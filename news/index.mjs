/**
* @vue/shared v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
/*! #__NO_SIDE_EFFECTS__ */
// @__NO_SIDE_EFFECTS__
function yt(e) {
  const t = /* @__PURE__ */ Object.create(null);
  for (const n of e.split(","))
    t[n] = 1;
  return (n) => n in t;
}
const ie = {}.NODE_ENV !== "production" ? Object.freeze({}) : {}, _t = {}.NODE_ENV !== "production" ? Object.freeze([]) : [], me = () => {
}, _f = () => !1, Dn = (e) => e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && // uppercase letter
(e.charCodeAt(2) > 122 || e.charCodeAt(2) < 97), _n = (e) => e.startsWith("onUpdate:"), pe = Object.assign, Is = (e, t) => {
  const n = e.indexOf(t);
  n > -1 && e.splice(n, 1);
}, $f = Object.prototype.hasOwnProperty, $ = (e, t) => $f.call(e, t), Q = Array.isArray, Qt = (e) => pr(e) === "[object Map]", no = (e) => pr(e) === "[object Set]", Z = (e) => typeof e == "function", Ae = (e) => typeof e == "string", Mt = (e) => typeof e == "symbol", ce = (e) => e !== null && typeof e == "object", ws = (e) => (ce(e) || Z(e)) && Z(e.then) && Z(e.catch), ro = Object.prototype.toString, pr = (e) => ro.call(e), Ps = (e) => pr(e).slice(8, -1), so = (e) => pr(e) === "[object Object]", Cs = (e) => Ae(e) && e !== "NaN" && e[0] !== "-" && "" + parseInt(e, 10) === e, hn = /* @__PURE__ */ yt(
  // the leading comma is intentional so empty string "" is also included
  ",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"
), el = /* @__PURE__ */ yt(
  "bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo"
), hr = (e) => {
  const t = /* @__PURE__ */ Object.create(null);
  return (n) => t[n] || (t[n] = e(n));
}, tl = /-(\w)/g, Ve = hr(
  (e) => e.replace(tl, (t, n) => n ? n.toUpperCase() : "")
), nl = /\B([A-Z])/g, Rt = hr(
  (e) => e.replace(nl, "-$1").toLowerCase()
), gr = hr((e) => e.charAt(0).toUpperCase() + e.slice(1)), Ht = hr(
  (e) => e ? `on${gr(e)}` : ""
), Bt = (e, t) => !Object.is(e, t), ln = (e, ...t) => {
  for (let n = 0; n < e.length; n++)
    e[n](...t);
}, $n = (e, t, n, r = !1) => {
  Object.defineProperty(e, t, {
    configurable: !0,
    enumerable: !1,
    writable: r,
    value: n
  });
}, rl = (e) => {
  const t = parseFloat(e);
  return isNaN(t) ? e : t;
};
let ni;
const Nn = () => ni || (ni = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : typeof global < "u" ? global : {});
function nn(e) {
  if (Q(e)) {
    const t = {};
    for (let n = 0; n < e.length; n++) {
      const r = e[n], s = Ae(r) ? fl(r) : nn(r);
      if (s)
        for (const i in s)
          t[i] = s[i];
    }
    return t;
  } else if (Ae(e) || ce(e))
    return e;
}
const sl = /;(?![^(]*\))/g, il = /:([^]+)/, ol = /\/\*[^]*?\*\//g;
function fl(e) {
  const t = {};
  return e.replace(ol, "").split(sl).forEach((n) => {
    if (n) {
      const r = n.split(il);
      r.length > 1 && (t[r[0].trim()] = r[1].trim());
    }
  }), t;
}
function gt(e) {
  let t = "";
  if (Ae(e))
    t = e;
  else if (Q(e))
    for (let n = 0; n < e.length; n++) {
      const r = gt(e[n]);
      r && (t += r + " ");
    }
  else if (ce(e))
    for (const n in e)
      e[n] && (t += n + " ");
  return t.trim();
}
const ll = "html,body,base,head,link,meta,style,title,address,article,aside,footer,header,hgroup,h1,h2,h3,h4,h5,h6,nav,section,div,dd,dl,dt,figcaption,figure,picture,hr,img,li,main,ol,p,pre,ul,a,b,abbr,bdi,bdo,br,cite,code,data,dfn,em,i,kbd,mark,q,rp,rt,ruby,s,samp,small,span,strong,sub,sup,time,u,var,wbr,area,audio,map,track,video,embed,object,param,source,canvas,script,noscript,del,ins,caption,col,colgroup,table,thead,tbody,td,th,tr,button,datalist,fieldset,form,input,label,legend,meter,optgroup,option,output,progress,select,textarea,details,dialog,menu,summary,template,blockquote,iframe,tfoot", cl = "svg,animate,animateMotion,animateTransform,circle,clipPath,color-profile,defs,desc,discard,ellipse,feBlend,feColorMatrix,feComponentTransfer,feComposite,feConvolveMatrix,feDiffuseLighting,feDisplacementMap,feDistantLight,feDropShadow,feFlood,feFuncA,feFuncB,feFuncG,feFuncR,feGaussianBlur,feImage,feMerge,feMergeNode,feMorphology,feOffset,fePointLight,feSpecularLighting,feSpotLight,feTile,feTurbulence,filter,foreignObject,g,hatch,hatchpath,image,line,linearGradient,marker,mask,mesh,meshgradient,meshpatch,meshrow,metadata,mpath,path,pattern,polygon,polyline,radialGradient,rect,set,solidcolor,stop,switch,symbol,text,textPath,title,tspan,unknown,use,view", ul = "annotation,annotation-xml,maction,maligngroup,malignmark,math,menclose,merror,mfenced,mfrac,mfraction,mglyph,mi,mlabeledtr,mlongdiv,mmultiscripts,mn,mo,mover,mpadded,mphantom,mprescripts,mroot,mrow,ms,mscarries,mscarry,msgroup,msline,mspace,msqrt,msrow,mstack,mstyle,msub,msubsup,msup,mtable,mtd,mtext,mtr,munder,munderover,none,semantics", al = /* @__PURE__ */ yt(ll), Al = /* @__PURE__ */ yt(cl), dl = /* @__PURE__ */ yt(ul), pl = "itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly", hl = /* @__PURE__ */ yt(pl);
function io(e) {
  return !!e || e === "";
}
const oo = (e) => !!(e && e.__v_isRef === !0), Ur = (e) => Ae(e) ? e : e == null ? "" : Q(e) || ce(e) && (e.toString === ro || !Z(e.toString)) ? oo(e) ? Ur(e.value) : JSON.stringify(e, fo, 2) : String(e), fo = (e, t) => oo(t) ? fo(e, t.value) : Qt(t) ? {
  [`Map(${t.size})`]: [...t.entries()].reduce(
    (n, [r, s], i) => (n[Mr(r, i) + " =>"] = s, n),
    {}
  )
} : no(t) ? {
  [`Set(${t.size})`]: [...t.values()].map((n) => Mr(n))
} : Mt(t) ? Mr(t) : ce(t) && !Q(t) && !so(t) ? String(t) : t, Mr = (e, t = "") => {
  var n;
  return (
    // Symbol.description in es2019+ so we need to cast here to pass
    // the lib: es2016 check
    Mt(e) ? `Symbol(${(n = e.description) != null ? n : t})` : e
  );
};
/**
* @vue/reactivity v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
function qe(e, ...t) {
  console.warn(`[Vue warn] ${e}`, ...t);
}
let Ne;
class gl {
  constructor(t = !1) {
    this.detached = t, this._active = !0, this.effects = [], this.cleanups = [], this._isPaused = !1, this.parent = Ne, !t && Ne && (this.index = (Ne.scopes || (Ne.scopes = [])).push(
      this
    ) - 1);
  }
  get active() {
    return this._active;
  }
  pause() {
    if (this._active) {
      this._isPaused = !0;
      let t, n;
      if (this.scopes)
        for (t = 0, n = this.scopes.length; t < n; t++)
          this.scopes[t].pause();
      for (t = 0, n = this.effects.length; t < n; t++)
        this.effects[t].pause();
    }
  }
  /**
   * Resumes the effect scope, including all child scopes and effects.
   */
  resume() {
    if (this._active && this._isPaused) {
      this._isPaused = !1;
      let t, n;
      if (this.scopes)
        for (t = 0, n = this.scopes.length; t < n; t++)
          this.scopes[t].resume();
      for (t = 0, n = this.effects.length; t < n; t++)
        this.effects[t].resume();
    }
  }
  run(t) {
    if (this._active) {
      const n = Ne;
      try {
        return Ne = this, t();
      } finally {
        Ne = n;
      }
    } else
      ({}).NODE_ENV !== "production" && qe("cannot run an inactive effect scope.");
  }
  /**
   * This should only be called on non-detached scopes
   * @internal
   */
  on() {
    Ne = this;
  }
  /**
   * This should only be called on non-detached scopes
   * @internal
   */
  off() {
    Ne = this.parent;
  }
  stop(t) {
    if (this._active) {
      this._active = !1;
      let n, r;
      for (n = 0, r = this.effects.length; n < r; n++)
        this.effects[n].stop();
      for (this.effects.length = 0, n = 0, r = this.cleanups.length; n < r; n++)
        this.cleanups[n]();
      if (this.cleanups.length = 0, this.scopes) {
        for (n = 0, r = this.scopes.length; n < r; n++)
          this.scopes[n].stop(!0);
        this.scopes.length = 0;
      }
      if (!this.detached && this.parent && !t) {
        const s = this.parent.scopes.pop();
        s && s !== this && (this.parent.scopes[this.index] = s, s.index = this.index);
      }
      this.parent = void 0;
    }
  }
}
function xs() {
  return Ne;
}
function lo(e, t = !1) {
  Ne ? Ne.cleanups.push(e) : {}.NODE_ENV !== "production" && !t && qe(
    "onScopeDispose() is called when there is no active effect scope to be associated with."
  );
}
let ne;
const Tr = /* @__PURE__ */ new WeakSet();
class co {
  constructor(t) {
    this.fn = t, this.deps = void 0, this.depsTail = void 0, this.flags = 5, this.next = void 0, this.cleanup = void 0, this.scheduler = void 0, Ne && Ne.active && Ne.effects.push(this);
  }
  pause() {
    this.flags |= 64;
  }
  resume() {
    this.flags & 64 && (this.flags &= -65, Tr.has(this) && (Tr.delete(this), this.trigger()));
  }
  /**
   * @internal
   */
  notify() {
    this.flags & 2 && !(this.flags & 32) || this.flags & 8 || ao(this);
  }
  run() {
    if (!(this.flags & 1))
      return this.fn();
    this.flags |= 2, ri(this), Ao(this);
    const t = ne, n = We;
    ne = this, We = !0;
    try {
      return this.fn();
    } finally {
      ({}).NODE_ENV !== "production" && ne !== this && qe(
        "Active effect was not restored correctly - this is likely a Vue internal bug."
      ), po(this), ne = t, We = n, this.flags &= -3;
    }
  }
  stop() {
    if (this.flags & 1) {
      for (let t = this.deps; t; t = t.nextDep)
        Ss(t);
      this.deps = this.depsTail = void 0, ri(this), this.onStop && this.onStop(), this.flags &= -2;
    }
  }
  trigger() {
    this.flags & 64 ? Tr.add(this) : this.scheduler ? this.scheduler() : this.runIfDirty();
  }
  /**
   * @internal
   */
  runIfDirty() {
    Kr(this) && this.run();
  }
  get dirty() {
    return Kr(this);
  }
}
let uo = 0, gn, vn;
function ao(e, t = !1) {
  if (e.flags |= 8, t) {
    e.next = vn, vn = e;
    return;
  }
  e.next = gn, gn = e;
}
function Ds() {
  uo++;
}
function Ns() {
  if (--uo > 0)
    return;
  if (vn) {
    let t = vn;
    for (vn = void 0; t; ) {
      const n = t.next;
      t.next = void 0, t.flags &= -9, t = n;
    }
  }
  let e;
  for (; gn; ) {
    let t = gn;
    for (gn = void 0; t; ) {
      const n = t.next;
      if (t.next = void 0, t.flags &= -9, t.flags & 1)
        try {
          t.trigger();
        } catch (r) {
          e || (e = r);
        }
      t = n;
    }
  }
  if (e)
    throw e;
}
function Ao(e) {
  for (let t = e.deps; t; t = t.nextDep)
    t.version = -1, t.prevActiveLink = t.dep.activeLink, t.dep.activeLink = t;
}
function po(e) {
  let t, n = e.depsTail, r = n;
  for (; r; ) {
    const s = r.prevDep;
    r.version === -1 ? (r === n && (n = s), Ss(r), vl(r)) : t = r, r.dep.activeLink = r.prevActiveLink, r.prevActiveLink = void 0, r = s;
  }
  e.deps = t, e.depsTail = n;
}
function Kr(e) {
  for (let t = e.deps; t; t = t.nextDep)
    if (t.dep.version !== t.version || t.dep.computed && (ho(t.dep.computed) || t.dep.version !== t.version))
      return !0;
  return !!e._dirty;
}
function ho(e) {
  if (e.flags & 4 && !(e.flags & 16) || (e.flags &= -17, e.globalVersion === yn))
    return;
  e.globalVersion = yn;
  const t = e.dep;
  if (e.flags |= 2, t.version > 0 && !e.isSSR && e.deps && !Kr(e)) {
    e.flags &= -3;
    return;
  }
  const n = ne, r = We;
  ne = e, We = !0;
  try {
    Ao(e);
    const s = e.fn(e._value);
    (t.version === 0 || Bt(s, e._value)) && (e._value = s, t.version++);
  } catch (s) {
    throw t.version++, s;
  } finally {
    ne = n, We = r, po(e), e.flags &= -3;
  }
}
function Ss(e, t = !1) {
  const { dep: n, prevSub: r, nextSub: s } = e;
  if (r && (r.nextSub = s, e.prevSub = void 0), s && (s.prevSub = r, e.nextSub = void 0), {}.NODE_ENV !== "production" && n.subsHead === e && (n.subsHead = s), n.subs === e && (n.subs = r, !r && n.computed)) {
    n.computed.flags &= -5;
    for (let i = n.computed.deps; i; i = i.nextDep)
      Ss(i, !0);
  }
  !t && !--n.sc && n.map && n.map.delete(n.key);
}
function vl(e) {
  const { prevDep: t, nextDep: n } = e;
  t && (t.nextDep = n, e.prevDep = void 0), n && (n.prevDep = t, e.nextDep = void 0);
}
let We = !0;
const go = [];
function bt() {
  go.push(We), We = !1;
}
function It() {
  const e = go.pop();
  We = e === void 0 ? !0 : e;
}
function ri(e) {
  const { cleanup: t } = e;
  if (e.cleanup = void 0, t) {
    const n = ne;
    ne = void 0;
    try {
      t();
    } finally {
      ne = n;
    }
  }
}
let yn = 0;
class ml {
  constructor(t, n) {
    this.sub = t, this.dep = n, this.version = n.version, this.nextDep = this.prevDep = this.nextSub = this.prevSub = this.prevActiveLink = void 0;
  }
}
class Bs {
  constructor(t) {
    this.computed = t, this.version = 0, this.activeLink = void 0, this.subs = void 0, this.map = void 0, this.key = void 0, this.sc = 0, {}.NODE_ENV !== "production" && (this.subsHead = void 0);
  }
  track(t) {
    if (!ne || !We || ne === this.computed)
      return;
    let n = this.activeLink;
    if (n === void 0 || n.sub !== ne)
      n = this.activeLink = new ml(ne, this), ne.deps ? (n.prevDep = ne.depsTail, ne.depsTail.nextDep = n, ne.depsTail = n) : ne.deps = ne.depsTail = n, vo(n);
    else if (n.version === -1 && (n.version = this.version, n.nextDep)) {
      const r = n.nextDep;
      r.prevDep = n.prevDep, n.prevDep && (n.prevDep.nextDep = r), n.prevDep = ne.depsTail, n.nextDep = void 0, ne.depsTail.nextDep = n, ne.depsTail = n, ne.deps === n && (ne.deps = r);
    }
    return {}.NODE_ENV !== "production" && ne.onTrack && ne.onTrack(
      pe(
        {
          effect: ne
        },
        t
      )
    ), n;
  }
  trigger(t) {
    this.version++, yn++, this.notify(t);
  }
  notify(t) {
    Ds();
    try {
      if ({}.NODE_ENV !== "production")
        for (let n = this.subsHead; n; n = n.nextSub)
          n.sub.onTrigger && !(n.sub.flags & 8) && n.sub.onTrigger(
            pe(
              {
                effect: n.sub
              },
              t
            )
          );
      for (let n = this.subs; n; n = n.prevSub)
        n.sub.notify() && n.sub.dep.notify();
    } finally {
      Ns();
    }
  }
}
function vo(e) {
  if (e.dep.sc++, e.sub.flags & 4) {
    const t = e.dep.computed;
    if (t && !e.dep.subs) {
      t.flags |= 20;
      for (let r = t.deps; r; r = r.nextDep)
        vo(r);
    }
    const n = e.dep.subs;
    n !== e && (e.prevSub = n, n && (n.nextSub = e)), {}.NODE_ENV !== "production" && e.dep.subsHead === void 0 && (e.dep.subsHead = e), e.dep.subs = e;
  }
}
const qr = /* @__PURE__ */ new WeakMap(), Yt = Symbol(
  {}.NODE_ENV !== "production" ? "Object iterate" : ""
), _r = Symbol(
  {}.NODE_ENV !== "production" ? "Map keys iterate" : ""
), bn = Symbol(
  {}.NODE_ENV !== "production" ? "Array iterate" : ""
);
function ve(e, t, n) {
  if (We && ne) {
    let r = qr.get(e);
    r || qr.set(e, r = /* @__PURE__ */ new Map());
    let s = r.get(n);
    s || (r.set(n, s = new Bs()), s.map = r, s.key = n), {}.NODE_ENV !== "production" ? s.track({
      target: e,
      type: t,
      key: n
    }) : s.track();
  }
}
function it(e, t, n, r, s, i) {
  const o = qr.get(e);
  if (!o) {
    yn++;
    return;
  }
  const f = (c) => {
    c && ({}.NODE_ENV !== "production" ? c.trigger({
      target: e,
      type: t,
      key: n,
      newValue: r,
      oldValue: s,
      oldTarget: i
    }) : c.trigger());
  };
  if (Ds(), t === "clear")
    o.forEach(f);
  else {
    const c = Q(e), A = c && Cs(n);
    if (c && n === "length") {
      const u = Number(r);
      o.forEach((l, p) => {
        (p === "length" || p === bn || !Mt(p) && p >= u) && f(l);
      });
    } else
      switch ((n !== void 0 || o.has(void 0)) && f(o.get(n)), A && f(o.get(bn)), t) {
        case "add":
          c ? A && f(o.get("length")) : (f(o.get(Yt)), Qt(e) && f(o.get(_r)));
          break;
        case "delete":
          c || (f(o.get(Yt)), Qt(e) && f(o.get(_r)));
          break;
        case "set":
          Qt(e) && f(o.get(Yt));
          break;
      }
  }
  Ns();
}
function Kt(e) {
  const t = q(e);
  return t === e ? t : (ve(t, "iterate", bn), Be(e) ? t : t.map(we));
}
function vr(e) {
  return ve(e = q(e), "iterate", bn), e;
}
const El = {
  __proto__: null,
  [Symbol.iterator]() {
    return Lr(this, Symbol.iterator, we);
  },
  concat(...e) {
    return Kt(this).concat(
      ...e.map((t) => Q(t) ? Kt(t) : t)
    );
  },
  entries() {
    return Lr(this, "entries", (e) => (e[1] = we(e[1]), e));
  },
  every(e, t) {
    return at(this, "every", e, t, void 0, arguments);
  },
  filter(e, t) {
    return at(this, "filter", e, t, (n) => n.map(we), arguments);
  },
  find(e, t) {
    return at(this, "find", e, t, we, arguments);
  },
  findIndex(e, t) {
    return at(this, "findIndex", e, t, void 0, arguments);
  },
  findLast(e, t) {
    return at(this, "findLast", e, t, we, arguments);
  },
  findLastIndex(e, t) {
    return at(this, "findLastIndex", e, t, void 0, arguments);
  },
  // flat, flatMap could benefit from ARRAY_ITERATE but are not straight-forward to implement
  forEach(e, t) {
    return at(this, "forEach", e, t, void 0, arguments);
  },
  includes(...e) {
    return kr(this, "includes", e);
  },
  indexOf(...e) {
    return kr(this, "indexOf", e);
  },
  join(e) {
    return Kt(this).join(e);
  },
  // keys() iterator only reads `length`, no optimisation required
  lastIndexOf(...e) {
    return kr(this, "lastIndexOf", e);
  },
  map(e, t) {
    return at(this, "map", e, t, void 0, arguments);
  },
  pop() {
    return cn(this, "pop");
  },
  push(...e) {
    return cn(this, "push", e);
  },
  reduce(e, ...t) {
    return si(this, "reduce", e, t);
  },
  reduceRight(e, ...t) {
    return si(this, "reduceRight", e, t);
  },
  shift() {
    return cn(this, "shift");
  },
  // slice could use ARRAY_ITERATE but also seems to beg for range tracking
  some(e, t) {
    return at(this, "some", e, t, void 0, arguments);
  },
  splice(...e) {
    return cn(this, "splice", e);
  },
  toReversed() {
    return Kt(this).toReversed();
  },
  toSorted(e) {
    return Kt(this).toSorted(e);
  },
  toSpliced(...e) {
    return Kt(this).toSpliced(...e);
  },
  unshift(...e) {
    return cn(this, "unshift", e);
  },
  values() {
    return Lr(this, "values", we);
  }
};
function Lr(e, t, n) {
  const r = vr(e), s = r[t]();
  return r !== e && !Be(e) && (s._next = s.next, s.next = () => {
    const i = s._next();
    return i.value && (i.value = n(i.value)), i;
  }), s;
}
const yl = Array.prototype;
function at(e, t, n, r, s, i) {
  const o = vr(e), f = o !== e && !Be(e), c = o[t];
  if (c !== yl[t]) {
    const l = c.apply(e, i);
    return f ? we(l) : l;
  }
  let A = n;
  o !== e && (f ? A = function(l, p) {
    return n.call(this, we(l), p, e);
  } : n.length > 2 && (A = function(l, p) {
    return n.call(this, l, p, e);
  }));
  const u = c.call(o, A, r);
  return f && s ? s(u) : u;
}
function si(e, t, n, r) {
  const s = vr(e);
  let i = n;
  return s !== e && (Be(e) ? n.length > 3 && (i = function(o, f, c) {
    return n.call(this, o, f, c, e);
  }) : i = function(o, f, c) {
    return n.call(this, o, we(f), c, e);
  }), s[t](i, ...r);
}
function kr(e, t, n) {
  const r = q(e);
  ve(r, "iterate", bn);
  const s = r[t](...n);
  return (s === -1 || s === !1) && er(n[0]) ? (n[0] = q(n[0]), r[t](...n)) : s;
}
function cn(e, t, n = []) {
  bt(), Ds();
  const r = q(e)[t].apply(e, n);
  return Ns(), It(), r;
}
const bl = /* @__PURE__ */ yt("__proto__,__v_isRef,__isVue"), mo = new Set(
  /* @__PURE__ */ Object.getOwnPropertyNames(Symbol).filter((e) => e !== "arguments" && e !== "caller").map((e) => Symbol[e]).filter(Mt)
);
function Il(e) {
  Mt(e) || (e = String(e));
  const t = q(this);
  return ve(t, "has", e), t.hasOwnProperty(e);
}
class Eo {
  constructor(t = !1, n = !1) {
    this._isReadonly = t, this._isShallow = n;
  }
  get(t, n, r) {
    if (n === "__v_skip")
      return t.__v_skip;
    const s = this._isReadonly, i = this._isShallow;
    if (n === "__v_isReactive")
      return !s;
    if (n === "__v_isReadonly")
      return s;
    if (n === "__v_isShallow")
      return i;
    if (n === "__v_raw")
      return r === (s ? i ? Co : Po : i ? wo : Io).get(t) || // receiver is not the reactive proxy, but has the same prototype
      // this means the receiver is a user proxy of the reactive proxy
      Object.getPrototypeOf(t) === Object.getPrototypeOf(r) ? t : void 0;
    const o = Q(t);
    if (!s) {
      let c;
      if (o && (c = El[n]))
        return c;
      if (n === "hasOwnProperty")
        return Il;
    }
    const f = Reflect.get(
      t,
      n,
      // if this is a proxy wrapping a ref, return methods using the raw ref
      // as receiver so that we don't have to call `toRaw` on the ref in all
      // its class methods
      de(t) ? t : r
    );
    return (Mt(n) ? mo.has(n) : bl(n)) || (s || ve(t, "get", n), i) ? f : de(f) ? o && Cs(n) ? f : f.value : ce(f) ? s ? Os(f) : Rs(f) : f;
  }
}
class yo extends Eo {
  constructor(t = !1) {
    super(!1, t);
  }
  set(t, n, r, s) {
    let i = t[n];
    if (!this._isShallow) {
      const c = Et(i);
      if (!Be(r) && !Et(r) && (i = q(i), r = q(r)), !Q(t) && de(i) && !de(r))
        return c ? !1 : (i.value = r, !0);
    }
    const o = Q(t) && Cs(n) ? Number(n) < t.length : $(t, n), f = Reflect.set(
      t,
      n,
      r,
      de(t) ? t : s
    );
    return t === q(s) && (o ? Bt(r, i) && it(t, "set", n, r, i) : it(t, "add", n, r)), f;
  }
  deleteProperty(t, n) {
    const r = $(t, n), s = t[n], i = Reflect.deleteProperty(t, n);
    return i && r && it(t, "delete", n, void 0, s), i;
  }
  has(t, n) {
    const r = Reflect.has(t, n);
    return (!Mt(n) || !mo.has(n)) && ve(t, "has", n), r;
  }
  ownKeys(t) {
    return ve(
      t,
      "iterate",
      Q(t) ? "length" : Yt
    ), Reflect.ownKeys(t);
  }
}
class bo extends Eo {
  constructor(t = !1) {
    super(!0, t);
  }
  set(t, n) {
    return {}.NODE_ENV !== "production" && qe(
      `Set operation on key "${String(n)}" failed: target is readonly.`,
      t
    ), !0;
  }
  deleteProperty(t, n) {
    return {}.NODE_ENV !== "production" && qe(
      `Delete operation on key "${String(n)}" failed: target is readonly.`,
      t
    ), !0;
  }
}
const wl = /* @__PURE__ */ new yo(), Pl = /* @__PURE__ */ new bo(), Cl = /* @__PURE__ */ new yo(!0), xl = /* @__PURE__ */ new bo(!0), $r = (e) => e, Ln = (e) => Reflect.getPrototypeOf(e);
function Dl(e, t, n) {
  return function(...r) {
    const s = this.__v_raw, i = q(s), o = Qt(i), f = e === "entries" || e === Symbol.iterator && o, c = e === "keys" && o, A = s[e](...r), u = n ? $r : t ? es : we;
    return !t && ve(
      i,
      "iterate",
      c ? _r : Yt
    ), {
      // iterator protocol
      next() {
        const { value: l, done: p } = A.next();
        return p ? { value: l, done: p } : {
          value: f ? [u(l[0]), u(l[1])] : u(l),
          done: p
        };
      },
      // iterable protocol
      [Symbol.iterator]() {
        return this;
      }
    };
  };
}
function kn(e) {
  return function(...t) {
    if ({}.NODE_ENV !== "production") {
      const n = t[0] ? `on key "${t[0]}" ` : "";
      qe(
        `${gr(e)} operation ${n}failed: target is readonly.`,
        q(this)
      );
    }
    return e === "delete" ? !1 : e === "clear" ? void 0 : this;
  };
}
function Nl(e, t) {
  const n = {
    get(s) {
      const i = this.__v_raw, o = q(i), f = q(s);
      e || (Bt(s, f) && ve(o, "get", s), ve(o, "get", f));
      const { has: c } = Ln(o), A = t ? $r : e ? es : we;
      if (c.call(o, s))
        return A(i.get(s));
      if (c.call(o, f))
        return A(i.get(f));
      i !== o && i.get(s);
    },
    get size() {
      const s = this.__v_raw;
      return !e && ve(q(s), "iterate", Yt), Reflect.get(s, "size", s);
    },
    has(s) {
      const i = this.__v_raw, o = q(i), f = q(s);
      return e || (Bt(s, f) && ve(o, "has", s), ve(o, "has", f)), s === f ? i.has(s) : i.has(s) || i.has(f);
    },
    forEach(s, i) {
      const o = this, f = o.__v_raw, c = q(f), A = t ? $r : e ? es : we;
      return !e && ve(c, "iterate", Yt), f.forEach((u, l) => s.call(i, A(u), A(l), o));
    }
  };
  return pe(
    n,
    e ? {
      add: kn("add"),
      set: kn("set"),
      delete: kn("delete"),
      clear: kn("clear")
    } : {
      add(s) {
        !t && !Be(s) && !Et(s) && (s = q(s));
        const i = q(this);
        return Ln(i).has.call(i, s) || (i.add(s), it(i, "add", s, s)), this;
      },
      set(s, i) {
        !t && !Be(i) && !Et(i) && (i = q(i));
        const o = q(this), { has: f, get: c } = Ln(o);
        let A = f.call(o, s);
        A ? {}.NODE_ENV !== "production" && ii(o, f, s) : (s = q(s), A = f.call(o, s));
        const u = c.call(o, s);
        return o.set(s, i), A ? Bt(i, u) && it(o, "set", s, i, u) : it(o, "add", s, i), this;
      },
      delete(s) {
        const i = q(this), { has: o, get: f } = Ln(i);
        let c = o.call(i, s);
        c ? {}.NODE_ENV !== "production" && ii(i, o, s) : (s = q(s), c = o.call(i, s));
        const A = f ? f.call(i, s) : void 0, u = i.delete(s);
        return c && it(i, "delete", s, void 0, A), u;
      },
      clear() {
        const s = q(this), i = s.size !== 0, o = {}.NODE_ENV !== "production" ? Qt(s) ? new Map(s) : new Set(s) : void 0, f = s.clear();
        return i && it(
          s,
          "clear",
          void 0,
          void 0,
          o
        ), f;
      }
    }
  ), [
    "keys",
    "values",
    "entries",
    Symbol.iterator
  ].forEach((s) => {
    n[s] = Dl(s, e, t);
  }), n;
}
function mr(e, t) {
  const n = Nl(e, t);
  return (r, s, i) => s === "__v_isReactive" ? !e : s === "__v_isReadonly" ? e : s === "__v_raw" ? r : Reflect.get(
    $(n, s) && s in r ? n : r,
    s,
    i
  );
}
const Sl = {
  get: /* @__PURE__ */ mr(!1, !1)
}, Bl = {
  get: /* @__PURE__ */ mr(!1, !0)
}, Rl = {
  get: /* @__PURE__ */ mr(!0, !1)
}, Ol = {
  get: /* @__PURE__ */ mr(!0, !0)
};
function ii(e, t, n) {
  const r = q(n);
  if (r !== n && t.call(e, r)) {
    const s = Ps(e);
    qe(
      `Reactive ${s} contains both the raw and reactive versions of the same object${s === "Map" ? " as keys" : ""}, which can lead to inconsistencies. Avoid differentiating between the raw and reactive versions of an object and only use the reactive version if possible.`
    );
  }
}
const Io = /* @__PURE__ */ new WeakMap(), wo = /* @__PURE__ */ new WeakMap(), Po = /* @__PURE__ */ new WeakMap(), Co = /* @__PURE__ */ new WeakMap();
function Ml(e) {
  switch (e) {
    case "Object":
    case "Array":
      return 1;
    case "Map":
    case "Set":
    case "WeakMap":
    case "WeakSet":
      return 2;
    default:
      return 0;
  }
}
function Tl(e) {
  return e.__v_skip || !Object.isExtensible(e) ? 0 : Ml(Ps(e));
}
function Rs(e) {
  return Et(e) ? e : Er(
    e,
    !1,
    wl,
    Sl,
    Io
  );
}
function Ll(e) {
  return Er(
    e,
    !1,
    Cl,
    Bl,
    wo
  );
}
function Os(e) {
  return Er(
    e,
    !0,
    Pl,
    Rl,
    Po
  );
}
function ft(e) {
  return Er(
    e,
    !0,
    xl,
    Ol,
    Co
  );
}
function Er(e, t, n, r, s) {
  if (!ce(e))
    return {}.NODE_ENV !== "production" && qe(
      `value cannot be made ${t ? "readonly" : "reactive"}: ${String(
        e
      )}`
    ), e;
  if (e.__v_raw && !(t && e.__v_isReactive))
    return e;
  const i = s.get(e);
  if (i)
    return i;
  const o = Tl(e);
  if (o === 0)
    return e;
  const f = new Proxy(
    e,
    o === 2 ? r : n
  );
  return s.set(e, f), f;
}
function Zt(e) {
  return Et(e) ? Zt(e.__v_raw) : !!(e && e.__v_isReactive);
}
function Et(e) {
  return !!(e && e.__v_isReadonly);
}
function Be(e) {
  return !!(e && e.__v_isShallow);
}
function er(e) {
  return e ? !!e.__v_raw : !1;
}
function q(e) {
  const t = e && e.__v_raw;
  return t ? q(t) : e;
}
function kl(e) {
  return !$(e, "__v_skip") && Object.isExtensible(e) && $n(e, "__v_skip", !0), e;
}
const we = (e) => ce(e) ? Rs(e) : e, es = (e) => ce(e) ? Os(e) : e;
function de(e) {
  return e ? e.__v_isRef === !0 : !1;
}
function ze(e) {
  return xo(e, !1);
}
function ts(e) {
  return xo(e, !0);
}
function xo(e, t) {
  return de(e) ? e : new Hl(e, t);
}
class Hl {
  constructor(t, n) {
    this.dep = new Bs(), this.__v_isRef = !0, this.__v_isShallow = !1, this._rawValue = n ? t : q(t), this._value = n ? t : we(t), this.__v_isShallow = n;
  }
  get value() {
    return {}.NODE_ENV !== "production" ? this.dep.track({
      target: this,
      type: "get",
      key: "value"
    }) : this.dep.track(), this._value;
  }
  set value(t) {
    const n = this._rawValue, r = this.__v_isShallow || Be(t) || Et(t);
    t = r ? t : q(t), Bt(t, n) && (this._rawValue = t, this._value = r ? t : we(t), {}.NODE_ENV !== "production" ? this.dep.trigger({
      target: this,
      type: "set",
      key: "value",
      newValue: t,
      oldValue: n
    }) : this.dep.trigger());
  }
}
function ht(e) {
  return de(e) ? e.value : e;
}
function zt(e) {
  return Z(e) ? e() : ht(e);
}
const jl = {
  get: (e, t, n) => t === "__v_raw" ? e : ht(Reflect.get(e, t, n)),
  set: (e, t, n, r) => {
    const s = e[t];
    return de(s) && !de(n) ? (s.value = n, !0) : Reflect.set(e, t, n, r);
  }
};
function Do(e) {
  return Zt(e) ? e : new Proxy(e, jl);
}
class zl {
  constructor(t, n, r) {
    this.fn = t, this.setter = n, this._value = void 0, this.dep = new Bs(this), this.__v_isRef = !0, this.deps = void 0, this.depsTail = void 0, this.flags = 16, this.globalVersion = yn - 1, this.next = void 0, this.effect = this, this.__v_isReadonly = !n, this.isSSR = r;
  }
  /**
   * @internal
   */
  notify() {
    if (this.flags |= 16, !(this.flags & 8) && // avoid infinite self recursion
    ne !== this)
      return ao(this, !0), !0;
  }
  get value() {
    const t = {}.NODE_ENV !== "production" ? this.dep.track({
      target: this,
      type: "get",
      key: "value"
    }) : this.dep.track();
    return ho(this), t && (t.version = this.dep.version), this._value;
  }
  set value(t) {
    this.setter ? this.setter(t) : {}.NODE_ENV !== "production" && qe("Write operation failed: computed value is readonly");
  }
}
function Jl(e, t, n = !1) {
  let r, s;
  Z(e) ? r = e : (r = e.get, s = e.set);
  const i = new zl(r, s, n);
  return {}.NODE_ENV !== "production" && t && !n && (i.onTrack = t.onTrack, i.onTrigger = t.onTrigger), i;
}
const Hn = {}, tr = /* @__PURE__ */ new WeakMap();
let jt;
function Xl(e, t = !1, n = jt) {
  if (n) {
    let r = tr.get(n);
    r || tr.set(n, r = []), r.push(e);
  } else
    ({}).NODE_ENV !== "production" && !t && qe(
      "onWatcherCleanup() was called when there was no active watcher to associate with."
    );
}
function Ql(e, t, n = ie) {
  const { immediate: r, deep: s, once: i, scheduler: o, augmentJob: f, call: c } = n, A = (R) => {
    (n.onWarn || qe)(
      "Invalid watch source: ",
      R,
      "A watch source can only be a getter/effect function, a ref, a reactive object, or an array of these types."
    );
  }, u = (R) => s ? R : Be(R) || s === !1 || s === 0 ? Nt(R, 1) : Nt(R);
  let l, p, g, v, h = !1, y = !1;
  if (de(e) ? (p = () => e.value, h = Be(e)) : Zt(e) ? (p = () => u(e), h = !0) : Q(e) ? (y = !0, h = e.some((R) => Zt(R) || Be(R)), p = () => e.map((R) => {
    if (de(R))
      return R.value;
    if (Zt(R))
      return u(R);
    if (Z(R))
      return c ? c(R, 2) : R();
    ({}).NODE_ENV !== "production" && A(R);
  })) : Z(e) ? t ? p = c ? () => c(e, 2) : e : p = () => {
    if (g) {
      bt();
      try {
        g();
      } finally {
        It();
      }
    }
    const R = jt;
    jt = l;
    try {
      return c ? c(e, 3, [v]) : e(v);
    } finally {
      jt = R;
    }
  } : (p = me, {}.NODE_ENV !== "production" && A(e)), t && s) {
    const R = p, U = s === !0 ? 1 / 0 : s;
    p = () => Nt(R(), U);
  }
  const x = xs(), H = () => {
    l.stop(), x && x.active && Is(x.effects, l);
  };
  if (i && t) {
    const R = t;
    t = (...U) => {
      R(...U), H();
    };
  }
  let T = y ? new Array(e.length).fill(Hn) : Hn;
  const G = (R) => {
    if (!(!(l.flags & 1) || !l.dirty && !R))
      if (t) {
        const U = l.run();
        if (s || h || (y ? U.some((re, ee) => Bt(re, T[ee])) : Bt(U, T))) {
          g && g();
          const re = jt;
          jt = l;
          try {
            const ee = [
              U,
              // pass undefined as the old value when it's changed for the first time
              T === Hn ? void 0 : y && T[0] === Hn ? [] : T,
              v
            ];
            c ? c(t, 3, ee) : (
              // @ts-expect-error
              t(...ee)
            ), T = U;
          } finally {
            jt = re;
          }
        }
      } else
        l.run();
  };
  return f && f(G), l = new co(p), l.scheduler = o ? () => o(G, !1) : G, v = (R) => Xl(R, !1, l), g = l.onStop = () => {
    const R = tr.get(l);
    if (R) {
      if (c)
        c(R, 4);
      else
        for (const U of R)
          U();
      tr.delete(l);
    }
  }, {}.NODE_ENV !== "production" && (l.onTrack = n.onTrack, l.onTrigger = n.onTrigger), t ? r ? G(!0) : T = l.run() : o ? o(G.bind(null, !0), !0) : l.run(), H.pause = l.pause.bind(l), H.resume = l.resume.bind(l), H.stop = H, H;
}
function Nt(e, t = 1 / 0, n) {
  if (t <= 0 || !ce(e) || e.__v_skip || (n = n || /* @__PURE__ */ new Set(), n.has(e)))
    return e;
  if (n.add(e), t--, de(e))
    Nt(e.value, t, n);
  else if (Q(e))
    for (let r = 0; r < e.length; r++)
      Nt(e[r], t, n);
  else if (no(e) || Qt(e))
    e.forEach((r) => {
      Nt(r, t, n);
    });
  else if (so(e)) {
    for (const r in e)
      Nt(e[r], t, n);
    for (const r of Object.getOwnPropertySymbols(e))
      Object.prototype.propertyIsEnumerable.call(e, r) && Nt(e[r], t, n);
  }
  return e;
}
/**
* @vue/runtime-core v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
const Ft = [];
function Jn(e) {
  Ft.push(e);
}
function Xn() {
  Ft.pop();
}
let Hr = !1;
function O(e, ...t) {
  if (Hr)
    return;
  Hr = !0, bt();
  const n = Ft.length ? Ft[Ft.length - 1].component : null, r = n && n.appContext.config.warnHandler, s = Yl();
  if (r)
    rn(
      r,
      n,
      11,
      [
        // eslint-disable-next-line no-restricted-syntax
        e + t.map((i) => {
          var o, f;
          return (f = (o = i.toString) == null ? void 0 : o.call(i)) != null ? f : JSON.stringify(i);
        }).join(""),
        n && n.proxy,
        s.map(
          ({ vnode: i }) => `at <${Cr(n, i.type)}>`
        ).join(`
`),
        s
      ]
    );
  else {
    const i = [`[Vue warn]: ${e}`, ...t];
    s.length && i.push(`
`, ...Zl(s)), console.warn(...i);
  }
  It(), Hr = !1;
}
function Yl() {
  let e = Ft[Ft.length - 1];
  if (!e)
    return [];
  const t = [];
  for (; e; ) {
    const n = t[0];
    n && n.vnode === e ? n.recurseCount++ : t.push({
      vnode: e,
      recurseCount: 0
    });
    const r = e.component && e.component.parent;
    e = r && r.vnode;
  }
  return t;
}
function Zl(e) {
  const t = [];
  return e.forEach((n, r) => {
    t.push(...r === 0 ? [] : [`
`], ...Fl(n));
  }), t;
}
function Fl({ vnode: e, recurseCount: t }) {
  const n = t > 0 ? `... (${t} recursive calls)` : "", r = e.component ? e.component.parent == null : !1, s = ` at <${Cr(
    e.component,
    e.type,
    r
  )}`, i = ">" + n;
  return e.props ? [s, ...Gl(e.props), i] : [s + i];
}
function Gl(e) {
  const t = [], n = Object.keys(e);
  return n.slice(0, 3).forEach((r) => {
    t.push(...No(r, e[r]));
  }), n.length > 3 && t.push(" ..."), t;
}
function No(e, t, n) {
  return Ae(t) ? (t = JSON.stringify(t), n ? t : [`${e}=${t}`]) : typeof t == "number" || typeof t == "boolean" || t == null ? n ? t : [`${e}=${t}`] : de(t) ? (t = No(e, q(t.value), !0), n ? t : [`${e}=Ref<`, t, ">"]) : Z(t) ? [`${e}=fn${t.name ? `<${t.name}>` : ""}`] : (t = q(t), n ? t : [`${e}=`, t]);
}
const Ms = {
  sp: "serverPrefetch hook",
  bc: "beforeCreate hook",
  c: "created hook",
  bm: "beforeMount hook",
  m: "mounted hook",
  bu: "beforeUpdate hook",
  u: "updated",
  bum: "beforeUnmount hook",
  um: "unmounted hook",
  a: "activated hook",
  da: "deactivated hook",
  ec: "errorCaptured hook",
  rtc: "renderTracked hook",
  rtg: "renderTriggered hook",
  0: "setup function",
  1: "render function",
  2: "watcher getter",
  3: "watcher callback",
  4: "watcher cleanup function",
  5: "native event handler",
  6: "component event handler",
  7: "vnode hook",
  8: "directive hook",
  9: "transition hook",
  10: "app errorHandler",
  11: "app warnHandler",
  12: "ref function",
  13: "async component loader",
  14: "scheduler flush",
  15: "component update",
  16: "app unmount cleanup function"
};
function rn(e, t, n, r) {
  try {
    return r ? e(...r) : e();
  } catch (s) {
    Sn(s, t, n);
  }
}
function ct(e, t, n, r) {
  if (Z(e)) {
    const s = rn(e, t, n, r);
    return s && ws(s) && s.catch((i) => {
      Sn(i, t, n);
    }), s;
  }
  if (Q(e)) {
    const s = [];
    for (let i = 0; i < e.length; i++)
      s.push(ct(e[i], t, n, r));
    return s;
  } else
    ({}).NODE_ENV !== "production" && O(
      `Invalid value type passed to callWithAsyncErrorHandling(): ${typeof e}`
    );
}
function Sn(e, t, n, r = !0) {
  const s = t ? t.vnode : null, { errorHandler: i, throwUnhandledErrorInProduction: o } = t && t.appContext.config || ie;
  if (t) {
    let f = t.parent;
    const c = t.proxy, A = {}.NODE_ENV !== "production" ? Ms[n] : `https://vuejs.org/error-reference/#runtime-${n}`;
    for (; f; ) {
      const u = f.ec;
      if (u) {
        for (let l = 0; l < u.length; l++)
          if (u[l](e, c, A) === !1)
            return;
      }
      f = f.parent;
    }
    if (i) {
      bt(), rn(i, null, 10, [
        e,
        c,
        A
      ]), It();
      return;
    }
  }
  Vl(e, n, s, r, o);
}
function Vl(e, t, n, r = !0, s = !1) {
  if ({}.NODE_ENV !== "production") {
    const i = Ms[t];
    if (n && Jn(n), O(`Unhandled error${i ? ` during execution of ${i}` : ""}`), n && Xn(), r)
      throw e;
    console.error(e);
  } else {
    if (s)
      throw e;
    console.error(e);
  }
}
const Se = [];
let rt = -1;
const $t = [];
let xt = null, qt = 0;
const So = /* @__PURE__ */ Promise.resolve();
let nr = null;
const Wl = 100;
function Vt(e) {
  const t = nr || So;
  return e ? t.then(this ? e.bind(this) : e) : t;
}
function Ul(e) {
  let t = rt + 1, n = Se.length;
  for (; t < n; ) {
    const r = t + n >>> 1, s = Se[r], i = In(s);
    i < e || i === e && s.flags & 2 ? t = r + 1 : n = r;
  }
  return t;
}
function yr(e) {
  if (!(e.flags & 1)) {
    const t = In(e), n = Se[Se.length - 1];
    !n || // fast path when the job id is larger than the tail
    !(e.flags & 2) && t >= In(n) ? Se.push(e) : Se.splice(Ul(t), 0, e), e.flags |= 1, Bo();
  }
}
function Bo() {
  nr || (nr = So.then(Mo));
}
function Ro(e) {
  Q(e) ? $t.push(...e) : xt && e.id === -1 ? xt.splice(qt + 1, 0, e) : e.flags & 1 || ($t.push(e), e.flags |= 1), Bo();
}
function oi(e, t, n = rt + 1) {
  for ({}.NODE_ENV !== "production" && (t = t || /* @__PURE__ */ new Map()); n < Se.length; n++) {
    const r = Se[n];
    if (r && r.flags & 2) {
      if (e && r.id !== e.uid || {}.NODE_ENV !== "production" && Ts(t, r))
        continue;
      Se.splice(n, 1), n--, r.flags & 4 && (r.flags &= -2), r(), r.flags & 4 || (r.flags &= -2);
    }
  }
}
function Oo(e) {
  if ($t.length) {
    const t = [...new Set($t)].sort(
      (n, r) => In(n) - In(r)
    );
    if ($t.length = 0, xt) {
      xt.push(...t);
      return;
    }
    for (xt = t, {}.NODE_ENV !== "production" && (e = e || /* @__PURE__ */ new Map()), qt = 0; qt < xt.length; qt++) {
      const n = xt[qt];
      ({}).NODE_ENV !== "production" && Ts(e, n) || (n.flags & 4 && (n.flags &= -2), n.flags & 8 || n(), n.flags &= -2);
    }
    xt = null, qt = 0;
  }
}
const In = (e) => e.id == null ? e.flags & 2 ? -1 : 1 / 0 : e.id;
function Mo(e) {
  ({}).NODE_ENV !== "production" && (e = e || /* @__PURE__ */ new Map());
  const t = {}.NODE_ENV !== "production" ? (n) => Ts(e, n) : me;
  try {
    for (rt = 0; rt < Se.length; rt++) {
      const n = Se[rt];
      if (n && !(n.flags & 8)) {
        if ({}.NODE_ENV !== "production" && t(n))
          continue;
        n.flags & 4 && (n.flags &= -2), rn(
          n,
          n.i,
          n.i ? 15 : 14
        ), n.flags & 4 || (n.flags &= -2);
      }
    }
  } finally {
    for (; rt < Se.length; rt++) {
      const n = Se[rt];
      n && (n.flags &= -2);
    }
    rt = -1, Se.length = 0, Oo(e), nr = null, (Se.length || $t.length) && Mo(e);
  }
}
function Ts(e, t) {
  const n = e.get(t) || 0;
  if (n > Wl) {
    const r = t.i, s = r && pf(r.type);
    return Sn(
      `Maximum recursive updates exceeded${s ? ` in component <${s}>` : ""}. This means you have a reactive effect that is mutating its own dependencies and thus recursively triggering itself. Possible sources include component template, render function, updated hook or watcher source function.`,
      null,
      10
    ), !0;
  }
  return e.set(t, n + 1), !1;
}
let lt = !1;
const Qn = /* @__PURE__ */ new Map();
({}).NODE_ENV !== "production" && (Nn().__VUE_HMR_RUNTIME__ = {
  createRecord: jr(To),
  rerender: jr(_l),
  reload: jr($l)
});
const Wt = /* @__PURE__ */ new Map();
function Kl(e) {
  const t = e.type.__hmrId;
  let n = Wt.get(t);
  n || (To(t, e.type), n = Wt.get(t)), n.instances.add(e);
}
function ql(e) {
  Wt.get(e.type.__hmrId).instances.delete(e);
}
function To(e, t) {
  return Wt.has(e) ? !1 : (Wt.set(e, {
    initialDef: rr(t),
    instances: /* @__PURE__ */ new Set()
  }), !0);
}
function rr(e) {
  return hf(e) ? e.__vccOpts : e;
}
function _l(e, t) {
  const n = Wt.get(e);
  n && (n.initialDef.render = t, [...n.instances].forEach((r) => {
    t && (r.render = t, rr(r.type).render = t), r.renderCache = [], lt = !0, r.update(), lt = !1;
  }));
}
function $l(e, t) {
  const n = Wt.get(e);
  if (!n)
    return;
  t = rr(t), fi(n.initialDef, t);
  const r = [...n.instances];
  for (let s = 0; s < r.length; s++) {
    const i = r[s], o = rr(i.type);
    let f = Qn.get(o);
    f || (o !== n.initialDef && fi(o, t), Qn.set(o, f = /* @__PURE__ */ new Set())), f.add(i), i.appContext.propsCache.delete(i.type), i.appContext.emitsCache.delete(i.type), i.appContext.optionsCache.delete(i.type), i.ceReload ? (f.add(i), i.ceReload(t.styles), f.delete(i)) : i.parent ? yr(() => {
      lt = !0, i.parent.update(), lt = !1, f.delete(i);
    }) : i.appContext.reload ? i.appContext.reload() : typeof window < "u" ? window.location.reload() : console.warn(
      "[HMR] Root or manually mounted instance modified. Full reload required."
    ), i.root.ce && i !== i.root && i.root.ce._removeChildStyle(o);
  }
  Ro(() => {
    Qn.clear();
  });
}
function fi(e, t) {
  pe(e, t);
  for (const n in e)
    n !== "__file" && !(n in t) && delete e[n];
}
function jr(e) {
  return (t, n) => {
    try {
      return e(t, n);
    } catch (r) {
      console.error(r), console.warn(
        "[HMR] Something went wrong during Vue component hot-reload. Full reload required."
      );
    }
  };
}
let ot, dn = [], ns = !1;
function Bn(e, ...t) {
  ot ? ot.emit(e, ...t) : ns || dn.push({ event: e, args: t });
}
function Lo(e, t) {
  var n, r;
  ot = e, ot ? (ot.enabled = !0, dn.forEach(({ event: s, args: i }) => ot.emit(s, ...i)), dn = []) : /* handle late devtools injection - only do this if we are in an actual */ /* browser environment to avoid the timer handle stalling test runner exit */ /* (#4815) */ typeof window < "u" && // some envs mock window but not fully
  window.HTMLElement && // also exclude jsdom
  // eslint-disable-next-line no-restricted-syntax
  !((r = (n = window.navigator) == null ? void 0 : n.userAgent) != null && r.includes("jsdom")) ? ((t.__VUE_DEVTOOLS_HOOK_REPLAY__ = t.__VUE_DEVTOOLS_HOOK_REPLAY__ || []).push((i) => {
    Lo(i, t);
  }), setTimeout(() => {
    ot || (t.__VUE_DEVTOOLS_HOOK_REPLAY__ = null, ns = !0, dn = []);
  }, 3e3)) : (ns = !0, dn = []);
}
function ec(e, t) {
  Bn("app:init", e, t, {
    Fragment: Oe,
    Text: On,
    Comment: Je,
    Static: Fn
  });
}
function tc(e) {
  Bn("app:unmount", e);
}
const nc = /* @__PURE__ */ Ls(
  "component:added"
  /* COMPONENT_ADDED */
), ko = /* @__PURE__ */ Ls(
  "component:updated"
  /* COMPONENT_UPDATED */
), rc = /* @__PURE__ */ Ls(
  "component:removed"
  /* COMPONENT_REMOVED */
), sc = (e) => {
  ot && typeof ot.cleanupBuffer == "function" && // remove the component if it wasn't buffered
  !ot.cleanupBuffer(e) && rc(e);
};
/*! #__NO_SIDE_EFFECTS__ */
// @__NO_SIDE_EFFECTS__
function Ls(e) {
  return (t) => {
    Bn(
      e,
      t.appContext.app,
      t.uid,
      t.parent ? t.parent.uid : void 0,
      t
    );
  };
}
const ic = /* @__PURE__ */ Ho(
  "perf:start"
  /* PERFORMANCE_START */
), oc = /* @__PURE__ */ Ho(
  "perf:end"
  /* PERFORMANCE_END */
);
function Ho(e) {
  return (t, n, r) => {
    Bn(e, t.appContext.app, t.uid, t, n, r);
  };
}
function fc(e, t, n) {
  Bn(
    "component:emit",
    e.appContext.app,
    e,
    t,
    n
  );
}
let He = null, jo = null;
function sr(e) {
  const t = He;
  return He = e, jo = e && e.type.__scopeId || null, t;
}
function lc(e, t = He, n) {
  if (!t || e._n)
    return e;
  const r = (...s) => {
    r._d && mi(-1);
    const i = sr(t);
    let o;
    try {
      o = e(...s);
    } finally {
      sr(i), r._d && mi(1);
    }
    return {}.NODE_ENV !== "production" && ko(t), o;
  };
  return r._n = !0, r._c = !0, r._d = !0, r;
}
function zo(e) {
  el(e) && O("Do not use built-in directive ids as custom directive id: " + e);
}
function Lt(e, t, n, r) {
  const s = e.dirs, i = t && t.dirs;
  for (let o = 0; o < s.length; o++) {
    const f = s[o];
    i && (f.oldValue = i[o].value);
    let c = f.dir[r];
    c && (bt(), ct(c, n, 8, [
      e.el,
      f,
      e,
      t
    ]), It());
  }
}
const cc = Symbol("_vte"), uc = (e) => e.__isTeleport;
function ks(e, t) {
  e.shapeFlag & 6 && e.component ? (e.transition = t, ks(e.component.subTree, t)) : e.shapeFlag & 128 ? (e.ssContent.transition = t.clone(e.ssContent), e.ssFallback.transition = t.clone(e.ssFallback)) : e.transition = t;
}
function Jo(e) {
  e.ids = [e.ids[0] + e.ids[2]++ + "-", 0, 0];
}
const ac = /* @__PURE__ */ new WeakSet();
function ir(e, t, n, r, s = !1) {
  if (Q(e)) {
    e.forEach(
      (v, h) => ir(
        v,
        t && (Q(t) ? t[h] : t),
        n,
        r,
        s
      )
    );
    return;
  }
  if (mn(r) && !s) {
    r.shapeFlag & 512 && r.type.__asyncResolved && r.component.subTree.component && ir(e, t, n, r.component.subTree);
    return;
  }
  const i = r.shapeFlag & 4 ? Zs(r.component) : r.el, o = s ? null : i, { i: f, r: c } = e;
  if ({}.NODE_ENV !== "production" && !f) {
    O(
      "Missing ref owner context. ref cannot be used on hoisted vnodes. A vnode with ref must be created inside the render function."
    );
    return;
  }
  const A = t && t.r, u = f.refs === ie ? f.refs = {} : f.refs, l = f.setupState, p = q(l), g = l === ie ? () => !1 : (v) => ({}).NODE_ENV !== "production" && ($(p, v) && !de(p[v]) && O(
    `Template ref "${v}" used on a non-ref value. It will not work in the production build.`
  ), ac.has(p[v])) ? !1 : $(p, v);
  if (A != null && A !== c && (Ae(A) ? (u[A] = null, g(A) && (l[A] = null)) : de(A) && (A.value = null)), Z(c))
    rn(c, f, 12, [o, u]);
  else {
    const v = Ae(c), h = de(c);
    if (v || h) {
      const y = () => {
        if (e.f) {
          const x = v ? g(c) ? l[c] : u[c] : c.value;
          s ? Q(x) && Is(x, i) : Q(x) ? x.includes(i) || x.push(i) : v ? (u[c] = [i], g(c) && (l[c] = u[c])) : (c.value = [i], e.k && (u[e.k] = c.value));
        } else
          v ? (u[c] = o, g(c) && (l[c] = o)) : h ? (c.value = o, e.k && (u[e.k] = o)) : {}.NODE_ENV !== "production" && O("Invalid template ref type:", c, `(${typeof c})`);
      };
      o ? (y.id = -1, ke(y, n)) : y();
    } else
      ({}).NODE_ENV !== "production" && O("Invalid template ref type:", c, `(${typeof c})`);
  }
}
Nn().requestIdleCallback;
Nn().cancelIdleCallback;
const mn = (e) => !!e.type.__asyncLoader, Hs = (e) => e.type.__isKeepAlive;
function Ac(e, t) {
  Xo(e, "a", t);
}
function dc(e, t) {
  Xo(e, "da", t);
}
function Xo(e, t, n = ye) {
  const r = e.__wdc || (e.__wdc = () => {
    let s = n;
    for (; s; ) {
      if (s.isDeactivated)
        return;
      s = s.parent;
    }
    return e();
  });
  if (br(t, r, n), n) {
    let s = n.parent;
    for (; s && s.parent; )
      Hs(s.parent.vnode) && pc(r, t, n, s), s = s.parent;
  }
}
function pc(e, t, n, r) {
  const s = br(
    t,
    e,
    r,
    !0
    /* prepend */
  );
  Qo(() => {
    Is(r[t], s);
  }, n);
}
function br(e, t, n = ye, r = !1) {
  if (n) {
    const s = n[e] || (n[e] = []), i = t.__weh || (t.__weh = (...o) => {
      bt();
      const f = Mn(n), c = ct(t, n, e, o);
      return f(), It(), c;
    });
    return r ? s.unshift(i) : s.push(i), i;
  } else if ({}.NODE_ENV !== "production") {
    const s = Ht(Ms[e].replace(/ hook$/, ""));
    O(
      `${s} is called when there is no active component instance to be associated with. Lifecycle injection APIs can only be used during execution of setup(). If you are using async setup(), make sure to register lifecycle hooks before the first await statement.`
    );
  }
}
const wt = (e) => (t, n = ye) => {
  (!Pn || e === "sp") && br(e, (...r) => t(...r), n);
}, hc = wt("bm"), Rn = wt("m"), gc = wt(
  "bu"
), vc = wt("u"), mc = wt(
  "bum"
), Qo = wt("um"), Ec = wt(
  "sp"
), yc = wt("rtg"), bc = wt("rtc");
function Ic(e, t = ye) {
  br("ec", e, t);
}
const wc = Symbol.for("v-ndc");
function js(e, t, n, r) {
  let s;
  const i = n && n[r], o = Q(e);
  if (o || Ae(e)) {
    const f = o && Zt(e);
    let c = !1;
    f && (c = !Be(e), e = vr(e)), s = new Array(e.length);
    for (let A = 0, u = e.length; A < u; A++)
      s[A] = t(
        c ? we(e[A]) : e[A],
        A,
        void 0,
        i && i[A]
      );
  } else if (typeof e == "number") {
    ({}).NODE_ENV !== "production" && !Number.isInteger(e) && O(`The v-for range expect an integer value but got ${e}.`), s = new Array(e);
    for (let f = 0; f < e; f++)
      s[f] = t(f + 1, f, void 0, i && i[f]);
  } else if (ce(e))
    if (e[Symbol.iterator])
      s = Array.from(
        e,
        (f, c) => t(f, c, void 0, i && i[c])
      );
    else {
      const f = Object.keys(e);
      s = new Array(f.length);
      for (let c = 0, A = f.length; c < A; c++) {
        const u = f[c];
        s[c] = t(e[u], u, c, i && i[c]);
      }
    }
  else
    s = [];
  return n && (n[r] = s), s;
}
const rs = (e) => e ? Af(e) ? Zs(e) : rs(e.parent) : null, Gt = (
  // Move PURE marker to new line to workaround compiler discarding it
  // due to type annotation
  /* @__PURE__ */ pe(/* @__PURE__ */ Object.create(null), {
    $: (e) => e,
    $el: (e) => e.vnode.el,
    $data: (e) => e.data,
    $props: (e) => ({}).NODE_ENV !== "production" ? ft(e.props) : e.props,
    $attrs: (e) => ({}).NODE_ENV !== "production" ? ft(e.attrs) : e.attrs,
    $slots: (e) => ({}).NODE_ENV !== "production" ? ft(e.slots) : e.slots,
    $refs: (e) => ({}).NODE_ENV !== "production" ? ft(e.refs) : e.refs,
    $parent: (e) => rs(e.parent),
    $root: (e) => rs(e.root),
    $host: (e) => e.ce,
    $emit: (e) => e.emit,
    $options: (e) => Js(e),
    $forceUpdate: (e) => e.f || (e.f = () => {
      yr(e.update);
    }),
    $nextTick: (e) => e.n || (e.n = Vt.bind(e.proxy)),
    $watch: (e) => nu.bind(e)
  })
), zs = (e) => e === "_" || e === "$", zr = (e, t) => e !== ie && !e.__isScriptSetup && $(e, t), Yo = {
  get({ _: e }, t) {
    if (t === "__v_skip")
      return !0;
    const { ctx: n, setupState: r, data: s, props: i, accessCache: o, type: f, appContext: c } = e;
    if ({}.NODE_ENV !== "production" && t === "__isVue")
      return !0;
    let A;
    if (t[0] !== "$") {
      const g = o[t];
      if (g !== void 0)
        switch (g) {
          case 1:
            return r[t];
          case 2:
            return s[t];
          case 4:
            return n[t];
          case 3:
            return i[t];
        }
      else {
        if (zr(r, t))
          return o[t] = 1, r[t];
        if (s !== ie && $(s, t))
          return o[t] = 2, s[t];
        if (
          // only cache other properties when instance has declared (thus stable)
          // props
          (A = e.propsOptions[0]) && $(A, t)
        )
          return o[t] = 3, i[t];
        if (n !== ie && $(n, t))
          return o[t] = 4, n[t];
        ss && (o[t] = 0);
      }
    }
    const u = Gt[t];
    let l, p;
    if (u)
      return t === "$attrs" ? (ve(e.attrs, "get", ""), {}.NODE_ENV !== "production" && lr()) : {}.NODE_ENV !== "production" && t === "$slots" && ve(e, "get", t), u(e);
    if (
      // css module (injected by vue-loader)
      (l = f.__cssModules) && (l = l[t])
    )
      return l;
    if (n !== ie && $(n, t))
      return o[t] = 4, n[t];
    if (
      // global properties
      p = c.config.globalProperties, $(p, t)
    )
      return p[t];
    ({}).NODE_ENV !== "production" && He && (!Ae(t) || // #1091 avoid internal isRef/isVNode checks on component instance leading
    // to infinite warning loop
    t.indexOf("__v") !== 0) && (s !== ie && zs(t[0]) && $(s, t) ? O(
      `Property ${JSON.stringify(
        t
      )} must be accessed via $data because it starts with a reserved character ("$" or "_") and is not proxied on the render context.`
    ) : e === He && O(
      `Property ${JSON.stringify(t)} was accessed during render but is not defined on instance.`
    ));
  },
  set({ _: e }, t, n) {
    const { data: r, setupState: s, ctx: i } = e;
    return zr(s, t) ? (s[t] = n, !0) : {}.NODE_ENV !== "production" && s.__isScriptSetup && $(s, t) ? (O(`Cannot mutate <script setup> binding "${t}" from Options API.`), !1) : r !== ie && $(r, t) ? (r[t] = n, !0) : $(e.props, t) ? ({}.NODE_ENV !== "production" && O(`Attempting to mutate prop "${t}". Props are readonly.`), !1) : t[0] === "$" && t.slice(1) in e ? ({}.NODE_ENV !== "production" && O(
      `Attempting to mutate public property "${t}". Properties starting with $ are reserved and readonly.`
    ), !1) : ({}.NODE_ENV !== "production" && t in e.appContext.config.globalProperties ? Object.defineProperty(i, t, {
      enumerable: !0,
      configurable: !0,
      value: n
    }) : i[t] = n, !0);
  },
  has({
    _: { data: e, setupState: t, accessCache: n, ctx: r, appContext: s, propsOptions: i }
  }, o) {
    let f;
    return !!n[o] || e !== ie && $(e, o) || zr(t, o) || (f = i[0]) && $(f, o) || $(r, o) || $(Gt, o) || $(s.config.globalProperties, o);
  },
  defineProperty(e, t, n) {
    return n.get != null ? e._.accessCache[t] = 0 : $(n, "value") && this.set(e, t, n.value, null), Reflect.defineProperty(e, t, n);
  }
};
({}).NODE_ENV !== "production" && (Yo.ownKeys = (e) => (O(
  "Avoid app logic that relies on enumerating keys on a component instance. The keys will be empty in production mode to avoid performance overhead."
), Reflect.ownKeys(e)));
function Pc(e) {
  const t = {};
  return Object.defineProperty(t, "_", {
    configurable: !0,
    enumerable: !1,
    get: () => e
  }), Object.keys(Gt).forEach((n) => {
    Object.defineProperty(t, n, {
      configurable: !0,
      enumerable: !1,
      get: () => Gt[n](e),
      // intercepted by the proxy so no need for implementation,
      // but needed to prevent set errors
      set: me
    });
  }), t;
}
function Cc(e) {
  const {
    ctx: t,
    propsOptions: [n]
  } = e;
  n && Object.keys(n).forEach((r) => {
    Object.defineProperty(t, r, {
      enumerable: !0,
      configurable: !0,
      get: () => e.props[r],
      set: me
    });
  });
}
function xc(e) {
  const { ctx: t, setupState: n } = e;
  Object.keys(q(n)).forEach((r) => {
    if (!n.__isScriptSetup) {
      if (zs(r[0])) {
        O(
          `setup() return property ${JSON.stringify(
            r
          )} should not start with "$" or "_" which are reserved prefixes for Vue internals.`
        );
        return;
      }
      Object.defineProperty(t, r, {
        enumerable: !0,
        configurable: !0,
        get: () => n[r],
        set: me
      });
    }
  });
}
function li(e) {
  return Q(e) ? e.reduce(
    (t, n) => (t[n] = null, t),
    {}
  ) : e;
}
function Dc() {
  const e = /* @__PURE__ */ Object.create(null);
  return (t, n) => {
    e[n] ? O(`${t} property "${n}" is already defined in ${e[n]}.`) : e[n] = t;
  };
}
let ss = !0;
function Nc(e) {
  const t = Js(e), n = e.proxy, r = e.ctx;
  ss = !1, t.beforeCreate && ci(t.beforeCreate, e, "bc");
  const {
    // state
    data: s,
    computed: i,
    methods: o,
    watch: f,
    provide: c,
    inject: A,
    // lifecycle
    created: u,
    beforeMount: l,
    mounted: p,
    beforeUpdate: g,
    updated: v,
    activated: h,
    deactivated: y,
    beforeDestroy: x,
    beforeUnmount: H,
    destroyed: T,
    unmounted: G,
    render: R,
    renderTracked: U,
    renderTriggered: re,
    errorCaptured: ee,
    serverPrefetch: oe,
    // public API
    expose: j,
    inheritAttrs: J,
    // assets
    components: fe,
    directives: Tt,
    filters: F
  } = t, k = {}.NODE_ENV !== "production" ? Dc() : null;
  if ({}.NODE_ENV !== "production") {
    const [D] = e.propsOptions;
    if (D)
      for (const I in D)
        k("Props", I);
  }
  if (A && Sc(A, r, k), o)
    for (const D in o) {
      const I = o[D];
      Z(I) ? ({}.NODE_ENV !== "production" ? Object.defineProperty(r, D, {
        value: I.bind(n),
        configurable: !0,
        enumerable: !0,
        writable: !0
      }) : r[D] = I.bind(n), {}.NODE_ENV !== "production" && k("Methods", D)) : {}.NODE_ENV !== "production" && O(
        `Method "${D}" has type "${typeof I}" in the component definition. Did you reference the function correctly?`
      );
    }
  if (s) {
    ({}).NODE_ENV !== "production" && !Z(s) && O(
      "The data option must be a function. Plain object usage is no longer supported."
    );
    const D = s.call(n, n);
    if ({}.NODE_ENV !== "production" && ws(D) && O(
      "data() returned a Promise - note data() cannot be async; If you intend to perform data fetching before component renders, use async setup() + <Suspense>."
    ), !ce(D))
      ({}).NODE_ENV !== "production" && O("data() should return an object.");
    else if (e.data = Rs(D), {}.NODE_ENV !== "production")
      for (const I in D)
        k("Data", I), zs(I[0]) || Object.defineProperty(r, I, {
          configurable: !0,
          enumerable: !0,
          get: () => D[I],
          set: me
        });
  }
  if (ss = !0, i)
    for (const D in i) {
      const I = i[D], X = Z(I) ? I.bind(n, n) : Z(I.get) ? I.get.bind(n, n) : me;
      ({}).NODE_ENV !== "production" && X === me && O(`Computed property "${D}" has no getter.`);
      const V = !Z(I) && Z(I.set) ? I.set.bind(n) : {}.NODE_ENV !== "production" ? () => {
        O(
          `Write operation failed: computed property "${D}" is readonly.`
        );
      } : me, te = Ge({
        get: X,
        set: V
      });
      Object.defineProperty(r, D, {
        enumerable: !0,
        configurable: !0,
        get: () => te.value,
        set: (le) => te.value = le
      }), {}.NODE_ENV !== "production" && k("Computed", D);
    }
  if (f)
    for (const D in f)
      Zo(f[D], r, n, D);
  if (c) {
    const D = Z(c) ? c.call(n) : c;
    Reflect.ownKeys(D).forEach((I) => {
      Lc(I, D[I]);
    });
  }
  u && ci(u, e, "c");
  function N(D, I) {
    Q(I) ? I.forEach((X) => D(X.bind(n))) : I && D(I.bind(n));
  }
  if (N(hc, l), N(Rn, p), N(gc, g), N(vc, v), N(Ac, h), N(dc, y), N(Ic, ee), N(bc, U), N(yc, re), N(mc, H), N(Qo, G), N(Ec, oe), Q(j))
    if (j.length) {
      const D = e.exposed || (e.exposed = {});
      j.forEach((I) => {
        Object.defineProperty(D, I, {
          get: () => n[I],
          set: (X) => n[I] = X
        });
      });
    } else
      e.exposed || (e.exposed = {});
  R && e.render === me && (e.render = R), J != null && (e.inheritAttrs = J), fe && (e.components = fe), Tt && (e.directives = Tt), oe && Jo(e);
}
function Sc(e, t, n = me) {
  Q(e) && (e = is(e));
  for (const r in e) {
    const s = e[r];
    let i;
    ce(s) ? "default" in s ? i = Yn(
      s.from || r,
      s.default,
      !0
    ) : i = Yn(s.from || r) : i = Yn(s), de(i) ? Object.defineProperty(t, r, {
      enumerable: !0,
      configurable: !0,
      get: () => i.value,
      set: (o) => i.value = o
    }) : t[r] = i, {}.NODE_ENV !== "production" && n("Inject", r);
  }
}
function ci(e, t, n) {
  ct(
    Q(e) ? e.map((r) => r.bind(t.proxy)) : e.bind(t.proxy),
    t,
    n
  );
}
function Zo(e, t, n, r) {
  let s = r.includes(".") ? rf(n, r) : () => n[r];
  if (Ae(e)) {
    const i = t[e];
    Z(i) ? Ue(s, i) : {}.NODE_ENV !== "production" && O(`Invalid watch handler specified by key "${e}"`, i);
  } else if (Z(e))
    Ue(s, e.bind(n));
  else if (ce(e))
    if (Q(e))
      e.forEach((i) => Zo(i, t, n, r));
    else {
      const i = Z(e.handler) ? e.handler.bind(n) : t[e.handler];
      Z(i) ? Ue(s, i, e) : {}.NODE_ENV !== "production" && O(`Invalid watch handler specified by key "${e.handler}"`, i);
    }
  else
    ({}).NODE_ENV !== "production" && O(`Invalid watch option: "${r}"`, e);
}
function Js(e) {
  const t = e.type, { mixins: n, extends: r } = t, {
    mixins: s,
    optionsCache: i,
    config: { optionMergeStrategies: o }
  } = e.appContext, f = i.get(t);
  let c;
  return f ? c = f : !s.length && !n && !r ? c = t : (c = {}, s.length && s.forEach(
    (A) => or(c, A, o, !0)
  ), or(c, t, o)), ce(t) && i.set(t, c), c;
}
function or(e, t, n, r = !1) {
  const { mixins: s, extends: i } = t;
  i && or(e, i, n, !0), s && s.forEach(
    (o) => or(e, o, n, !0)
  );
  for (const o in t)
    if (r && o === "expose")
      ({}).NODE_ENV !== "production" && O(
        '"expose" option is ignored when declared in mixins or extends. It should only be declared in the base component itself.'
      );
    else {
      const f = Bc[o] || n && n[o];
      e[o] = f ? f(e[o], t[o]) : t[o];
    }
  return e;
}
const Bc = {
  data: ui,
  props: ai,
  emits: ai,
  // objects
  methods: pn,
  computed: pn,
  // lifecycle
  beforeCreate: De,
  created: De,
  beforeMount: De,
  mounted: De,
  beforeUpdate: De,
  updated: De,
  beforeDestroy: De,
  beforeUnmount: De,
  destroyed: De,
  unmounted: De,
  activated: De,
  deactivated: De,
  errorCaptured: De,
  serverPrefetch: De,
  // assets
  components: pn,
  directives: pn,
  // watch
  watch: Oc,
  // provide / inject
  provide: ui,
  inject: Rc
};
function ui(e, t) {
  return t ? e ? function() {
    return pe(
      Z(e) ? e.call(this, this) : e,
      Z(t) ? t.call(this, this) : t
    );
  } : t : e;
}
function Rc(e, t) {
  return pn(is(e), is(t));
}
function is(e) {
  if (Q(e)) {
    const t = {};
    for (let n = 0; n < e.length; n++)
      t[e[n]] = e[n];
    return t;
  }
  return e;
}
function De(e, t) {
  return e ? [...new Set([].concat(e, t))] : t;
}
function pn(e, t) {
  return e ? pe(/* @__PURE__ */ Object.create(null), e, t) : t;
}
function ai(e, t) {
  return e ? Q(e) && Q(t) ? [.../* @__PURE__ */ new Set([...e, ...t])] : pe(
    /* @__PURE__ */ Object.create(null),
    li(e),
    li(t ?? {})
  ) : t;
}
function Oc(e, t) {
  if (!e)
    return t;
  if (!t)
    return e;
  const n = pe(/* @__PURE__ */ Object.create(null), e);
  for (const r in t)
    n[r] = De(e[r], t[r]);
  return n;
}
function Fo() {
  return {
    app: null,
    config: {
      isNativeTag: _f,
      performance: !1,
      globalProperties: {},
      optionMergeStrategies: {},
      errorHandler: void 0,
      warnHandler: void 0,
      compilerOptions: {}
    },
    mixins: [],
    components: {},
    directives: {},
    provides: /* @__PURE__ */ Object.create(null),
    optionsCache: /* @__PURE__ */ new WeakMap(),
    propsCache: /* @__PURE__ */ new WeakMap(),
    emitsCache: /* @__PURE__ */ new WeakMap()
  };
}
let Mc = 0;
function Tc(e, t) {
  return function(r, s = null) {
    Z(r) || (r = pe({}, r)), s != null && !ce(s) && ({}.NODE_ENV !== "production" && O("root props passed to app.mount() must be an object."), s = null);
    const i = Fo(), o = /* @__PURE__ */ new WeakSet(), f = [];
    let c = !1;
    const A = i.app = {
      _uid: Mc++,
      _component: r,
      _props: s,
      _container: null,
      _context: i,
      _instance: null,
      version: wi,
      get config() {
        return i.config;
      },
      set config(u) {
        ({}).NODE_ENV !== "production" && O(
          "app.config cannot be replaced. Modify individual options instead."
        );
      },
      use(u, ...l) {
        return o.has(u) ? {}.NODE_ENV !== "production" && O("Plugin has already been applied to target app.") : u && Z(u.install) ? (o.add(u), u.install(A, ...l)) : Z(u) ? (o.add(u), u(A, ...l)) : {}.NODE_ENV !== "production" && O(
          'A plugin must either be a function or an object with an "install" function.'
        ), A;
      },
      mixin(u) {
        return i.mixins.includes(u) ? {}.NODE_ENV !== "production" && O(
          "Mixin has already been applied to target app" + (u.name ? `: ${u.name}` : "")
        ) : i.mixins.push(u), A;
      },
      component(u, l) {
        return {}.NODE_ENV !== "production" && us(u, i.config), l ? ({}.NODE_ENV !== "production" && i.components[u] && O(`Component "${u}" has already been registered in target app.`), i.components[u] = l, A) : i.components[u];
      },
      directive(u, l) {
        return {}.NODE_ENV !== "production" && zo(u), l ? ({}.NODE_ENV !== "production" && i.directives[u] && O(`Directive "${u}" has already been registered in target app.`), i.directives[u] = l, A) : i.directives[u];
      },
      mount(u, l, p) {
        if (c)
          ({}).NODE_ENV !== "production" && O(
            "App has already been mounted.\nIf you want to remount the same app, move your app creation logic into a factory function and create fresh app instances for each mount - e.g. `const createMyApp = () => createApp(App)`"
          );
        else {
          ({}).NODE_ENV !== "production" && u.__vue_app__ && O(
            "There is already an app instance mounted on the host container.\n If you want to mount another app on the same host container, you need to unmount the previous app by calling `app.unmount()` first."
          );
          const g = A._ceVNode || vt(r, s);
          return g.appContext = i, p === !0 ? p = "svg" : p === !1 && (p = void 0), {}.NODE_ENV !== "production" && (i.reload = () => {
            e(
              Ot(g),
              u,
              p
            );
          }), l && t ? t(g, u) : e(g, u, p), c = !0, A._container = u, u.__vue_app__ = A, {}.NODE_ENV !== "production" && (A._instance = g.component, ec(A, wi)), Zs(g.component);
        }
      },
      onUnmount(u) {
        ({}).NODE_ENV !== "production" && typeof u != "function" && O(
          `Expected function as first argument to app.onUnmount(), but got ${typeof u}`
        ), f.push(u);
      },
      unmount() {
        c ? (ct(
          f,
          A._instance,
          16
        ), e(null, A._container), {}.NODE_ENV !== "production" && (A._instance = null, tc(A)), delete A._container.__vue_app__) : {}.NODE_ENV !== "production" && O("Cannot unmount an app that is not mounted.");
      },
      provide(u, l) {
        return {}.NODE_ENV !== "production" && u in i.provides && O(
          `App already provides property with key "${String(u)}". It will be overwritten with the new value.`
        ), i.provides[u] = l, A;
      },
      runWithContext(u) {
        const l = en;
        en = A;
        try {
          return u();
        } finally {
          en = l;
        }
      }
    };
    return A;
  };
}
let en = null;
function Lc(e, t) {
  if (!ye)
    ({}).NODE_ENV !== "production" && O("provide() can only be used inside setup().");
  else {
    let n = ye.provides;
    const r = ye.parent && ye.parent.provides;
    r === n && (n = ye.provides = Object.create(r)), n[e] = t;
  }
}
function Yn(e, t, n = !1) {
  const r = ye || He;
  if (r || en) {
    const s = en ? en._context.provides : r ? r.parent == null ? r.vnode.appContext && r.vnode.appContext.provides : r.parent.provides : void 0;
    if (s && e in s)
      return s[e];
    if (arguments.length > 1)
      return n && Z(t) ? t.call(r && r.proxy) : t;
    ({}).NODE_ENV !== "production" && O(`injection "${String(e)}" not found.`);
  } else
    ({}).NODE_ENV !== "production" && O("inject() can only be used inside setup() or functional components.");
}
const Go = {}, Vo = () => Object.create(Go), Wo = (e) => Object.getPrototypeOf(e) === Go;
function kc(e, t, n, r = !1) {
  const s = {}, i = Vo();
  e.propsDefaults = /* @__PURE__ */ Object.create(null), Uo(e, t, s, i);
  for (const o in e.propsOptions[0])
    o in s || (s[o] = void 0);
  ({}).NODE_ENV !== "production" && qo(t || {}, s, e), n ? e.props = r ? s : Ll(s) : e.type.props ? e.props = s : e.props = i, e.attrs = i;
}
function Hc(e) {
  for (; e; ) {
    if (e.type.__hmrId)
      return !0;
    e = e.parent;
  }
}
function jc(e, t, n, r) {
  const {
    props: s,
    attrs: i,
    vnode: { patchFlag: o }
  } = e, f = q(s), [c] = e.propsOptions;
  let A = !1;
  if (
    // always force full diff in dev
    // - #1942 if hmr is enabled with sfc component
    // - vite#872 non-sfc component used by sfc component
    !({}.NODE_ENV !== "production" && Hc(e)) && (r || o > 0) && !(o & 16)
  ) {
    if (o & 8) {
      const u = e.vnode.dynamicProps;
      for (let l = 0; l < u.length; l++) {
        let p = u[l];
        if (Ir(e.emitsOptions, p))
          continue;
        const g = t[p];
        if (c)
          if ($(i, p))
            g !== i[p] && (i[p] = g, A = !0);
          else {
            const v = Ve(p);
            s[v] = os(
              c,
              f,
              v,
              g,
              e,
              !1
            );
          }
        else
          g !== i[p] && (i[p] = g, A = !0);
      }
    }
  } else {
    Uo(e, t, s, i) && (A = !0);
    let u;
    for (const l in f)
      (!t || // for camelCase
      !$(t, l) && // it's possible the original props was passed in as kebab-case
      // and converted to camelCase (#955)
      ((u = Rt(l)) === l || !$(t, u))) && (c ? n && // for camelCase
      (n[l] !== void 0 || // for kebab-case
      n[u] !== void 0) && (s[l] = os(
        c,
        f,
        l,
        void 0,
        e,
        !0
      )) : delete s[l]);
    if (i !== f)
      for (const l in i)
        (!t || !$(t, l)) && (delete i[l], A = !0);
  }
  A && it(e.attrs, "set", ""), {}.NODE_ENV !== "production" && qo(t || {}, s, e);
}
function Uo(e, t, n, r) {
  const [s, i] = e.propsOptions;
  let o = !1, f;
  if (t)
    for (let c in t) {
      if (hn(c))
        continue;
      const A = t[c];
      let u;
      s && $(s, u = Ve(c)) ? !i || !i.includes(u) ? n[u] = A : (f || (f = {}))[u] = A : Ir(e.emitsOptions, c) || (!(c in r) || A !== r[c]) && (r[c] = A, o = !0);
    }
  if (i) {
    const c = q(n), A = f || ie;
    for (let u = 0; u < i.length; u++) {
      const l = i[u];
      n[l] = os(
        s,
        c,
        l,
        A[l],
        e,
        !$(A, l)
      );
    }
  }
  return o;
}
function os(e, t, n, r, s, i) {
  const o = e[n];
  if (o != null) {
    const f = $(o, "default");
    if (f && r === void 0) {
      const c = o.default;
      if (o.type !== Function && !o.skipFactory && Z(c)) {
        const { propsDefaults: A } = s;
        if (n in A)
          r = A[n];
        else {
          const u = Mn(s);
          r = A[n] = c.call(
            null,
            t
          ), u();
        }
      } else
        r = c;
      s.ce && s.ce._setProp(n, r);
    }
    o[
      0
      /* shouldCast */
    ] && (i && !f ? r = !1 : o[
      1
      /* shouldCastTrue */
    ] && (r === "" || r === Rt(n)) && (r = !0));
  }
  return r;
}
const zc = /* @__PURE__ */ new WeakMap();
function Ko(e, t, n = !1) {
  const r = n ? zc : t.propsCache, s = r.get(e);
  if (s)
    return s;
  const i = e.props, o = {}, f = [];
  let c = !1;
  if (!Z(e)) {
    const u = (l) => {
      c = !0;
      const [p, g] = Ko(l, t, !0);
      pe(o, p), g && f.push(...g);
    };
    !n && t.mixins.length && t.mixins.forEach(u), e.extends && u(e.extends), e.mixins && e.mixins.forEach(u);
  }
  if (!i && !c)
    return ce(e) && r.set(e, _t), _t;
  if (Q(i))
    for (let u = 0; u < i.length; u++) {
      ({}).NODE_ENV !== "production" && !Ae(i[u]) && O("props must be strings when using array syntax.", i[u]);
      const l = Ve(i[u]);
      Ai(l) && (o[l] = ie);
    }
  else if (i) {
    ({}).NODE_ENV !== "production" && !ce(i) && O("invalid props options", i);
    for (const u in i) {
      const l = Ve(u);
      if (Ai(l)) {
        const p = i[u], g = o[l] = Q(p) || Z(p) ? { type: p } : pe({}, p), v = g.type;
        let h = !1, y = !0;
        if (Q(v))
          for (let x = 0; x < v.length; ++x) {
            const H = v[x], T = Z(H) && H.name;
            if (T === "Boolean") {
              h = !0;
              break;
            } else
              T === "String" && (y = !1);
          }
        else
          h = Z(v) && v.name === "Boolean";
        g[
          0
          /* shouldCast */
        ] = h, g[
          1
          /* shouldCastTrue */
        ] = y, (h || $(g, "default")) && f.push(l);
      }
    }
  }
  const A = [o, f];
  return ce(e) && r.set(e, A), A;
}
function Ai(e) {
  return e[0] !== "$" && !hn(e) ? !0 : ({}.NODE_ENV !== "production" && O(`Invalid prop name: "${e}" is a reserved property.`), !1);
}
function Jc(e) {
  return e === null ? "null" : typeof e == "function" ? e.name || "" : typeof e == "object" && e.constructor && e.constructor.name || "";
}
function qo(e, t, n) {
  const r = q(t), s = n.propsOptions[0], i = Object.keys(e).map((o) => Ve(o));
  for (const o in s) {
    let f = s[o];
    f != null && Xc(
      o,
      r[o],
      f,
      {}.NODE_ENV !== "production" ? ft(r) : r,
      !i.includes(o)
    );
  }
}
function Xc(e, t, n, r, s) {
  const { type: i, required: o, validator: f, skipCheck: c } = n;
  if (o && s) {
    O('Missing required prop: "' + e + '"');
    return;
  }
  if (!(t == null && !o)) {
    if (i != null && i !== !0 && !c) {
      let A = !1;
      const u = Q(i) ? i : [i], l = [];
      for (let p = 0; p < u.length && !A; p++) {
        const { valid: g, expectedType: v } = Yc(t, u[p]);
        l.push(v || ""), A = g;
      }
      if (!A) {
        O(Zc(e, t, l));
        return;
      }
    }
    f && !f(t, r) && O('Invalid prop: custom validator check failed for prop "' + e + '".');
  }
}
const Qc = /* @__PURE__ */ yt(
  "String,Number,Boolean,Function,Symbol,BigInt"
);
function Yc(e, t) {
  let n;
  const r = Jc(t);
  if (r === "null")
    n = e === null;
  else if (Qc(r)) {
    const s = typeof e;
    n = s === r.toLowerCase(), !n && s === "object" && (n = e instanceof t);
  } else
    r === "Object" ? n = ce(e) : r === "Array" ? n = Q(e) : n = e instanceof t;
  return {
    valid: n,
    expectedType: r
  };
}
function Zc(e, t, n) {
  if (n.length === 0)
    return `Prop type [] for prop "${e}" won't match anything. Did you mean to use type Array instead?`;
  let r = `Invalid prop: type check failed for prop "${e}". Expected ${n.map(gr).join(" | ")}`;
  const s = n[0], i = Ps(t), o = di(t, s), f = di(t, i);
  return n.length === 1 && pi(s) && !Fc(s, i) && (r += ` with value ${o}`), r += `, got ${i} `, pi(i) && (r += `with value ${f}.`), r;
}
function di(e, t) {
  return t === "String" ? `"${e}"` : t === "Number" ? `${Number(e)}` : `${e}`;
}
function pi(e) {
  return ["string", "number", "boolean"].some((n) => e.toLowerCase() === n);
}
function Fc(...e) {
  return e.some((t) => t.toLowerCase() === "boolean");
}
const _o = (e) => e[0] === "_" || e === "$stable", Xs = (e) => Q(e) ? e.map(Fe) : [Fe(e)], Gc = (e, t, n) => {
  if (t._n)
    return t;
  const r = lc((...s) => ({}.NODE_ENV !== "production" && ye && (!n || n.root === ye.root) && O(
    `Slot "${e}" invoked outside of the render function: this will not track dependencies used in the slot. Invoke the slot function inside the render function instead.`
  ), Xs(t(...s))), n);
  return r._c = !1, r;
}, $o = (e, t, n) => {
  const r = e._ctx;
  for (const s in e) {
    if (_o(s))
      continue;
    const i = e[s];
    if (Z(i))
      t[s] = Gc(s, i, r);
    else if (i != null) {
      ({}).NODE_ENV !== "production" && O(
        `Non-function value encountered for slot "${s}". Prefer function slots for better performance.`
      );
      const o = Xs(i);
      t[s] = () => o;
    }
  }
}, ef = (e, t) => {
  ({}).NODE_ENV !== "production" && !Hs(e.vnode) && O(
    "Non-function value encountered for default slot. Prefer function slots for better performance."
  );
  const n = Xs(t);
  e.slots.default = () => n;
}, fs = (e, t, n) => {
  for (const r in t)
    (n || r !== "_") && (e[r] = t[r]);
}, Vc = (e, t, n) => {
  const r = e.slots = Vo();
  if (e.vnode.shapeFlag & 32) {
    const s = t._;
    s ? (fs(r, t, n), n && $n(r, "_", s, !0)) : $o(t, r);
  } else
    t && ef(e, t);
}, Wc = (e, t, n) => {
  const { vnode: r, slots: s } = e;
  let i = !0, o = ie;
  if (r.shapeFlag & 32) {
    const f = t._;
    f ? {}.NODE_ENV !== "production" && lt ? (fs(s, t, n), it(e, "set", "$slots")) : n && f === 1 ? i = !1 : fs(s, t, n) : (i = !t.$stable, $o(t, s)), o = t;
  } else
    t && (ef(e, t), o = { default: 1 });
  if (i)
    for (const f in s)
      !_o(f) && o[f] == null && delete s[f];
};
let un, St;
function At(e, t) {
  e.appContext.config.performance && fr() && St.mark(`vue-${t}-${e.uid}`), {}.NODE_ENV !== "production" && ic(e, t, fr() ? St.now() : Date.now());
}
function dt(e, t) {
  if (e.appContext.config.performance && fr()) {
    const n = `vue-${t}-${e.uid}`, r = n + ":end";
    St.mark(r), St.measure(
      `<${Cr(e, e.type)}> ${t}`,
      n,
      r
    ), St.clearMarks(n), St.clearMarks(r);
  }
  ({}).NODE_ENV !== "production" && oc(e, t, fr() ? St.now() : Date.now());
}
function fr() {
  return un !== void 0 || (typeof window < "u" && window.performance ? (un = !0, St = window.performance) : un = !1), un;
}
function Uc() {
  const e = [];
  if ({}.NODE_ENV !== "production" && e.length) {
    const t = e.length > 1;
    console.warn(
      `Feature flag${t ? "s" : ""} ${e.join(", ")} ${t ? "are" : "is"} not explicitly defined. You are running the esm-bundler build of Vue, which expects these compile-time feature flags to be globally injected via the bundler config in order to get better tree-shaking in the production bundle.

For more details, see https://link.vuejs.org/feature-flags.`
    );
  }
}
const ke = cu;
function Kc(e) {
  return qc(e);
}
function qc(e, t) {
  Uc();
  const n = Nn();
  n.__VUE__ = !0, {}.NODE_ENV !== "production" && Lo(n.__VUE_DEVTOOLS_GLOBAL_HOOK__, n);
  const {
    insert: r,
    remove: s,
    patchProp: i,
    createElement: o,
    createText: f,
    createComment: c,
    setText: A,
    setElementText: u,
    parentNode: l,
    nextSibling: p,
    setScopeId: g = me,
    insertStaticContent: v
  } = e, h = (a, d, E, P = null, b = null, w = null, M = void 0, B = null, S = {}.NODE_ENV !== "production" && lt ? !1 : !!d.dynamicChildren) => {
    if (a === d)
      return;
    a && !an(a, d) && (P = $e(a), ue(a, b, w, !0), a = null), d.patchFlag === -2 && (S = !1, d.dynamicChildren = null);
    const { type: C, ref: Y, shapeFlag: L } = d;
    switch (C) {
      case On:
        y(a, d, E, P);
        break;
      case Je:
        x(a, d, E, P);
        break;
      case Fn:
        a == null ? H(d, E, P, M) : {}.NODE_ENV !== "production" && T(a, d, E, M);
        break;
      case Oe:
        Tt(
          a,
          d,
          E,
          P,
          b,
          w,
          M,
          B,
          S
        );
        break;
      default:
        L & 1 ? U(
          a,
          d,
          E,
          P,
          b,
          w,
          M,
          B,
          S
        ) : L & 6 ? F(
          a,
          d,
          E,
          P,
          b,
          w,
          M,
          B,
          S
        ) : L & 64 || L & 128 ? C.process(
          a,
          d,
          E,
          P,
          b,
          w,
          M,
          B,
          S,
          Ce
        ) : {}.NODE_ENV !== "production" && O("Invalid VNode type:", C, `(${typeof C})`);
    }
    Y != null && b && ir(Y, a && a.ref, w, d || a, !d);
  }, y = (a, d, E, P) => {
    if (a == null)
      r(
        d.el = f(d.children),
        E,
        P
      );
    else {
      const b = d.el = a.el;
      d.children !== a.children && A(b, d.children);
    }
  }, x = (a, d, E, P) => {
    a == null ? r(
      d.el = c(d.children || ""),
      E,
      P
    ) : d.el = a.el;
  }, H = (a, d, E, P) => {
    [a.el, a.anchor] = v(
      a.children,
      d,
      E,
      P,
      a.el,
      a.anchor
    );
  }, T = (a, d, E, P) => {
    if (d.children !== a.children) {
      const b = p(a.anchor);
      R(a), [d.el, d.anchor] = v(
        d.children,
        E,
        b,
        P
      );
    } else
      d.el = a.el, d.anchor = a.anchor;
  }, G = ({ el: a, anchor: d }, E, P) => {
    let b;
    for (; a && a !== d; )
      b = p(a), r(a, E, P), a = b;
    r(d, E, P);
  }, R = ({ el: a, anchor: d }) => {
    let E;
    for (; a && a !== d; )
      E = p(a), s(a), a = E;
    s(d);
  }, U = (a, d, E, P, b, w, M, B, S) => {
    d.type === "svg" ? M = "svg" : d.type === "math" && (M = "mathml"), a == null ? re(
      d,
      E,
      P,
      b,
      w,
      M,
      B,
      S
    ) : j(
      a,
      d,
      b,
      w,
      M,
      B,
      S
    );
  }, re = (a, d, E, P, b, w, M, B) => {
    let S, C;
    const { props: Y, shapeFlag: L, transition: z, dirs: W } = a;
    if (S = a.el = o(
      a.type,
      w,
      Y && Y.is,
      Y
    ), L & 8 ? u(S, a.children) : L & 16 && oe(
      a.children,
      S,
      null,
      P,
      b,
      Jr(a, w),
      M,
      B
    ), W && Lt(a, null, P, "created"), ee(S, a, a.scopeId, M, P), Y) {
      for (const ae in Y)
        ae !== "value" && !hn(ae) && i(S, ae, null, Y[ae], w, P);
      "value" in Y && i(S, "value", null, Y.value, w), (C = Y.onVnodeBeforeMount) && tt(C, P, a);
    }
    ({}).NODE_ENV !== "production" && ($n(S, "__vnode", a, !0), $n(S, "__vueParentComponent", P, !0)), W && Lt(a, null, P, "beforeMount");
    const _ = _c(b, z);
    _ && z.beforeEnter(S), r(S, d, E), ((C = Y && Y.onVnodeMounted) || _ || W) && ke(() => {
      C && tt(C, P, a), _ && z.enter(S), W && Lt(a, null, P, "mounted");
    }, b);
  }, ee = (a, d, E, P, b) => {
    if (E && g(a, E), P)
      for (let w = 0; w < P.length; w++)
        g(a, P[w]);
    if (b) {
      let w = b.subTree;
      if ({}.NODE_ENV !== "production" && w.patchFlag > 0 && w.patchFlag & 2048 && (w = Qs(w.children) || w), d === w || ff(w.type) && (w.ssContent === d || w.ssFallback === d)) {
        const M = b.vnode;
        ee(
          a,
          M,
          M.scopeId,
          M.slotScopeIds,
          b.parent
        );
      }
    }
  }, oe = (a, d, E, P, b, w, M, B, S = 0) => {
    for (let C = S; C < a.length; C++) {
      const Y = a[C] = B ? Dt(a[C]) : Fe(a[C]);
      h(
        null,
        Y,
        d,
        E,
        P,
        b,
        w,
        M,
        B
      );
    }
  }, j = (a, d, E, P, b, w, M) => {
    const B = d.el = a.el;
    ({}).NODE_ENV !== "production" && (B.__vnode = d);
    let { patchFlag: S, dynamicChildren: C, dirs: Y } = d;
    S |= a.patchFlag & 16;
    const L = a.props || ie, z = d.props || ie;
    let W;
    if (E && kt(E, !1), (W = z.onVnodeBeforeUpdate) && tt(W, E, d, a), Y && Lt(d, a, E, "beforeUpdate"), E && kt(E, !0), {}.NODE_ENV !== "production" && lt && (S = 0, M = !1, C = null), (L.innerHTML && z.innerHTML == null || L.textContent && z.textContent == null) && u(B, ""), C ? (J(
      a.dynamicChildren,
      C,
      B,
      E,
      P,
      Jr(d, b),
      w
    ), {}.NODE_ENV !== "production" && Zn(a, d)) : M || X(
      a,
      d,
      B,
      null,
      E,
      P,
      Jr(d, b),
      w,
      !1
    ), S > 0) {
      if (S & 16)
        fe(B, L, z, E, b);
      else if (S & 2 && L.class !== z.class && i(B, "class", null, z.class, b), S & 4 && i(B, "style", L.style, z.style, b), S & 8) {
        const _ = d.dynamicProps;
        for (let ae = 0; ae < _.length; ae++) {
          const se = _[ae], Te = L[se], Ie = z[se];
          (Ie !== Te || se === "value") && i(B, se, Te, Ie, b, E);
        }
      }
      S & 1 && a.children !== d.children && u(B, d.children);
    } else
      !M && C == null && fe(B, L, z, E, b);
    ((W = z.onVnodeUpdated) || Y) && ke(() => {
      W && tt(W, E, d, a), Y && Lt(d, a, E, "updated");
    }, P);
  }, J = (a, d, E, P, b, w, M) => {
    for (let B = 0; B < d.length; B++) {
      const S = a[B], C = d[B], Y = (
        // oldVNode may be an errored async setup() component inside Suspense
        // which will not have a mounted element
        S.el && // - In the case of a Fragment, we need to provide the actual parent
        // of the Fragment itself so it can move its children.
        (S.type === Oe || // - In the case of different nodes, there is going to be a replacement
        // which also requires the correct parent container
        !an(S, C) || // - In the case of a component, it could contain anything.
        S.shapeFlag & 70) ? l(S.el) : (
          // In other cases, the parent container is not actually used so we
          // just pass the block element here to avoid a DOM parentNode call.
          E
        )
      );
      h(
        S,
        C,
        Y,
        null,
        P,
        b,
        w,
        M,
        !0
      );
    }
  }, fe = (a, d, E, P, b) => {
    if (d !== E) {
      if (d !== ie)
        for (const w in d)
          !hn(w) && !(w in E) && i(
            a,
            w,
            d[w],
            null,
            b,
            P
          );
      for (const w in E) {
        if (hn(w))
          continue;
        const M = E[w], B = d[w];
        M !== B && w !== "value" && i(a, w, B, M, b, P);
      }
      "value" in E && i(a, "value", d.value, E.value, b);
    }
  }, Tt = (a, d, E, P, b, w, M, B, S) => {
    const C = d.el = a ? a.el : f(""), Y = d.anchor = a ? a.anchor : f("");
    let { patchFlag: L, dynamicChildren: z, slotScopeIds: W } = d;
    ({}).NODE_ENV !== "production" && // #5523 dev root fragment may inherit directives
    (lt || L & 2048) && (L = 0, S = !1, z = null), W && (B = B ? B.concat(W) : W), a == null ? (r(C, E, P), r(Y, E, P), oe(
      // #10007
      // such fragment like `<></>` will be compiled into
      // a fragment which doesn't have a children.
      // In this case fallback to an empty array
      d.children || [],
      E,
      Y,
      b,
      w,
      M,
      B,
      S
    )) : L > 0 && L & 64 && z && // #2715 the previous fragment could've been a BAILed one as a result
    // of renderSlot() with no valid children
    a.dynamicChildren ? (J(
      a.dynamicChildren,
      z,
      E,
      b,
      w,
      M,
      B
    ), {}.NODE_ENV !== "production" ? Zn(a, d) : (
      // #2080 if the stable fragment has a key, it's a <template v-for> that may
      //  get moved around. Make sure all root level vnodes inherit el.
      // #2134 or if it's a component root, it may also get moved around
      // as the component is being moved.
      (d.key != null || b && d === b.subTree) && Zn(
        a,
        d,
        !0
        /* shallow */
      )
    )) : X(
      a,
      d,
      E,
      Y,
      b,
      w,
      M,
      B,
      S
    );
  }, F = (a, d, E, P, b, w, M, B, S) => {
    d.slotScopeIds = B, a == null ? d.shapeFlag & 512 ? b.ctx.activate(
      d,
      E,
      P,
      M,
      S
    ) : k(
      d,
      E,
      P,
      b,
      w,
      M,
      S
    ) : N(a, d, S);
  }, k = (a, d, E, P, b, w, M) => {
    const B = a.component = vu(
      a,
      P,
      b
    );
    if ({}.NODE_ENV !== "production" && B.type.__hmrId && Kl(B), {}.NODE_ENV !== "production" && (Jn(a), At(B, "mount")), Hs(a) && (B.ctx.renderer = Ce), {}.NODE_ENV !== "production" && At(B, "init"), Eu(B, !1, M), {}.NODE_ENV !== "production" && dt(B, "init"), B.asyncDep) {
      if ({}.NODE_ENV !== "production" && lt && (a.el = null), b && b.registerDep(B, D, M), !a.el) {
        const S = B.subTree = vt(Je);
        x(null, S, d, E);
      }
    } else
      D(
        B,
        a,
        d,
        E,
        b,
        w,
        M
      );
    ({}).NODE_ENV !== "production" && (Xn(), dt(B, "mount"));
  }, N = (a, d, E) => {
    const P = d.component = a.component;
    if (fu(a, d, E))
      if (P.asyncDep && !P.asyncResolved) {
        ({}).NODE_ENV !== "production" && Jn(d), I(P, d, E), {}.NODE_ENV !== "production" && Xn();
        return;
      } else
        P.next = d, P.update();
    else
      d.el = a.el, P.vnode = d;
  }, D = (a, d, E, P, b, w, M) => {
    const B = () => {
      if (a.isMounted) {
        let { next: L, bu: z, u: W, parent: _, vnode: ae } = a;
        {
          const Le = tf(a);
          if (Le) {
            L && (L.el = ae.el, I(a, L, M)), Le.asyncDep.then(() => {
              a.isUnmounted || B();
            });
            return;
          }
        }
        let se = L, Te;
        ({}).NODE_ENV !== "production" && Jn(L || a.vnode), kt(a, !1), L ? (L.el = ae.el, I(a, L, M)) : L = ae, z && ln(z), (Te = L.props && L.props.onVnodeBeforeUpdate) && tt(Te, _, L, ae), kt(a, !0), {}.NODE_ENV !== "production" && At(a, "render");
        const Ie = Xr(a);
        ({}).NODE_ENV !== "production" && dt(a, "render");
        const Ze = a.subTree;
        a.subTree = Ie, {}.NODE_ENV !== "production" && At(a, "patch"), h(
          Ze,
          Ie,
          // parent may have changed if it's in a teleport
          l(Ze.el),
          // anchor may have changed if it's in a fragment
          $e(Ze),
          a,
          b,
          w
        ), {}.NODE_ENV !== "production" && dt(a, "patch"), L.el = Ie.el, se === null && lu(a, Ie.el), W && ke(W, b), (Te = L.props && L.props.onVnodeUpdated) && ke(
          () => tt(Te, _, L, ae),
          b
        ), {}.NODE_ENV !== "production" && ko(a), {}.NODE_ENV !== "production" && Xn();
      } else {
        let L;
        const { el: z, props: W } = d, { bm: _, m: ae, parent: se, root: Te, type: Ie } = a, Ze = mn(d);
        if (kt(a, !1), _ && ln(_), !Ze && (L = W && W.onVnodeBeforeMount) && tt(L, se, d), kt(a, !0), z && Ct) {
          const Le = () => {
            ({}).NODE_ENV !== "production" && At(a, "render"), a.subTree = Xr(a), {}.NODE_ENV !== "production" && dt(a, "render"), {}.NODE_ENV !== "production" && At(a, "hydrate"), Ct(
              z,
              a.subTree,
              a,
              b,
              null
            ), {}.NODE_ENV !== "production" && dt(a, "hydrate");
          };
          Ze && Ie.__asyncHydrate ? Ie.__asyncHydrate(
            z,
            a,
            Le
          ) : Le();
        } else {
          Te.ce && Te.ce._injectChildStyle(Ie), {}.NODE_ENV !== "production" && At(a, "render");
          const Le = a.subTree = Xr(a);
          ({}).NODE_ENV !== "production" && dt(a, "render"), {}.NODE_ENV !== "production" && At(a, "patch"), h(
            null,
            Le,
            E,
            P,
            a,
            b,
            w
          ), {}.NODE_ENV !== "production" && dt(a, "patch"), d.el = Le.el;
        }
        if (ae && ke(ae, b), !Ze && (L = W && W.onVnodeMounted)) {
          const Le = d;
          ke(
            () => tt(L, se, Le),
            b
          );
        }
        (d.shapeFlag & 256 || se && mn(se.vnode) && se.vnode.shapeFlag & 256) && a.a && ke(a.a, b), a.isMounted = !0, {}.NODE_ENV !== "production" && nc(a), d = E = P = null;
      }
    };
    a.scope.on();
    const S = a.effect = new co(B);
    a.scope.off();
    const C = a.update = S.run.bind(S), Y = a.job = S.runIfDirty.bind(S);
    Y.i = a, Y.id = a.uid, S.scheduler = () => yr(Y), kt(a, !0), {}.NODE_ENV !== "production" && (S.onTrack = a.rtc ? (L) => ln(a.rtc, L) : void 0, S.onTrigger = a.rtg ? (L) => ln(a.rtg, L) : void 0), C();
  }, I = (a, d, E) => {
    d.component = a;
    const P = a.vnode.props;
    a.vnode = d, a.next = null, jc(a, d.props, P, E), Wc(a, d.children, E), bt(), oi(a), It();
  }, X = (a, d, E, P, b, w, M, B, S = !1) => {
    const C = a && a.children, Y = a ? a.shapeFlag : 0, L = d.children, { patchFlag: z, shapeFlag: W } = d;
    if (z > 0) {
      if (z & 128) {
        te(
          C,
          L,
          E,
          P,
          b,
          w,
          M,
          B,
          S
        );
        return;
      } else if (z & 256) {
        V(
          C,
          L,
          E,
          P,
          b,
          w,
          M,
          B,
          S
        );
        return;
      }
    }
    W & 8 ? (Y & 16 && Me(C, b, w), L !== C && u(E, L)) : Y & 16 ? W & 16 ? te(
      C,
      L,
      E,
      P,
      b,
      w,
      M,
      B,
      S
    ) : Me(C, b, w, !0) : (Y & 8 && u(E, ""), W & 16 && oe(
      L,
      E,
      P,
      b,
      w,
      M,
      B,
      S
    ));
  }, V = (a, d, E, P, b, w, M, B, S) => {
    a = a || _t, d = d || _t;
    const C = a.length, Y = d.length, L = Math.min(C, Y);
    let z;
    for (z = 0; z < L; z++) {
      const W = d[z] = S ? Dt(d[z]) : Fe(d[z]);
      h(
        a[z],
        W,
        E,
        null,
        b,
        w,
        M,
        B,
        S
      );
    }
    C > Y ? Me(
      a,
      b,
      w,
      !0,
      !1,
      L
    ) : oe(
      d,
      E,
      P,
      b,
      w,
      M,
      B,
      S,
      L
    );
  }, te = (a, d, E, P, b, w, M, B, S) => {
    let C = 0;
    const Y = d.length;
    let L = a.length - 1, z = Y - 1;
    for (; C <= L && C <= z; ) {
      const W = a[C], _ = d[C] = S ? Dt(d[C]) : Fe(d[C]);
      if (an(W, _))
        h(
          W,
          _,
          E,
          null,
          b,
          w,
          M,
          B,
          S
        );
      else
        break;
      C++;
    }
    for (; C <= L && C <= z; ) {
      const W = a[L], _ = d[z] = S ? Dt(d[z]) : Fe(d[z]);
      if (an(W, _))
        h(
          W,
          _,
          E,
          null,
          b,
          w,
          M,
          B,
          S
        );
      else
        break;
      L--, z--;
    }
    if (C > L) {
      if (C <= z) {
        const W = z + 1, _ = W < Y ? d[W].el : P;
        for (; C <= z; )
          h(
            null,
            d[C] = S ? Dt(d[C]) : Fe(d[C]),
            E,
            _,
            b,
            w,
            M,
            B,
            S
          ), C++;
      }
    } else if (C > z)
      for (; C <= L; )
        ue(a[C], b, w, !0), C++;
    else {
      const W = C, _ = C, ae = /* @__PURE__ */ new Map();
      for (C = _; C <= z; C++) {
        const xe = d[C] = S ? Dt(d[C]) : Fe(d[C]);
        xe.key != null && ({}.NODE_ENV !== "production" && ae.has(xe.key) && O(
          "Duplicate keys found during update:",
          JSON.stringify(xe.key),
          "Make sure keys are unique."
        ), ae.set(xe.key, C));
      }
      let se, Te = 0;
      const Ie = z - _ + 1;
      let Ze = !1, Le = 0;
      const fn = new Array(Ie);
      for (C = 0; C < Ie; C++)
        fn[C] = 0;
      for (C = W; C <= L; C++) {
        const xe = a[C];
        if (Te >= Ie) {
          ue(xe, b, w, !0);
          continue;
        }
        let et;
        if (xe.key != null)
          et = ae.get(xe.key);
        else
          for (se = _; se <= z; se++)
            if (fn[se - _] === 0 && an(xe, d[se])) {
              et = se;
              break;
            }
        et === void 0 ? ue(xe, b, w, !0) : (fn[et - _] = C + 1, et >= Le ? Le = et : Ze = !0, h(
          xe,
          d[et],
          E,
          null,
          b,
          w,
          M,
          B,
          S
        ), Te++);
      }
      const ei = Ze ? $c(fn) : _t;
      for (se = ei.length - 1, C = Ie - 1; C >= 0; C--) {
        const xe = _ + C, et = d[xe], ti = xe + 1 < Y ? d[xe + 1].el : P;
        fn[C] === 0 ? h(
          null,
          et,
          E,
          ti,
          b,
          w,
          M,
          B,
          S
        ) : Ze && (se < 0 || C !== ei[se] ? le(et, E, ti, 2) : se--);
      }
    }
  }, le = (a, d, E, P, b = null) => {
    const { el: w, type: M, transition: B, children: S, shapeFlag: C } = a;
    if (C & 6) {
      le(a.component.subTree, d, E, P);
      return;
    }
    if (C & 128) {
      a.suspense.move(d, E, P);
      return;
    }
    if (C & 64) {
      M.move(a, d, E, Ce);
      return;
    }
    if (M === Oe) {
      r(w, d, E);
      for (let L = 0; L < S.length; L++)
        le(S[L], d, E, P);
      r(a.anchor, d, E);
      return;
    }
    if (M === Fn) {
      G(a, d, E);
      return;
    }
    if (P !== 2 && C & 1 && B)
      if (P === 0)
        B.beforeEnter(w), r(w, d, E), ke(() => B.enter(w), b);
      else {
        const { leave: L, delayLeave: z, afterLeave: W } = B, _ = () => r(w, d, E), ae = () => {
          L(w, () => {
            _(), W && W();
          });
        };
        z ? z(w, _, ae) : ae();
      }
    else
      r(w, d, E);
  }, ue = (a, d, E, P = !1, b = !1) => {
    const {
      type: w,
      props: M,
      ref: B,
      children: S,
      dynamicChildren: C,
      shapeFlag: Y,
      patchFlag: L,
      dirs: z,
      cacheIndex: W
    } = a;
    if (L === -2 && (b = !1), B != null && ir(B, null, E, a, !0), W != null && (d.renderCache[W] = void 0), Y & 256) {
      d.ctx.deactivate(a);
      return;
    }
    const _ = Y & 1 && z, ae = !mn(a);
    let se;
    if (ae && (se = M && M.onVnodeBeforeUnmount) && tt(se, d, a), Y & 6)
      Pt(a.component, E, P);
    else {
      if (Y & 128) {
        a.suspense.unmount(E, P);
        return;
      }
      _ && Lt(a, null, d, "beforeUnmount"), Y & 64 ? a.type.remove(
        a,
        d,
        E,
        Ce,
        P
      ) : C && // #5154
      // when v-once is used inside a block, setBlockTracking(-1) marks the
      // parent block with hasOnce: true
      // so that it doesn't take the fast path during unmount - otherwise
      // components nested in v-once are never unmounted.
      !C.hasOnce && // #1153: fast path should not be taken for non-stable (v-for) fragments
      (w !== Oe || L > 0 && L & 64) ? Me(
        C,
        d,
        E,
        !1,
        !0
      ) : (w === Oe && L & 384 || !b && Y & 16) && Me(S, d, E), P && he(a);
    }
    (ae && (se = M && M.onVnodeUnmounted) || _) && ke(() => {
      se && tt(se, d, a), _ && Lt(a, null, d, "unmounted");
    }, E);
  }, he = (a) => {
    const { type: d, el: E, anchor: P, transition: b } = a;
    if (d === Oe) {
      ({}).NODE_ENV !== "production" && a.patchFlag > 0 && a.patchFlag & 2048 && b && !b.persisted ? a.children.forEach((M) => {
        M.type === Je ? s(M.el) : he(M);
      }) : Re(E, P);
      return;
    }
    if (d === Fn) {
      R(a);
      return;
    }
    const w = () => {
      s(E), b && !b.persisted && b.afterLeave && b.afterLeave();
    };
    if (a.shapeFlag & 1 && b && !b.persisted) {
      const { leave: M, delayLeave: B } = b, S = () => M(E, w);
      B ? B(a.el, w, S) : S();
    } else
      w();
  }, Re = (a, d) => {
    let E;
    for (; a !== d; )
      E = p(a), s(a), a = E;
    s(d);
  }, Pt = (a, d, E) => {
    ({}).NODE_ENV !== "production" && a.type.__hmrId && ql(a);
    const { bum: P, scope: b, job: w, subTree: M, um: B, m: S, a: C } = a;
    hi(S), hi(C), P && ln(P), b.stop(), w && (w.flags |= 8, ue(M, a, d, E)), B && ke(B, d), ke(() => {
      a.isUnmounted = !0;
    }, d), d && d.pendingBranch && !d.isUnmounted && a.asyncDep && !a.asyncResolved && a.suspenseId === d.pendingId && (d.deps--, d.deps === 0 && d.resolve()), {}.NODE_ENV !== "production" && sc(a);
  }, Me = (a, d, E, P = !1, b = !1, w = 0) => {
    for (let M = w; M < a.length; M++)
      ue(a[M], d, E, P, b);
  }, $e = (a) => {
    if (a.shapeFlag & 6)
      return $e(a.component.subTree);
    if (a.shapeFlag & 128)
      return a.suspense.next();
    const d = p(a.anchor || a.el), E = d && d[cc];
    return E ? p(E) : d;
  };
  let Ye = !1;
  const be = (a, d, E) => {
    a == null ? d._vnode && ue(d._vnode, null, null, !0) : h(
      d._vnode || null,
      a,
      d,
      null,
      null,
      null,
      E
    ), d._vnode = a, Ye || (Ye = !0, oi(), Oo(), Ye = !1);
  }, Ce = {
    p: h,
    um: ue,
    m: le,
    r: he,
    mt: k,
    mc: oe,
    pc: X,
    pbc: J,
    n: $e,
    o: e
  };
  let ut, Ct;
  return t && ([ut, Ct] = t(
    Ce
  )), {
    render: be,
    hydrate: ut,
    createApp: Tc(be, ut)
  };
}
function Jr({ type: e, props: t }, n) {
  return n === "svg" && e === "foreignObject" || n === "mathml" && e === "annotation-xml" && t && t.encoding && t.encoding.includes("html") ? void 0 : n;
}
function kt({ effect: e, job: t }, n) {
  n ? (e.flags |= 32, t.flags |= 4) : (e.flags &= -33, t.flags &= -5);
}
function _c(e, t) {
  return (!e || e && !e.pendingBranch) && t && !t.persisted;
}
function Zn(e, t, n = !1) {
  const r = e.children, s = t.children;
  if (Q(r) && Q(s))
    for (let i = 0; i < r.length; i++) {
      const o = r[i];
      let f = s[i];
      f.shapeFlag & 1 && !f.dynamicChildren && ((f.patchFlag <= 0 || f.patchFlag === 32) && (f = s[i] = Dt(s[i]), f.el = o.el), !n && f.patchFlag !== -2 && Zn(o, f)), f.type === On && (f.el = o.el), {}.NODE_ENV !== "production" && f.type === Je && !f.el && (f.el = o.el);
    }
}
function $c(e) {
  const t = e.slice(), n = [0];
  let r, s, i, o, f;
  const c = e.length;
  for (r = 0; r < c; r++) {
    const A = e[r];
    if (A !== 0) {
      if (s = n[n.length - 1], e[s] < A) {
        t[r] = s, n.push(r);
        continue;
      }
      for (i = 0, o = n.length - 1; i < o; )
        f = i + o >> 1, e[n[f]] < A ? i = f + 1 : o = f;
      A < e[n[i]] && (i > 0 && (t[r] = n[i - 1]), n[i] = r);
    }
  }
  for (i = n.length, o = n[i - 1]; i-- > 0; )
    n[i] = o, o = t[o];
  return n;
}
function tf(e) {
  const t = e.subTree.component;
  if (t)
    return t.asyncDep && !t.asyncResolved ? t : tf(t);
}
function hi(e) {
  if (e)
    for (let t = 0; t < e.length; t++)
      e[t].flags |= 8;
}
const eu = Symbol.for("v-scx"), tu = () => {
  {
    const e = Yn(eu);
    return e || {}.NODE_ENV !== "production" && O(
      "Server rendering context not provided. Make sure to only call useSSRContext() conditionally in the server build."
    ), e;
  }
};
function Ue(e, t, n) {
  return {}.NODE_ENV !== "production" && !Z(t) && O(
    "`watch(fn, options?)` signature has been moved to a separate API. Use `watchEffect(fn, options?)` instead. `watch` now only supports `watch(source, cb, options?) signature."
  ), nf(e, t, n);
}
function nf(e, t, n = ie) {
  const { immediate: r, deep: s, flush: i, once: o } = n;
  ({}).NODE_ENV !== "production" && !t && (r !== void 0 && O(
    'watch() "immediate" option is only respected when using the watch(source, callback, options?) signature.'
  ), s !== void 0 && O(
    'watch() "deep" option is only respected when using the watch(source, callback, options?) signature.'
  ), o !== void 0 && O(
    'watch() "once" option is only respected when using the watch(source, callback, options?) signature.'
  ));
  const f = pe({}, n);
  ({}).NODE_ENV !== "production" && (f.onWarn = O);
  const c = t && r || !t && i !== "post";
  let A;
  if (Pn) {
    if (i === "sync") {
      const g = tu();
      A = g.__watcherHandles || (g.__watcherHandles = []);
    } else if (!c) {
      const g = () => {
      };
      return g.stop = me, g.resume = me, g.pause = me, g;
    }
  }
  const u = ye;
  f.call = (g, v, h) => ct(g, u, v, h);
  let l = !1;
  i === "post" ? f.scheduler = (g) => {
    ke(g, u && u.suspense);
  } : i !== "sync" && (l = !0, f.scheduler = (g, v) => {
    v ? g() : yr(g);
  }), f.augmentJob = (g) => {
    t && (g.flags |= 4), l && (g.flags |= 2, u && (g.id = u.uid, g.i = u));
  };
  const p = Ql(e, t, f);
  return Pn && (A ? A.push(p) : c && p()), p;
}
function nu(e, t, n) {
  const r = this.proxy, s = Ae(e) ? e.includes(".") ? rf(r, e) : () => r[e] : e.bind(r, r);
  let i;
  Z(t) ? i = t : (i = t.handler, n = t);
  const o = Mn(this), f = nf(s, i.bind(r), n);
  return o(), f;
}
function rf(e, t) {
  const n = t.split(".");
  return () => {
    let r = e;
    for (let s = 0; s < n.length && r; s++)
      r = r[n[s]];
    return r;
  };
}
const ru = (e, t) => t === "modelValue" || t === "model-value" ? e.modelModifiers : e[`${t}Modifiers`] || e[`${Ve(t)}Modifiers`] || e[`${Rt(t)}Modifiers`];
function su(e, t, ...n) {
  if (e.isUnmounted)
    return;
  const r = e.vnode.props || ie;
  if ({}.NODE_ENV !== "production") {
    const {
      emitsOptions: u,
      propsOptions: [l]
    } = e;
    if (u)
      if (!(t in u))
        (!l || !(Ht(Ve(t)) in l)) && O(
          `Component emitted event "${t}" but it is neither declared in the emits option nor as an "${Ht(Ve(t))}" prop.`
        );
      else {
        const p = u[t];
        Z(p) && (p(...n) || O(
          `Invalid event arguments: event validation failed for event "${t}".`
        ));
      }
  }
  let s = n;
  const i = t.startsWith("update:"), o = i && ru(r, t.slice(7));
  if (o && (o.trim && (s = n.map((u) => Ae(u) ? u.trim() : u)), o.number && (s = n.map(rl))), {}.NODE_ENV !== "production" && fc(e, t, s), {}.NODE_ENV !== "production") {
    const u = t.toLowerCase();
    u !== t && r[Ht(u)] && O(
      `Event "${u}" is emitted in component ${Cr(
        e,
        e.type
      )} but the handler is registered for "${t}". Note that HTML attributes are case-insensitive and you cannot use v-on to listen to camelCase events when using in-DOM templates. You should probably use "${Rt(
        t
      )}" instead of "${t}".`
    );
  }
  let f, c = r[f = Ht(t)] || // also try camelCase event handler (#2249)
  r[f = Ht(Ve(t))];
  !c && i && (c = r[f = Ht(Rt(t))]), c && ct(
    c,
    e,
    6,
    s
  );
  const A = r[f + "Once"];
  if (A) {
    if (!e.emitted)
      e.emitted = {};
    else if (e.emitted[f])
      return;
    e.emitted[f] = !0, ct(
      A,
      e,
      6,
      s
    );
  }
}
function sf(e, t, n = !1) {
  const r = t.emitsCache, s = r.get(e);
  if (s !== void 0)
    return s;
  const i = e.emits;
  let o = {}, f = !1;
  if (!Z(e)) {
    const c = (A) => {
      const u = sf(A, t, !0);
      u && (f = !0, pe(o, u));
    };
    !n && t.mixins.length && t.mixins.forEach(c), e.extends && c(e.extends), e.mixins && e.mixins.forEach(c);
  }
  return !i && !f ? (ce(e) && r.set(e, null), null) : (Q(i) ? i.forEach((c) => o[c] = null) : pe(o, i), ce(e) && r.set(e, o), o);
}
function Ir(e, t) {
  return !e || !Dn(t) ? !1 : (t = t.slice(2).replace(/Once$/, ""), $(e, t[0].toLowerCase() + t.slice(1)) || $(e, Rt(t)) || $(e, t));
}
let ls = !1;
function lr() {
  ls = !0;
}
function Xr(e) {
  const {
    type: t,
    vnode: n,
    proxy: r,
    withProxy: s,
    propsOptions: [i],
    slots: o,
    attrs: f,
    emit: c,
    render: A,
    renderCache: u,
    props: l,
    data: p,
    setupState: g,
    ctx: v,
    inheritAttrs: h
  } = e, y = sr(e);
  let x, H;
  ({}).NODE_ENV !== "production" && (ls = !1);
  try {
    if (n.shapeFlag & 4) {
      const R = s || r, U = {}.NODE_ENV !== "production" && g.__isScriptSetup ? new Proxy(R, {
        get(re, ee, oe) {
          return O(
            `Property '${String(
              ee
            )}' was accessed via 'this'. Avoid using 'this' in templates.`
          ), Reflect.get(re, ee, oe);
        }
      }) : R;
      x = Fe(
        A.call(
          U,
          R,
          u,
          {}.NODE_ENV !== "production" ? ft(l) : l,
          g,
          p,
          v
        )
      ), H = f;
    } else {
      const R = t;
      ({}).NODE_ENV !== "production" && f === l && lr(), x = Fe(
        R.length > 1 ? R(
          {}.NODE_ENV !== "production" ? ft(l) : l,
          {}.NODE_ENV !== "production" ? {
            get attrs() {
              return lr(), ft(f);
            },
            slots: o,
            emit: c
          } : { attrs: f, slots: o, emit: c }
        ) : R(
          {}.NODE_ENV !== "production" ? ft(l) : l,
          null
        )
      ), H = t.props ? f : iu(f);
    }
  } catch (R) {
    En.length = 0, Sn(R, e, 1), x = vt(Je);
  }
  let T = x, G;
  if ({}.NODE_ENV !== "production" && x.patchFlag > 0 && x.patchFlag & 2048 && ([T, G] = of(x)), H && h !== !1) {
    const R = Object.keys(H), { shapeFlag: U } = T;
    if (R.length) {
      if (U & 7)
        i && R.some(_n) && (H = ou(
          H,
          i
        )), T = Ot(T, H, !1, !0);
      else if ({}.NODE_ENV !== "production" && !ls && T.type !== Je) {
        const re = Object.keys(f), ee = [], oe = [];
        for (let j = 0, J = re.length; j < J; j++) {
          const fe = re[j];
          Dn(fe) ? _n(fe) || ee.push(fe[2].toLowerCase() + fe.slice(3)) : oe.push(fe);
        }
        oe.length && O(
          `Extraneous non-props attributes (${oe.join(", ")}) were passed to component but could not be automatically inherited because component renders fragment or text or teleport root nodes.`
        ), ee.length && O(
          `Extraneous non-emits event listeners (${ee.join(", ")}) were passed to component but could not be automatically inherited because component renders fragment or text root nodes. If the listener is intended to be a component custom event listener only, declare it using the "emits" option.`
        );
      }
    }
  }
  return n.dirs && ({}.NODE_ENV !== "production" && !gi(T) && O(
    "Runtime directive used on component with non-element root node. The directives will not function as intended."
  ), T = Ot(T, null, !1, !0), T.dirs = T.dirs ? T.dirs.concat(n.dirs) : n.dirs), n.transition && ({}.NODE_ENV !== "production" && !gi(T) && O(
    "Component inside <Transition> renders non-element root node that cannot be animated."
  ), ks(T, n.transition)), {}.NODE_ENV !== "production" && G ? G(T) : x = T, sr(y), x;
}
const of = (e) => {
  const t = e.children, n = e.dynamicChildren, r = Qs(t, !1);
  if (r) {
    if ({}.NODE_ENV !== "production" && r.patchFlag > 0 && r.patchFlag & 2048)
      return of(r);
  } else
    return [e, void 0];
  const s = t.indexOf(r), i = n ? n.indexOf(r) : -1, o = (f) => {
    t[s] = f, n && (i > -1 ? n[i] = f : f.patchFlag > 0 && (e.dynamicChildren = [...n, f]));
  };
  return [Fe(r), o];
};
function Qs(e, t = !0) {
  let n;
  for (let r = 0; r < e.length; r++) {
    const s = e[r];
    if (wr(s)) {
      if (s.type !== Je || s.children === "v-if") {
        if (n)
          return;
        if (n = s, {}.NODE_ENV !== "production" && t && n.patchFlag > 0 && n.patchFlag & 2048)
          return Qs(n.children);
      }
    } else
      return;
  }
  return n;
}
const iu = (e) => {
  let t;
  for (const n in e)
    (n === "class" || n === "style" || Dn(n)) && ((t || (t = {}))[n] = e[n]);
  return t;
}, ou = (e, t) => {
  const n = {};
  for (const r in e)
    (!_n(r) || !(r.slice(9) in t)) && (n[r] = e[r]);
  return n;
}, gi = (e) => e.shapeFlag & 7 || e.type === Je;
function fu(e, t, n) {
  const { props: r, children: s, component: i } = e, { props: o, children: f, patchFlag: c } = t, A = i.emitsOptions;
  if ({}.NODE_ENV !== "production" && (s || f) && lt || t.dirs || t.transition)
    return !0;
  if (n && c >= 0) {
    if (c & 1024)
      return !0;
    if (c & 16)
      return r ? vi(r, o, A) : !!o;
    if (c & 8) {
      const u = t.dynamicProps;
      for (let l = 0; l < u.length; l++) {
        const p = u[l];
        if (o[p] !== r[p] && !Ir(A, p))
          return !0;
      }
    }
  } else
    return (s || f) && (!f || !f.$stable) ? !0 : r === o ? !1 : r ? o ? vi(r, o, A) : !0 : !!o;
  return !1;
}
function vi(e, t, n) {
  const r = Object.keys(t);
  if (r.length !== Object.keys(e).length)
    return !0;
  for (let s = 0; s < r.length; s++) {
    const i = r[s];
    if (t[i] !== e[i] && !Ir(n, i))
      return !0;
  }
  return !1;
}
function lu({ vnode: e, parent: t }, n) {
  for (; t; ) {
    const r = t.subTree;
    if (r.suspense && r.suspense.activeBranch === e && (r.el = e.el), r === e)
      (e = t.vnode).el = n, t = t.parent;
    else
      break;
  }
}
const ff = (e) => e.__isSuspense;
function cu(e, t) {
  t && t.pendingBranch ? Q(e) ? t.effects.push(...e) : t.effects.push(e) : Ro(e);
}
const Oe = Symbol.for("v-fgt"), On = Symbol.for("v-txt"), Je = Symbol.for("v-cmt"), Fn = Symbol.for("v-stc"), En = [];
let je = null;
function Ee(e = !1) {
  En.push(je = e ? null : []);
}
function uu() {
  En.pop(), je = En[En.length - 1] || null;
}
let wn = 1;
function mi(e, t = !1) {
  wn += e, e < 0 && je && t && (je.hasOnce = !0);
}
function lf(e) {
  return e.dynamicChildren = wn > 0 ? je || _t : null, uu(), wn > 0 && je && je.push(e), e;
}
function Qe(e, t, n, r, s, i) {
  return lf(
    st(
      e,
      t,
      n,
      r,
      s,
      i,
      !0
    )
  );
}
function tn(e, t, n, r, s) {
  return lf(
    vt(
      e,
      t,
      n,
      r,
      s,
      !0
    )
  );
}
function wr(e) {
  return e ? e.__v_isVNode === !0 : !1;
}
function an(e, t) {
  if ({}.NODE_ENV !== "production" && t.shapeFlag & 6 && e.component) {
    const n = Qn.get(t.type);
    if (n && n.has(e.component))
      return e.shapeFlag &= -257, t.shapeFlag &= -513, !1;
  }
  return e.type === t.type && e.key === t.key;
}
const au = (...e) => uf(
  ...e
), cf = ({ key: e }) => e ?? null, Gn = ({
  ref: e,
  ref_key: t,
  ref_for: n
}) => (typeof e == "number" && (e = "" + e), e != null ? Ae(e) || de(e) || Z(e) ? { i: He, r: e, k: t, f: !!n } : e : null);
function st(e, t = null, n = null, r = 0, s = null, i = e === Oe ? 0 : 1, o = !1, f = !1) {
  const c = {
    __v_isVNode: !0,
    __v_skip: !0,
    type: e,
    props: t,
    key: t && cf(t),
    ref: t && Gn(t),
    scopeId: jo,
    slotScopeIds: null,
    children: n,
    component: null,
    suspense: null,
    ssContent: null,
    ssFallback: null,
    dirs: null,
    transition: null,
    el: null,
    anchor: null,
    target: null,
    targetStart: null,
    targetAnchor: null,
    staticCount: 0,
    shapeFlag: i,
    patchFlag: r,
    dynamicProps: s,
    dynamicChildren: null,
    appContext: null,
    ctx: He
  };
  return f ? (Ys(c, n), i & 128 && e.normalize(c)) : n && (c.shapeFlag |= Ae(n) ? 8 : 16), {}.NODE_ENV !== "production" && c.key !== c.key && O("VNode created with invalid key (NaN). VNode type:", c.type), wn > 0 && // avoid a block node from tracking itself
  !o && // has current parent block
  je && // presence of a patch flag indicates this node needs patching on updates.
  // component nodes also should always be patched, because even if the
  // component doesn't need to update, it needs to persist the instance on to
  // the next vnode so that it can be properly unmounted later.
  (c.patchFlag > 0 || i & 6) && // the EVENTS flag is only for hydration and if it is the only flag, the
  // vnode should not be considered dynamic due to handler caching.
  c.patchFlag !== 32 && je.push(c), c;
}
const vt = {}.NODE_ENV !== "production" ? au : uf;
function uf(e, t = null, n = null, r = 0, s = null, i = !1) {
  if ((!e || e === wc) && ({}.NODE_ENV !== "production" && !e && O(`Invalid vnode type when creating vnode: ${e}.`), e = Je), wr(e)) {
    const f = Ot(
      e,
      t,
      !0
      /* mergeRef: true */
    );
    return n && Ys(f, n), wn > 0 && !i && je && (f.shapeFlag & 6 ? je[je.indexOf(e)] = f : je.push(f)), f.patchFlag = -2, f;
  }
  if (hf(e) && (e = e.__vccOpts), t) {
    t = Au(t);
    let { class: f, style: c } = t;
    f && !Ae(f) && (t.class = gt(f)), ce(c) && (er(c) && !Q(c) && (c = pe({}, c)), t.style = nn(c));
  }
  const o = Ae(e) ? 1 : ff(e) ? 128 : uc(e) ? 64 : ce(e) ? 4 : Z(e) ? 2 : 0;
  return {}.NODE_ENV !== "production" && o & 4 && er(e) && (e = q(e), O(
    "Vue received a Component that was made a reactive object. This can lead to unnecessary performance overhead and should be avoided by marking the component with `markRaw` or using `shallowRef` instead of `ref`.",
    `
Component that was made reactive: `,
    e
  )), st(
    e,
    t,
    n,
    r,
    s,
    o,
    i,
    !0
  );
}
function Au(e) {
  return e ? er(e) || Wo(e) ? pe({}, e) : e : null;
}
function Ot(e, t, n = !1, r = !1) {
  const { props: s, ref: i, patchFlag: o, children: f, transition: c } = e, A = t ? pu(s || {}, t) : s, u = {
    __v_isVNode: !0,
    __v_skip: !0,
    type: e.type,
    props: A,
    key: A && cf(A),
    ref: t && t.ref ? (
      // #2078 in the case of <component :is="vnode" ref="extra"/>
      // if the vnode itself already has a ref, cloneVNode will need to merge
      // the refs so the single vnode can be set on multiple refs
      n && i ? Q(i) ? i.concat(Gn(t)) : [i, Gn(t)] : Gn(t)
    ) : i,
    scopeId: e.scopeId,
    slotScopeIds: e.slotScopeIds,
    children: {}.NODE_ENV !== "production" && o === -1 && Q(f) ? f.map(af) : f,
    target: e.target,
    targetStart: e.targetStart,
    targetAnchor: e.targetAnchor,
    staticCount: e.staticCount,
    shapeFlag: e.shapeFlag,
    // if the vnode is cloned with extra props, we can no longer assume its
    // existing patch flag to be reliable and need to add the FULL_PROPS flag.
    // note: preserve flag for fragments since they use the flag for children
    // fast paths only.
    patchFlag: t && e.type !== Oe ? o === -1 ? 16 : o | 16 : o,
    dynamicProps: e.dynamicProps,
    dynamicChildren: e.dynamicChildren,
    appContext: e.appContext,
    dirs: e.dirs,
    transition: c,
    // These should technically only be non-null on mounted VNodes. However,
    // they *should* be copied for kept-alive vnodes. So we just always copy
    // them since them being non-null during a mount doesn't affect the logic as
    // they will simply be overwritten.
    component: e.component,
    suspense: e.suspense,
    ssContent: e.ssContent && Ot(e.ssContent),
    ssFallback: e.ssFallback && Ot(e.ssFallback),
    el: e.el,
    anchor: e.anchor,
    ctx: e.ctx,
    ce: e.ce
  };
  return c && r && ks(
    u,
    c.clone(u)
  ), u;
}
function af(e) {
  const t = Ot(e);
  return Q(e.children) && (t.children = e.children.map(af)), t;
}
function du(e = " ", t = 0) {
  return vt(On, null, e, t);
}
function Ei(e = "", t = !1) {
  return t ? (Ee(), tn(Je, null, e)) : vt(Je, null, e);
}
function Fe(e) {
  return e == null || typeof e == "boolean" ? vt(Je) : Q(e) ? vt(
    Oe,
    null,
    // #3666, avoid reference pollution when reusing vnode
    e.slice()
  ) : wr(e) ? Dt(e) : vt(On, null, String(e));
}
function Dt(e) {
  return e.el === null && e.patchFlag !== -1 || e.memo ? e : Ot(e);
}
function Ys(e, t) {
  let n = 0;
  const { shapeFlag: r } = e;
  if (t == null)
    t = null;
  else if (Q(t))
    n = 16;
  else if (typeof t == "object")
    if (r & 65) {
      const s = t.default;
      s && (s._c && (s._d = !1), Ys(e, s()), s._c && (s._d = !0));
      return;
    } else {
      n = 32;
      const s = t._;
      !s && !Wo(t) ? t._ctx = He : s === 3 && He && (He.slots._ === 1 ? t._ = 1 : (t._ = 2, e.patchFlag |= 1024));
    }
  else
    Z(t) ? (t = { default: t, _ctx: He }, n = 32) : (t = String(t), r & 64 ? (n = 16, t = [du(t)]) : n = 8);
  e.children = t, e.shapeFlag |= n;
}
function pu(...e) {
  const t = {};
  for (let n = 0; n < e.length; n++) {
    const r = e[n];
    for (const s in r)
      if (s === "class")
        t.class !== r.class && (t.class = gt([t.class, r.class]));
      else if (s === "style")
        t.style = nn([t.style, r.style]);
      else if (Dn(s)) {
        const i = t[s], o = r[s];
        o && i !== o && !(Q(i) && i.includes(o)) && (t[s] = i ? [].concat(i, o) : o);
      } else
        s !== "" && (t[s] = r[s]);
  }
  return t;
}
function tt(e, t, n, r = null) {
  ct(e, t, 7, [
    n,
    r
  ]);
}
const hu = Fo();
let gu = 0;
function vu(e, t, n) {
  const r = e.type, s = (t ? t.appContext : e.appContext) || hu, i = {
    uid: gu++,
    vnode: e,
    type: r,
    parent: t,
    appContext: s,
    root: null,
    // to be immediately set
    next: null,
    subTree: null,
    // will be set synchronously right after creation
    effect: null,
    update: null,
    // will be set synchronously right after creation
    job: null,
    scope: new gl(
      !0
      /* detached */
    ),
    render: null,
    proxy: null,
    exposed: null,
    exposeProxy: null,
    withProxy: null,
    provides: t ? t.provides : Object.create(s.provides),
    ids: t ? t.ids : ["", 0, 0],
    accessCache: null,
    renderCache: [],
    // local resolved assets
    components: null,
    directives: null,
    // resolved props and emits options
    propsOptions: Ko(r, s),
    emitsOptions: sf(r, s),
    // emit
    emit: null,
    // to be set immediately
    emitted: null,
    // props default value
    propsDefaults: ie,
    // inheritAttrs
    inheritAttrs: r.inheritAttrs,
    // state
    ctx: ie,
    data: ie,
    props: ie,
    attrs: ie,
    slots: ie,
    refs: ie,
    setupState: ie,
    setupContext: null,
    // suspense related
    suspense: n,
    suspenseId: n ? n.pendingId : 0,
    asyncDep: null,
    asyncResolved: !1,
    // lifecycle hooks
    // not using enums here because it results in computed properties
    isMounted: !1,
    isUnmounted: !1,
    isDeactivated: !1,
    bc: null,
    c: null,
    bm: null,
    m: null,
    bu: null,
    u: null,
    um: null,
    bum: null,
    da: null,
    a: null,
    rtg: null,
    rtc: null,
    ec: null,
    sp: null
  };
  return {}.NODE_ENV !== "production" ? i.ctx = Pc(i) : i.ctx = { _: i }, i.root = t ? t.root : i, i.emit = su.bind(null, i), e.ce && e.ce(i), i;
}
let ye = null;
const Pr = () => ye || He;
let cr, cs;
{
  const e = Nn(), t = (n, r) => {
    let s;
    return (s = e[n]) || (s = e[n] = []), s.push(r), (i) => {
      s.length > 1 ? s.forEach((o) => o(i)) : s[0](i);
    };
  };
  cr = t(
    "__VUE_INSTANCE_SETTERS__",
    (n) => ye = n
  ), cs = t(
    "__VUE_SSR_SETTERS__",
    (n) => Pn = n
  );
}
const Mn = (e) => {
  const t = ye;
  return cr(e), e.scope.on(), () => {
    e.scope.off(), cr(t);
  };
}, yi = () => {
  ye && ye.scope.off(), cr(null);
}, mu = /* @__PURE__ */ yt("slot,component");
function us(e, { isNativeTag: t }) {
  (mu(e) || t(e)) && O(
    "Do not use built-in or reserved HTML elements as component id: " + e
  );
}
function Af(e) {
  return e.vnode.shapeFlag & 4;
}
let Pn = !1;
function Eu(e, t = !1, n = !1) {
  t && cs(t);
  const { props: r, children: s } = e.vnode, i = Af(e);
  kc(e, r, i, t), Vc(e, s, n);
  const o = i ? yu(e, t) : void 0;
  return t && cs(!1), o;
}
function yu(e, t) {
  var n;
  const r = e.type;
  if ({}.NODE_ENV !== "production") {
    if (r.name && us(r.name, e.appContext.config), r.components) {
      const i = Object.keys(r.components);
      for (let o = 0; o < i.length; o++)
        us(i[o], e.appContext.config);
    }
    if (r.directives) {
      const i = Object.keys(r.directives);
      for (let o = 0; o < i.length; o++)
        zo(i[o]);
    }
    r.compilerOptions && bu() && O(
      '"compilerOptions" is only supported when using a build of Vue that includes the runtime compiler. Since you are using a runtime-only build, the options should be passed via your build tool config instead.'
    );
  }
  e.accessCache = /* @__PURE__ */ Object.create(null), e.proxy = new Proxy(e.ctx, Yo), {}.NODE_ENV !== "production" && Cc(e);
  const { setup: s } = r;
  if (s) {
    bt();
    const i = e.setupContext = s.length > 1 ? wu(e) : null, o = Mn(e), f = rn(
      s,
      e,
      0,
      [
        {}.NODE_ENV !== "production" ? ft(e.props) : e.props,
        i
      ]
    ), c = ws(f);
    if (It(), o(), (c || e.sp) && !mn(e) && Jo(e), c) {
      if (f.then(yi, yi), t)
        return f.then((A) => {
          bi(e, A, t);
        }).catch((A) => {
          Sn(A, e, 0);
        });
      if (e.asyncDep = f, {}.NODE_ENV !== "production" && !e.suspense) {
        const A = (n = r.name) != null ? n : "Anonymous";
        O(
          `Component <${A}>: setup function returned a promise, but no <Suspense> boundary was found in the parent component tree. A component with async setup() must be nested in a <Suspense> in order to be rendered.`
        );
      }
    } else
      bi(e, f, t);
  } else
    df(e, t);
}
function bi(e, t, n) {
  Z(t) ? e.type.__ssrInlineRender ? e.ssrRender = t : e.render = t : ce(t) ? ({}.NODE_ENV !== "production" && wr(t) && O(
    "setup() should not return VNodes directly - return a render function instead."
  ), {}.NODE_ENV !== "production" && (e.devtoolsRawSetupState = t), e.setupState = Do(t), {}.NODE_ENV !== "production" && xc(e)) : {}.NODE_ENV !== "production" && t !== void 0 && O(
    `setup() should return an object. Received: ${t === null ? "null" : typeof t}`
  ), df(e, n);
}
let as;
const bu = () => !as;
function df(e, t, n) {
  const r = e.type;
  if (!e.render) {
    if (!t && as && !r.render) {
      const s = r.template || Js(e).template;
      if (s) {
        ({}).NODE_ENV !== "production" && At(e, "compile");
        const { isCustomElement: i, compilerOptions: o } = e.appContext.config, { delimiters: f, compilerOptions: c } = r, A = pe(
          pe(
            {
              isCustomElement: i,
              delimiters: f
            },
            o
          ),
          c
        );
        r.render = as(s, A), {}.NODE_ENV !== "production" && dt(e, "compile");
      }
    }
    e.render = r.render || me;
  }
  {
    const s = Mn(e);
    bt();
    try {
      Nc(e);
    } finally {
      It(), s();
    }
  }
  ({}).NODE_ENV !== "production" && !r.render && e.render === me && !t && (r.template ? O(
    'Component provided template option but runtime compilation is not supported in this build of Vue. Configure your bundler to alias "vue" to "vue/dist/vue.esm-bundler.js".'
  ) : O("Component is missing template or render function: ", r));
}
const Ii = {}.NODE_ENV !== "production" ? {
  get(e, t) {
    return lr(), ve(e, "get", ""), e[t];
  },
  set() {
    return O("setupContext.attrs is readonly."), !1;
  },
  deleteProperty() {
    return O("setupContext.attrs is readonly."), !1;
  }
} : {
  get(e, t) {
    return ve(e, "get", ""), e[t];
  }
};
function Iu(e) {
  return new Proxy(e.slots, {
    get(t, n) {
      return ve(e, "get", "$slots"), t[n];
    }
  });
}
function wu(e) {
  const t = (n) => {
    if ({}.NODE_ENV !== "production" && (e.exposed && O("expose() should be called only once per setup()."), n != null)) {
      let r = typeof n;
      r === "object" && (Q(n) ? r = "array" : de(n) && (r = "ref")), r !== "object" && O(
        `expose() should be passed a plain object, received ${r}.`
      );
    }
    e.exposed = n || {};
  };
  if ({}.NODE_ENV !== "production") {
    let n, r;
    return Object.freeze({
      get attrs() {
        return n || (n = new Proxy(e.attrs, Ii));
      },
      get slots() {
        return r || (r = Iu(e));
      },
      get emit() {
        return (s, ...i) => e.emit(s, ...i);
      },
      expose: t
    });
  } else
    return {
      attrs: new Proxy(e.attrs, Ii),
      slots: e.slots,
      emit: e.emit,
      expose: t
    };
}
function Zs(e) {
  return e.exposed ? e.exposeProxy || (e.exposeProxy = new Proxy(Do(kl(e.exposed)), {
    get(t, n) {
      if (n in t)
        return t[n];
      if (n in Gt)
        return Gt[n](e);
    },
    has(t, n) {
      return n in t || n in Gt;
    }
  })) : e.proxy;
}
const Pu = /(?:^|[-_])(\w)/g, Cu = (e) => e.replace(Pu, (t) => t.toUpperCase()).replace(/[-_]/g, "");
function pf(e, t = !0) {
  return Z(e) ? e.displayName || e.name : e.name || t && e.__name;
}
function Cr(e, t, n = !1) {
  let r = pf(t);
  if (!r && t.__file) {
    const s = t.__file.match(/([^/\\]+)\.\w+$/);
    s && (r = s[1]);
  }
  if (!r && e && e.parent) {
    const s = (i) => {
      for (const o in i)
        if (i[o] === t)
          return o;
    };
    r = s(
      e.components || e.parent.type.components
    ) || s(e.appContext.components);
  }
  return r ? Cu(r) : n ? "App" : "Anonymous";
}
function hf(e) {
  return Z(e) && "__vccOpts" in e;
}
const Ge = (e, t) => {
  const n = Jl(e, t, Pn);
  if ({}.NODE_ENV !== "production") {
    const r = Pr();
    r && r.appContext.config.warnRecursiveComputed && (n._warnRecursive = !0);
  }
  return n;
};
function xu() {
  if ({}.NODE_ENV === "production" || typeof window > "u")
    return;
  const e = { style: "color:#3ba776" }, t = { style: "color:#1677ff" }, n = { style: "color:#f5222d" }, r = { style: "color:#eb2f96" }, s = {
    __vue_custom_formatter: !0,
    header(l) {
      return ce(l) ? l.__isVue ? ["div", e, "VueInstance"] : de(l) ? [
        "div",
        {},
        ["span", e, u(l)],
        "<",
        // avoid debugger accessing value affecting behavior
        f("_value" in l ? l._value : l),
        ">"
      ] : Zt(l) ? [
        "div",
        {},
        ["span", e, Be(l) ? "ShallowReactive" : "Reactive"],
        "<",
        f(l),
        `>${Et(l) ? " (readonly)" : ""}`
      ] : Et(l) ? [
        "div",
        {},
        ["span", e, Be(l) ? "ShallowReadonly" : "Readonly"],
        "<",
        f(l),
        ">"
      ] : null : null;
    },
    hasBody(l) {
      return l && l.__isVue;
    },
    body(l) {
      if (l && l.__isVue)
        return [
          "div",
          {},
          ...i(l.$)
        ];
    }
  };
  function i(l) {
    const p = [];
    l.type.props && l.props && p.push(o("props", q(l.props))), l.setupState !== ie && p.push(o("setup", l.setupState)), l.data !== ie && p.push(o("data", q(l.data)));
    const g = c(l, "computed");
    g && p.push(o("computed", g));
    const v = c(l, "inject");
    return v && p.push(o("injected", v)), p.push([
      "div",
      {},
      [
        "span",
        {
          style: r.style + ";opacity:0.66"
        },
        "$ (internal): "
      ],
      ["object", { object: l }]
    ]), p;
  }
  function o(l, p) {
    return p = pe({}, p), Object.keys(p).length ? [
      "div",
      { style: "line-height:1.25em;margin-bottom:0.6em" },
      [
        "div",
        {
          style: "color:#476582"
        },
        l
      ],
      [
        "div",
        {
          style: "padding-left:1.25em"
        },
        ...Object.keys(p).map((g) => [
          "div",
          {},
          ["span", r, g + ": "],
          f(p[g], !1)
        ])
      ]
    ] : ["span", {}];
  }
  function f(l, p = !0) {
    return typeof l == "number" ? ["span", t, l] : typeof l == "string" ? ["span", n, JSON.stringify(l)] : typeof l == "boolean" ? ["span", r, l] : ce(l) ? ["object", { object: p ? q(l) : l }] : ["span", n, String(l)];
  }
  function c(l, p) {
    const g = l.type;
    if (Z(g))
      return;
    const v = {};
    for (const h in l.ctx)
      A(g, h, p) && (v[h] = l.ctx[h]);
    return v;
  }
  function A(l, p, g) {
    const v = l[g];
    if (Q(v) && v.includes(p) || ce(v) && p in v || l.extends && A(l.extends, p, g) || l.mixins && l.mixins.some((h) => A(h, p, g)))
      return !0;
  }
  function u(l) {
    return Be(l) ? "ShallowRef" : l.effect ? "ComputedRef" : "Ref";
  }
  window.devtoolsFormatters ? window.devtoolsFormatters.push(s) : window.devtoolsFormatters = [s];
}
const wi = "3.5.13", mt = {}.NODE_ENV !== "production" ? O : me;
/**
* @vue/runtime-dom v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
let As;
const Pi = typeof window < "u" && window.trustedTypes;
if (Pi)
  try {
    As = /* @__PURE__ */ Pi.createPolicy("vue", {
      createHTML: (e) => e
    });
  } catch (e) {
    ({}).NODE_ENV !== "production" && mt(`Error creating trusted types policy: ${e}`);
  }
const gf = As ? (e) => As.createHTML(e) : (e) => e, Du = "http://www.w3.org/2000/svg", Nu = "http://www.w3.org/1998/Math/MathML", pt = typeof document < "u" ? document : null, Ci = pt && /* @__PURE__ */ pt.createElement("template"), Su = {
  insert: (e, t, n) => {
    t.insertBefore(e, n || null);
  },
  remove: (e) => {
    const t = e.parentNode;
    t && t.removeChild(e);
  },
  createElement: (e, t, n, r) => {
    const s = t === "svg" ? pt.createElementNS(Du, e) : t === "mathml" ? pt.createElementNS(Nu, e) : n ? pt.createElement(e, { is: n }) : pt.createElement(e);
    return e === "select" && r && r.multiple != null && s.setAttribute("multiple", r.multiple), s;
  },
  createText: (e) => pt.createTextNode(e),
  createComment: (e) => pt.createComment(e),
  setText: (e, t) => {
    e.nodeValue = t;
  },
  setElementText: (e, t) => {
    e.textContent = t;
  },
  parentNode: (e) => e.parentNode,
  nextSibling: (e) => e.nextSibling,
  querySelector: (e) => pt.querySelector(e),
  setScopeId(e, t) {
    e.setAttribute(t, "");
  },
  // __UNSAFE__
  // Reason: innerHTML.
  // Static content here can only come from compiled templates.
  // As long as the user only uses trusted templates, this is safe.
  insertStaticContent(e, t, n, r, s, i) {
    const o = n ? n.previousSibling : t.lastChild;
    if (s && (s === i || s.nextSibling))
      for (; t.insertBefore(s.cloneNode(!0), n), !(s === i || !(s = s.nextSibling)); )
        ;
    else {
      Ci.innerHTML = gf(
        r === "svg" ? `<svg>${e}</svg>` : r === "mathml" ? `<math>${e}</math>` : e
      );
      const f = Ci.content;
      if (r === "svg" || r === "mathml") {
        const c = f.firstChild;
        for (; c.firstChild; )
          f.appendChild(c.firstChild);
        f.removeChild(c);
      }
      t.insertBefore(f, n);
    }
    return [
      // first
      o ? o.nextSibling : t.firstChild,
      // last
      n ? n.previousSibling : t.lastChild
    ];
  }
}, Bu = Symbol("_vtc");
function Ru(e, t, n) {
  const r = e[Bu];
  r && (t = (t ? [t, ...r] : [...r]).join(" ")), t == null ? e.removeAttribute("class") : n ? e.setAttribute("class", t) : e.className = t;
}
const xi = Symbol("_vod"), Ou = Symbol("_vsh"), Mu = Symbol({}.NODE_ENV !== "production" ? "CSS_VAR_TEXT" : ""), Tu = /(^|;)\s*display\s*:/;
function Lu(e, t, n) {
  const r = e.style, s = Ae(n);
  let i = !1;
  if (n && !s) {
    if (t)
      if (Ae(t))
        for (const o of t.split(";")) {
          const f = o.slice(0, o.indexOf(":")).trim();
          n[f] == null && Vn(r, f, "");
        }
      else
        for (const o in t)
          n[o] == null && Vn(r, o, "");
    for (const o in n)
      o === "display" && (i = !0), Vn(r, o, n[o]);
  } else if (s) {
    if (t !== n) {
      const o = r[Mu];
      o && (n += ";" + o), r.cssText = n, i = Tu.test(n);
    }
  } else
    t && e.removeAttribute("style");
  xi in e && (e[xi] = i ? r.display : "", e[Ou] && (r.display = "none"));
}
const ku = /[^\\];\s*$/, Di = /\s*!important$/;
function Vn(e, t, n) {
  if (Q(n))
    n.forEach((r) => Vn(e, t, r));
  else if (n == null && (n = ""), {}.NODE_ENV !== "production" && ku.test(n) && mt(
    `Unexpected semicolon at the end of '${t}' style value: '${n}'`
  ), t.startsWith("--"))
    e.setProperty(t, n);
  else {
    const r = Hu(e, t);
    Di.test(n) ? e.setProperty(
      Rt(r),
      n.replace(Di, ""),
      "important"
    ) : e[r] = n;
  }
}
const Ni = ["Webkit", "Moz", "ms"], Qr = {};
function Hu(e, t) {
  const n = Qr[t];
  if (n)
    return n;
  let r = Ve(t);
  if (r !== "filter" && r in e)
    return Qr[t] = r;
  r = gr(r);
  for (let s = 0; s < Ni.length; s++) {
    const i = Ni[s] + r;
    if (i in e)
      return Qr[t] = i;
  }
  return t;
}
const Si = "http://www.w3.org/1999/xlink";
function Bi(e, t, n, r, s, i = hl(t)) {
  r && t.startsWith("xlink:") ? n == null ? e.removeAttributeNS(Si, t.slice(6, t.length)) : e.setAttributeNS(Si, t, n) : n == null || i && !io(n) ? e.removeAttribute(t) : e.setAttribute(
    t,
    i ? "" : Mt(n) ? String(n) : n
  );
}
function Ri(e, t, n, r, s) {
  if (t === "innerHTML" || t === "textContent") {
    n != null && (e[t] = t === "innerHTML" ? gf(n) : n);
    return;
  }
  const i = e.tagName;
  if (t === "value" && i !== "PROGRESS" && // custom elements may use _value internally
  !i.includes("-")) {
    const f = i === "OPTION" ? e.getAttribute("value") || "" : e.value, c = n == null ? (
      // #11647: value should be set as empty string for null and undefined,
      // but <input type="checkbox"> should be set as 'on'.
      e.type === "checkbox" ? "on" : ""
    ) : String(n);
    (f !== c || !("_value" in e)) && (e.value = c), n == null && e.removeAttribute(t), e._value = n;
    return;
  }
  let o = !1;
  if (n === "" || n == null) {
    const f = typeof e[t];
    f === "boolean" ? n = io(n) : n == null && f === "string" ? (n = "", o = !0) : f === "number" && (n = 0, o = !0);
  }
  try {
    e[t] = n;
  } catch (f) {
    ({}).NODE_ENV !== "production" && !o && mt(
      `Failed setting prop "${t}" on <${i.toLowerCase()}>: value ${n} is invalid.`,
      f
    );
  }
  o && e.removeAttribute(s || t);
}
function ju(e, t, n, r) {
  e.addEventListener(t, n, r);
}
function zu(e, t, n, r) {
  e.removeEventListener(t, n, r);
}
const Oi = Symbol("_vei");
function Ju(e, t, n, r, s = null) {
  const i = e[Oi] || (e[Oi] = {}), o = i[t];
  if (r && o)
    o.value = {}.NODE_ENV !== "production" ? Ti(r, t) : r;
  else {
    const [f, c] = Xu(t);
    if (r) {
      const A = i[t] = Zu(
        {}.NODE_ENV !== "production" ? Ti(r, t) : r,
        s
      );
      ju(e, f, A, c);
    } else
      o && (zu(e, f, o, c), i[t] = void 0);
  }
}
const Mi = /(?:Once|Passive|Capture)$/;
function Xu(e) {
  let t;
  if (Mi.test(e)) {
    t = {};
    let r;
    for (; r = e.match(Mi); )
      e = e.slice(0, e.length - r[0].length), t[r[0].toLowerCase()] = !0;
  }
  return [e[2] === ":" ? e.slice(3) : Rt(e.slice(2)), t];
}
let Yr = 0;
const Qu = /* @__PURE__ */ Promise.resolve(), Yu = () => Yr || (Qu.then(() => Yr = 0), Yr = Date.now());
function Zu(e, t) {
  const n = (r) => {
    if (!r._vts)
      r._vts = Date.now();
    else if (r._vts <= n.attached)
      return;
    ct(
      Fu(r, n.value),
      t,
      5,
      [r]
    );
  };
  return n.value = e, n.attached = Yu(), n;
}
function Ti(e, t) {
  return Z(e) || Q(e) ? e : (mt(
    `Wrong type passed as event handler to ${t} - did you forget @ or : in front of your prop?
Expected function or array of functions, received type ${typeof e}.`
  ), me);
}
function Fu(e, t) {
  if (Q(t)) {
    const n = e.stopImmediatePropagation;
    return e.stopImmediatePropagation = () => {
      n.call(e), e._stopped = !0;
    }, t.map(
      (r) => (s) => !s._stopped && r && r(s)
    );
  } else
    return t;
}
const Li = (e) => e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && // lowercase letter
e.charCodeAt(2) > 96 && e.charCodeAt(2) < 123, Gu = (e, t, n, r, s, i) => {
  const o = s === "svg";
  t === "class" ? Ru(e, r, o) : t === "style" ? Lu(e, n, r) : Dn(t) ? _n(t) || Ju(e, t, n, r, i) : (t[0] === "." ? (t = t.slice(1), !0) : t[0] === "^" ? (t = t.slice(1), !1) : Vu(e, t, r, o)) ? (Ri(e, t, r), !e.tagName.includes("-") && (t === "value" || t === "checked" || t === "selected") && Bi(e, t, r, o, i, t !== "value")) : /* #11081 force set props for possible async custom element */ e._isVueCE && (/[A-Z]/.test(t) || !Ae(r)) ? Ri(e, Ve(t), r, i, t) : (t === "true-value" ? e._trueValue = r : t === "false-value" && (e._falseValue = r), Bi(e, t, r, o));
};
function Vu(e, t, n, r) {
  if (r)
    return !!(t === "innerHTML" || t === "textContent" || t in e && Li(t) && Z(n));
  if (t === "spellcheck" || t === "draggable" || t === "translate" || t === "form" || t === "list" && e.tagName === "INPUT" || t === "type" && e.tagName === "TEXTAREA")
    return !1;
  if (t === "width" || t === "height") {
    const s = e.tagName;
    if (s === "IMG" || s === "VIDEO" || s === "CANVAS" || s === "SOURCE")
      return !1;
  }
  return Li(t) && Ae(n) ? !1 : t in e;
}
const Wu = /* @__PURE__ */ pe({ patchProp: Gu }, Su);
let ki;
function Uu() {
  return ki || (ki = Kc(Wu));
}
const Ku = (...e) => {
  const t = Uu().createApp(...e);
  ({}).NODE_ENV !== "production" && (_u(t), $u(t));
  const { mount: n } = t;
  return t.mount = (r) => {
    const s = ea(r);
    if (!s)
      return;
    const i = t._component;
    !Z(i) && !i.render && !i.template && (i.template = s.innerHTML), s.nodeType === 1 && (s.textContent = "");
    const o = n(s, !1, qu(s));
    return s instanceof Element && (s.removeAttribute("v-cloak"), s.setAttribute("data-v-app", "")), o;
  }, t;
};
function qu(e) {
  if (e instanceof SVGElement)
    return "svg";
  if (typeof MathMLElement == "function" && e instanceof MathMLElement)
    return "mathml";
}
function _u(e) {
  Object.defineProperty(e.config, "isNativeTag", {
    value: (t) => al(t) || Al(t) || dl(t),
    writable: !1
  });
}
function $u(e) {
  {
    const t = e.config.isCustomElement;
    Object.defineProperty(e.config, "isCustomElement", {
      get() {
        return t;
      },
      set() {
        mt(
          "The `isCustomElement` config option is deprecated. Use `compilerOptions.isCustomElement` instead."
        );
      }
    });
    const n = e.config.compilerOptions, r = 'The `compilerOptions` config option is only respected when using a build of Vue.js that includes the runtime compiler (aka "full build"). Since you are using the runtime-only build, `compilerOptions` must be passed to `@vue/compiler-dom` in the build setup instead.\n- For vue-loader: pass it via vue-loader\'s `compilerOptions` loader option.\n- For vue-cli: see https://cli.vuejs.org/guide/webpack.html#modifying-options-of-a-loader\n- For vite: pass it via @vitejs/plugin-vue options. See https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue#example-for-passing-options-to-vuecompiler-sfc';
    Object.defineProperty(e.config, "compilerOptions", {
      get() {
        return mt(r), n;
      },
      set() {
        mt(r);
      }
    });
  }
}
function ea(e) {
  if (Ae(e)) {
    const t = document.querySelector(e);
    return {}.NODE_ENV !== "production" && !t && mt(
      `Failed to mount app: mount target selector "${e}" returned null.`
    ), t;
  }
  return {}.NODE_ENV !== "production" && window.ShadowRoot && e instanceof window.ShadowRoot && e.mode === "closed" && mt(
    'mounting on a ShadowRoot with `{mode: "closed"}` may lead to unpredictable bugs'
  ), e;
}
/**
* vue v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
function ta() {
  xu();
}
({}).NODE_ENV !== "production" && ta();
function vf(e) {
  return xs() ? (lo(e), !0) : !1;
}
function Fs(e) {
  return typeof e == "function" ? e() : ht(e);
}
const na = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const ra = Object.prototype.toString, sa = (e) => ra.call(e) === "[object Object]", ia = () => {
};
function oa(e) {
  return e || Pr();
}
function fa(e, t = !0, n) {
  oa() ? Rn(e, n) : t ? e() : Vt(e);
}
const Gs = na ? window : void 0;
function Jt(e) {
  var t;
  const n = Fs(e);
  return (t = n == null ? void 0 : n.$el) != null ? t : n;
}
function la(...e) {
  let t, n, r, s;
  if (typeof e[0] == "string" || Array.isArray(e[0]) ? ([n, r, s] = e, t = Gs) : [t, n, r, s] = e, !t)
    return ia;
  Array.isArray(n) || (n = [n]), Array.isArray(r) || (r = [r]);
  const i = [], o = () => {
    i.forEach((u) => u()), i.length = 0;
  }, f = (u, l, p, g) => (u.addEventListener(l, p, g), () => u.removeEventListener(l, p, g)), c = Ue(
    () => [Jt(t), Fs(s)],
    ([u, l]) => {
      if (o(), !u)
        return;
      const p = sa(l) ? { ...l } : l;
      i.push(
        ...n.flatMap((g) => r.map((v) => f(u, g, v, p)))
      );
    },
    { immediate: !0, flush: "post" }
  ), A = () => {
    c(), o();
  };
  return vf(A), A;
}
function ca() {
  const e = ze(!1), t = Pr();
  return t && Rn(() => {
    e.value = !0;
  }, t), e;
}
function ua(e) {
  const t = ca();
  return Ge(() => (t.value, !!e()));
}
function aa(e, t, n = {}) {
  const { window: r = Gs, ...s } = n;
  let i;
  const o = ua(() => r && "ResizeObserver" in r), f = () => {
    i && (i.disconnect(), i = void 0);
  }, c = Ge(() => {
    const l = Fs(e);
    return Array.isArray(l) ? l.map((p) => Jt(p)) : [Jt(l)];
  }), A = Ue(
    c,
    (l) => {
      if (f(), o.value && r) {
        i = new ResizeObserver(t);
        for (const p of l)
          p && i.observe(p, s);
      }
    },
    { immediate: !0, flush: "post" }
  ), u = () => {
    f(), A();
  };
  return vf(u), {
    isSupported: o,
    stop: u
  };
}
function Aa(e, t = { width: 0, height: 0 }, n = {}) {
  const { window: r = Gs, box: s = "content-box" } = n, i = Ge(() => {
    var l, p;
    return (p = (l = Jt(e)) == null ? void 0 : l.namespaceURI) == null ? void 0 : p.includes("svg");
  }), o = ze(t.width), f = ze(t.height), { stop: c } = aa(
    e,
    ([l]) => {
      const p = s === "border-box" ? l.borderBoxSize : s === "content-box" ? l.contentBoxSize : l.devicePixelContentBoxSize;
      if (r && i.value) {
        const g = Jt(e);
        if (g) {
          const v = g.getBoundingClientRect();
          o.value = v.width, f.value = v.height;
        }
      } else if (p) {
        const g = Array.isArray(p) ? p : [p];
        o.value = g.reduce((v, { inlineSize: h }) => v + h, 0), f.value = g.reduce((v, { blockSize: h }) => v + h, 0);
      } else
        o.value = l.contentRect.width, f.value = l.contentRect.height;
    },
    n
  );
  fa(() => {
    const l = Jt(e);
    l && (o.value = "offsetWidth" in l ? l.offsetWidth : t.width, f.value = "offsetHeight" in l ? l.offsetHeight : t.height);
  });
  const A = Ue(
    () => Jt(e),
    (l) => {
      o.value = l ? t.width : 0, f.value = l ? t.height : 0;
    }
  );
  function u() {
    c(), A();
  }
  return {
    width: o,
    height: f,
    stop: u
  };
}
var mf = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function Ef(e) {
  return e && e.__esModule && Object.prototype.hasOwnProperty.call(e, "default") ? e.default : e;
}
var yf = { exports: {} };
(function(e, t) {
  (function(n, r) {
    e.exports = r();
  })(mf, function() {
    var n = 1e3, r = 6e4, s = 36e5, i = "millisecond", o = "second", f = "minute", c = "hour", A = "day", u = "week", l = "month", p = "quarter", g = "year", v = "date", h = "Invalid Date", y = /^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/, x = /\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g, H = { name: "en", weekdays: "Sunday_Monday_Tuesday_Wednesday_Thursday_Friday_Saturday".split("_"), months: "January_February_March_April_May_June_July_August_September_October_November_December".split("_"), ordinal: function(F) {
      var k = ["th", "st", "nd", "rd"], N = F % 100;
      return "[" + F + (k[(N - 20) % 10] || k[N] || k[0]) + "]";
    } }, T = function(F, k, N) {
      var D = String(F);
      return !D || D.length >= k ? F : "" + Array(k + 1 - D.length).join(N) + F;
    }, G = { s: T, z: function(F) {
      var k = -F.utcOffset(), N = Math.abs(k), D = Math.floor(N / 60), I = N % 60;
      return (k <= 0 ? "+" : "-") + T(D, 2, "0") + ":" + T(I, 2, "0");
    }, m: function F(k, N) {
      if (k.date() < N.date())
        return -F(N, k);
      var D = 12 * (N.year() - k.year()) + (N.month() - k.month()), I = k.clone().add(D, l), X = N - I < 0, V = k.clone().add(D + (X ? -1 : 1), l);
      return +(-(D + (N - I) / (X ? I - V : V - I)) || 0);
    }, a: function(F) {
      return F < 0 ? Math.ceil(F) || 0 : Math.floor(F);
    }, p: function(F) {
      return { M: l, y: g, w: u, d: A, D: v, h: c, m: f, s: o, ms: i, Q: p }[F] || String(F || "").toLowerCase().replace(/s$/, "");
    }, u: function(F) {
      return F === void 0;
    } }, R = "en", U = {};
    U[R] = H;
    var re = "$isDayjsObject", ee = function(F) {
      return F instanceof fe || !(!F || !F[re]);
    }, oe = function F(k, N, D) {
      var I;
      if (!k)
        return R;
      if (typeof k == "string") {
        var X = k.toLowerCase();
        U[X] && (I = X), N && (U[X] = N, I = X);
        var V = k.split("-");
        if (!I && V.length > 1)
          return F(V[0]);
      } else {
        var te = k.name;
        U[te] = k, I = te;
      }
      return !D && I && (R = I), I || !D && R;
    }, j = function(F, k) {
      if (ee(F))
        return F.clone();
      var N = typeof k == "object" ? k : {};
      return N.date = F, N.args = arguments, new fe(N);
    }, J = G;
    J.l = oe, J.i = ee, J.w = function(F, k) {
      return j(F, { locale: k.$L, utc: k.$u, x: k.$x, $offset: k.$offset });
    };
    var fe = function() {
      function F(N) {
        this.$L = oe(N.locale, null, !0), this.parse(N), this.$x = this.$x || N.x || {}, this[re] = !0;
      }
      var k = F.prototype;
      return k.parse = function(N) {
        this.$d = function(D) {
          var I = D.date, X = D.utc;
          if (I === null)
            return /* @__PURE__ */ new Date(NaN);
          if (J.u(I))
            return /* @__PURE__ */ new Date();
          if (I instanceof Date)
            return new Date(I);
          if (typeof I == "string" && !/Z$/i.test(I)) {
            var V = I.match(y);
            if (V) {
              var te = V[2] - 1 || 0, le = (V[7] || "0").substring(0, 3);
              return X ? new Date(Date.UTC(V[1], te, V[3] || 1, V[4] || 0, V[5] || 0, V[6] || 0, le)) : new Date(V[1], te, V[3] || 1, V[4] || 0, V[5] || 0, V[6] || 0, le);
            }
          }
          return new Date(I);
        }(N), this.init();
      }, k.init = function() {
        var N = this.$d;
        this.$y = N.getFullYear(), this.$M = N.getMonth(), this.$D = N.getDate(), this.$W = N.getDay(), this.$H = N.getHours(), this.$m = N.getMinutes(), this.$s = N.getSeconds(), this.$ms = N.getMilliseconds();
      }, k.$utils = function() {
        return J;
      }, k.isValid = function() {
        return this.$d.toString() !== h;
      }, k.isSame = function(N, D) {
        var I = j(N);
        return this.startOf(D) <= I && I <= this.endOf(D);
      }, k.isAfter = function(N, D) {
        return j(N) < this.startOf(D);
      }, k.isBefore = function(N, D) {
        return this.endOf(D) < j(N);
      }, k.$g = function(N, D, I) {
        return J.u(N) ? this[D] : this.set(I, N);
      }, k.unix = function() {
        return Math.floor(this.valueOf() / 1e3);
      }, k.valueOf = function() {
        return this.$d.getTime();
      }, k.startOf = function(N, D) {
        var I = this, X = !!J.u(D) || D, V = J.p(N), te = function(Ye, be) {
          var Ce = J.w(I.$u ? Date.UTC(I.$y, be, Ye) : new Date(I.$y, be, Ye), I);
          return X ? Ce : Ce.endOf(A);
        }, le = function(Ye, be) {
          return J.w(I.toDate()[Ye].apply(I.toDate("s"), (X ? [0, 0, 0, 0] : [23, 59, 59, 999]).slice(be)), I);
        }, ue = this.$W, he = this.$M, Re = this.$D, Pt = "set" + (this.$u ? "UTC" : "");
        switch (V) {
          case g:
            return X ? te(1, 0) : te(31, 11);
          case l:
            return X ? te(1, he) : te(0, he + 1);
          case u:
            var Me = this.$locale().weekStart || 0, $e = (ue < Me ? ue + 7 : ue) - Me;
            return te(X ? Re - $e : Re + (6 - $e), he);
          case A:
          case v:
            return le(Pt + "Hours", 0);
          case c:
            return le(Pt + "Minutes", 1);
          case f:
            return le(Pt + "Seconds", 2);
          case o:
            return le(Pt + "Milliseconds", 3);
          default:
            return this.clone();
        }
      }, k.endOf = function(N) {
        return this.startOf(N, !1);
      }, k.$set = function(N, D) {
        var I, X = J.p(N), V = "set" + (this.$u ? "UTC" : ""), te = (I = {}, I[A] = V + "Date", I[v] = V + "Date", I[l] = V + "Month", I[g] = V + "FullYear", I[c] = V + "Hours", I[f] = V + "Minutes", I[o] = V + "Seconds", I[i] = V + "Milliseconds", I)[X], le = X === A ? this.$D + (D - this.$W) : D;
        if (X === l || X === g) {
          var ue = this.clone().set(v, 1);
          ue.$d[te](le), ue.init(), this.$d = ue.set(v, Math.min(this.$D, ue.daysInMonth())).$d;
        } else
          te && this.$d[te](le);
        return this.init(), this;
      }, k.set = function(N, D) {
        return this.clone().$set(N, D);
      }, k.get = function(N) {
        return this[J.p(N)]();
      }, k.add = function(N, D) {
        var I, X = this;
        N = Number(N);
        var V = J.p(D), te = function(he) {
          var Re = j(X);
          return J.w(Re.date(Re.date() + Math.round(he * N)), X);
        };
        if (V === l)
          return this.set(l, this.$M + N);
        if (V === g)
          return this.set(g, this.$y + N);
        if (V === A)
          return te(1);
        if (V === u)
          return te(7);
        var le = (I = {}, I[f] = r, I[c] = s, I[o] = n, I)[V] || 1, ue = this.$d.getTime() + N * le;
        return J.w(ue, this);
      }, k.subtract = function(N, D) {
        return this.add(-1 * N, D);
      }, k.format = function(N) {
        var D = this, I = this.$locale();
        if (!this.isValid())
          return I.invalidDate || h;
        var X = N || "YYYY-MM-DDTHH:mm:ssZ", V = J.z(this), te = this.$H, le = this.$m, ue = this.$M, he = I.weekdays, Re = I.months, Pt = I.meridiem, Me = function(be, Ce, ut, Ct) {
          return be && (be[Ce] || be(D, X)) || ut[Ce].slice(0, Ct);
        }, $e = function(be) {
          return J.s(te % 12 || 12, be, "0");
        }, Ye = Pt || function(be, Ce, ut) {
          var Ct = be < 12 ? "AM" : "PM";
          return ut ? Ct.toLowerCase() : Ct;
        };
        return X.replace(x, function(be, Ce) {
          return Ce || function(ut) {
            switch (ut) {
              case "YY":
                return String(D.$y).slice(-2);
              case "YYYY":
                return J.s(D.$y, 4, "0");
              case "M":
                return ue + 1;
              case "MM":
                return J.s(ue + 1, 2, "0");
              case "MMM":
                return Me(I.monthsShort, ue, Re, 3);
              case "MMMM":
                return Me(Re, ue);
              case "D":
                return D.$D;
              case "DD":
                return J.s(D.$D, 2, "0");
              case "d":
                return String(D.$W);
              case "dd":
                return Me(I.weekdaysMin, D.$W, he, 2);
              case "ddd":
                return Me(I.weekdaysShort, D.$W, he, 3);
              case "dddd":
                return he[D.$W];
              case "H":
                return String(te);
              case "HH":
                return J.s(te, 2, "0");
              case "h":
                return $e(1);
              case "hh":
                return $e(2);
              case "a":
                return Ye(te, le, !0);
              case "A":
                return Ye(te, le, !1);
              case "m":
                return String(le);
              case "mm":
                return J.s(le, 2, "0");
              case "s":
                return String(D.$s);
              case "ss":
                return J.s(D.$s, 2, "0");
              case "SSS":
                return J.s(D.$ms, 3, "0");
              case "Z":
                return V;
            }
            return null;
          }(be) || V.replace(":", "");
        });
      }, k.utcOffset = function() {
        return 15 * -Math.round(this.$d.getTimezoneOffset() / 15);
      }, k.diff = function(N, D, I) {
        var X, V = this, te = J.p(D), le = j(N), ue = (le.utcOffset() - this.utcOffset()) * r, he = this - le, Re = function() {
          return J.m(V, le);
        };
        switch (te) {
          case g:
            X = Re() / 12;
            break;
          case l:
            X = Re();
            break;
          case p:
            X = Re() / 3;
            break;
          case u:
            X = (he - ue) / 6048e5;
            break;
          case A:
            X = (he - ue) / 864e5;
            break;
          case c:
            X = he / s;
            break;
          case f:
            X = he / r;
            break;
          case o:
            X = he / n;
            break;
          default:
            X = he;
        }
        return I ? X : J.a(X);
      }, k.daysInMonth = function() {
        return this.endOf(l).$D;
      }, k.$locale = function() {
        return U[this.$L];
      }, k.locale = function(N, D) {
        if (!N)
          return this.$L;
        var I = this.clone(), X = oe(N, D, !0);
        return X && (I.$L = X), I;
      }, k.clone = function() {
        return J.w(this.$d, this);
      }, k.toDate = function() {
        return new Date(this.valueOf());
      }, k.toJSON = function() {
        return this.isValid() ? this.toISOString() : null;
      }, k.toISOString = function() {
        return this.$d.toISOString();
      }, k.toString = function() {
        return this.$d.toUTCString();
      }, F;
    }(), Tt = fe.prototype;
    return j.prototype = Tt, [["$ms", i], ["$s", o], ["$m", f], ["$H", c], ["$W", A], ["$M", l], ["$y", g], ["$D", v]].forEach(function(F) {
      Tt[F[1]] = function(k) {
        return this.$g(k, F[0], F[1]);
      };
    }), j.extend = function(F, k) {
      return F.$i || (F(k, fe, j), F.$i = !0), j;
    }, j.locale = oe, j.isDayjs = ee, j.unix = function(F) {
      return j(1e3 * F);
    }, j.en = U[R], j.Ls = U, j.p = {}, j;
  });
})(yf);
var da = yf.exports;
const ur = /* @__PURE__ */ Ef(da);
var bf = { exports: {} };
(function(e, t) {
  (function(n, r) {
    e.exports = r();
  })(mf, function() {
    var n = "minute", r = /[+-]\d\d(?::?\d\d)?/g, s = /([+-]|\d\d)/g;
    return function(i, o, f) {
      var c = o.prototype;
      f.utc = function(h) {
        var y = { date: h, utc: !0, args: arguments };
        return new o(y);
      }, c.utc = function(h) {
        var y = f(this.toDate(), { locale: this.$L, utc: !0 });
        return h ? y.add(this.utcOffset(), n) : y;
      }, c.local = function() {
        return f(this.toDate(), { locale: this.$L, utc: !1 });
      };
      var A = c.parse;
      c.parse = function(h) {
        h.utc && (this.$u = !0), this.$utils().u(h.$offset) || (this.$offset = h.$offset), A.call(this, h);
      };
      var u = c.init;
      c.init = function() {
        if (this.$u) {
          var h = this.$d;
          this.$y = h.getUTCFullYear(), this.$M = h.getUTCMonth(), this.$D = h.getUTCDate(), this.$W = h.getUTCDay(), this.$H = h.getUTCHours(), this.$m = h.getUTCMinutes(), this.$s = h.getUTCSeconds(), this.$ms = h.getUTCMilliseconds();
        } else
          u.call(this);
      };
      var l = c.utcOffset;
      c.utcOffset = function(h, y) {
        var x = this.$utils().u;
        if (x(h))
          return this.$u ? 0 : x(this.$offset) ? l.call(this) : this.$offset;
        if (typeof h == "string" && (h = function(R) {
          R === void 0 && (R = "");
          var U = R.match(r);
          if (!U)
            return null;
          var re = ("" + U[0]).match(s) || ["-", 0, 0], ee = re[0], oe = 60 * +re[1] + +re[2];
          return oe === 0 ? 0 : ee === "+" ? oe : -oe;
        }(h), h === null))
          return this;
        var H = Math.abs(h) <= 16 ? 60 * h : h, T = this;
        if (y)
          return T.$offset = H, T.$u = h === 0, T;
        if (h !== 0) {
          var G = this.$u ? this.toDate().getTimezoneOffset() : -1 * this.utcOffset();
          (T = this.local().add(H + G, n)).$offset = H, T.$x.$localOffset = G;
        } else
          T = this.utc();
        return T;
      };
      var p = c.format;
      c.format = function(h) {
        var y = h || (this.$u ? "YYYY-MM-DDTHH:mm:ss[Z]" : "");
        return p.call(this, y);
      }, c.valueOf = function() {
        var h = this.$utils().u(this.$offset) ? 0 : this.$offset + (this.$x.$localOffset || this.$d.getTimezoneOffset());
        return this.$d.valueOf() - 6e4 * h;
      }, c.isUTC = function() {
        return !!this.$u;
      }, c.toISOString = function() {
        return this.toDate().toISOString();
      }, c.toString = function() {
        return this.toDate().toUTCString();
      };
      var g = c.toDate;
      c.toDate = function(h) {
        return h === "s" && this.$offset ? f(this.format("YYYY-MM-DD HH:mm:ss:SSS")).toDate() : g.call(this);
      };
      var v = c.diff;
      c.diff = function(h, y, x) {
        if (h && this.$u === h.$u)
          return v.call(this, h, y, x);
        var H = this.local(), T = f(h).local();
        return v.call(H, T, y, x);
      };
    };
  });
})(bf);
var pa = bf.exports;
const ha = /* @__PURE__ */ Ef(pa);
ur.extend(ha);
function If() {
  return ur.utc().format("YYYY-MM-DD");
}
function wf(e) {
  if (!e)
    return "";
  const t = ur(), n = ur(e), r = t.diff(n, "second");
  if (r < 60)
    return `${r}s ago`;
  const s = t.diff(n, "minute");
  if (s < 60)
    return `${s}m ago`;
  const i = t.diff(n, "hour");
  if (i < 24)
    return `${i}h ago`;
  const o = t.diff(n, "day");
  return o < 7 ? `${o}d ago` : t.diff(n, "year") <= 1 ? n.format("MMM D") : n.format("MMM YYYY");
}
function ga(e) {
  return xs() ? (lo(e), !0) : !1;
}
function Vs(e) {
  return typeof e == "function" ? e() : ht(e);
}
const va = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const ma = Object.prototype.toString, Ea = (e) => ma.call(e) === "[object Object]", ya = () => {
};
function ba(e, t) {
  function n(...r) {
    return new Promise((s, i) => {
      Promise.resolve(e(() => t.apply(this, r), { fn: t, thisArg: this, args: r })).then(s).catch(i);
    });
  }
  return n;
}
const Pf = (e) => e();
function Ia(e = Pf) {
  const t = ze(!0);
  function n() {
    t.value = !1;
  }
  function r() {
    t.value = !0;
  }
  const s = (...i) => {
    t.value && e(...i);
  };
  return { isActive: Os(t), pause: n, resume: r, eventFilter: s };
}
function wa(e) {
  return e || Pr();
}
function Pa(e, t, n = {}) {
  const {
    eventFilter: r = Pf,
    ...s
  } = n;
  return Ue(
    e,
    ba(
      r,
      t
    ),
    s
  );
}
function Ca(e, t, n = {}) {
  const {
    eventFilter: r,
    ...s
  } = n, { eventFilter: i, pause: o, resume: f, isActive: c } = Ia(r);
  return { stop: Pa(
    e,
    t,
    {
      ...s,
      eventFilter: i
    }
  ), pause: o, resume: f, isActive: c };
}
function xa(e, t = !0, n) {
  wa() ? Rn(e, n) : t ? e() : Vt(e);
}
const ds = va ? window : void 0;
function Da(e) {
  var t;
  const n = Vs(e);
  return (t = n == null ? void 0 : n.$el) != null ? t : n;
}
function Hi(...e) {
  let t, n, r, s;
  if (typeof e[0] == "string" || Array.isArray(e[0]) ? ([n, r, s] = e, t = ds) : [t, n, r, s] = e, !t)
    return ya;
  Array.isArray(n) || (n = [n]), Array.isArray(r) || (r = [r]);
  const i = [], o = () => {
    i.forEach((u) => u()), i.length = 0;
  }, f = (u, l, p, g) => (u.addEventListener(l, p, g), () => u.removeEventListener(l, p, g)), c = Ue(
    () => [Da(t), Vs(s)],
    ([u, l]) => {
      if (o(), !u)
        return;
      const p = Ea(l) ? { ...l } : l;
      i.push(
        ...n.flatMap((g) => r.map((v) => f(u, g, v, p)))
      );
    },
    { immediate: !0, flush: "post" }
  ), A = () => {
    c(), o();
  };
  return ga(A), A;
}
const jn = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {}, zn = "__vueuse_ssr_handlers__", Na = /* @__PURE__ */ Sa();
function Sa() {
  return zn in jn || (jn[zn] = jn[zn] || {}), jn[zn];
}
function Ba(e, t) {
  return Na[e] || t;
}
function Ra(e) {
  return e == null ? "any" : e instanceof Set ? "set" : e instanceof Map ? "map" : e instanceof Date ? "date" : typeof e == "boolean" ? "boolean" : typeof e == "string" ? "string" : typeof e == "object" ? "object" : Number.isNaN(e) ? "any" : "number";
}
const Oa = {
  boolean: {
    read: (e) => e === "true",
    write: (e) => String(e)
  },
  object: {
    read: (e) => JSON.parse(e),
    write: (e) => JSON.stringify(e)
  },
  number: {
    read: (e) => Number.parseFloat(e),
    write: (e) => String(e)
  },
  any: {
    read: (e) => e,
    write: (e) => String(e)
  },
  string: {
    read: (e) => e,
    write: (e) => String(e)
  },
  map: {
    read: (e) => new Map(JSON.parse(e)),
    write: (e) => JSON.stringify(Array.from(e.entries()))
  },
  set: {
    read: (e) => new Set(JSON.parse(e)),
    write: (e) => JSON.stringify(Array.from(e))
  },
  date: {
    read: (e) => new Date(e),
    write: (e) => e.toISOString()
  }
}, ji = "vueuse-storage";
function Cf(e, t, n, r = {}) {
  var s;
  const {
    flush: i = "pre",
    deep: o = !0,
    listenToStorageChanges: f = !0,
    writeDefaults: c = !0,
    mergeDefaults: A = !1,
    shallow: u,
    window: l = ds,
    eventFilter: p,
    onError: g = (j) => {
      console.error(j);
    },
    initOnMounted: v
  } = r, h = (u ? ts : ze)(typeof t == "function" ? t() : t);
  if (!n)
    try {
      n = Ba("getDefaultStorage", () => {
        var j;
        return (j = ds) == null ? void 0 : j.localStorage;
      })();
    } catch (j) {
      g(j);
    }
  if (!n)
    return h;
  const y = Vs(t), x = Ra(y), H = (s = r.serializer) != null ? s : Oa[x], { pause: T, resume: G } = Ca(
    h,
    () => U(h.value),
    { flush: i, deep: o, eventFilter: p }
  );
  l && f && xa(() => {
    n instanceof Storage ? Hi(l, "storage", ee) : Hi(l, ji, oe), v && ee();
  }), v || ee();
  function R(j, J) {
    if (l) {
      const fe = {
        key: e,
        oldValue: j,
        newValue: J,
        storageArea: n
      };
      l.dispatchEvent(n instanceof Storage ? new StorageEvent("storage", fe) : new CustomEvent(ji, {
        detail: fe
      }));
    }
  }
  function U(j) {
    try {
      const J = n.getItem(e);
      if (j == null)
        R(J, null), n.removeItem(e);
      else {
        const fe = H.write(j);
        J !== fe && (n.setItem(e, fe), R(J, fe));
      }
    } catch (J) {
      g(J);
    }
  }
  function re(j) {
    const J = j ? j.newValue : n.getItem(e);
    if (J == null)
      return c && y != null && n.setItem(e, H.write(y)), y;
    if (!j && A) {
      const fe = H.read(J);
      return typeof A == "function" ? A(fe, y) : x === "object" && !Array.isArray(fe) ? { ...y, ...fe } : fe;
    } else
      return typeof J != "string" ? J : H.read(J);
  }
  function ee(j) {
    if (!(j && j.storageArea !== n)) {
      if (j && j.key == null) {
        h.value = y;
        return;
      }
      if (!(j && j.key !== e)) {
        T();
        try {
          (j == null ? void 0 : j.newValue) !== H.write(h.value) && (h.value = re(j));
        } catch (J) {
          g(J);
        } finally {
          j ? Vt(G) : G();
        }
      }
    }
  }
  function oe(j) {
    ee(j.detail);
  }
  return h;
}
function Ma() {
  return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (e) => {
    const t = Math.random() * 16 | 0;
    return (e === "x" ? t : t & 3 | 8).toString(16);
  });
}
function Ta() {
  const e = navigator.userAgent.toLowerCase();
  return /android/.test(e) ? "android" : /iphone|ipad|ipod/.test(e) ? "ios" : "pc";
}
const Cn = Ta() === "pc1";
function La() {
  const e = Ma();
  return Cf("mx-deviceId", e).value;
}
const Zr = Cf("mx-news-read-map", {
  // date: '2025-01-01',
  // asasd: true
});
function ka(e) {
  const t = If();
  t !== Zr.value.date && (Zr.value = { date: t }), Zr.value[e] = !0;
}
function xf(e, t) {
  return function() {
    return e.apply(t, arguments);
  };
}
const { toString: Ha } = Object.prototype, { getPrototypeOf: Ws } = Object, xr = ((e) => (t) => {
  const n = Ha.call(t);
  return e[n] || (e[n] = n.slice(8, -1).toLowerCase());
})(/* @__PURE__ */ Object.create(null)), _e = (e) => (e = e.toLowerCase(), (t) => xr(t) === e), Dr = (e) => (t) => typeof t === e, { isArray: sn } = Array, xn = Dr("undefined");
function ja(e) {
  return e !== null && !xn(e) && e.constructor !== null && !xn(e.constructor) && Xe(e.constructor.isBuffer) && e.constructor.isBuffer(e);
}
const Df = _e("ArrayBuffer");
function za(e) {
  let t;
  return typeof ArrayBuffer < "u" && ArrayBuffer.isView ? t = ArrayBuffer.isView(e) : t = e && e.buffer && Df(e.buffer), t;
}
const Ja = Dr("string"), Xe = Dr("function"), Nf = Dr("number"), Nr = (e) => e !== null && typeof e == "object", Xa = (e) => e === !0 || e === !1, Wn = (e) => {
  if (xr(e) !== "object")
    return !1;
  const t = Ws(e);
  return (t === null || t === Object.prototype || Object.getPrototypeOf(t) === null) && !(Symbol.toStringTag in e) && !(Symbol.iterator in e);
}, Qa = _e("Date"), Ya = _e("File"), Za = _e("Blob"), Fa = _e("FileList"), Ga = (e) => Nr(e) && Xe(e.pipe), Va = (e) => {
  let t;
  return e && (typeof FormData == "function" && e instanceof FormData || Xe(e.append) && ((t = xr(e)) === "formdata" || // detect form-data instance
  t === "object" && Xe(e.toString) && e.toString() === "[object FormData]"));
}, Wa = _e("URLSearchParams"), [Ua, Ka, qa, _a] = ["ReadableStream", "Request", "Response", "Headers"].map(_e), $a = (e) => e.trim ? e.trim() : e.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, "");
function Tn(e, t, { allOwnKeys: n = !1 } = {}) {
  if (e === null || typeof e > "u")
    return;
  let r, s;
  if (typeof e != "object" && (e = [e]), sn(e))
    for (r = 0, s = e.length; r < s; r++)
      t.call(null, e[r], r, e);
  else {
    const i = n ? Object.getOwnPropertyNames(e) : Object.keys(e), o = i.length;
    let f;
    for (r = 0; r < o; r++)
      f = i[r], t.call(null, e[f], f, e);
  }
}
function Sf(e, t) {
  t = t.toLowerCase();
  const n = Object.keys(e);
  let r = n.length, s;
  for (; r-- > 0; )
    if (s = n[r], t === s.toLowerCase())
      return s;
  return null;
}
const Xt = (() => typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : global)(), Bf = (e) => !xn(e) && e !== Xt;
function ps() {
  const { caseless: e } = Bf(this) && this || {}, t = {}, n = (r, s) => {
    const i = e && Sf(t, s) || s;
    Wn(t[i]) && Wn(r) ? t[i] = ps(t[i], r) : Wn(r) ? t[i] = ps({}, r) : sn(r) ? t[i] = r.slice() : t[i] = r;
  };
  for (let r = 0, s = arguments.length; r < s; r++)
    arguments[r] && Tn(arguments[r], n);
  return t;
}
const eA = (e, t, n, { allOwnKeys: r } = {}) => (Tn(t, (s, i) => {
  n && Xe(s) ? e[i] = xf(s, n) : e[i] = s;
}, { allOwnKeys: r }), e), tA = (e) => (e.charCodeAt(0) === 65279 && (e = e.slice(1)), e), nA = (e, t, n, r) => {
  e.prototype = Object.create(t.prototype, r), e.prototype.constructor = e, Object.defineProperty(e, "super", {
    value: t.prototype
  }), n && Object.assign(e.prototype, n);
}, rA = (e, t, n, r) => {
  let s, i, o;
  const f = {};
  if (t = t || {}, e == null)
    return t;
  do {
    for (s = Object.getOwnPropertyNames(e), i = s.length; i-- > 0; )
      o = s[i], (!r || r(o, e, t)) && !f[o] && (t[o] = e[o], f[o] = !0);
    e = n !== !1 && Ws(e);
  } while (e && (!n || n(e, t)) && e !== Object.prototype);
  return t;
}, sA = (e, t, n) => {
  e = String(e), (n === void 0 || n > e.length) && (n = e.length), n -= t.length;
  const r = e.indexOf(t, n);
  return r !== -1 && r === n;
}, iA = (e) => {
  if (!e)
    return null;
  if (sn(e))
    return e;
  let t = e.length;
  if (!Nf(t))
    return null;
  const n = new Array(t);
  for (; t-- > 0; )
    n[t] = e[t];
  return n;
}, oA = ((e) => (t) => e && t instanceof e)(typeof Uint8Array < "u" && Ws(Uint8Array)), fA = (e, t) => {
  const r = (e && e[Symbol.iterator]).call(e);
  let s;
  for (; (s = r.next()) && !s.done; ) {
    const i = s.value;
    t.call(e, i[0], i[1]);
  }
}, lA = (e, t) => {
  let n;
  const r = [];
  for (; (n = e.exec(t)) !== null; )
    r.push(n);
  return r;
}, cA = _e("HTMLFormElement"), uA = (e) => e.toLowerCase().replace(
  /[-_\s]([a-z\d])(\w*)/g,
  function(n, r, s) {
    return r.toUpperCase() + s;
  }
), zi = (({ hasOwnProperty: e }) => (t, n) => e.call(t, n))(Object.prototype), aA = _e("RegExp"), Rf = (e, t) => {
  const n = Object.getOwnPropertyDescriptors(e), r = {};
  Tn(n, (s, i) => {
    let o;
    (o = t(s, i, e)) !== !1 && (r[i] = o || s);
  }), Object.defineProperties(e, r);
}, AA = (e) => {
  Rf(e, (t, n) => {
    if (Xe(e) && ["arguments", "caller", "callee"].indexOf(n) !== -1)
      return !1;
    const r = e[n];
    if (Xe(r)) {
      if (t.enumerable = !1, "writable" in t) {
        t.writable = !1;
        return;
      }
      t.set || (t.set = () => {
        throw Error("Can not rewrite read-only method '" + n + "'");
      });
    }
  });
}, dA = (e, t) => {
  const n = {}, r = (s) => {
    s.forEach((i) => {
      n[i] = !0;
    });
  };
  return sn(e) ? r(e) : r(String(e).split(t)), n;
}, pA = () => {
}, hA = (e, t) => e != null && Number.isFinite(e = +e) ? e : t, Fr = "abcdefghijklmnopqrstuvwxyz", Ji = "0123456789", Of = {
  DIGIT: Ji,
  ALPHA: Fr,
  ALPHA_DIGIT: Fr + Fr.toUpperCase() + Ji
}, gA = (e = 16, t = Of.ALPHA_DIGIT) => {
  let n = "";
  const { length: r } = t;
  for (; e--; )
    n += t[Math.random() * r | 0];
  return n;
};
function vA(e) {
  return !!(e && Xe(e.append) && e[Symbol.toStringTag] === "FormData" && e[Symbol.iterator]);
}
const mA = (e) => {
  const t = new Array(10), n = (r, s) => {
    if (Nr(r)) {
      if (t.indexOf(r) >= 0)
        return;
      if (!("toJSON" in r)) {
        t[s] = r;
        const i = sn(r) ? [] : {};
        return Tn(r, (o, f) => {
          const c = n(o, s + 1);
          !xn(c) && (i[f] = c);
        }), t[s] = void 0, i;
      }
    }
    return r;
  };
  return n(e, 0);
}, EA = _e("AsyncFunction"), yA = (e) => e && (Nr(e) || Xe(e)) && Xe(e.then) && Xe(e.catch), Mf = ((e, t) => e ? setImmediate : t ? ((n, r) => (Xt.addEventListener("message", ({ source: s, data: i }) => {
  s === Xt && i === n && r.length && r.shift()();
}, !1), (s) => {
  r.push(s), Xt.postMessage(n, "*");
}))(`axios@${Math.random()}`, []) : (n) => setTimeout(n))(
  typeof setImmediate == "function",
  Xe(Xt.postMessage)
), bA = typeof queueMicrotask < "u" ? queueMicrotask.bind(Xt) : typeof process < "u" && process.nextTick || Mf, m = {
  isArray: sn,
  isArrayBuffer: Df,
  isBuffer: ja,
  isFormData: Va,
  isArrayBufferView: za,
  isString: Ja,
  isNumber: Nf,
  isBoolean: Xa,
  isObject: Nr,
  isPlainObject: Wn,
  isReadableStream: Ua,
  isRequest: Ka,
  isResponse: qa,
  isHeaders: _a,
  isUndefined: xn,
  isDate: Qa,
  isFile: Ya,
  isBlob: Za,
  isRegExp: aA,
  isFunction: Xe,
  isStream: Ga,
  isURLSearchParams: Wa,
  isTypedArray: oA,
  isFileList: Fa,
  forEach: Tn,
  merge: ps,
  extend: eA,
  trim: $a,
  stripBOM: tA,
  inherits: nA,
  toFlatObject: rA,
  kindOf: xr,
  kindOfTest: _e,
  endsWith: sA,
  toArray: iA,
  forEachEntry: fA,
  matchAll: lA,
  isHTMLForm: cA,
  hasOwnProperty: zi,
  hasOwnProp: zi,
  // an alias to avoid ESLint no-prototype-builtins detection
  reduceDescriptors: Rf,
  freezeMethods: AA,
  toObjectSet: dA,
  toCamelCase: uA,
  noop: pA,
  toFiniteNumber: hA,
  findKey: Sf,
  global: Xt,
  isContextDefined: Bf,
  ALPHABET: Of,
  generateString: gA,
  isSpecCompliantForm: vA,
  toJSONObject: mA,
  isAsyncFn: EA,
  isThenable: yA,
  setImmediate: Mf,
  asap: bA
};
function K(e, t, n, r, s) {
  Error.call(this), Error.captureStackTrace ? Error.captureStackTrace(this, this.constructor) : this.stack = new Error().stack, this.message = e, this.name = "AxiosError", t && (this.code = t), n && (this.config = n), r && (this.request = r), s && (this.response = s, this.status = s.status ? s.status : null);
}
m.inherits(K, Error, {
  toJSON: function() {
    return {
      // Standard
      message: this.message,
      name: this.name,
      // Microsoft
      description: this.description,
      number: this.number,
      // Mozilla
      fileName: this.fileName,
      lineNumber: this.lineNumber,
      columnNumber: this.columnNumber,
      stack: this.stack,
      // Axios
      config: m.toJSONObject(this.config),
      code: this.code,
      status: this.status
    };
  }
});
const Tf = K.prototype, Lf = {};
[
  "ERR_BAD_OPTION_VALUE",
  "ERR_BAD_OPTION",
  "ECONNABORTED",
  "ETIMEDOUT",
  "ERR_NETWORK",
  "ERR_FR_TOO_MANY_REDIRECTS",
  "ERR_DEPRECATED",
  "ERR_BAD_RESPONSE",
  "ERR_BAD_REQUEST",
  "ERR_CANCELED",
  "ERR_NOT_SUPPORT",
  "ERR_INVALID_URL"
  // eslint-disable-next-line func-names
].forEach((e) => {
  Lf[e] = { value: e };
});
Object.defineProperties(K, Lf);
Object.defineProperty(Tf, "isAxiosError", { value: !0 });
K.from = (e, t, n, r, s, i) => {
  const o = Object.create(Tf);
  return m.toFlatObject(e, o, function(c) {
    return c !== Error.prototype;
  }, (f) => f !== "isAxiosError"), K.call(o, e.message, t, n, r, s), o.cause = e, o.name = e.name, i && Object.assign(o, i), o;
};
const IA = null;
function hs(e) {
  return m.isPlainObject(e) || m.isArray(e);
}
function kf(e) {
  return m.endsWith(e, "[]") ? e.slice(0, -2) : e;
}
function Xi(e, t, n) {
  return e ? e.concat(t).map(function(s, i) {
    return s = kf(s), !n && i ? "[" + s + "]" : s;
  }).join(n ? "." : "") : t;
}
function wA(e) {
  return m.isArray(e) && !e.some(hs);
}
const PA = m.toFlatObject(m, {}, null, function(t) {
  return /^is[A-Z]/.test(t);
});
function Sr(e, t, n) {
  if (!m.isObject(e))
    throw new TypeError("target must be an object");
  t = t || new FormData(), n = m.toFlatObject(n, {
    metaTokens: !0,
    dots: !1,
    indexes: !1
  }, !1, function(h, y) {
    return !m.isUndefined(y[h]);
  });
  const r = n.metaTokens, s = n.visitor || u, i = n.dots, o = n.indexes, c = (n.Blob || typeof Blob < "u" && Blob) && m.isSpecCompliantForm(t);
  if (!m.isFunction(s))
    throw new TypeError("visitor must be a function");
  function A(v) {
    if (v === null)
      return "";
    if (m.isDate(v))
      return v.toISOString();
    if (!c && m.isBlob(v))
      throw new K("Blob is not supported. Use a Buffer instead.");
    return m.isArrayBuffer(v) || m.isTypedArray(v) ? c && typeof Blob == "function" ? new Blob([v]) : Buffer.from(v) : v;
  }
  function u(v, h, y) {
    let x = v;
    if (v && !y && typeof v == "object") {
      if (m.endsWith(h, "{}"))
        h = r ? h : h.slice(0, -2), v = JSON.stringify(v);
      else if (m.isArray(v) && wA(v) || (m.isFileList(v) || m.endsWith(h, "[]")) && (x = m.toArray(v)))
        return h = kf(h), x.forEach(function(T, G) {
          !(m.isUndefined(T) || T === null) && t.append(
            // eslint-disable-next-line no-nested-ternary
            o === !0 ? Xi([h], G, i) : o === null ? h : h + "[]",
            A(T)
          );
        }), !1;
    }
    return hs(v) ? !0 : (t.append(Xi(y, h, i), A(v)), !1);
  }
  const l = [], p = Object.assign(PA, {
    defaultVisitor: u,
    convertValue: A,
    isVisitable: hs
  });
  function g(v, h) {
    if (!m.isUndefined(v)) {
      if (l.indexOf(v) !== -1)
        throw Error("Circular reference detected in " + h.join("."));
      l.push(v), m.forEach(v, function(x, H) {
        (!(m.isUndefined(x) || x === null) && s.call(
          t,
          x,
          m.isString(H) ? H.trim() : H,
          h,
          p
        )) === !0 && g(x, h ? h.concat(H) : [H]);
      }), l.pop();
    }
  }
  if (!m.isObject(e))
    throw new TypeError("data must be an object");
  return g(e), t;
}
function Qi(e) {
  const t = {
    "!": "%21",
    "'": "%27",
    "(": "%28",
    ")": "%29",
    "~": "%7E",
    "%20": "+",
    "%00": "\0"
  };
  return encodeURIComponent(e).replace(/[!'()~]|%20|%00/g, function(r) {
    return t[r];
  });
}
function Us(e, t) {
  this._pairs = [], e && Sr(e, this, t);
}
const Hf = Us.prototype;
Hf.append = function(t, n) {
  this._pairs.push([t, n]);
};
Hf.toString = function(t) {
  const n = t ? function(r) {
    return t.call(this, r, Qi);
  } : Qi;
  return this._pairs.map(function(s) {
    return n(s[0]) + "=" + n(s[1]);
  }, "").join("&");
};
function CA(e) {
  return encodeURIComponent(e).replace(/%3A/gi, ":").replace(/%24/g, "$").replace(/%2C/gi, ",").replace(/%20/g, "+").replace(/%5B/gi, "[").replace(/%5D/gi, "]");
}
function jf(e, t, n) {
  if (!t)
    return e;
  const r = n && n.encode || CA;
  m.isFunction(n) && (n = {
    serialize: n
  });
  const s = n && n.serialize;
  let i;
  if (s ? i = s(t, n) : i = m.isURLSearchParams(t) ? t.toString() : new Us(t, n).toString(r), i) {
    const o = e.indexOf("#");
    o !== -1 && (e = e.slice(0, o)), e += (e.indexOf("?") === -1 ? "?" : "&") + i;
  }
  return e;
}
class xA {
  constructor() {
    this.handlers = [];
  }
  /**
   * Add a new interceptor to the stack
   *
   * @param {Function} fulfilled The function to handle `then` for a `Promise`
   * @param {Function} rejected The function to handle `reject` for a `Promise`
   *
   * @return {Number} An ID used to remove interceptor later
   */
  use(t, n, r) {
    return this.handlers.push({
      fulfilled: t,
      rejected: n,
      synchronous: r ? r.synchronous : !1,
      runWhen: r ? r.runWhen : null
    }), this.handlers.length - 1;
  }
  /**
   * Remove an interceptor from the stack
   *
   * @param {Number} id The ID that was returned by `use`
   *
   * @returns {Boolean} `true` if the interceptor was removed, `false` otherwise
   */
  eject(t) {
    this.handlers[t] && (this.handlers[t] = null);
  }
  /**
   * Clear all interceptors from the stack
   *
   * @returns {void}
   */
  clear() {
    this.handlers && (this.handlers = []);
  }
  /**
   * Iterate over all the registered interceptors
   *
   * This method is particularly useful for skipping over any
   * interceptors that may have become `null` calling `eject`.
   *
   * @param {Function} fn The function to call for each interceptor
   *
   * @returns {void}
   */
  forEach(t) {
    m.forEach(this.handlers, function(r) {
      r !== null && t(r);
    });
  }
}
const Yi = xA, zf = {
  silentJSONParsing: !0,
  forcedJSONParsing: !0,
  clarifyTimeoutError: !1
}, DA = typeof URLSearchParams < "u" ? URLSearchParams : Us, NA = typeof FormData < "u" ? FormData : null, SA = typeof Blob < "u" ? Blob : null, BA = {
  isBrowser: !0,
  classes: {
    URLSearchParams: DA,
    FormData: NA,
    Blob: SA
  },
  protocols: ["http", "https", "file", "blob", "url", "data"]
}, Ks = typeof window < "u" && typeof document < "u", gs = typeof navigator == "object" && navigator || void 0, RA = Ks && (!gs || ["ReactNative", "NativeScript", "NS"].indexOf(gs.product) < 0), OA = (() => typeof WorkerGlobalScope < "u" && // eslint-disable-next-line no-undef
self instanceof WorkerGlobalScope && typeof self.importScripts == "function")(), MA = Ks && window.location.href || "http://localhost", TA = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  hasBrowserEnv: Ks,
  hasStandardBrowserEnv: RA,
  hasStandardBrowserWebWorkerEnv: OA,
  navigator: gs,
  origin: MA
}, Symbol.toStringTag, { value: "Module" })), Pe = {
  ...TA,
  ...BA
};
function LA(e, t) {
  return Sr(e, new Pe.classes.URLSearchParams(), Object.assign({
    visitor: function(n, r, s, i) {
      return Pe.isNode && m.isBuffer(n) ? (this.append(r, n.toString("base64")), !1) : i.defaultVisitor.apply(this, arguments);
    }
  }, t));
}
function kA(e) {
  return m.matchAll(/\w+|\[(\w*)]/g, e).map((t) => t[0] === "[]" ? "" : t[1] || t[0]);
}
function HA(e) {
  const t = {}, n = Object.keys(e);
  let r;
  const s = n.length;
  let i;
  for (r = 0; r < s; r++)
    i = n[r], t[i] = e[i];
  return t;
}
function Jf(e) {
  function t(n, r, s, i) {
    let o = n[i++];
    if (o === "__proto__")
      return !0;
    const f = Number.isFinite(+o), c = i >= n.length;
    return o = !o && m.isArray(s) ? s.length : o, c ? (m.hasOwnProp(s, o) ? s[o] = [s[o], r] : s[o] = r, !f) : ((!s[o] || !m.isObject(s[o])) && (s[o] = []), t(n, r, s[o], i) && m.isArray(s[o]) && (s[o] = HA(s[o])), !f);
  }
  if (m.isFormData(e) && m.isFunction(e.entries)) {
    const n = {};
    return m.forEachEntry(e, (r, s) => {
      t(kA(r), s, n, 0);
    }), n;
  }
  return null;
}
function jA(e, t, n) {
  if (m.isString(e))
    try {
      return (t || JSON.parse)(e), m.trim(e);
    } catch (r) {
      if (r.name !== "SyntaxError")
        throw r;
    }
  return (n || JSON.stringify)(e);
}
const qs = {
  transitional: zf,
  adapter: ["xhr", "http", "fetch"],
  transformRequest: [function(t, n) {
    const r = n.getContentType() || "", s = r.indexOf("application/json") > -1, i = m.isObject(t);
    if (i && m.isHTMLForm(t) && (t = new FormData(t)), m.isFormData(t))
      return s ? JSON.stringify(Jf(t)) : t;
    if (m.isArrayBuffer(t) || m.isBuffer(t) || m.isStream(t) || m.isFile(t) || m.isBlob(t) || m.isReadableStream(t))
      return t;
    if (m.isArrayBufferView(t))
      return t.buffer;
    if (m.isURLSearchParams(t))
      return n.setContentType("application/x-www-form-urlencoded;charset=utf-8", !1), t.toString();
    let f;
    if (i) {
      if (r.indexOf("application/x-www-form-urlencoded") > -1)
        return LA(t, this.formSerializer).toString();
      if ((f = m.isFileList(t)) || r.indexOf("multipart/form-data") > -1) {
        const c = this.env && this.env.FormData;
        return Sr(
          f ? { "files[]": t } : t,
          c && new c(),
          this.formSerializer
        );
      }
    }
    return i || s ? (n.setContentType("application/json", !1), jA(t)) : t;
  }],
  transformResponse: [function(t) {
    const n = this.transitional || qs.transitional, r = n && n.forcedJSONParsing, s = this.responseType === "json";
    if (m.isResponse(t) || m.isReadableStream(t))
      return t;
    if (t && m.isString(t) && (r && !this.responseType || s)) {
      const o = !(n && n.silentJSONParsing) && s;
      try {
        return JSON.parse(t);
      } catch (f) {
        if (o)
          throw f.name === "SyntaxError" ? K.from(f, K.ERR_BAD_RESPONSE, this, null, this.response) : f;
      }
    }
    return t;
  }],
  /**
   * A timeout in milliseconds to abort a request. If set to 0 (default) a
   * timeout is not created.
   */
  timeout: 0,
  xsrfCookieName: "XSRF-TOKEN",
  xsrfHeaderName: "X-XSRF-TOKEN",
  maxContentLength: -1,
  maxBodyLength: -1,
  env: {
    FormData: Pe.classes.FormData,
    Blob: Pe.classes.Blob
  },
  validateStatus: function(t) {
    return t >= 200 && t < 300;
  },
  headers: {
    common: {
      Accept: "application/json, text/plain, */*",
      "Content-Type": void 0
    }
  }
};
m.forEach(["delete", "get", "head", "post", "put", "patch"], (e) => {
  qs.headers[e] = {};
});
const _s = qs, zA = m.toObjectSet([
  "age",
  "authorization",
  "content-length",
  "content-type",
  "etag",
  "expires",
  "from",
  "host",
  "if-modified-since",
  "if-unmodified-since",
  "last-modified",
  "location",
  "max-forwards",
  "proxy-authorization",
  "referer",
  "retry-after",
  "user-agent"
]), JA = (e) => {
  const t = {};
  let n, r, s;
  return e && e.split(`
`).forEach(function(o) {
    s = o.indexOf(":"), n = o.substring(0, s).trim().toLowerCase(), r = o.substring(s + 1).trim(), !(!n || t[n] && zA[n]) && (n === "set-cookie" ? t[n] ? t[n].push(r) : t[n] = [r] : t[n] = t[n] ? t[n] + ", " + r : r);
  }), t;
}, Zi = Symbol("internals");
function An(e) {
  return e && String(e).trim().toLowerCase();
}
function Un(e) {
  return e === !1 || e == null ? e : m.isArray(e) ? e.map(Un) : String(e);
}
function XA(e) {
  const t = /* @__PURE__ */ Object.create(null), n = /([^\s,;=]+)\s*(?:=\s*([^,;]+))?/g;
  let r;
  for (; r = n.exec(e); )
    t[r[1]] = r[2];
  return t;
}
const QA = (e) => /^[-_a-zA-Z0-9^`|~,!#$%&'*+.]+$/.test(e.trim());
function Gr(e, t, n, r, s) {
  if (m.isFunction(r))
    return r.call(this, t, n);
  if (s && (t = n), !!m.isString(t)) {
    if (m.isString(r))
      return t.indexOf(r) !== -1;
    if (m.isRegExp(r))
      return r.test(t);
  }
}
function YA(e) {
  return e.trim().toLowerCase().replace(/([a-z\d])(\w*)/g, (t, n, r) => n.toUpperCase() + r);
}
function ZA(e, t) {
  const n = m.toCamelCase(" " + t);
  ["get", "set", "has"].forEach((r) => {
    Object.defineProperty(e, r + n, {
      value: function(s, i, o) {
        return this[r].call(this, t, s, i, o);
      },
      configurable: !0
    });
  });
}
class Br {
  constructor(t) {
    t && this.set(t);
  }
  set(t, n, r) {
    const s = this;
    function i(f, c, A) {
      const u = An(c);
      if (!u)
        throw new Error("header name must be a non-empty string");
      const l = m.findKey(s, u);
      (!l || s[l] === void 0 || A === !0 || A === void 0 && s[l] !== !1) && (s[l || c] = Un(f));
    }
    const o = (f, c) => m.forEach(f, (A, u) => i(A, u, c));
    if (m.isPlainObject(t) || t instanceof this.constructor)
      o(t, n);
    else if (m.isString(t) && (t = t.trim()) && !QA(t))
      o(JA(t), n);
    else if (m.isHeaders(t))
      for (const [f, c] of t.entries())
        i(c, f, r);
    else
      t != null && i(n, t, r);
    return this;
  }
  get(t, n) {
    if (t = An(t), t) {
      const r = m.findKey(this, t);
      if (r) {
        const s = this[r];
        if (!n)
          return s;
        if (n === !0)
          return XA(s);
        if (m.isFunction(n))
          return n.call(this, s, r);
        if (m.isRegExp(n))
          return n.exec(s);
        throw new TypeError("parser must be boolean|regexp|function");
      }
    }
  }
  has(t, n) {
    if (t = An(t), t) {
      const r = m.findKey(this, t);
      return !!(r && this[r] !== void 0 && (!n || Gr(this, this[r], r, n)));
    }
    return !1;
  }
  delete(t, n) {
    const r = this;
    let s = !1;
    function i(o) {
      if (o = An(o), o) {
        const f = m.findKey(r, o);
        f && (!n || Gr(r, r[f], f, n)) && (delete r[f], s = !0);
      }
    }
    return m.isArray(t) ? t.forEach(i) : i(t), s;
  }
  clear(t) {
    const n = Object.keys(this);
    let r = n.length, s = !1;
    for (; r--; ) {
      const i = n[r];
      (!t || Gr(this, this[i], i, t, !0)) && (delete this[i], s = !0);
    }
    return s;
  }
  normalize(t) {
    const n = this, r = {};
    return m.forEach(this, (s, i) => {
      const o = m.findKey(r, i);
      if (o) {
        n[o] = Un(s), delete n[i];
        return;
      }
      const f = t ? YA(i) : String(i).trim();
      f !== i && delete n[i], n[f] = Un(s), r[f] = !0;
    }), this;
  }
  concat(...t) {
    return this.constructor.concat(this, ...t);
  }
  toJSON(t) {
    const n = /* @__PURE__ */ Object.create(null);
    return m.forEach(this, (r, s) => {
      r != null && r !== !1 && (n[s] = t && m.isArray(r) ? r.join(", ") : r);
    }), n;
  }
  [Symbol.iterator]() {
    return Object.entries(this.toJSON())[Symbol.iterator]();
  }
  toString() {
    return Object.entries(this.toJSON()).map(([t, n]) => t + ": " + n).join(`
`);
  }
  get [Symbol.toStringTag]() {
    return "AxiosHeaders";
  }
  static from(t) {
    return t instanceof this ? t : new this(t);
  }
  static concat(t, ...n) {
    const r = new this(t);
    return n.forEach((s) => r.set(s)), r;
  }
  static accessor(t) {
    const r = (this[Zi] = this[Zi] = {
      accessors: {}
    }).accessors, s = this.prototype;
    function i(o) {
      const f = An(o);
      r[f] || (ZA(s, o), r[f] = !0);
    }
    return m.isArray(t) ? t.forEach(i) : i(t), this;
  }
}
Br.accessor(["Content-Type", "Content-Length", "Accept", "Accept-Encoding", "User-Agent", "Authorization"]);
m.reduceDescriptors(Br.prototype, ({ value: e }, t) => {
  let n = t[0].toUpperCase() + t.slice(1);
  return {
    get: () => e,
    set(r) {
      this[n] = r;
    }
  };
});
m.freezeMethods(Br);
const Ke = Br;
function Vr(e, t) {
  const n = this || _s, r = t || n, s = Ke.from(r.headers);
  let i = r.data;
  return m.forEach(e, function(f) {
    i = f.call(n, i, s.normalize(), t ? t.status : void 0);
  }), s.normalize(), i;
}
function Xf(e) {
  return !!(e && e.__CANCEL__);
}
function on(e, t, n) {
  K.call(this, e ?? "canceled", K.ERR_CANCELED, t, n), this.name = "CanceledError";
}
m.inherits(on, K, {
  __CANCEL__: !0
});
function Qf(e, t, n) {
  const r = n.config.validateStatus;
  !n.status || !r || r(n.status) ? e(n) : t(new K(
    "Request failed with status code " + n.status,
    [K.ERR_BAD_REQUEST, K.ERR_BAD_RESPONSE][Math.floor(n.status / 100) - 4],
    n.config,
    n.request,
    n
  ));
}
function FA(e) {
  const t = /^([-+\w]{1,25})(:?\/\/|:)/.exec(e);
  return t && t[1] || "";
}
function GA(e, t) {
  e = e || 10;
  const n = new Array(e), r = new Array(e);
  let s = 0, i = 0, o;
  return t = t !== void 0 ? t : 1e3, function(c) {
    const A = Date.now(), u = r[i];
    o || (o = A), n[s] = c, r[s] = A;
    let l = i, p = 0;
    for (; l !== s; )
      p += n[l++], l = l % e;
    if (s = (s + 1) % e, s === i && (i = (i + 1) % e), A - o < t)
      return;
    const g = u && A - u;
    return g ? Math.round(p * 1e3 / g) : void 0;
  };
}
function VA(e, t) {
  let n = 0, r = 1e3 / t, s, i;
  const o = (A, u = Date.now()) => {
    n = u, s = null, i && (clearTimeout(i), i = null), e.apply(null, A);
  };
  return [(...A) => {
    const u = Date.now(), l = u - n;
    l >= r ? o(A, u) : (s = A, i || (i = setTimeout(() => {
      i = null, o(s);
    }, r - l)));
  }, () => s && o(s)];
}
const ar = (e, t, n = 3) => {
  let r = 0;
  const s = GA(50, 250);
  return VA((i) => {
    const o = i.loaded, f = i.lengthComputable ? i.total : void 0, c = o - r, A = s(c), u = o <= f;
    r = o;
    const l = {
      loaded: o,
      total: f,
      progress: f ? o / f : void 0,
      bytes: c,
      rate: A || void 0,
      estimated: A && f && u ? (f - o) / A : void 0,
      event: i,
      lengthComputable: f != null,
      [t ? "download" : "upload"]: !0
    };
    e(l);
  }, n);
}, Fi = (e, t) => {
  const n = e != null;
  return [(r) => t[0]({
    lengthComputable: n,
    total: e,
    loaded: r
  }), t[1]];
}, Gi = (e) => (...t) => m.asap(() => e(...t)), WA = Pe.hasStandardBrowserEnv ? ((e, t) => (n) => (n = new URL(n, Pe.origin), e.protocol === n.protocol && e.host === n.host && (t || e.port === n.port)))(
  new URL(Pe.origin),
  Pe.navigator && /(msie|trident)/i.test(Pe.navigator.userAgent)
) : () => !0, UA = Pe.hasStandardBrowserEnv ? (
  // Standard browser envs support document.cookie
  {
    write(e, t, n, r, s, i) {
      const o = [e + "=" + encodeURIComponent(t)];
      m.isNumber(n) && o.push("expires=" + new Date(n).toGMTString()), m.isString(r) && o.push("path=" + r), m.isString(s) && o.push("domain=" + s), i === !0 && o.push("secure"), document.cookie = o.join("; ");
    },
    read(e) {
      const t = document.cookie.match(new RegExp("(^|;\\s*)(" + e + ")=([^;]*)"));
      return t ? decodeURIComponent(t[3]) : null;
    },
    remove(e) {
      this.write(e, "", Date.now() - 864e5);
    }
  }
) : (
  // Non-standard browser env (web workers, react-native) lack needed support.
  {
    write() {
    },
    read() {
      return null;
    },
    remove() {
    }
  }
);
function KA(e) {
  return /^([a-z][a-z\d+\-.]*:)?\/\//i.test(e);
}
function qA(e, t) {
  return t ? e.replace(/\/?\/$/, "") + "/" + t.replace(/^\/+/, "") : e;
}
function Yf(e, t) {
  return e && !KA(t) ? qA(e, t) : t;
}
const Vi = (e) => e instanceof Ke ? { ...e } : e;
function Ut(e, t) {
  t = t || {};
  const n = {};
  function r(A, u, l, p) {
    return m.isPlainObject(A) && m.isPlainObject(u) ? m.merge.call({ caseless: p }, A, u) : m.isPlainObject(u) ? m.merge({}, u) : m.isArray(u) ? u.slice() : u;
  }
  function s(A, u, l, p) {
    if (m.isUndefined(u)) {
      if (!m.isUndefined(A))
        return r(void 0, A, l, p);
    } else
      return r(A, u, l, p);
  }
  function i(A, u) {
    if (!m.isUndefined(u))
      return r(void 0, u);
  }
  function o(A, u) {
    if (m.isUndefined(u)) {
      if (!m.isUndefined(A))
        return r(void 0, A);
    } else
      return r(void 0, u);
  }
  function f(A, u, l) {
    if (l in t)
      return r(A, u);
    if (l in e)
      return r(void 0, A);
  }
  const c = {
    url: i,
    method: i,
    data: i,
    baseURL: o,
    transformRequest: o,
    transformResponse: o,
    paramsSerializer: o,
    timeout: o,
    timeoutMessage: o,
    withCredentials: o,
    withXSRFToken: o,
    adapter: o,
    responseType: o,
    xsrfCookieName: o,
    xsrfHeaderName: o,
    onUploadProgress: o,
    onDownloadProgress: o,
    decompress: o,
    maxContentLength: o,
    maxBodyLength: o,
    beforeRedirect: o,
    transport: o,
    httpAgent: o,
    httpsAgent: o,
    cancelToken: o,
    socketPath: o,
    responseEncoding: o,
    validateStatus: f,
    headers: (A, u, l) => s(Vi(A), Vi(u), l, !0)
  };
  return m.forEach(Object.keys(Object.assign({}, e, t)), function(u) {
    const l = c[u] || s, p = l(e[u], t[u], u);
    m.isUndefined(p) && l !== f || (n[u] = p);
  }), n;
}
const Zf = (e) => {
  const t = Ut({}, e);
  let { data: n, withXSRFToken: r, xsrfHeaderName: s, xsrfCookieName: i, headers: o, auth: f } = t;
  t.headers = o = Ke.from(o), t.url = jf(Yf(t.baseURL, t.url), e.params, e.paramsSerializer), f && o.set(
    "Authorization",
    "Basic " + btoa((f.username || "") + ":" + (f.password ? unescape(encodeURIComponent(f.password)) : ""))
  );
  let c;
  if (m.isFormData(n)) {
    if (Pe.hasStandardBrowserEnv || Pe.hasStandardBrowserWebWorkerEnv)
      o.setContentType(void 0);
    else if ((c = o.getContentType()) !== !1) {
      const [A, ...u] = c ? c.split(";").map((l) => l.trim()).filter(Boolean) : [];
      o.setContentType([A || "multipart/form-data", ...u].join("; "));
    }
  }
  if (Pe.hasStandardBrowserEnv && (r && m.isFunction(r) && (r = r(t)), r || r !== !1 && WA(t.url))) {
    const A = s && i && UA.read(i);
    A && o.set(s, A);
  }
  return t;
}, _A = typeof XMLHttpRequest < "u", $A = _A && function(e) {
  return new Promise(function(n, r) {
    const s = Zf(e);
    let i = s.data;
    const o = Ke.from(s.headers).normalize();
    let { responseType: f, onUploadProgress: c, onDownloadProgress: A } = s, u, l, p, g, v;
    function h() {
      g && g(), v && v(), s.cancelToken && s.cancelToken.unsubscribe(u), s.signal && s.signal.removeEventListener("abort", u);
    }
    let y = new XMLHttpRequest();
    y.open(s.method.toUpperCase(), s.url, !0), y.timeout = s.timeout;
    function x() {
      if (!y)
        return;
      const T = Ke.from(
        "getAllResponseHeaders" in y && y.getAllResponseHeaders()
      ), R = {
        data: !f || f === "text" || f === "json" ? y.responseText : y.response,
        status: y.status,
        statusText: y.statusText,
        headers: T,
        config: e,
        request: y
      };
      Qf(function(re) {
        n(re), h();
      }, function(re) {
        r(re), h();
      }, R), y = null;
    }
    "onloadend" in y ? y.onloadend = x : y.onreadystatechange = function() {
      !y || y.readyState !== 4 || y.status === 0 && !(y.responseURL && y.responseURL.indexOf("file:") === 0) || setTimeout(x);
    }, y.onabort = function() {
      y && (r(new K("Request aborted", K.ECONNABORTED, e, y)), y = null);
    }, y.onerror = function() {
      r(new K("Network Error", K.ERR_NETWORK, e, y)), y = null;
    }, y.ontimeout = function() {
      let G = s.timeout ? "timeout of " + s.timeout + "ms exceeded" : "timeout exceeded";
      const R = s.transitional || zf;
      s.timeoutErrorMessage && (G = s.timeoutErrorMessage), r(new K(
        G,
        R.clarifyTimeoutError ? K.ETIMEDOUT : K.ECONNABORTED,
        e,
        y
      )), y = null;
    }, i === void 0 && o.setContentType(null), "setRequestHeader" in y && m.forEach(o.toJSON(), function(G, R) {
      y.setRequestHeader(R, G);
    }), m.isUndefined(s.withCredentials) || (y.withCredentials = !!s.withCredentials), f && f !== "json" && (y.responseType = s.responseType), A && ([p, v] = ar(A, !0), y.addEventListener("progress", p)), c && y.upload && ([l, g] = ar(c), y.upload.addEventListener("progress", l), y.upload.addEventListener("loadend", g)), (s.cancelToken || s.signal) && (u = (T) => {
      y && (r(!T || T.type ? new on(null, e, y) : T), y.abort(), y = null);
    }, s.cancelToken && s.cancelToken.subscribe(u), s.signal && (s.signal.aborted ? u() : s.signal.addEventListener("abort", u)));
    const H = FA(s.url);
    if (H && Pe.protocols.indexOf(H) === -1) {
      r(new K("Unsupported protocol " + H + ":", K.ERR_BAD_REQUEST, e));
      return;
    }
    y.send(i || null);
  });
}, ed = (e, t) => {
  const { length: n } = e = e ? e.filter(Boolean) : [];
  if (t || n) {
    let r = new AbortController(), s;
    const i = function(A) {
      if (!s) {
        s = !0, f();
        const u = A instanceof Error ? A : this.reason;
        r.abort(u instanceof K ? u : new on(u instanceof Error ? u.message : u));
      }
    };
    let o = t && setTimeout(() => {
      o = null, i(new K(`timeout ${t} of ms exceeded`, K.ETIMEDOUT));
    }, t);
    const f = () => {
      e && (o && clearTimeout(o), o = null, e.forEach((A) => {
        A.unsubscribe ? A.unsubscribe(i) : A.removeEventListener("abort", i);
      }), e = null);
    };
    e.forEach((A) => A.addEventListener("abort", i));
    const { signal: c } = r;
    return c.unsubscribe = () => m.asap(f), c;
  }
}, td = ed, nd = function* (e, t) {
  let n = e.byteLength;
  if (!t || n < t) {
    yield e;
    return;
  }
  let r = 0, s;
  for (; r < n; )
    s = r + t, yield e.slice(r, s), r = s;
}, rd = async function* (e, t) {
  for await (const n of sd(e))
    yield* nd(n, t);
}, sd = async function* (e) {
  if (e[Symbol.asyncIterator]) {
    yield* e;
    return;
  }
  const t = e.getReader();
  try {
    for (; ; ) {
      const { done: n, value: r } = await t.read();
      if (n)
        break;
      yield r;
    }
  } finally {
    await t.cancel();
  }
}, Wi = (e, t, n, r) => {
  const s = rd(e, t);
  let i = 0, o, f = (c) => {
    o || (o = !0, r && r(c));
  };
  return new ReadableStream({
    async pull(c) {
      try {
        const { done: A, value: u } = await s.next();
        if (A) {
          f(), c.close();
          return;
        }
        let l = u.byteLength;
        if (n) {
          let p = i += l;
          n(p);
        }
        c.enqueue(new Uint8Array(u));
      } catch (A) {
        throw f(A), A;
      }
    },
    cancel(c) {
      return f(c), s.return();
    }
  }, {
    highWaterMark: 2
  });
}, Rr = typeof fetch == "function" && typeof Request == "function" && typeof Response == "function", Ff = Rr && typeof ReadableStream == "function", id = Rr && (typeof TextEncoder == "function" ? ((e) => (t) => e.encode(t))(new TextEncoder()) : async (e) => new Uint8Array(await new Response(e).arrayBuffer())), Gf = (e, ...t) => {
  try {
    return !!e(...t);
  } catch {
    return !1;
  }
}, od = Ff && Gf(() => {
  let e = !1;
  const t = new Request(Pe.origin, {
    body: new ReadableStream(),
    method: "POST",
    get duplex() {
      return e = !0, "half";
    }
  }).headers.has("Content-Type");
  return e && !t;
}), Ui = 64 * 1024, vs = Ff && Gf(() => m.isReadableStream(new Response("").body)), Ar = {
  stream: vs && ((e) => e.body)
};
Rr && ((e) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((t) => {
    !Ar[t] && (Ar[t] = m.isFunction(e[t]) ? (n) => n[t]() : (n, r) => {
      throw new K(`Response type '${t}' is not supported`, K.ERR_NOT_SUPPORT, r);
    });
  });
})(new Response());
const fd = async (e) => {
  if (e == null)
    return 0;
  if (m.isBlob(e))
    return e.size;
  if (m.isSpecCompliantForm(e))
    return (await new Request(Pe.origin, {
      method: "POST",
      body: e
    }).arrayBuffer()).byteLength;
  if (m.isArrayBufferView(e) || m.isArrayBuffer(e))
    return e.byteLength;
  if (m.isURLSearchParams(e) && (e = e + ""), m.isString(e))
    return (await id(e)).byteLength;
}, ld = async (e, t) => {
  const n = m.toFiniteNumber(e.getContentLength());
  return n ?? fd(t);
}, cd = Rr && (async (e) => {
  let {
    url: t,
    method: n,
    data: r,
    signal: s,
    cancelToken: i,
    timeout: o,
    onDownloadProgress: f,
    onUploadProgress: c,
    responseType: A,
    headers: u,
    withCredentials: l = "same-origin",
    fetchOptions: p
  } = Zf(e);
  A = A ? (A + "").toLowerCase() : "text";
  let g = td([s, i && i.toAbortSignal()], o), v;
  const h = g && g.unsubscribe && (() => {
    g.unsubscribe();
  });
  let y;
  try {
    if (c && od && n !== "get" && n !== "head" && (y = await ld(u, r)) !== 0) {
      let R = new Request(t, {
        method: "POST",
        body: r,
        duplex: "half"
      }), U;
      if (m.isFormData(r) && (U = R.headers.get("content-type")) && u.setContentType(U), R.body) {
        const [re, ee] = Fi(
          y,
          ar(Gi(c))
        );
        r = Wi(R.body, Ui, re, ee);
      }
    }
    m.isString(l) || (l = l ? "include" : "omit");
    const x = "credentials" in Request.prototype;
    v = new Request(t, {
      ...p,
      signal: g,
      method: n.toUpperCase(),
      headers: u.normalize().toJSON(),
      body: r,
      duplex: "half",
      credentials: x ? l : void 0
    });
    let H = await fetch(v);
    const T = vs && (A === "stream" || A === "response");
    if (vs && (f || T && h)) {
      const R = {};
      ["status", "statusText", "headers"].forEach((oe) => {
        R[oe] = H[oe];
      });
      const U = m.toFiniteNumber(H.headers.get("content-length")), [re, ee] = f && Fi(
        U,
        ar(Gi(f), !0)
      ) || [];
      H = new Response(
        Wi(H.body, Ui, re, () => {
          ee && ee(), h && h();
        }),
        R
      );
    }
    A = A || "text";
    let G = await Ar[m.findKey(Ar, A) || "text"](H, e);
    return !T && h && h(), await new Promise((R, U) => {
      Qf(R, U, {
        data: G,
        headers: Ke.from(H.headers),
        status: H.status,
        statusText: H.statusText,
        config: e,
        request: v
      });
    });
  } catch (x) {
    throw h && h(), x && x.name === "TypeError" && /fetch/i.test(x.message) ? Object.assign(
      new K("Network Error", K.ERR_NETWORK, e, v),
      {
        cause: x.cause || x
      }
    ) : K.from(x, x && x.code, e, v);
  }
}), ms = {
  http: IA,
  xhr: $A,
  fetch: cd
};
m.forEach(ms, (e, t) => {
  if (e) {
    try {
      Object.defineProperty(e, "name", { value: t });
    } catch {
    }
    Object.defineProperty(e, "adapterName", { value: t });
  }
});
const Ki = (e) => `- ${e}`, ud = (e) => m.isFunction(e) || e === null || e === !1, Vf = {
  getAdapter: (e) => {
    e = m.isArray(e) ? e : [e];
    const { length: t } = e;
    let n, r;
    const s = {};
    for (let i = 0; i < t; i++) {
      n = e[i];
      let o;
      if (r = n, !ud(n) && (r = ms[(o = String(n)).toLowerCase()], r === void 0))
        throw new K(`Unknown adapter '${o}'`);
      if (r)
        break;
      s[o || "#" + i] = r;
    }
    if (!r) {
      const i = Object.entries(s).map(
        ([f, c]) => `adapter ${f} ` + (c === !1 ? "is not supported by the environment" : "is not available in the build")
      );
      let o = t ? i.length > 1 ? `since :
` + i.map(Ki).join(`
`) : " " + Ki(i[0]) : "as no adapter specified";
      throw new K(
        "There is no suitable adapter to dispatch the request " + o,
        "ERR_NOT_SUPPORT"
      );
    }
    return r;
  },
  adapters: ms
};
function Wr(e) {
  if (e.cancelToken && e.cancelToken.throwIfRequested(), e.signal && e.signal.aborted)
    throw new on(null, e);
}
function qi(e) {
  return Wr(e), e.headers = Ke.from(e.headers), e.data = Vr.call(
    e,
    e.transformRequest
  ), ["post", "put", "patch"].indexOf(e.method) !== -1 && e.headers.setContentType("application/x-www-form-urlencoded", !1), Vf.getAdapter(e.adapter || _s.adapter)(e).then(function(r) {
    return Wr(e), r.data = Vr.call(
      e,
      e.transformResponse,
      r
    ), r.headers = Ke.from(r.headers), r;
  }, function(r) {
    return Xf(r) || (Wr(e), r && r.response && (r.response.data = Vr.call(
      e,
      e.transformResponse,
      r.response
    ), r.response.headers = Ke.from(r.response.headers))), Promise.reject(r);
  });
}
const Wf = "1.7.9", Or = {};
["object", "boolean", "number", "function", "string", "symbol"].forEach((e, t) => {
  Or[e] = function(r) {
    return typeof r === e || "a" + (t < 1 ? "n " : " ") + e;
  };
});
const _i = {};
Or.transitional = function(t, n, r) {
  function s(i, o) {
    return "[Axios v" + Wf + "] Transitional option '" + i + "'" + o + (r ? ". " + r : "");
  }
  return (i, o, f) => {
    if (t === !1)
      throw new K(
        s(o, " has been removed" + (n ? " in " + n : "")),
        K.ERR_DEPRECATED
      );
    return n && !_i[o] && (_i[o] = !0, console.warn(
      s(
        o,
        " has been deprecated since v" + n + " and will be removed in the near future"
      )
    )), t ? t(i, o, f) : !0;
  };
};
Or.spelling = function(t) {
  return (n, r) => (console.warn(`${r} is likely a misspelling of ${t}`), !0);
};
function ad(e, t, n) {
  if (typeof e != "object")
    throw new K("options must be an object", K.ERR_BAD_OPTION_VALUE);
  const r = Object.keys(e);
  let s = r.length;
  for (; s-- > 0; ) {
    const i = r[s], o = t[i];
    if (o) {
      const f = e[i], c = f === void 0 || o(f, i, e);
      if (c !== !0)
        throw new K("option " + i + " must be " + c, K.ERR_BAD_OPTION_VALUE);
      continue;
    }
    if (n !== !0)
      throw new K("Unknown option " + i, K.ERR_BAD_OPTION);
  }
}
const Kn = {
  assertOptions: ad,
  validators: Or
}, nt = Kn.validators;
class dr {
  constructor(t) {
    this.defaults = t, this.interceptors = {
      request: new Yi(),
      response: new Yi()
    };
  }
  /**
   * Dispatch a request
   *
   * @param {String|Object} configOrUrl The config specific for this request (merged with this.defaults)
   * @param {?Object} config
   *
   * @returns {Promise} The Promise to be fulfilled
   */
  async request(t, n) {
    try {
      return await this._request(t, n);
    } catch (r) {
      if (r instanceof Error) {
        let s = {};
        Error.captureStackTrace ? Error.captureStackTrace(s) : s = new Error();
        const i = s.stack ? s.stack.replace(/^.+\n/, "") : "";
        try {
          r.stack ? i && !String(r.stack).endsWith(i.replace(/^.+\n.+\n/, "")) && (r.stack += `
` + i) : r.stack = i;
        } catch {
        }
      }
      throw r;
    }
  }
  _request(t, n) {
    typeof t == "string" ? (n = n || {}, n.url = t) : n = t || {}, n = Ut(this.defaults, n);
    const { transitional: r, paramsSerializer: s, headers: i } = n;
    r !== void 0 && Kn.assertOptions(r, {
      silentJSONParsing: nt.transitional(nt.boolean),
      forcedJSONParsing: nt.transitional(nt.boolean),
      clarifyTimeoutError: nt.transitional(nt.boolean)
    }, !1), s != null && (m.isFunction(s) ? n.paramsSerializer = {
      serialize: s
    } : Kn.assertOptions(s, {
      encode: nt.function,
      serialize: nt.function
    }, !0)), Kn.assertOptions(n, {
      baseUrl: nt.spelling("baseURL"),
      withXsrfToken: nt.spelling("withXSRFToken")
    }, !0), n.method = (n.method || this.defaults.method || "get").toLowerCase();
    let o = i && m.merge(
      i.common,
      i[n.method]
    );
    i && m.forEach(
      ["delete", "get", "head", "post", "put", "patch", "common"],
      (v) => {
        delete i[v];
      }
    ), n.headers = Ke.concat(o, i);
    const f = [];
    let c = !0;
    this.interceptors.request.forEach(function(h) {
      typeof h.runWhen == "function" && h.runWhen(n) === !1 || (c = c && h.synchronous, f.unshift(h.fulfilled, h.rejected));
    });
    const A = [];
    this.interceptors.response.forEach(function(h) {
      A.push(h.fulfilled, h.rejected);
    });
    let u, l = 0, p;
    if (!c) {
      const v = [qi.bind(this), void 0];
      for (v.unshift.apply(v, f), v.push.apply(v, A), p = v.length, u = Promise.resolve(n); l < p; )
        u = u.then(v[l++], v[l++]);
      return u;
    }
    p = f.length;
    let g = n;
    for (l = 0; l < p; ) {
      const v = f[l++], h = f[l++];
      try {
        g = v(g);
      } catch (y) {
        h.call(this, y);
        break;
      }
    }
    try {
      u = qi.call(this, g);
    } catch (v) {
      return Promise.reject(v);
    }
    for (l = 0, p = A.length; l < p; )
      u = u.then(A[l++], A[l++]);
    return u;
  }
  getUri(t) {
    t = Ut(this.defaults, t);
    const n = Yf(t.baseURL, t.url);
    return jf(n, t.params, t.paramsSerializer);
  }
}
m.forEach(["delete", "get", "head", "options"], function(t) {
  dr.prototype[t] = function(n, r) {
    return this.request(Ut(r || {}, {
      method: t,
      url: n,
      data: (r || {}).data
    }));
  };
});
m.forEach(["post", "put", "patch"], function(t) {
  function n(r) {
    return function(i, o, f) {
      return this.request(Ut(f || {}, {
        method: t,
        headers: r ? {
          "Content-Type": "multipart/form-data"
        } : {},
        url: i,
        data: o
      }));
    };
  }
  dr.prototype[t] = n(), dr.prototype[t + "Form"] = n(!0);
});
const qn = dr;
class $s {
  constructor(t) {
    if (typeof t != "function")
      throw new TypeError("executor must be a function.");
    let n;
    this.promise = new Promise(function(i) {
      n = i;
    });
    const r = this;
    this.promise.then((s) => {
      if (!r._listeners)
        return;
      let i = r._listeners.length;
      for (; i-- > 0; )
        r._listeners[i](s);
      r._listeners = null;
    }), this.promise.then = (s) => {
      let i;
      const o = new Promise((f) => {
        r.subscribe(f), i = f;
      }).then(s);
      return o.cancel = function() {
        r.unsubscribe(i);
      }, o;
    }, t(function(i, o, f) {
      r.reason || (r.reason = new on(i, o, f), n(r.reason));
    });
  }
  /**
   * Throws a `CanceledError` if cancellation has been requested.
   */
  throwIfRequested() {
    if (this.reason)
      throw this.reason;
  }
  /**
   * Subscribe to the cancel signal
   */
  subscribe(t) {
    if (this.reason) {
      t(this.reason);
      return;
    }
    this._listeners ? this._listeners.push(t) : this._listeners = [t];
  }
  /**
   * Unsubscribe from the cancel signal
   */
  unsubscribe(t) {
    if (!this._listeners)
      return;
    const n = this._listeners.indexOf(t);
    n !== -1 && this._listeners.splice(n, 1);
  }
  toAbortSignal() {
    const t = new AbortController(), n = (r) => {
      t.abort(r);
    };
    return this.subscribe(n), t.signal.unsubscribe = () => this.unsubscribe(n), t.signal;
  }
  /**
   * Returns an object that contains a new `CancelToken` and a function that, when called,
   * cancels the `CancelToken`.
   */
  static source() {
    let t;
    return {
      token: new $s(function(s) {
        t = s;
      }),
      cancel: t
    };
  }
}
const Ad = $s;
function dd(e) {
  return function(n) {
    return e.apply(null, n);
  };
}
function pd(e) {
  return m.isObject(e) && e.isAxiosError === !0;
}
const Es = {
  Continue: 100,
  SwitchingProtocols: 101,
  Processing: 102,
  EarlyHints: 103,
  Ok: 200,
  Created: 201,
  Accepted: 202,
  NonAuthoritativeInformation: 203,
  NoContent: 204,
  ResetContent: 205,
  PartialContent: 206,
  MultiStatus: 207,
  AlreadyReported: 208,
  ImUsed: 226,
  MultipleChoices: 300,
  MovedPermanently: 301,
  Found: 302,
  SeeOther: 303,
  NotModified: 304,
  UseProxy: 305,
  Unused: 306,
  TemporaryRedirect: 307,
  PermanentRedirect: 308,
  BadRequest: 400,
  Unauthorized: 401,
  PaymentRequired: 402,
  Forbidden: 403,
  NotFound: 404,
  MethodNotAllowed: 405,
  NotAcceptable: 406,
  ProxyAuthenticationRequired: 407,
  RequestTimeout: 408,
  Conflict: 409,
  Gone: 410,
  LengthRequired: 411,
  PreconditionFailed: 412,
  PayloadTooLarge: 413,
  UriTooLong: 414,
  UnsupportedMediaType: 415,
  RangeNotSatisfiable: 416,
  ExpectationFailed: 417,
  ImATeapot: 418,
  MisdirectedRequest: 421,
  UnprocessableEntity: 422,
  Locked: 423,
  FailedDependency: 424,
  TooEarly: 425,
  UpgradeRequired: 426,
  PreconditionRequired: 428,
  TooManyRequests: 429,
  RequestHeaderFieldsTooLarge: 431,
  UnavailableForLegalReasons: 451,
  InternalServerError: 500,
  NotImplemented: 501,
  BadGateway: 502,
  ServiceUnavailable: 503,
  GatewayTimeout: 504,
  HttpVersionNotSupported: 505,
  VariantAlsoNegotiates: 506,
  InsufficientStorage: 507,
  LoopDetected: 508,
  NotExtended: 510,
  NetworkAuthenticationRequired: 511
};
Object.entries(Es).forEach(([e, t]) => {
  Es[t] = e;
});
const hd = Es;
function Uf(e) {
  const t = new qn(e), n = xf(qn.prototype.request, t);
  return m.extend(n, qn.prototype, t, { allOwnKeys: !0 }), m.extend(n, t, null, { allOwnKeys: !0 }), n.create = function(s) {
    return Uf(Ut(e, s));
  }, n;
}
const ge = Uf(_s);
ge.Axios = qn;
ge.CanceledError = on;
ge.CancelToken = Ad;
ge.isCancel = Xf;
ge.VERSION = Wf;
ge.toFormData = Sr;
ge.AxiosError = K;
ge.Cancel = ge.CanceledError;
ge.all = function(t) {
  return Promise.all(t);
};
ge.spread = dd;
ge.isAxiosError = pd;
ge.mergeConfig = Ut;
ge.AxiosHeaders = Ke;
ge.formToJSON = (e) => Jf(m.isHTMLForm(e) ? new FormData(e) : e);
ge.getAdapter = Vf.getAdapter;
ge.HttpStatusCode = hd;
ge.default = ge;
const gd = ge, vd = "https://mxnewsdata.pages.dev/news_en", md = "https://mxnews.pages.dev/";
async function Ed(e = "usa") {
  const t = If(), r = (await gd.get(`${vd}/${t}/${e}.json`)).data, s = [];
  return typeof r == "object" && !r.code && Object.keys(r).forEach((i) => {
    const o = r[i];
    if (!o.cached || !o.image)
      return;
    const f = {
      ...o,
      hash: i,
      category: o.category,
      title: o.title,
      tag: o.source,
      date: wf(o.pubDate),
      imageList: [o.image, ...o.images || []],
      link: `${md}/#/detail/${o.category.toLowerCase()}/${i}`
    };
    s.push(f);
  }), {
    data: s
  };
}
const yd = "https://api.taboola.com/2.0/json/maxthonbrowser-us-english/recommendations.get", $i = Date.now();
async function bd(e = "Trending", t = "init") {
  const n = {
    placements: [],
    // 用户信息
    user: {
      // 请求单元，第一次请求是 init，后续用上次返回的值
      session: t,
      // 设备ID，每个设备随机生成一个，后续从本地取，比如 'a3ad232c-9f08-45c5-9051-051409edb405'
      device: La(),
      // 以下固定：用户IP和设备信息
      // realip: '192.199.248.75',
      agent: navigator.userAgent
    },
    view: {
      // 页面ID，每次刷新随机生成一个，可以用时间戳，比如 'a558e7763d614902a3689c69b23c25a7'
      id: `view_${e}_${$i}`
    },
    app: {
      // 傲游key
      apiKey: "6d24fb0e092f4d688803aa8dd2b40fc43a8dad37",
      // SERVER or CLIENT
      origin: "CLIENT",
      // 以下固定
      type: "MOBILE",
      name: "maxthonbrowser-us-english"
    },
    source: {
      // 页面ID，每次刷新随机生成一个，可以用时间戳，比如 'a558e7763d614902a3689c69b23c25a7'
      id: `source_${e}_${$i}`,
      // "home" or "article" or "video"
      type: "HOME",
      // 当前网址
      url: location.href
    }
  };
  for (let u = 0; u < 9; u++) {
    const l = u + 1;
    n.placements.push({
      // Editorial Trending 1
      name: l % 3 === 0 ? `AD Editorial ${e} ${l}` : `Editorial ${e} ${l}`,
      // 以下固定
      recCount: 1,
      organicType: "mix"
    });
  }
  const s = await (await fetch(yd, {
    method: "post",
    credentials: "include",
    body: JSON.stringify(n)
  })).json(), i = [], o = [];
  s.placements.forEach((u) => {
    var v;
    const l = u.list[0], p = !l.categories, g = {
      ...l,
      hash: l["raw-id"],
      category: p ? "ad" : l.categories[0],
      title: l.name,
      tag: l.branding,
      date: p ? l.origin : wf(l.created),
      imageList: ((v = l.thumbnail) == null ? void 0 : v.map((h) => h.url)) || [],
      link: l.url,
      visibleEvent: u.events.visible
    };
    p ? o.push(g) : i.push(g);
  });
  const f = i.length, c = o.length, A = Math.floor(f / c);
  for (let u = c - 1; u >= 0; u--) {
    const l = A * (u + 1);
    i.splice(l, 0, o[u]);
  }
  return {
    session: s.user.session,
    data: i
  };
}
const Id = {
  // 获取新闻
  getNewsList(e, t) {
    return Cn ? Ed(e) : bd(e, t);
  }
};
const wd = { class: "mx-skeleton" }, Kf = {
  __name: "MxSkeleton",
  props: {
    // type: row, image, border
    // width, height 宽高
    // center 是否居中
    rows: { type: Array, default: null }
  },
  setup(e) {
    const t = e, n = ze([]);
    r(t.rows);
    function r(s) {
      s.forEach((i) => {
        if (i.repeat)
          for (let o = 0; o < i.repeat; o++)
            i.rows ? r(i.rows) : n.value.push(i);
        else
          n.value.push(i);
      });
    }
    return (s, i) => (Ee(), Qe("div", wd, [
      (Ee(!0), Qe(Oe, null, js(n.value, (o, f) => (Ee(), Qe("div", {
        key: f,
        class: gt([
          "mx-skeleton-item",
          `is-type-${o.type || "row"}`,
          {
            "is-center": o.center,
            "is-border": o.border
          }
        ]),
        style: nn({
          width: o.width,
          height: o.height
        })
      }, null, 6))), 128))
    ]));
  }
};
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const eo = () => {
};
function ys(e, t = !1, n = "Timeout") {
  return new Promise((r, s) => {
    setTimeout(t ? () => s(n) : r, e);
  });
}
function bs(e, t = !1) {
  function n(l, { flush: p = "sync", deep: g = !1, timeout: v, throwOnTimeout: h } = {}) {
    let y = null;
    const H = [new Promise((T) => {
      y = Ue(
        e,
        (G) => {
          l(G) !== t && (y ? y() : Vt(() => y == null ? void 0 : y()), T(G));
        },
        {
          flush: p,
          deep: g,
          immediate: !0
        }
      );
    })];
    return v != null && H.push(
      ys(v, h).then(() => zt(e)).finally(() => y == null ? void 0 : y())
    ), Promise.race(H);
  }
  function r(l, p) {
    if (!de(l))
      return n((G) => G === l, p);
    const { flush: g = "sync", deep: v = !1, timeout: h, throwOnTimeout: y } = p ?? {};
    let x = null;
    const T = [new Promise((G) => {
      x = Ue(
        [e, l],
        ([R, U]) => {
          t !== (R === U) && (x ? x() : Vt(() => x == null ? void 0 : x()), G(R));
        },
        {
          flush: g,
          deep: v,
          immediate: !0
        }
      );
    })];
    return h != null && T.push(
      ys(h, y).then(() => zt(e)).finally(() => (x == null || x(), zt(e)))
    ), Promise.race(T);
  }
  function s(l) {
    return n((p) => !!p, l);
  }
  function i(l) {
    return r(null, l);
  }
  function o(l) {
    return r(void 0, l);
  }
  function f(l) {
    return n(Number.isNaN, l);
  }
  function c(l, p) {
    return n((g) => {
      const v = Array.from(g);
      return v.includes(l) || v.includes(zt(l));
    }, p);
  }
  function A(l) {
    return u(1, l);
  }
  function u(l = 1, p) {
    let g = -1;
    return n(() => (g += 1, g >= l), p);
  }
  return Array.isArray(zt(e)) ? {
    toMatch: n,
    toContains: c,
    changed: A,
    changedTimes: u,
    get not() {
      return bs(e, !t);
    }
  } : {
    toMatch: n,
    toBe: r,
    toBeTruthy: s,
    toBeNull: i,
    toBeNaN: f,
    toBeUndefined: o,
    changed: A,
    changedTimes: u,
    get not() {
      return bs(e, !t);
    }
  };
}
function Pd(e) {
  return bs(e);
}
function Cd(e, t, n) {
  const {
    immediate: r = !0,
    delay: s = 0,
    onError: i = eo,
    onSuccess: o = eo,
    resetOnExecute: f = !0,
    shallow: c = !0,
    throwError: A
  } = n ?? {}, u = c ? ts(t) : ze(t), l = ze(!1), p = ze(!1), g = ts(void 0);
  async function v(x = 0, ...H) {
    f && (u.value = t), g.value = void 0, l.value = !1, p.value = !0, x > 0 && await ys(x);
    const T = typeof e == "function" ? e(...H) : e;
    try {
      const G = await T;
      u.value = G, l.value = !0, o(G);
    } catch (G) {
      if (g.value = G, i(G), A)
        throw G;
    } finally {
      p.value = !1;
    }
    return u.value;
  }
  r && v(s);
  const h = {
    state: u,
    isReady: l,
    isLoading: p,
    error: g,
    execute: v
  };
  function y() {
    return new Promise((x, H) => {
      Pd(p).toBe(!1).then(() => x(h)).catch(H);
    });
  }
  return {
    ...h,
    then(x, H) {
      return y().then(x, H);
    }
  };
}
async function xd(e) {
  return new Promise((t, n) => {
    const r = new Image(), { src: s, srcset: i, sizes: o, class: f, loading: c, crossorigin: A, referrerPolicy: u, width: l, height: p, decoding: g, fetchPriority: v, ismap: h, usemap: y } = e;
    r.src = s, i != null && (r.srcset = i), o != null && (r.sizes = o), f != null && (r.className = f), c != null && (r.loading = c), A != null && (r.crossOrigin = A), u != null && (r.referrerPolicy = u), l != null && (r.width = l), p != null && (r.height = p), g != null && (r.decoding = g), v != null && (r.fetchPriority = v), h != null && (r.isMap = h), y != null && (r.useMap = y), r.onload = () => t(r), r.onerror = n;
  });
}
function Dd(e, t = {}) {
  const n = Cd(
    () => xd(zt(e)),
    void 0,
    {
      resetOnExecute: !0,
      ...t
    }
  );
  return Ue(
    () => zt(e),
    () => n.execute(t.delay),
    { deep: !0 }
  ), n;
}
const Nd = "data:image/png;base64,";
const Sd = ["src", "data-src"], Bd = {
  __name: "MxImage",
  props: {
    // 图片地址
    src: { type: String, default: "" },
    // 是否圆角
    radius: { type: String, default: "" }
  },
  setup(e) {
    const t = e, { isLoading: n, error: r } = Dd({ src: t.src });
    return (s, i) => (Ee(), Qe("div", {
      class: "mx-image",
      style: nn({ "border-radius": e.radius })
    }, [
      ht(n) ? (Ee(), tn(Kf, {
        key: 0,
        rows: [{ type: "image" }]
      })) : (Ee(), Qe("img", {
        key: 1,
        src: ht(r) ? ht(Nd) : t.src,
        "data-src": ht(r) ? e.src : null,
        class: gt(["mx-image-inner", { "is-error": ht(r) }])
      }, null, 10, Sd))
    ], 4));
  }
};
const Rd = { class: "mx-news-content" }, Od = ["innerHTML"], Md = { class: "mx-news-source" }, Td = { class: "mx-news-date" }, to = {
  __name: "MxNewsItem",
  props: {
    // 数据
    data: { type: Object, default: null },
    // 是否顶部大图
    isTop: { type: Boolean, default: !1 }
  },
  setup(e) {
    const t = e, n = Ge(() => {
      if (t.data.imageList.length > 1) {
        const o = t.isTop ? 1 : 3;
        return t.data.imageList.slice(0, o);
      } else
        return t.data.imageList;
    }), r = Ge(() => t.isTop ? !1 : n.value.length === 1), s = ze(null);
    Rn(() => {
      if (Cn)
        return;
      const o = new IntersectionObserver(
        (f) => {
          f.forEach((c) => {
            c.isIntersecting && (fetch(t.data.visibleEvent), o.disconnect());
          });
        },
        { threshold: 0.5 }
      );
      o.observe(s.value);
    });
    function i() {
      window.open(t.data.link), Cn && ka();
    }
    return (o, f) => (Ee(), Qe("div", {
      ref_key: "itemRef",
      ref: s,
      class: gt(["mx-news-item", {
        "is-cols": r.value,
        "is-top": e.isTop
      }]),
      onClick: i
    }, [
      st("div", Rd, [
        st("div", {
          class: gt(["mx-news-title", e.isTop ? "is-top" : "is-list"]),
          innerHTML: e.data.title
        }, null, 10, Od),
        st("div", {
          class: gt(["mx-news-meta", e.isTop ? "is-top" : "is-list"])
        }, [
          st("span", Md, Ur(e.data.tag), 1),
          f[0] || (f[0] = st("span", null, "·", -1)),
          st("span", Td, Ur(e.data.date), 1)
        ], 2)
      ]),
      st("div", {
        class: "mx-news-imgs",
        style: nn(`--mx-news-image-length: ${n.value.length}`)
      }, [
        (Ee(!0), Qe(Oe, null, js(n.value, (c) => (Ee(), tn(Bd, {
          key: c,
          src: c
        }, null, 8, ["src"]))), 128))
      ], 4)
    ], 2));
  }
};
const Ld = { class: "mx-news-list" }, kd = {
  key: 1,
  class: "mx-news-empty"
}, Hd = {
  key: 1,
  class: "mx-news-loading"
}, qf = {
  __name: "MxNewsList",
  props: {
    // 挂载节点选择器
    container: { type: String, default: null },
    // 滚动容器选择器：可选，默认 body
    scrollbar: { type: String, default: null },
    // 分类：可选，默认 Trending
    category: { type: String, default: null },
    // 是否需要顶部大图：默认 true
    needTop: { type: Boolean, default: !0 },
    // 最大条目：默认空，无限制
    maxLength: { type: Number, default: null },
    // 宽度过大时自动两列布局
    autoCols: { type: Boolean, default: !1 }
  },
  setup(e) {
    const t = e, n = document.querySelector(t.container), r = t.scrollbar ? document.querySelector(t.scrollbar) : document, s = t.scrollbar ? document.querySelector(t.scrollbar) : document.documentElement, { width: i } = Aa(n), o = Ge(() => t.autoCols && i.value >= 768), f = ze(null), c = Ge(() => t.needTop && f.value ? f.value[0] : null), A = Ge(() => {
      var x, H;
      const y = t.needTop ? 1 : 0;
      return t.maxLength ? (x = f.value) == null ? void 0 : x.slice(y, t.maxLength) : (H = f.value) == null ? void 0 : H.slice(y);
    }), u = Ge(() => {
      const y = [{ width: "80%", center: !0 }, { type: "image" }, { type: "border" }], x = [{ repeat: 3, rows: [{ width: "40%" }, { type: "row" }, { width: "60%" }, { type: "border" }] }];
      return t.needTop ? [...y, ...x] : [...x];
    }), l = ze(!1), p = ze("init"), g = Ge(() => t.category ? t.category : Cn ? "usa" : "Trending");
    async function v(y) {
      if (l.value)
        return;
      l.value = !0;
      const x = await Id.getNewsList(g.value, p.value);
      l.value = !1, y && (f.value = []), f.value.push(...x.data), p.value = x.session, Vt(() => {
        h();
      });
    }
    la(r, "scroll", h);
    function h() {
      var T;
      if (Cn || n.style.display === "none" || t.maxLength && t.maxLength < ((T = f.value) == null ? void 0 : T.length))
        return;
      const { clientHeight: y, scrollTop: x, scrollHeight: H } = s;
      y + x >= H - 100 && v();
    }
    return v(!0), (y, x) => (Ee(), Qe("div", Ld, [
      f.value ? f.value.length ? (Ee(), Qe(Oe, { key: 2 }, [
        e.needTop ? (Ee(), tn(to, {
          key: 0,
          data: c.value,
          "is-top": !0
        }, null, 8, ["data"])) : Ei("", !0),
        st("div", {
          class: gt(["mx-news-items", { "is-cols": o.value }])
        }, [
          (Ee(!0), Qe(Oe, null, js(A.value, (H, T) => (Ee(), tn(to, {
            key: T,
            data: H,
            "is-top": !1
          }, null, 8, ["data"]))), 128))
        ], 2),
        l.value ? (Ee(), Qe("div", Hd, " Loading... ")) : Ei("", !0)
      ], 64)) : (Ee(), Qe("div", kd, " No Data ")) : (Ee(), tn(Kf, {
        key: 0,
        rows: u.value
      }, null, 8, ["rows"]))
    ]));
  }
};
function jd(e = {}) {
  const t = Ku(qf, e), n = document.querySelector(e.container);
  return t.mount(n), {
    destroy() {
      t.unmount(), document.body.removeChild(n);
    }
  };
}
function zd(e) {
  e.component("MyComponent", qf);
}
export {
  jd as default,
  jd as init,
  zd as register
};
