/**
* @vue/shared v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
/*! #__NO_SIDE_EFFECTS__ */
// @__NO_SIDE_EFFECTS__
function At(e) {
  const t = /* @__PURE__ */ Object.create(null);
  for (const n of e.split(","))
    t[n] = 1;
  return (n) => n in t;
}
const ee = {}.NODE_ENV !== "production" ? Object.freeze({}) : {}, Jt = {}.NODE_ENV !== "production" ? Object.freeze([]) : [], pe = () => {
}, Ao = () => !1, an = (e) => e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && // uppercase letter
(e.charCodeAt(2) > 122 || e.charCodeAt(2) < 97), Bn = (e) => e.startsWith("onUpdate:"), ue = Object.assign, Si = (e, t) => {
  const n = e.indexOf(t);
  n > -1 && e.splice(n, 1);
}, ao = Object.prototype.hasOwnProperty, W = (e, t) => ao.call(e, t), z = Array.isArray, kt = (e) => Yn(e) === "[object Map]", Qr = (e) => Yn(e) === "[object Set]", X = (e) => typeof e == "function", fe = (e) => typeof e == "string", Ct = (e) => typeof e == "symbol", ie = (e) => e !== null && typeof e == "object", Li = (e) => (ie(e) || X(e)) && X(e.then) && X(e.catch), Yr = Object.prototype.toString, Yn = (e) => Yr.call(e), ki = (e) => Yn(e).slice(8, -1), Zr = (e) => Yn(e) === "[object Object]", Oi = (e) => fe(e) && e !== "NaN" && e[0] !== "-" && "" + parseInt(e, 10) === e, en = /* @__PURE__ */ At(
  // the leading comma is intentional so empty string "" is also included
  ",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"
), po = /* @__PURE__ */ At(
  "bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo"
), Zn = (e) => {
  const t = /* @__PURE__ */ Object.create(null);
  return (n) => t[n] || (t[n] = e(n));
}, go = /-(\w)/g, Je = Zn(
  (e) => e.replace(go, (t, n) => n ? n.toUpperCase() : "")
), ho = /\B([A-Z])/g, Pt = Zn(
  (e) => e.replace(ho, "-$1").toLowerCase()
), Gn = Zn((e) => e.charAt(0).toUpperCase() + e.slice(1)), Bt = Zn(
  (e) => e ? `on${Gn(e)}` : ""
), yt = (e, t) => !Object.is(e, t), Wt = (e, ...t) => {
  for (let n = 0; n < e.length; n++)
    e[n](...t);
}, Mn = (e, t, n, i = !1) => {
  Object.defineProperty(e, t, {
    configurable: !0,
    enumerable: !1,
    writable: i,
    value: n
  });
}, vo = (e) => {
  const t = parseFloat(e);
  return isNaN(t) ? e : t;
};
let ir;
const dn = () => ir || (ir = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : typeof global < "u" ? global : {});
function Gt(e) {
  if (z(e)) {
    const t = {};
    for (let n = 0; n < e.length; n++) {
      const i = e[n], r = fe(i) ? bo(i) : Gt(i);
      if (r)
        for (const s in r)
          t[s] = r[s];
    }
    return t;
  } else if (fe(e) || ie(e))
    return e;
}
const Eo = /;(?![^(]*\))/g, mo = /:([^]+)/, Io = /\/\*[^]*?\*\//g;
function bo(e) {
  const t = {};
  return e.replace(Io, "").split(Eo).forEach((n) => {
    if (n) {
      const i = n.split(mo);
      i.length > 1 && (t[i[0].trim()] = i[1].trim());
    }
  }), t;
}
function ot(e) {
  let t = "";
  if (fe(e))
    t = e;
  else if (z(e))
    for (let n = 0; n < e.length; n++) {
      const i = ot(e[n]);
      i && (t += i + " ");
    }
  else if (ie(e))
    for (const n in e)
      e[n] && (t += n + " ");
  return t.trim();
}
const yo = "html,body,base,head,link,meta,style,title,address,article,aside,footer,header,hgroup,h1,h2,h3,h4,h5,h6,nav,section,div,dd,dl,dt,figcaption,figure,picture,hr,img,li,main,ol,p,pre,ul,a,b,abbr,bdi,bdo,br,cite,code,data,dfn,em,i,kbd,mark,q,rp,rt,ruby,s,samp,small,span,strong,sub,sup,time,u,var,wbr,area,audio,map,track,video,embed,object,param,source,canvas,script,noscript,del,ins,caption,col,colgroup,table,thead,tbody,td,th,tr,button,datalist,fieldset,form,input,label,legend,meter,optgroup,option,output,progress,select,textarea,details,dialog,menu,summary,template,blockquote,iframe,tfoot", Po = "svg,animate,animateMotion,animateTransform,circle,clipPath,color-profile,defs,desc,discard,ellipse,feBlend,feColorMatrix,feComponentTransfer,feComposite,feConvolveMatrix,feDiffuseLighting,feDisplacementMap,feDistantLight,feDropShadow,feFlood,feFuncA,feFuncB,feFuncG,feFuncR,feGaussianBlur,feImage,feMerge,feMergeNode,feMorphology,feOffset,fePointLight,feSpecularLighting,feSpotLight,feTile,feTurbulence,filter,foreignObject,g,hatch,hatchpath,image,line,linearGradient,marker,mask,mesh,meshgradient,meshpatch,meshrow,metadata,mpath,path,pattern,polygon,polyline,radialGradient,rect,set,solidcolor,stop,switch,symbol,text,textPath,title,tspan,unknown,use,view", wo = "annotation,annotation-xml,maction,maligngroup,malignmark,math,menclose,merror,mfenced,mfrac,mfraction,mglyph,mi,mlabeledtr,mlongdiv,mmultiscripts,mn,mo,mover,mpadded,mphantom,mprescripts,mroot,mrow,ms,mscarries,mscarry,msgroup,msline,mspace,msqrt,msrow,mstack,mstyle,msub,msubsup,msup,mtable,mtd,mtext,mtr,munder,munderover,none,semantics", Co = /* @__PURE__ */ At(yo), xo = /* @__PURE__ */ At(Po), Do = /* @__PURE__ */ At(wo), No = "itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly", Bo = /* @__PURE__ */ At(No);
function Gr(e) {
  return !!e || e === "";
}
const Fr = (e) => !!(e && e.__v_isRef === !0), ai = (e) => fe(e) ? e : e == null ? "" : z(e) || ie(e) && (e.toString === Yr || !X(e.toString)) ? Fr(e) ? ai(e.value) : JSON.stringify(e, Vr, 2) : String(e), Vr = (e, t) => Fr(t) ? Vr(e, t.value) : kt(t) ? {
  [`Map(${t.size})`]: [...t.entries()].reduce(
    (n, [i, r], s) => (n[ti(i, s) + " =>"] = r, n),
    {}
  )
} : Qr(t) ? {
  [`Set(${t.size})`]: [...t.values()].map((n) => ti(n))
} : Ct(t) ? ti(t) : ie(t) && !z(t) && !Zr(t) ? String(t) : t, ti = (e, t = "") => {
  var n;
  return (
    // Symbol.description in es2019+ so we need to cast here to pass
    // the lib: es2016 check
    Ct(e) ? `Symbol(${(n = e.description) != null ? n : t})` : e
  );
};
/**
* @vue/reactivity v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
function Ze(e, ...t) {
  console.warn(`[Vue warn] ${e}`, ...t);
}
let we;
class Mo {
  constructor(t = !1) {
    this.detached = t, this._active = !0, this.effects = [], this.cleanups = [], this._isPaused = !1, this.parent = we, !t && we && (this.index = (we.scopes || (we.scopes = [])).push(
      this
    ) - 1);
  }
  get active() {
    return this._active;
  }
  pause() {
    if (this._active) {
      this._isPaused = !0;
      let t, n;
      if (this.scopes)
        for (t = 0, n = this.scopes.length; t < n; t++)
          this.scopes[t].pause();
      for (t = 0, n = this.effects.length; t < n; t++)
        this.effects[t].pause();
    }
  }
  /**
   * Resumes the effect scope, including all child scopes and effects.
   */
  resume() {
    if (this._active && this._isPaused) {
      this._isPaused = !1;
      let t, n;
      if (this.scopes)
        for (t = 0, n = this.scopes.length; t < n; t++)
          this.scopes[t].resume();
      for (t = 0, n = this.effects.length; t < n; t++)
        this.effects[t].resume();
    }
  }
  run(t) {
    if (this._active) {
      const n = we;
      try {
        return we = this, t();
      } finally {
        we = n;
      }
    } else
      ({}).NODE_ENV !== "production" && Ze("cannot run an inactive effect scope.");
  }
  /**
   * This should only be called on non-detached scopes
   * @internal
   */
  on() {
    we = this;
  }
  /**
   * This should only be called on non-detached scopes
   * @internal
   */
  off() {
    we = this.parent;
  }
  stop(t) {
    if (this._active) {
      this._active = !1;
      let n, i;
      for (n = 0, i = this.effects.length; n < i; n++)
        this.effects[n].stop();
      for (this.effects.length = 0, n = 0, i = this.cleanups.length; n < i; n++)
        this.cleanups[n]();
      if (this.cleanups.length = 0, this.scopes) {
        for (n = 0, i = this.scopes.length; n < i; n++)
          this.scopes[n].stop(!0);
        this.scopes.length = 0;
      }
      if (!this.detached && this.parent && !t) {
        const r = this.parent.scopes.pop();
        r && r !== this && (this.parent.scopes[this.index] = r, r.index = this.index);
      }
      this.parent = void 0;
    }
  }
}
function Wr() {
  return we;
}
function So(e, t = !1) {
  we ? we.cleanups.push(e) : {}.NODE_ENV !== "production" && !t && Ze(
    "onScopeDispose() is called when there is no active effect scope to be associated with."
  );
}
let _;
const ni = /* @__PURE__ */ new WeakSet();
class Kr {
  constructor(t) {
    this.fn = t, this.deps = void 0, this.depsTail = void 0, this.flags = 5, this.next = void 0, this.cleanup = void 0, this.scheduler = void 0, we && we.active && we.effects.push(this);
  }
  pause() {
    this.flags |= 64;
  }
  resume() {
    this.flags & 64 && (this.flags &= -65, ni.has(this) && (ni.delete(this), this.trigger()));
  }
  /**
   * @internal
   */
  notify() {
    this.flags & 2 && !(this.flags & 32) || this.flags & 8 || qr(this);
  }
  run() {
    if (!(this.flags & 1))
      return this.fn();
    this.flags |= 2, rr(this), _r(this);
    const t = _, n = Qe;
    _ = this, Qe = !0;
    try {
      return this.fn();
    } finally {
      ({}).NODE_ENV !== "production" && _ !== this && Ze(
        "Active effect was not restored correctly - this is likely a Vue internal bug."
      ), $r(this), _ = t, Qe = n, this.flags &= -3;
    }
  }
  stop() {
    if (this.flags & 1) {
      for (let t = this.deps; t; t = t.nextDep)
        Ti(t);
      this.deps = this.depsTail = void 0, rr(this), this.onStop && this.onStop(), this.flags &= -2;
    }
  }
  trigger() {
    this.flags & 64 ? ni.add(this) : this.scheduler ? this.scheduler() : this.runIfDirty();
  }
  /**
   * @internal
   */
  runIfDirty() {
    di(this) && this.run();
  }
  get dirty() {
    return di(this);
  }
}
let Ur = 0, tn, nn;
function qr(e, t = !1) {
  if (e.flags |= 8, t) {
    e.next = nn, nn = e;
    return;
  }
  e.next = tn, tn = e;
}
function Hi() {
  Ur++;
}
function Ri() {
  if (--Ur > 0)
    return;
  if (nn) {
    let t = nn;
    for (nn = void 0; t; ) {
      const n = t.next;
      t.next = void 0, t.flags &= -9, t = n;
    }
  }
  let e;
  for (; tn; ) {
    let t = tn;
    for (tn = void 0; t; ) {
      const n = t.next;
      if (t.next = void 0, t.flags &= -9, t.flags & 1)
        try {
          t.trigger();
        } catch (i) {
          e || (e = i);
        }
      t = n;
    }
  }
  if (e)
    throw e;
}
function _r(e) {
  for (let t = e.deps; t; t = t.nextDep)
    t.version = -1, t.prevActiveLink = t.dep.activeLink, t.dep.activeLink = t;
}
function $r(e) {
  let t, n = e.depsTail, i = n;
  for (; i; ) {
    const r = i.prevDep;
    i.version === -1 ? (i === n && (n = r), Ti(i), Lo(i)) : t = i, i.dep.activeLink = i.prevActiveLink, i.prevActiveLink = void 0, i = r;
  }
  e.deps = t, e.depsTail = n;
}
function di(e) {
  for (let t = e.deps; t; t = t.nextDep)
    if (t.dep.version !== t.version || t.dep.computed && (es(t.dep.computed) || t.dep.version !== t.version))
      return !0;
  return !!e._dirty;
}
function es(e) {
  if (e.flags & 4 && !(e.flags & 16) || (e.flags &= -17, e.globalVersion === on))
    return;
  e.globalVersion = on;
  const t = e.dep;
  if (e.flags |= 2, t.version > 0 && !e.isSSR && e.deps && !di(e)) {
    e.flags &= -3;
    return;
  }
  const n = _, i = Qe;
  _ = e, Qe = !0;
  try {
    _r(e);
    const r = e.fn(e._value);
    (t.version === 0 || yt(r, e._value)) && (e._value = r, t.version++);
  } catch (r) {
    throw t.version++, r;
  } finally {
    _ = n, Qe = i, $r(e), e.flags &= -3;
  }
}
function Ti(e, t = !1) {
  const { dep: n, prevSub: i, nextSub: r } = e;
  if (i && (i.nextSub = r, e.prevSub = void 0), r && (r.prevSub = i, e.nextSub = void 0), {}.NODE_ENV !== "production" && n.subsHead === e && (n.subsHead = r), n.subs === e && (n.subs = i, !i && n.computed)) {
    n.computed.flags &= -5;
    for (let s = n.computed.deps; s; s = s.nextDep)
      Ti(s, !0);
  }
  !t && !--n.sc && n.map && n.map.delete(n.key);
}
function Lo(e) {
  const { prevDep: t, nextDep: n } = e;
  t && (t.nextDep = n, e.prevDep = void 0), n && (n.prevDep = t, e.nextDep = void 0);
}
let Qe = !0;
const ts = [];
function at() {
  ts.push(Qe), Qe = !1;
}
function dt() {
  const e = ts.pop();
  Qe = e === void 0 ? !0 : e;
}
function rr(e) {
  const { cleanup: t } = e;
  if (e.cleanup = void 0, t) {
    const n = _;
    _ = void 0;
    try {
      t();
    } finally {
      _ = n;
    }
  }
}
let on = 0;
class ko {
  constructor(t, n) {
    this.sub = t, this.dep = n, this.version = n.version, this.nextDep = this.prevDep = this.nextSub = this.prevSub = this.prevActiveLink = void 0;
  }
}
class zi {
  constructor(t) {
    this.computed = t, this.version = 0, this.activeLink = void 0, this.subs = void 0, this.map = void 0, this.key = void 0, this.sc = 0, {}.NODE_ENV !== "production" && (this.subsHead = void 0);
  }
  track(t) {
    if (!_ || !Qe || _ === this.computed)
      return;
    let n = this.activeLink;
    if (n === void 0 || n.sub !== _)
      n = this.activeLink = new ko(_, this), _.deps ? (n.prevDep = _.depsTail, _.depsTail.nextDep = n, _.depsTail = n) : _.deps = _.depsTail = n, ns(n);
    else if (n.version === -1 && (n.version = this.version, n.nextDep)) {
      const i = n.nextDep;
      i.prevDep = n.prevDep, n.prevDep && (n.prevDep.nextDep = i), n.prevDep = _.depsTail, n.nextDep = void 0, _.depsTail.nextDep = n, _.depsTail = n, _.deps === n && (_.deps = i);
    }
    return {}.NODE_ENV !== "production" && _.onTrack && _.onTrack(
      ue(
        {
          effect: _
        },
        t
      )
    ), n;
  }
  trigger(t) {
    this.version++, on++, this.notify(t);
  }
  notify(t) {
    Hi();
    try {
      if ({}.NODE_ENV !== "production")
        for (let n = this.subsHead; n; n = n.nextSub)
          n.sub.onTrigger && !(n.sub.flags & 8) && n.sub.onTrigger(
            ue(
              {
                effect: n.sub
              },
              t
            )
          );
      for (let n = this.subs; n; n = n.prevSub)
        n.sub.notify() && n.sub.dep.notify();
    } finally {
      Ri();
    }
  }
}
function ns(e) {
  if (e.dep.sc++, e.sub.flags & 4) {
    const t = e.dep.computed;
    if (t && !e.dep.subs) {
      t.flags |= 20;
      for (let i = t.deps; i; i = i.nextDep)
        ns(i);
    }
    const n = e.dep.subs;
    n !== e && (e.prevSub = n, n && (n.nextSub = e)), {}.NODE_ENV !== "production" && e.dep.subsHead === void 0 && (e.dep.subsHead = e), e.dep.subs = e;
  }
}
const pi = /* @__PURE__ */ new WeakMap(), Ot = Symbol(
  {}.NODE_ENV !== "production" ? "Object iterate" : ""
), gi = Symbol(
  {}.NODE_ENV !== "production" ? "Map keys iterate" : ""
), fn = Symbol(
  {}.NODE_ENV !== "production" ? "Array iterate" : ""
);
function de(e, t, n) {
  if (Qe && _) {
    let i = pi.get(e);
    i || pi.set(e, i = /* @__PURE__ */ new Map());
    let r = i.get(n);
    r || (i.set(n, r = new zi()), r.map = i, r.key = n), {}.NODE_ENV !== "production" ? r.track({
      target: e,
      type: t,
      key: n
    }) : r.track();
  }
}
function Ue(e, t, n, i, r, s) {
  const o = pi.get(e);
  if (!o) {
    on++;
    return;
  }
  const f = (u) => {
    u && ({}.NODE_ENV !== "production" ? u.trigger({
      target: e,
      type: t,
      key: n,
      newValue: i,
      oldValue: r,
      oldTarget: s
    }) : u.trigger());
  };
  if (Hi(), t === "clear")
    o.forEach(f);
  else {
    const u = z(e), d = u && Oi(n);
    if (u && n === "length") {
      const a = Number(i);
      o.forEach((l, p) => {
        (p === "length" || p === fn || !Ct(p) && p >= a) && f(l);
      });
    } else
      switch ((n !== void 0 || o.has(void 0)) && f(o.get(n)), d && f(o.get(fn)), t) {
        case "add":
          u ? d && f(o.get("length")) : (f(o.get(Ot)), kt(e) && f(o.get(gi)));
          break;
        case "delete":
          u || (f(o.get(Ot)), kt(e) && f(o.get(gi)));
          break;
        case "set":
          kt(e) && f(o.get(Ot));
          break;
      }
  }
  Ri();
}
function jt(e) {
  const t = G(e);
  return t === e ? t : (de(t, "iterate", fn), xe(e) ? t : t.map(Ie));
}
function Fn(e) {
  return de(e = G(e), "iterate", fn), e;
}
const Oo = {
  __proto__: null,
  [Symbol.iterator]() {
    return ii(this, Symbol.iterator, Ie);
  },
  concat(...e) {
    return jt(this).concat(
      ...e.map((t) => z(t) ? jt(t) : t)
    );
  },
  entries() {
    return ii(this, "entries", (e) => (e[1] = Ie(e[1]), e));
  },
  every(e, t) {
    return nt(this, "every", e, t, void 0, arguments);
  },
  filter(e, t) {
    return nt(this, "filter", e, t, (n) => n.map(Ie), arguments);
  },
  find(e, t) {
    return nt(this, "find", e, t, Ie, arguments);
  },
  findIndex(e, t) {
    return nt(this, "findIndex", e, t, void 0, arguments);
  },
  findLast(e, t) {
    return nt(this, "findLast", e, t, Ie, arguments);
  },
  findLastIndex(e, t) {
    return nt(this, "findLastIndex", e, t, void 0, arguments);
  },
  // flat, flatMap could benefit from ARRAY_ITERATE but are not straight-forward to implement
  forEach(e, t) {
    return nt(this, "forEach", e, t, void 0, arguments);
  },
  includes(...e) {
    return ri(this, "includes", e);
  },
  indexOf(...e) {
    return ri(this, "indexOf", e);
  },
  join(e) {
    return jt(this).join(e);
  },
  // keys() iterator only reads `length`, no optimisation required
  lastIndexOf(...e) {
    return ri(this, "lastIndexOf", e);
  },
  map(e, t) {
    return nt(this, "map", e, t, void 0, arguments);
  },
  pop() {
    return Kt(this, "pop");
  },
  push(...e) {
    return Kt(this, "push", e);
  },
  reduce(e, ...t) {
    return sr(this, "reduce", e, t);
  },
  reduceRight(e, ...t) {
    return sr(this, "reduceRight", e, t);
  },
  shift() {
    return Kt(this, "shift");
  },
  // slice could use ARRAY_ITERATE but also seems to beg for range tracking
  some(e, t) {
    return nt(this, "some", e, t, void 0, arguments);
  },
  splice(...e) {
    return Kt(this, "splice", e);
  },
  toReversed() {
    return jt(this).toReversed();
  },
  toSorted(e) {
    return jt(this).toSorted(e);
  },
  toSpliced(...e) {
    return jt(this).toSpliced(...e);
  },
  unshift(...e) {
    return Kt(this, "unshift", e);
  },
  values() {
    return ii(this, "values", Ie);
  }
};
function ii(e, t, n) {
  const i = Fn(e), r = i[t]();
  return i !== e && !xe(e) && (r._next = r.next, r.next = () => {
    const s = r._next();
    return s.value && (s.value = n(s.value)), s;
  }), r;
}
const Ho = Array.prototype;
function nt(e, t, n, i, r, s) {
  const o = Fn(e), f = o !== e && !xe(e), u = o[t];
  if (u !== Ho[t]) {
    const l = u.apply(e, s);
    return f ? Ie(l) : l;
  }
  let d = n;
  o !== e && (f ? d = function(l, p) {
    return n.call(this, Ie(l), p, e);
  } : n.length > 2 && (d = function(l, p) {
    return n.call(this, l, p, e);
  }));
  const a = u.call(o, d, i);
  return f && r ? r(a) : a;
}
function sr(e, t, n, i) {
  const r = Fn(e);
  let s = n;
  return r !== e && (xe(e) ? n.length > 3 && (s = function(o, f, u) {
    return n.call(this, o, f, u, e);
  }) : s = function(o, f, u) {
    return n.call(this, o, Ie(f), u, e);
  }), r[t](s, ...i);
}
function ri(e, t, n) {
  const i = G(e);
  de(i, "iterate", fn);
  const r = i[t](...n);
  return (r === -1 || r === !1) && Sn(n[0]) ? (n[0] = G(n[0]), i[t](...n)) : r;
}
function Kt(e, t, n = []) {
  at(), Hi();
  const i = G(e)[t].apply(e, n);
  return Ri(), dt(), i;
}
const Ro = /* @__PURE__ */ At("__proto__,__v_isRef,__isVue"), is = new Set(
  /* @__PURE__ */ Object.getOwnPropertyNames(Symbol).filter((e) => e !== "arguments" && e !== "caller").map((e) => Symbol[e]).filter(Ct)
);
function To(e) {
  Ct(e) || (e = String(e));
  const t = G(this);
  return de(t, "has", e), t.hasOwnProperty(e);
}
class rs {
  constructor(t = !1, n = !1) {
    this._isReadonly = t, this._isShallow = n;
  }
  get(t, n, i) {
    if (n === "__v_skip")
      return t.__v_skip;
    const r = this._isReadonly, s = this._isShallow;
    if (n === "__v_isReactive")
      return !r;
    if (n === "__v_isReadonly")
      return r;
    if (n === "__v_isShallow")
      return s;
    if (n === "__v_raw")
      return i === (r ? s ? us : cs : s ? ls : fs).get(t) || // receiver is not the reactive proxy, but has the same prototype
      // this means the receiver is a user proxy of the reactive proxy
      Object.getPrototypeOf(t) === Object.getPrototypeOf(i) ? t : void 0;
    const o = z(t);
    if (!r) {
      let u;
      if (o && (u = Oo[n]))
        return u;
      if (n === "hasOwnProperty")
        return To;
    }
    const f = Reflect.get(
      t,
      n,
      // if this is a proxy wrapping a ref, return methods using the raw ref
      // as receiver so that we don't have to call `toRaw` on the ref in all
      // its class methods
      ce(t) ? t : i
    );
    return (Ct(n) ? is.has(n) : Ro(n)) || (r || de(t, "get", n), s) ? f : ce(f) ? o && Oi(n) ? f : f.value : ie(f) ? r ? As(f) : ji(f) : f;
  }
}
class ss extends rs {
  constructor(t = !1) {
    super(!1, t);
  }
  set(t, n, i, r) {
    let s = t[n];
    if (!this._isShallow) {
      const u = ut(s);
      if (!xe(i) && !ut(i) && (s = G(s), i = G(i)), !z(t) && ce(s) && !ce(i))
        return u ? !1 : (s.value = i, !0);
    }
    const o = z(t) && Oi(n) ? Number(n) < t.length : W(t, n), f = Reflect.set(
      t,
      n,
      i,
      ce(t) ? t : r
    );
    return t === G(r) && (o ? yt(i, s) && Ue(t, "set", n, i, s) : Ue(t, "add", n, i)), f;
  }
  deleteProperty(t, n) {
    const i = W(t, n), r = t[n], s = Reflect.deleteProperty(t, n);
    return s && i && Ue(t, "delete", n, void 0, r), s;
  }
  has(t, n) {
    const i = Reflect.has(t, n);
    return (!Ct(n) || !is.has(n)) && de(t, "has", n), i;
  }
  ownKeys(t) {
    return de(
      t,
      "iterate",
      z(t) ? "length" : Ot
    ), Reflect.ownKeys(t);
  }
}
class os extends rs {
  constructor(t = !1) {
    super(!0, t);
  }
  set(t, n) {
    return {}.NODE_ENV !== "production" && Ze(
      `Set operation on key "${String(n)}" failed: target is readonly.`,
      t
    ), !0;
  }
  deleteProperty(t, n) {
    return {}.NODE_ENV !== "production" && Ze(
      `Delete operation on key "${String(n)}" failed: target is readonly.`,
      t
    ), !0;
  }
}
const zo = /* @__PURE__ */ new ss(), jo = /* @__PURE__ */ new os(), Xo = /* @__PURE__ */ new ss(!0), Jo = /* @__PURE__ */ new os(!0), hi = (e) => e, En = (e) => Reflect.getPrototypeOf(e);
function Qo(e, t, n) {
  return function(...i) {
    const r = this.__v_raw, s = G(r), o = kt(s), f = e === "entries" || e === Symbol.iterator && o, u = e === "keys" && o, d = r[e](...i), a = n ? hi : t ? vi : Ie;
    return !t && de(
      s,
      "iterate",
      u ? gi : Ot
    ), {
      // iterator protocol
      next() {
        const { value: l, done: p } = d.next();
        return p ? { value: l, done: p } : {
          value: f ? [a(l[0]), a(l[1])] : a(l),
          done: p
        };
      },
      // iterable protocol
      [Symbol.iterator]() {
        return this;
      }
    };
  };
}
function mn(e) {
  return function(...t) {
    if ({}.NODE_ENV !== "production") {
      const n = t[0] ? `on key "${t[0]}" ` : "";
      Ze(
        `${Gn(e)} operation ${n}failed: target is readonly.`,
        G(this)
      );
    }
    return e === "delete" ? !1 : e === "clear" ? void 0 : this;
  };
}
function Yo(e, t) {
  const n = {
    get(r) {
      const s = this.__v_raw, o = G(s), f = G(r);
      e || (yt(r, f) && de(o, "get", r), de(o, "get", f));
      const { has: u } = En(o), d = t ? hi : e ? vi : Ie;
      if (u.call(o, r))
        return d(s.get(r));
      if (u.call(o, f))
        return d(s.get(f));
      s !== o && s.get(r);
    },
    get size() {
      const r = this.__v_raw;
      return !e && de(G(r), "iterate", Ot), Reflect.get(r, "size", r);
    },
    has(r) {
      const s = this.__v_raw, o = G(s), f = G(r);
      return e || (yt(r, f) && de(o, "has", r), de(o, "has", f)), r === f ? s.has(r) : s.has(r) || s.has(f);
    },
    forEach(r, s) {
      const o = this, f = o.__v_raw, u = G(f), d = t ? hi : e ? vi : Ie;
      return !e && de(u, "iterate", Ot), f.forEach((a, l) => r.call(s, d(a), d(l), o));
    }
  };
  return ue(
    n,
    e ? {
      add: mn("add"),
      set: mn("set"),
      delete: mn("delete"),
      clear: mn("clear")
    } : {
      add(r) {
        !t && !xe(r) && !ut(r) && (r = G(r));
        const s = G(this);
        return En(s).has.call(s, r) || (s.add(r), Ue(s, "add", r, r)), this;
      },
      set(r, s) {
        !t && !xe(s) && !ut(s) && (s = G(s));
        const o = G(this), { has: f, get: u } = En(o);
        let d = f.call(o, r);
        d ? {}.NODE_ENV !== "production" && or(o, f, r) : (r = G(r), d = f.call(o, r));
        const a = u.call(o, r);
        return o.set(r, s), d ? yt(s, a) && Ue(o, "set", r, s, a) : Ue(o, "add", r, s), this;
      },
      delete(r) {
        const s = G(this), { has: o, get: f } = En(s);
        let u = o.call(s, r);
        u ? {}.NODE_ENV !== "production" && or(s, o, r) : (r = G(r), u = o.call(s, r));
        const d = f ? f.call(s, r) : void 0, a = s.delete(r);
        return u && Ue(s, "delete", r, void 0, d), a;
      },
      clear() {
        const r = G(this), s = r.size !== 0, o = {}.NODE_ENV !== "production" ? kt(r) ? new Map(r) : new Set(r) : void 0, f = r.clear();
        return s && Ue(
          r,
          "clear",
          void 0,
          void 0,
          o
        ), f;
      }
    }
  ), [
    "keys",
    "values",
    "entries",
    Symbol.iterator
  ].forEach((r) => {
    n[r] = Qo(r, e, t);
  }), n;
}
function Vn(e, t) {
  const n = Yo(e, t);
  return (i, r, s) => r === "__v_isReactive" ? !e : r === "__v_isReadonly" ? e : r === "__v_raw" ? i : Reflect.get(
    W(n, r) && r in i ? n : i,
    r,
    s
  );
}
const Zo = {
  get: /* @__PURE__ */ Vn(!1, !1)
}, Go = {
  get: /* @__PURE__ */ Vn(!1, !0)
}, Fo = {
  get: /* @__PURE__ */ Vn(!0, !1)
}, Vo = {
  get: /* @__PURE__ */ Vn(!0, !0)
};
function or(e, t, n) {
  const i = G(n);
  if (i !== n && t.call(e, i)) {
    const r = ki(e);
    Ze(
      `Reactive ${r} contains both the raw and reactive versions of the same object${r === "Map" ? " as keys" : ""}, which can lead to inconsistencies. Avoid differentiating between the raw and reactive versions of an object and only use the reactive version if possible.`
    );
  }
}
const fs = /* @__PURE__ */ new WeakMap(), ls = /* @__PURE__ */ new WeakMap(), cs = /* @__PURE__ */ new WeakMap(), us = /* @__PURE__ */ new WeakMap();
function Wo(e) {
  switch (e) {
    case "Object":
    case "Array":
      return 1;
    case "Map":
    case "Set":
    case "WeakMap":
    case "WeakSet":
      return 2;
    default:
      return 0;
  }
}
function Ko(e) {
  return e.__v_skip || !Object.isExtensible(e) ? 0 : Wo(ki(e));
}
function ji(e) {
  return ut(e) ? e : Wn(
    e,
    !1,
    zo,
    Zo,
    fs
  );
}
function Uo(e) {
  return Wn(
    e,
    !1,
    Xo,
    Go,
    ls
  );
}
function As(e) {
  return Wn(
    e,
    !0,
    jo,
    Fo,
    cs
  );
}
function _e(e) {
  return Wn(
    e,
    !0,
    Jo,
    Vo,
    us
  );
}
function Wn(e, t, n, i, r) {
  if (!ie(e))
    return {}.NODE_ENV !== "production" && Ze(
      `value cannot be made ${t ? "readonly" : "reactive"}: ${String(
        e
      )}`
    ), e;
  if (e.__v_raw && !(t && e.__v_isReactive))
    return e;
  const s = r.get(e);
  if (s)
    return s;
  const o = Ko(e);
  if (o === 0)
    return e;
  const f = new Proxy(
    e,
    o === 2 ? i : n
  );
  return r.set(e, f), f;
}
function Ht(e) {
  return ut(e) ? Ht(e.__v_raw) : !!(e && e.__v_isReactive);
}
function ut(e) {
  return !!(e && e.__v_isReadonly);
}
function xe(e) {
  return !!(e && e.__v_isShallow);
}
function Sn(e) {
  return e ? !!e.__v_raw : !1;
}
function G(e) {
  const t = e && e.__v_raw;
  return t ? G(t) : e;
}
function qo(e) {
  return !W(e, "__v_skip") && Object.isExtensible(e) && Mn(e, "__v_skip", !0), e;
}
const Ie = (e) => ie(e) ? ji(e) : e, vi = (e) => ie(e) ? As(e) : e;
function ce(e) {
  return e ? e.__v_isRef === !0 : !1;
}
function Ye(e) {
  return as(e, !1);
}
function fr(e) {
  return as(e, !0);
}
function as(e, t) {
  return ce(e) ? e : new _o(e, t);
}
class _o {
  constructor(t, n) {
    this.dep = new zi(), this.__v_isRef = !0, this.__v_isShallow = !1, this._rawValue = n ? t : G(t), this._value = n ? t : Ie(t), this.__v_isShallow = n;
  }
  get value() {
    return {}.NODE_ENV !== "production" ? this.dep.track({
      target: this,
      type: "get",
      key: "value"
    }) : this.dep.track(), this._value;
  }
  set value(t) {
    const n = this._rawValue, i = this.__v_isShallow || xe(t) || ut(t);
    t = i ? t : G(t), yt(t, n) && (this._rawValue = t, this._value = i ? t : Ie(t), {}.NODE_ENV !== "production" ? this.dep.trigger({
      target: this,
      type: "set",
      key: "value",
      newValue: t,
      oldValue: n
    }) : this.dep.trigger());
  }
}
function mt(e) {
  return ce(e) ? e.value : e;
}
function St(e) {
  return X(e) ? e() : mt(e);
}
const $o = {
  get: (e, t, n) => t === "__v_raw" ? e : mt(Reflect.get(e, t, n)),
  set: (e, t, n, i) => {
    const r = e[t];
    return ce(r) && !ce(n) ? (r.value = n, !0) : Reflect.set(e, t, n, i);
  }
};
function ds(e) {
  return Ht(e) ? e : new Proxy(e, $o);
}
class ef {
  constructor(t, n, i) {
    this.fn = t, this.setter = n, this._value = void 0, this.dep = new zi(this), this.__v_isRef = !0, this.deps = void 0, this.depsTail = void 0, this.flags = 16, this.globalVersion = on - 1, this.next = void 0, this.effect = this, this.__v_isReadonly = !n, this.isSSR = i;
  }
  /**
   * @internal
   */
  notify() {
    if (this.flags |= 16, !(this.flags & 8) && // avoid infinite self recursion
    _ !== this)
      return qr(this, !0), !0;
  }
  get value() {
    const t = {}.NODE_ENV !== "production" ? this.dep.track({
      target: this,
      type: "get",
      key: "value"
    }) : this.dep.track();
    return es(this), t && (t.version = this.dep.version), this._value;
  }
  set value(t) {
    this.setter ? this.setter(t) : {}.NODE_ENV !== "production" && Ze("Write operation failed: computed value is readonly");
  }
}
function tf(e, t, n = !1) {
  let i, r;
  X(e) ? i = e : (i = e.get, r = e.set);
  const s = new ef(i, r, n);
  return {}.NODE_ENV !== "production" && t && !n && (s.onTrack = t.onTrack, s.onTrigger = t.onTrigger), s;
}
const In = {}, Ln = /* @__PURE__ */ new WeakMap();
let Mt;
function nf(e, t = !1, n = Mt) {
  if (n) {
    let i = Ln.get(n);
    i || Ln.set(n, i = []), i.push(e);
  } else
    ({}).NODE_ENV !== "production" && !t && Ze(
      "onWatcherCleanup() was called when there was no active watcher to associate with."
    );
}
function rf(e, t, n = ee) {
  const { immediate: i, deep: r, once: s, scheduler: o, augmentJob: f, call: u } = n, d = (L) => {
    (n.onWarn || Ze)(
      "Invalid watch source: ",
      L,
      "A watch source can only be a getter/effect function, a ref, a reactive object, or an array of these types."
    );
  }, a = (L) => r ? L : xe(L) || r === !1 || r === 0 ? It(L, 1) : It(L);
  let l, p, h, w, m = !1, O = !1;
  if (ce(e) ? (p = () => e.value, m = xe(e)) : Ht(e) ? (p = () => a(e), m = !0) : z(e) ? (O = !0, m = e.some((L) => Ht(L) || xe(L)), p = () => e.map((L) => {
    if (ce(L))
      return L.value;
    if (Ht(L))
      return a(L);
    if (X(L))
      return u ? u(L, 2) : L();
    ({}).NODE_ENV !== "production" && d(L);
  })) : X(e) ? t ? p = u ? () => u(e, 2) : e : p = () => {
    if (h) {
      at();
      try {
        h();
      } finally {
        dt();
      }
    }
    const L = Mt;
    Mt = l;
    try {
      return u ? u(e, 3, [w]) : e(w);
    } finally {
      Mt = L;
    }
  } : (p = pe, {}.NODE_ENV !== "production" && d(e)), t && r) {
    const L = p, q = r === !0 ? 1 / 0 : r;
    p = () => It(L(), q);
  }
  const k = Wr(), Q = () => {
    l.stop(), k && k.active && Si(k.effects, l);
  };
  if (s && t) {
    const L = t;
    t = (...q) => {
      L(...q), Q();
    };
  }
  let R = O ? new Array(e.length).fill(In) : In;
  const K = (L) => {
    if (!(!(l.flags & 1) || !l.dirty && !L))
      if (t) {
        const q = l.run();
        if (r || m || (O ? q.some((ae, le) => yt(ae, R[le])) : yt(q, R))) {
          h && h();
          const ae = Mt;
          Mt = l;
          try {
            const le = [
              q,
              // pass undefined as the old value when it's changed for the first time
              R === In ? void 0 : O && R[0] === In ? [] : R,
              w
            ];
            u ? u(t, 3, le) : (
              // @ts-expect-error
              t(...le)
            ), R = q;
          } finally {
            Mt = ae;
          }
        }
      } else
        l.run();
  };
  return f && f(K), l = new Kr(p), l.scheduler = o ? () => o(K, !1) : K, w = (L) => nf(L, !1, l), h = l.onStop = () => {
    const L = Ln.get(l);
    if (L) {
      if (u)
        u(L, 4);
      else
        for (const q of L)
          q();
      Ln.delete(l);
    }
  }, {}.NODE_ENV !== "production" && (l.onTrack = n.onTrack, l.onTrigger = n.onTrigger), t ? i ? K(!0) : R = l.run() : o ? o(K.bind(null, !0), !0) : l.run(), Q.pause = l.pause.bind(l), Q.resume = l.resume.bind(l), Q.stop = Q, Q;
}
function It(e, t = 1 / 0, n) {
  if (t <= 0 || !ie(e) || e.__v_skip || (n = n || /* @__PURE__ */ new Set(), n.has(e)))
    return e;
  if (n.add(e), t--, ce(e))
    It(e.value, t, n);
  else if (z(e))
    for (let i = 0; i < e.length; i++)
      It(e[i], t, n);
  else if (Qr(e) || kt(e))
    e.forEach((i) => {
      It(i, t, n);
    });
  else if (Zr(e)) {
    for (const i in e)
      It(e[i], t, n);
    for (const i of Object.getOwnPropertySymbols(e))
      Object.prototype.propertyIsEnumerable.call(e, i) && It(e[i], t, n);
  }
  return e;
}
/**
* @vue/runtime-core v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
const Rt = [];
function bn(e) {
  Rt.push(e);
}
function yn() {
  Rt.pop();
}
let si = !1;
function N(e, ...t) {
  if (si)
    return;
  si = !0, at();
  const n = Rt.length ? Rt[Rt.length - 1].component : null, i = n && n.appContext.config.warnHandler, r = sf();
  if (i)
    Ft(
      i,
      n,
      11,
      [
        // eslint-disable-next-line no-restricted-syntax
        e + t.map((s) => {
          var o, f;
          return (f = (o = s.toString) == null ? void 0 : o.call(s)) != null ? f : JSON.stringify(s);
        }).join(""),
        n && n.proxy,
        r.map(
          ({ vnode: s }) => `at <${ei(n, s.type)}>`
        ).join(`
`),
        r
      ]
    );
  else {
    const s = [`[Vue warn]: ${e}`, ...t];
    r.length && s.push(`
`, ...of(r)), console.warn(...s);
  }
  dt(), si = !1;
}
function sf() {
  let e = Rt[Rt.length - 1];
  if (!e)
    return [];
  const t = [];
  for (; e; ) {
    const n = t[0];
    n && n.vnode === e ? n.recurseCount++ : t.push({
      vnode: e,
      recurseCount: 0
    });
    const i = e.component && e.component.parent;
    e = i && i.vnode;
  }
  return t;
}
function of(e) {
  const t = [];
  return e.forEach((n, i) => {
    t.push(...i === 0 ? [] : [`
`], ...ff(n));
  }), t;
}
function ff({ vnode: e, recurseCount: t }) {
  const n = t > 0 ? `... (${t} recursive calls)` : "", i = e.component ? e.component.parent == null : !1, r = ` at <${ei(
    e.component,
    e.type,
    i
  )}`, s = ">" + n;
  return e.props ? [r, ...lf(e.props), s] : [r + s];
}
function lf(e) {
  const t = [], n = Object.keys(e);
  return n.slice(0, 3).forEach((i) => {
    t.push(...ps(i, e[i]));
  }), n.length > 3 && t.push(" ..."), t;
}
function ps(e, t, n) {
  return fe(t) ? (t = JSON.stringify(t), n ? t : [`${e}=${t}`]) : typeof t == "number" || typeof t == "boolean" || t == null ? n ? t : [`${e}=${t}`] : ce(t) ? (t = ps(e, G(t.value), !0), n ? t : [`${e}=Ref<`, t, ">"]) : X(t) ? [`${e}=fn${t.name ? `<${t.name}>` : ""}`] : (t = G(t), n ? t : [`${e}=`, t]);
}
const Xi = {
  sp: "serverPrefetch hook",
  bc: "beforeCreate hook",
  c: "created hook",
  bm: "beforeMount hook",
  m: "mounted hook",
  bu: "beforeUpdate hook",
  u: "updated",
  bum: "beforeUnmount hook",
  um: "unmounted hook",
  a: "activated hook",
  da: "deactivated hook",
  ec: "errorCaptured hook",
  rtc: "renderTracked hook",
  rtg: "renderTriggered hook",
  0: "setup function",
  1: "render function",
  2: "watcher getter",
  3: "watcher callback",
  4: "watcher cleanup function",
  5: "native event handler",
  6: "component event handler",
  7: "vnode hook",
  8: "directive hook",
  9: "transition hook",
  10: "app errorHandler",
  11: "app warnHandler",
  12: "ref function",
  13: "async component loader",
  14: "scheduler flush",
  15: "component update",
  16: "app unmount cleanup function"
};
function Ft(e, t, n, i) {
  try {
    return i ? e(...i) : e();
  } catch (r) {
    pn(r, t, n);
  }
}
function et(e, t, n, i) {
  if (X(e)) {
    const r = Ft(e, t, n, i);
    return r && Li(r) && r.catch((s) => {
      pn(s, t, n);
    }), r;
  }
  if (z(e)) {
    const r = [];
    for (let s = 0; s < e.length; s++)
      r.push(et(e[s], t, n, i));
    return r;
  } else
    ({}).NODE_ENV !== "production" && N(
      `Invalid value type passed to callWithAsyncErrorHandling(): ${typeof e}`
    );
}
function pn(e, t, n, i = !0) {
  const r = t ? t.vnode : null, { errorHandler: s, throwUnhandledErrorInProduction: o } = t && t.appContext.config || ee;
  if (t) {
    let f = t.parent;
    const u = t.proxy, d = {}.NODE_ENV !== "production" ? Xi[n] : `https://vuejs.org/error-reference/#runtime-${n}`;
    for (; f; ) {
      const a = f.ec;
      if (a) {
        for (let l = 0; l < a.length; l++)
          if (a[l](e, u, d) === !1)
            return;
      }
      f = f.parent;
    }
    if (s) {
      at(), Ft(s, null, 10, [
        e,
        u,
        d
      ]), dt();
      return;
    }
  }
  cf(e, n, r, i, o);
}
function cf(e, t, n, i = !0, r = !1) {
  if ({}.NODE_ENV !== "production") {
    const s = Xi[t];
    if (n && bn(n), N(`Unhandled error${s ? ` during execution of ${s}` : ""}`), n && yn(), i)
      throw e;
    console.error(e);
  } else {
    if (r)
      throw e;
    console.error(e);
  }
}
const Ce = [];
let We = -1;
const Qt = [];
let vt = null, Xt = 0;
const gs = /* @__PURE__ */ Promise.resolve();
let kn = null;
const uf = 100;
function ln(e) {
  const t = kn || gs;
  return e ? t.then(this ? e.bind(this) : e) : t;
}
function Af(e) {
  let t = We + 1, n = Ce.length;
  for (; t < n; ) {
    const i = t + n >>> 1, r = Ce[i], s = cn(r);
    s < e || s === e && r.flags & 2 ? t = i + 1 : n = i;
  }
  return t;
}
function Kn(e) {
  if (!(e.flags & 1)) {
    const t = cn(e), n = Ce[Ce.length - 1];
    !n || // fast path when the job id is larger than the tail
    !(e.flags & 2) && t >= cn(n) ? Ce.push(e) : Ce.splice(Af(t), 0, e), e.flags |= 1, hs();
  }
}
function hs() {
  kn || (kn = gs.then(ms));
}
function vs(e) {
  z(e) ? Qt.push(...e) : vt && e.id === -1 ? vt.splice(Xt + 1, 0, e) : e.flags & 1 || (Qt.push(e), e.flags |= 1), hs();
}
function lr(e, t, n = We + 1) {
  for ({}.NODE_ENV !== "production" && (t = t || /* @__PURE__ */ new Map()); n < Ce.length; n++) {
    const i = Ce[n];
    if (i && i.flags & 2) {
      if (e && i.id !== e.uid || {}.NODE_ENV !== "production" && Ji(t, i))
        continue;
      Ce.splice(n, 1), n--, i.flags & 4 && (i.flags &= -2), i(), i.flags & 4 || (i.flags &= -2);
    }
  }
}
function Es(e) {
  if (Qt.length) {
    const t = [...new Set(Qt)].sort(
      (n, i) => cn(n) - cn(i)
    );
    if (Qt.length = 0, vt) {
      vt.push(...t);
      return;
    }
    for (vt = t, {}.NODE_ENV !== "production" && (e = e || /* @__PURE__ */ new Map()), Xt = 0; Xt < vt.length; Xt++) {
      const n = vt[Xt];
      ({}).NODE_ENV !== "production" && Ji(e, n) || (n.flags & 4 && (n.flags &= -2), n.flags & 8 || n(), n.flags &= -2);
    }
    vt = null, Xt = 0;
  }
}
const cn = (e) => e.id == null ? e.flags & 2 ? -1 : 1 / 0 : e.id;
function ms(e) {
  ({}).NODE_ENV !== "production" && (e = e || /* @__PURE__ */ new Map());
  const t = {}.NODE_ENV !== "production" ? (n) => Ji(e, n) : pe;
  try {
    for (We = 0; We < Ce.length; We++) {
      const n = Ce[We];
      if (n && !(n.flags & 8)) {
        if ({}.NODE_ENV !== "production" && t(n))
          continue;
        n.flags & 4 && (n.flags &= -2), Ft(
          n,
          n.i,
          n.i ? 15 : 14
        ), n.flags & 4 || (n.flags &= -2);
      }
    }
  } finally {
    for (; We < Ce.length; We++) {
      const n = Ce[We];
      n && (n.flags &= -2);
    }
    We = -1, Ce.length = 0, Es(e), kn = null, (Ce.length || Qt.length) && ms(e);
  }
}
function Ji(e, t) {
  const n = e.get(t) || 0;
  if (n > uf) {
    const i = t.i, r = i && $s(i.type);
    return pn(
      `Maximum recursive updates exceeded${r ? ` in component <${r}>` : ""}. This means you have a reactive effect that is mutating its own dependencies and thus recursively triggering itself. Possible sources include component template, render function, updated hook or watcher source function.`,
      null,
      10
    ), !0;
  }
  return e.set(t, n + 1), !1;
}
let $e = !1;
const Pn = /* @__PURE__ */ new Map();
({}).NODE_ENV !== "production" && (dn().__VUE_HMR_RUNTIME__ = {
  createRecord: oi(Is),
  rerender: oi(pf),
  reload: oi(gf)
});
const zt = /* @__PURE__ */ new Map();
function af(e) {
  const t = e.type.__hmrId;
  let n = zt.get(t);
  n || (Is(t, e.type), n = zt.get(t)), n.instances.add(e);
}
function df(e) {
  zt.get(e.type.__hmrId).instances.delete(e);
}
function Is(e, t) {
  return zt.has(e) ? !1 : (zt.set(e, {
    initialDef: On(t),
    instances: /* @__PURE__ */ new Set()
  }), !0);
}
function On(e) {
  return eo(e) ? e.__vccOpts : e;
}
function pf(e, t) {
  const n = zt.get(e);
  n && (n.initialDef.render = t, [...n.instances].forEach((i) => {
    t && (i.render = t, On(i.type).render = t), i.renderCache = [], $e = !0, i.update(), $e = !1;
  }));
}
function gf(e, t) {
  const n = zt.get(e);
  if (!n)
    return;
  t = On(t), cr(n.initialDef, t);
  const i = [...n.instances];
  for (let r = 0; r < i.length; r++) {
    const s = i[r], o = On(s.type);
    let f = Pn.get(o);
    f || (o !== n.initialDef && cr(o, t), Pn.set(o, f = /* @__PURE__ */ new Set())), f.add(s), s.appContext.propsCache.delete(s.type), s.appContext.emitsCache.delete(s.type), s.appContext.optionsCache.delete(s.type), s.ceReload ? (f.add(s), s.ceReload(t.styles), f.delete(s)) : s.parent ? Kn(() => {
      $e = !0, s.parent.update(), $e = !1, f.delete(s);
    }) : s.appContext.reload ? s.appContext.reload() : typeof window < "u" ? window.location.reload() : console.warn(
      "[HMR] Root or manually mounted instance modified. Full reload required."
    ), s.root.ce && s !== s.root && s.root.ce._removeChildStyle(o);
  }
  vs(() => {
    Pn.clear();
  });
}
function cr(e, t) {
  ue(e, t);
  for (const n in e)
    n !== "__file" && !(n in t) && delete e[n];
}
function oi(e) {
  return (t, n) => {
    try {
      return e(t, n);
    } catch (i) {
      console.error(i), console.warn(
        "[HMR] Something went wrong during Vue component hot-reload. Full reload required."
      );
    }
  };
}
let qe, _t = [], Ei = !1;
function gn(e, ...t) {
  qe ? qe.emit(e, ...t) : Ei || _t.push({ event: e, args: t });
}
function bs(e, t) {
  var n, i;
  qe = e, qe ? (qe.enabled = !0, _t.forEach(({ event: r, args: s }) => qe.emit(r, ...s)), _t = []) : /* handle late devtools injection - only do this if we are in an actual */ /* browser environment to avoid the timer handle stalling test runner exit */ /* (#4815) */ typeof window < "u" && // some envs mock window but not fully
  window.HTMLElement && // also exclude jsdom
  // eslint-disable-next-line no-restricted-syntax
  !((i = (n = window.navigator) == null ? void 0 : n.userAgent) != null && i.includes("jsdom")) ? ((t.__VUE_DEVTOOLS_HOOK_REPLAY__ = t.__VUE_DEVTOOLS_HOOK_REPLAY__ || []).push((s) => {
    bs(s, t);
  }), setTimeout(() => {
    qe || (t.__VUE_DEVTOOLS_HOOK_REPLAY__ = null, Ei = !0, _t = []);
  }, 3e3)) : (Ei = !0, _t = []);
}
function hf(e, t) {
  gn("app:init", e, t, {
    Fragment: Ne,
    Text: hn,
    Comment: He,
    Static: xn
  });
}
function vf(e) {
  gn("app:unmount", e);
}
const Ef = /* @__PURE__ */ Qi(
  "component:added"
  /* COMPONENT_ADDED */
), ys = /* @__PURE__ */ Qi(
  "component:updated"
  /* COMPONENT_UPDATED */
), mf = /* @__PURE__ */ Qi(
  "component:removed"
  /* COMPONENT_REMOVED */
), If = (e) => {
  qe && typeof qe.cleanupBuffer == "function" && // remove the component if it wasn't buffered
  !qe.cleanupBuffer(e) && mf(e);
};
/*! #__NO_SIDE_EFFECTS__ */
// @__NO_SIDE_EFFECTS__
function Qi(e) {
  return (t) => {
    gn(
      e,
      t.appContext.app,
      t.uid,
      t.parent ? t.parent.uid : void 0,
      t
    );
  };
}
const bf = /* @__PURE__ */ Ps(
  "perf:start"
  /* PERFORMANCE_START */
), yf = /* @__PURE__ */ Ps(
  "perf:end"
  /* PERFORMANCE_END */
);
function Ps(e) {
  return (t, n, i) => {
    gn(e, t.appContext.app, t.uid, t, n, i);
  };
}
function Pf(e, t, n) {
  gn(
    "component:emit",
    e.appContext.app,
    e,
    t,
    n
  );
}
let ke = null, ws = null;
function Hn(e) {
  const t = ke;
  return ke = e, ws = e && e.type.__scopeId || null, t;
}
function wf(e, t = ke, n) {
  if (!t || e._n)
    return e;
  const i = (...r) => {
    i._d && Ir(-1);
    const s = Hn(t);
    let o;
    try {
      o = e(...r);
    } finally {
      Hn(s), i._d && Ir(1);
    }
    return {}.NODE_ENV !== "production" && ys(t), o;
  };
  return i._n = !0, i._c = !0, i._d = !0, i;
}
function Cs(e) {
  po(e) && N("Do not use built-in directive ids as custom directive id: " + e);
}
function Dt(e, t, n, i) {
  const r = e.dirs, s = t && t.dirs;
  for (let o = 0; o < r.length; o++) {
    const f = r[o];
    s && (f.oldValue = s[o].value);
    let u = f.dir[i];
    u && (at(), et(u, n, 8, [
      e.el,
      f,
      e,
      t
    ]), dt());
  }
}
const Cf = Symbol("_vte"), xf = (e) => e.__isTeleport;
function Yi(e, t) {
  e.shapeFlag & 6 && e.component ? (e.transition = t, Yi(e.component.subTree, t)) : e.shapeFlag & 128 ? (e.ssContent.transition = t.clone(e.ssContent), e.ssFallback.transition = t.clone(e.ssFallback)) : e.transition = t;
}
function xs(e) {
  e.ids = [e.ids[0] + e.ids[2]++ + "-", 0, 0];
}
const Df = /* @__PURE__ */ new WeakSet();
function Rn(e, t, n, i, r = !1) {
  if (z(e)) {
    e.forEach(
      (w, m) => Rn(
        w,
        t && (z(t) ? t[m] : t),
        n,
        i,
        r
      )
    );
    return;
  }
  if (rn(i) && !r) {
    i.shapeFlag & 512 && i.type.__asyncResolved && i.component.subTree.component && Rn(e, t, n, i.component.subTree);
    return;
  }
  const s = i.shapeFlag & 4 ? _i(i.component) : i.el, o = r ? null : s, { i: f, r: u } = e;
  if ({}.NODE_ENV !== "production" && !f) {
    N(
      "Missing ref owner context. ref cannot be used on hoisted vnodes. A vnode with ref must be created inside the render function."
    );
    return;
  }
  const d = t && t.r, a = f.refs === ee ? f.refs = {} : f.refs, l = f.setupState, p = G(l), h = l === ee ? () => !1 : (w) => ({}).NODE_ENV !== "production" && (W(p, w) && !ce(p[w]) && N(
    `Template ref "${w}" used on a non-ref value. It will not work in the production build.`
  ), Df.has(p[w])) ? !1 : W(p, w);
  if (d != null && d !== u && (fe(d) ? (a[d] = null, h(d) && (l[d] = null)) : ce(d) && (d.value = null)), X(u))
    Ft(u, f, 12, [o, a]);
  else {
    const w = fe(u), m = ce(u);
    if (w || m) {
      const O = () => {
        if (e.f) {
          const k = w ? h(u) ? l[u] : a[u] : u.value;
          r ? z(k) && Si(k, s) : z(k) ? k.includes(s) || k.push(s) : w ? (a[u] = [s], h(u) && (l[u] = a[u])) : (u.value = [s], e.k && (a[e.k] = u.value));
        } else
          w ? (a[u] = o, h(u) && (l[u] = o)) : m ? (u.value = o, e.k && (a[e.k] = o)) : {}.NODE_ENV !== "production" && N("Invalid template ref type:", u, `(${typeof u})`);
      };
      o ? (O.id = -1, Le(O, n)) : O();
    } else
      ({}).NODE_ENV !== "production" && N("Invalid template ref type:", u, `(${typeof u})`);
  }
}
dn().requestIdleCallback;
dn().cancelIdleCallback;
const rn = (e) => !!e.type.__asyncLoader, Zi = (e) => e.type.__isKeepAlive;
function Nf(e, t) {
  Ds(e, "a", t);
}
function Bf(e, t) {
  Ds(e, "da", t);
}
function Ds(e, t, n = he) {
  const i = e.__wdc || (e.__wdc = () => {
    let r = n;
    for (; r; ) {
      if (r.isDeactivated)
        return;
      r = r.parent;
    }
    return e();
  });
  if (Un(t, i, n), n) {
    let r = n.parent;
    for (; r && r.parent; )
      Zi(r.parent.vnode) && Mf(i, t, n, r), r = r.parent;
  }
}
function Mf(e, t, n, i) {
  const r = Un(
    t,
    e,
    i,
    !0
    /* prepend */
  );
  Ns(() => {
    Si(i[t], r);
  }, n);
}
function Un(e, t, n = he, i = !1) {
  if (n) {
    const r = n[e] || (n[e] = []), s = t.__weh || (t.__weh = (...o) => {
      at();
      const f = vn(n), u = et(t, n, e, o);
      return f(), dt(), u;
    });
    return i ? r.unshift(s) : r.push(s), s;
  } else if ({}.NODE_ENV !== "production") {
    const r = Bt(Xi[e].replace(/ hook$/, ""));
    N(
      `${r} is called when there is no active component instance to be associated with. Lifecycle injection APIs can only be used during execution of setup(). If you are using async setup(), make sure to register lifecycle hooks before the first await statement.`
    );
  }
}
const pt = (e) => (t, n = he) => {
  (!An || e === "sp") && Un(e, (...i) => t(...i), n);
}, Sf = pt("bm"), qn = pt("m"), Lf = pt(
  "bu"
), kf = pt("u"), Of = pt(
  "bum"
), Ns = pt("um"), Hf = pt(
  "sp"
), Rf = pt("rtg"), Tf = pt("rtc");
function zf(e, t = he) {
  Un("ec", e, t);
}
const jf = Symbol.for("v-ndc");
function Gi(e, t, n, i) {
  let r;
  const s = n && n[i], o = z(e);
  if (o || fe(e)) {
    const f = o && Ht(e);
    let u = !1;
    f && (u = !xe(e), e = Fn(e)), r = new Array(e.length);
    for (let d = 0, a = e.length; d < a; d++)
      r[d] = t(
        u ? Ie(e[d]) : e[d],
        d,
        void 0,
        s && s[d]
      );
  } else if (typeof e == "number") {
    ({}).NODE_ENV !== "production" && !Number.isInteger(e) && N(`The v-for range expect an integer value but got ${e}.`), r = new Array(e);
    for (let f = 0; f < e; f++)
      r[f] = t(f + 1, f, void 0, s && s[f]);
  } else if (ie(e))
    if (e[Symbol.iterator])
      r = Array.from(
        e,
        (f, u) => t(f, u, void 0, s && s[u])
      );
    else {
      const f = Object.keys(e);
      r = new Array(f.length);
      for (let u = 0, d = f.length; u < d; u++) {
        const a = f[u];
        r[u] = t(e[a], a, u, s && s[u]);
      }
    }
  else
    r = [];
  return n && (n[i] = r), r;
}
const mi = (e) => e ? qs(e) ? _i(e) : mi(e.parent) : null, Tt = (
  // Move PURE marker to new line to workaround compiler discarding it
  // due to type annotation
  /* @__PURE__ */ ue(/* @__PURE__ */ Object.create(null), {
    $: (e) => e,
    $el: (e) => e.vnode.el,
    $data: (e) => e.data,
    $props: (e) => ({}).NODE_ENV !== "production" ? _e(e.props) : e.props,
    $attrs: (e) => ({}).NODE_ENV !== "production" ? _e(e.attrs) : e.attrs,
    $slots: (e) => ({}).NODE_ENV !== "production" ? _e(e.slots) : e.slots,
    $refs: (e) => ({}).NODE_ENV !== "production" ? _e(e.refs) : e.refs,
    $parent: (e) => mi(e.parent),
    $root: (e) => mi(e.root),
    $host: (e) => e.ce,
    $emit: (e) => e.emit,
    $options: (e) => Vi(e),
    $forceUpdate: (e) => e.f || (e.f = () => {
      Kn(e.update);
    }),
    $nextTick: (e) => e.n || (e.n = ln.bind(e.proxy)),
    $watch: (e) => El.bind(e)
  })
), Fi = (e) => e === "_" || e === "$", fi = (e, t) => e !== ee && !e.__isScriptSetup && W(e, t), Bs = {
  get({ _: e }, t) {
    if (t === "__v_skip")
      return !0;
    const { ctx: n, setupState: i, data: r, props: s, accessCache: o, type: f, appContext: u } = e;
    if ({}.NODE_ENV !== "production" && t === "__isVue")
      return !0;
    let d;
    if (t[0] !== "$") {
      const h = o[t];
      if (h !== void 0)
        switch (h) {
          case 1:
            return i[t];
          case 2:
            return r[t];
          case 4:
            return n[t];
          case 3:
            return s[t];
        }
      else {
        if (fi(i, t))
          return o[t] = 1, i[t];
        if (r !== ee && W(r, t))
          return o[t] = 2, r[t];
        if (
          // only cache other properties when instance has declared (thus stable)
          // props
          (d = e.propsOptions[0]) && W(d, t)
        )
          return o[t] = 3, s[t];
        if (n !== ee && W(n, t))
          return o[t] = 4, n[t];
        Ii && (o[t] = 0);
      }
    }
    const a = Tt[t];
    let l, p;
    if (a)
      return t === "$attrs" ? (de(e.attrs, "get", ""), {}.NODE_ENV !== "production" && jn()) : {}.NODE_ENV !== "production" && t === "$slots" && de(e, "get", t), a(e);
    if (
      // css module (injected by vue-loader)
      (l = f.__cssModules) && (l = l[t])
    )
      return l;
    if (n !== ee && W(n, t))
      return o[t] = 4, n[t];
    if (
      // global properties
      p = u.config.globalProperties, W(p, t)
    )
      return p[t];
    ({}).NODE_ENV !== "production" && ke && (!fe(t) || // #1091 avoid internal isRef/isVNode checks on component instance leading
    // to infinite warning loop
    t.indexOf("__v") !== 0) && (r !== ee && Fi(t[0]) && W(r, t) ? N(
      `Property ${JSON.stringify(
        t
      )} must be accessed via $data because it starts with a reserved character ("$" or "_") and is not proxied on the render context.`
    ) : e === ke && N(
      `Property ${JSON.stringify(t)} was accessed during render but is not defined on instance.`
    ));
  },
  set({ _: e }, t, n) {
    const { data: i, setupState: r, ctx: s } = e;
    return fi(r, t) ? (r[t] = n, !0) : {}.NODE_ENV !== "production" && r.__isScriptSetup && W(r, t) ? (N(`Cannot mutate <script setup> binding "${t}" from Options API.`), !1) : i !== ee && W(i, t) ? (i[t] = n, !0) : W(e.props, t) ? ({}.NODE_ENV !== "production" && N(`Attempting to mutate prop "${t}". Props are readonly.`), !1) : t[0] === "$" && t.slice(1) in e ? ({}.NODE_ENV !== "production" && N(
      `Attempting to mutate public property "${t}". Properties starting with $ are reserved and readonly.`
    ), !1) : ({}.NODE_ENV !== "production" && t in e.appContext.config.globalProperties ? Object.defineProperty(s, t, {
      enumerable: !0,
      configurable: !0,
      value: n
    }) : s[t] = n, !0);
  },
  has({
    _: { data: e, setupState: t, accessCache: n, ctx: i, appContext: r, propsOptions: s }
  }, o) {
    let f;
    return !!n[o] || e !== ee && W(e, o) || fi(t, o) || (f = s[0]) && W(f, o) || W(i, o) || W(Tt, o) || W(r.config.globalProperties, o);
  },
  defineProperty(e, t, n) {
    return n.get != null ? e._.accessCache[t] = 0 : W(n, "value") && this.set(e, t, n.value, null), Reflect.defineProperty(e, t, n);
  }
};
({}).NODE_ENV !== "production" && (Bs.ownKeys = (e) => (N(
  "Avoid app logic that relies on enumerating keys on a component instance. The keys will be empty in production mode to avoid performance overhead."
), Reflect.ownKeys(e)));
function Xf(e) {
  const t = {};
  return Object.defineProperty(t, "_", {
    configurable: !0,
    enumerable: !1,
    get: () => e
  }), Object.keys(Tt).forEach((n) => {
    Object.defineProperty(t, n, {
      configurable: !0,
      enumerable: !1,
      get: () => Tt[n](e),
      // intercepted by the proxy so no need for implementation,
      // but needed to prevent set errors
      set: pe
    });
  }), t;
}
function Jf(e) {
  const {
    ctx: t,
    propsOptions: [n]
  } = e;
  n && Object.keys(n).forEach((i) => {
    Object.defineProperty(t, i, {
      enumerable: !0,
      configurable: !0,
      get: () => e.props[i],
      set: pe
    });
  });
}
function Qf(e) {
  const { ctx: t, setupState: n } = e;
  Object.keys(G(n)).forEach((i) => {
    if (!n.__isScriptSetup) {
      if (Fi(i[0])) {
        N(
          `setup() return property ${JSON.stringify(
            i
          )} should not start with "$" or "_" which are reserved prefixes for Vue internals.`
        );
        return;
      }
      Object.defineProperty(t, i, {
        enumerable: !0,
        configurable: !0,
        get: () => n[i],
        set: pe
      });
    }
  });
}
function ur(e) {
  return z(e) ? e.reduce(
    (t, n) => (t[n] = null, t),
    {}
  ) : e;
}
function Yf() {
  const e = /* @__PURE__ */ Object.create(null);
  return (t, n) => {
    e[n] ? N(`${t} property "${n}" is already defined in ${e[n]}.`) : e[n] = t;
  };
}
let Ii = !0;
function Zf(e) {
  const t = Vi(e), n = e.proxy, i = e.ctx;
  Ii = !1, t.beforeCreate && Ar(t.beforeCreate, e, "bc");
  const {
    // state
    data: r,
    computed: s,
    methods: o,
    watch: f,
    provide: u,
    inject: d,
    // lifecycle
    created: a,
    beforeMount: l,
    mounted: p,
    beforeUpdate: h,
    updated: w,
    activated: m,
    deactivated: O,
    beforeDestroy: k,
    beforeUnmount: Q,
    destroyed: R,
    unmounted: K,
    render: L,
    renderTracked: q,
    renderTriggered: ae,
    errorCaptured: le,
    serverPrefetch: oe,
    // public API
    expose: te,
    inheritAttrs: F,
    // assets
    components: Ee,
    directives: xt,
    filters: J
  } = t, S = {}.NODE_ENV !== "production" ? Yf() : null;
  if ({}.NODE_ENV !== "production") {
    const [P] = e.propsOptions;
    if (P)
      for (const E in P)
        S("Props", E);
  }
  if (d && Gf(d, i, S), o)
    for (const P in o) {
      const E = o[P];
      X(E) ? ({}.NODE_ENV !== "production" ? Object.defineProperty(i, P, {
        value: E.bind(n),
        configurable: !0,
        enumerable: !0,
        writable: !0
      }) : i[P] = E.bind(n), {}.NODE_ENV !== "production" && S("Methods", P)) : {}.NODE_ENV !== "production" && N(
        `Method "${P}" has type "${typeof E}" in the component definition. Did you reference the function correctly?`
      );
    }
  if (r) {
    ({}).NODE_ENV !== "production" && !X(r) && N(
      "The data option must be a function. Plain object usage is no longer supported."
    );
    const P = r.call(n, n);
    if ({}.NODE_ENV !== "production" && Li(P) && N(
      "data() returned a Promise - note data() cannot be async; If you intend to perform data fetching before component renders, use async setup() + <Suspense>."
    ), !ie(P))
      ({}).NODE_ENV !== "production" && N("data() should return an object.");
    else if (e.data = ji(P), {}.NODE_ENV !== "production")
      for (const E in P)
        S("Data", E), Fi(E[0]) || Object.defineProperty(i, E, {
          configurable: !0,
          enumerable: !0,
          get: () => P[E],
          set: pe
        });
  }
  if (Ii = !0, s)
    for (const P in s) {
      const E = s[P], T = X(E) ? E.bind(n, n) : X(E.get) ? E.get.bind(n, n) : pe;
      ({}).NODE_ENV !== "production" && T === pe && N(`Computed property "${P}" has no getter.`);
      const Y = !X(E) && X(E.set) ? E.set.bind(n) : {}.NODE_ENV !== "production" ? () => {
        N(
          `Write operation failed: computed property "${P}" is readonly.`
        );
      } : pe, U = Xe({
        get: T,
        set: Y
      });
      Object.defineProperty(i, P, {
        enumerable: !0,
        configurable: !0,
        get: () => U.value,
        set: (ne) => U.value = ne
      }), {}.NODE_ENV !== "production" && S("Computed", P);
    }
  if (f)
    for (const P in f)
      Ms(f[P], i, n, P);
  if (u) {
    const P = X(u) ? u.call(n) : u;
    Reflect.ownKeys(P).forEach((E) => {
      qf(E, P[E]);
    });
  }
  a && Ar(a, e, "c");
  function C(P, E) {
    z(E) ? E.forEach((T) => P(T.bind(n))) : E && P(E.bind(n));
  }
  if (C(Sf, l), C(qn, p), C(Lf, h), C(kf, w), C(Nf, m), C(Bf, O), C(zf, le), C(Tf, q), C(Rf, ae), C(Of, Q), C(Ns, K), C(Hf, oe), z(te))
    if (te.length) {
      const P = e.exposed || (e.exposed = {});
      te.forEach((E) => {
        Object.defineProperty(P, E, {
          get: () => n[E],
          set: (T) => n[E] = T
        });
      });
    } else
      e.exposed || (e.exposed = {});
  L && e.render === pe && (e.render = L), F != null && (e.inheritAttrs = F), Ee && (e.components = Ee), xt && (e.directives = xt), oe && xs(e);
}
function Gf(e, t, n = pe) {
  z(e) && (e = bi(e));
  for (const i in e) {
    const r = e[i];
    let s;
    ie(r) ? "default" in r ? s = wn(
      r.from || i,
      r.default,
      !0
    ) : s = wn(r.from || i) : s = wn(r), ce(s) ? Object.defineProperty(t, i, {
      enumerable: !0,
      configurable: !0,
      get: () => s.value,
      set: (o) => s.value = o
    }) : t[i] = s, {}.NODE_ENV !== "production" && n("Inject", i);
  }
}
function Ar(e, t, n) {
  et(
    z(e) ? e.map((i) => i.bind(t.proxy)) : e.bind(t.proxy),
    t,
    n
  );
}
function Ms(e, t, n, i) {
  let r = i.includes(".") ? Ys(n, i) : () => n[i];
  if (fe(e)) {
    const s = t[e];
    X(s) ? ft(r, s) : {}.NODE_ENV !== "production" && N(`Invalid watch handler specified by key "${e}"`, s);
  } else if (X(e))
    ft(r, e.bind(n));
  else if (ie(e))
    if (z(e))
      e.forEach((s) => Ms(s, t, n, i));
    else {
      const s = X(e.handler) ? e.handler.bind(n) : t[e.handler];
      X(s) ? ft(r, s, e) : {}.NODE_ENV !== "production" && N(`Invalid watch handler specified by key "${e.handler}"`, s);
    }
  else
    ({}).NODE_ENV !== "production" && N(`Invalid watch option: "${i}"`, e);
}
function Vi(e) {
  const t = e.type, { mixins: n, extends: i } = t, {
    mixins: r,
    optionsCache: s,
    config: { optionMergeStrategies: o }
  } = e.appContext, f = s.get(t);
  let u;
  return f ? u = f : !r.length && !n && !i ? u = t : (u = {}, r.length && r.forEach(
    (d) => Tn(u, d, o, !0)
  ), Tn(u, t, o)), ie(t) && s.set(t, u), u;
}
function Tn(e, t, n, i = !1) {
  const { mixins: r, extends: s } = t;
  s && Tn(e, s, n, !0), r && r.forEach(
    (o) => Tn(e, o, n, !0)
  );
  for (const o in t)
    if (i && o === "expose")
      ({}).NODE_ENV !== "production" && N(
        '"expose" option is ignored when declared in mixins or extends. It should only be declared in the base component itself.'
      );
    else {
      const f = Ff[o] || n && n[o];
      e[o] = f ? f(e[o], t[o]) : t[o];
    }
  return e;
}
const Ff = {
  data: ar,
  props: dr,
  emits: dr,
  // objects
  methods: $t,
  computed: $t,
  // lifecycle
  beforeCreate: Pe,
  created: Pe,
  beforeMount: Pe,
  mounted: Pe,
  beforeUpdate: Pe,
  updated: Pe,
  beforeDestroy: Pe,
  beforeUnmount: Pe,
  destroyed: Pe,
  unmounted: Pe,
  activated: Pe,
  deactivated: Pe,
  errorCaptured: Pe,
  serverPrefetch: Pe,
  // assets
  components: $t,
  directives: $t,
  // watch
  watch: Wf,
  // provide / inject
  provide: ar,
  inject: Vf
};
function ar(e, t) {
  return t ? e ? function() {
    return ue(
      X(e) ? e.call(this, this) : e,
      X(t) ? t.call(this, this) : t
    );
  } : t : e;
}
function Vf(e, t) {
  return $t(bi(e), bi(t));
}
function bi(e) {
  if (z(e)) {
    const t = {};
    for (let n = 0; n < e.length; n++)
      t[e[n]] = e[n];
    return t;
  }
  return e;
}
function Pe(e, t) {
  return e ? [...new Set([].concat(e, t))] : t;
}
function $t(e, t) {
  return e ? ue(/* @__PURE__ */ Object.create(null), e, t) : t;
}
function dr(e, t) {
  return e ? z(e) && z(t) ? [.../* @__PURE__ */ new Set([...e, ...t])] : ue(
    /* @__PURE__ */ Object.create(null),
    ur(e),
    ur(t ?? {})
  ) : t;
}
function Wf(e, t) {
  if (!e)
    return t;
  if (!t)
    return e;
  const n = ue(/* @__PURE__ */ Object.create(null), e);
  for (const i in t)
    n[i] = Pe(e[i], t[i]);
  return n;
}
function Ss() {
  return {
    app: null,
    config: {
      isNativeTag: Ao,
      performance: !1,
      globalProperties: {},
      optionMergeStrategies: {},
      errorHandler: void 0,
      warnHandler: void 0,
      compilerOptions: {}
    },
    mixins: [],
    components: {},
    directives: {},
    provides: /* @__PURE__ */ Object.create(null),
    optionsCache: /* @__PURE__ */ new WeakMap(),
    propsCache: /* @__PURE__ */ new WeakMap(),
    emitsCache: /* @__PURE__ */ new WeakMap()
  };
}
let Kf = 0;
function Uf(e, t) {
  return function(i, r = null) {
    X(i) || (i = ue({}, i)), r != null && !ie(r) && ({}.NODE_ENV !== "production" && N("root props passed to app.mount() must be an object."), r = null);
    const s = Ss(), o = /* @__PURE__ */ new WeakSet(), f = [];
    let u = !1;
    const d = s.app = {
      _uid: Kf++,
      _component: i,
      _props: r,
      _container: null,
      _context: s,
      _instance: null,
      version: Cr,
      get config() {
        return s.config;
      },
      set config(a) {
        ({}).NODE_ENV !== "production" && N(
          "app.config cannot be replaced. Modify individual options instead."
        );
      },
      use(a, ...l) {
        return o.has(a) ? {}.NODE_ENV !== "production" && N("Plugin has already been applied to target app.") : a && X(a.install) ? (o.add(a), a.install(d, ...l)) : X(a) ? (o.add(a), a(d, ...l)) : {}.NODE_ENV !== "production" && N(
          'A plugin must either be a function or an object with an "install" function.'
        ), d;
      },
      mixin(a) {
        return s.mixins.includes(a) ? {}.NODE_ENV !== "production" && N(
          "Mixin has already been applied to target app" + (a.name ? `: ${a.name}` : "")
        ) : s.mixins.push(a), d;
      },
      component(a, l) {
        return {}.NODE_ENV !== "production" && xi(a, s.config), l ? ({}.NODE_ENV !== "production" && s.components[a] && N(`Component "${a}" has already been registered in target app.`), s.components[a] = l, d) : s.components[a];
      },
      directive(a, l) {
        return {}.NODE_ENV !== "production" && Cs(a), l ? ({}.NODE_ENV !== "production" && s.directives[a] && N(`Directive "${a}" has already been registered in target app.`), s.directives[a] = l, d) : s.directives[a];
      },
      mount(a, l, p) {
        if (u)
          ({}).NODE_ENV !== "production" && N(
            "App has already been mounted.\nIf you want to remount the same app, move your app creation logic into a factory function and create fresh app instances for each mount - e.g. `const createMyApp = () => createApp(App)`"
          );
        else {
          ({}).NODE_ENV !== "production" && a.__vue_app__ && N(
            "There is already an app instance mounted on the host container.\n If you want to mount another app on the same host container, you need to unmount the previous app by calling `app.unmount()` first."
          );
          const h = d._ceVNode || lt(i, r);
          return h.appContext = s, p === !0 ? p = "svg" : p === !1 && (p = void 0), {}.NODE_ENV !== "production" && (s.reload = () => {
            e(
              wt(h),
              a,
              p
            );
          }), l && t ? t(h, a) : e(h, a, p), u = !0, d._container = a, a.__vue_app__ = d, {}.NODE_ENV !== "production" && (d._instance = h.component, hf(d, Cr)), _i(h.component);
        }
      },
      onUnmount(a) {
        ({}).NODE_ENV !== "production" && typeof a != "function" && N(
          `Expected function as first argument to app.onUnmount(), but got ${typeof a}`
        ), f.push(a);
      },
      unmount() {
        u ? (et(
          f,
          d._instance,
          16
        ), e(null, d._container), {}.NODE_ENV !== "production" && (d._instance = null, vf(d)), delete d._container.__vue_app__) : {}.NODE_ENV !== "production" && N("Cannot unmount an app that is not mounted.");
      },
      provide(a, l) {
        return {}.NODE_ENV !== "production" && a in s.provides && N(
          `App already provides property with key "${String(a)}". It will be overwritten with the new value.`
        ), s.provides[a] = l, d;
      },
      runWithContext(a) {
        const l = Yt;
        Yt = d;
        try {
          return a();
        } finally {
          Yt = l;
        }
      }
    };
    return d;
  };
}
let Yt = null;
function qf(e, t) {
  if (!he)
    ({}).NODE_ENV !== "production" && N("provide() can only be used inside setup().");
  else {
    let n = he.provides;
    const i = he.parent && he.parent.provides;
    i === n && (n = he.provides = Object.create(i)), n[e] = t;
  }
}
function wn(e, t, n = !1) {
  const i = he || ke;
  if (i || Yt) {
    const r = Yt ? Yt._context.provides : i ? i.parent == null ? i.vnode.appContext && i.vnode.appContext.provides : i.parent.provides : void 0;
    if (r && e in r)
      return r[e];
    if (arguments.length > 1)
      return n && X(t) ? t.call(i && i.proxy) : t;
    ({}).NODE_ENV !== "production" && N(`injection "${String(e)}" not found.`);
  } else
    ({}).NODE_ENV !== "production" && N("inject() can only be used inside setup() or functional components.");
}
const Ls = {}, ks = () => Object.create(Ls), Os = (e) => Object.getPrototypeOf(e) === Ls;
function _f(e, t, n, i = !1) {
  const r = {}, s = ks();
  e.propsDefaults = /* @__PURE__ */ Object.create(null), Hs(e, t, r, s);
  for (const o in e.propsOptions[0])
    o in r || (r[o] = void 0);
  ({}).NODE_ENV !== "production" && Ts(t || {}, r, e), n ? e.props = i ? r : Uo(r) : e.type.props ? e.props = r : e.props = s, e.attrs = s;
}
function $f(e) {
  for (; e; ) {
    if (e.type.__hmrId)
      return !0;
    e = e.parent;
  }
}
function el(e, t, n, i) {
  const {
    props: r,
    attrs: s,
    vnode: { patchFlag: o }
  } = e, f = G(r), [u] = e.propsOptions;
  let d = !1;
  if (
    // always force full diff in dev
    // - #1942 if hmr is enabled with sfc component
    // - vite#872 non-sfc component used by sfc component
    !({}.NODE_ENV !== "production" && $f(e)) && (i || o > 0) && !(o & 16)
  ) {
    if (o & 8) {
      const a = e.vnode.dynamicProps;
      for (let l = 0; l < a.length; l++) {
        let p = a[l];
        if (_n(e.emitsOptions, p))
          continue;
        const h = t[p];
        if (u)
          if (W(s, p))
            h !== s[p] && (s[p] = h, d = !0);
          else {
            const w = Je(p);
            r[w] = yi(
              u,
              f,
              w,
              h,
              e,
              !1
            );
          }
        else
          h !== s[p] && (s[p] = h, d = !0);
      }
    }
  } else {
    Hs(e, t, r, s) && (d = !0);
    let a;
    for (const l in f)
      (!t || // for camelCase
      !W(t, l) && // it's possible the original props was passed in as kebab-case
      // and converted to camelCase (#955)
      ((a = Pt(l)) === l || !W(t, a))) && (u ? n && // for camelCase
      (n[l] !== void 0 || // for kebab-case
      n[a] !== void 0) && (r[l] = yi(
        u,
        f,
        l,
        void 0,
        e,
        !0
      )) : delete r[l]);
    if (s !== f)
      for (const l in s)
        (!t || !W(t, l)) && (delete s[l], d = !0);
  }
  d && Ue(e.attrs, "set", ""), {}.NODE_ENV !== "production" && Ts(t || {}, r, e);
}
function Hs(e, t, n, i) {
  const [r, s] = e.propsOptions;
  let o = !1, f;
  if (t)
    for (let u in t) {
      if (en(u))
        continue;
      const d = t[u];
      let a;
      r && W(r, a = Je(u)) ? !s || !s.includes(a) ? n[a] = d : (f || (f = {}))[a] = d : _n(e.emitsOptions, u) || (!(u in i) || d !== i[u]) && (i[u] = d, o = !0);
    }
  if (s) {
    const u = G(n), d = f || ee;
    for (let a = 0; a < s.length; a++) {
      const l = s[a];
      n[l] = yi(
        r,
        u,
        l,
        d[l],
        e,
        !W(d, l)
      );
    }
  }
  return o;
}
function yi(e, t, n, i, r, s) {
  const o = e[n];
  if (o != null) {
    const f = W(o, "default");
    if (f && i === void 0) {
      const u = o.default;
      if (o.type !== Function && !o.skipFactory && X(u)) {
        const { propsDefaults: d } = r;
        if (n in d)
          i = d[n];
        else {
          const a = vn(r);
          i = d[n] = u.call(
            null,
            t
          ), a();
        }
      } else
        i = u;
      r.ce && r.ce._setProp(n, i);
    }
    o[
      0
      /* shouldCast */
    ] && (s && !f ? i = !1 : o[
      1
      /* shouldCastTrue */
    ] && (i === "" || i === Pt(n)) && (i = !0));
  }
  return i;
}
const tl = /* @__PURE__ */ new WeakMap();
function Rs(e, t, n = !1) {
  const i = n ? tl : t.propsCache, r = i.get(e);
  if (r)
    return r;
  const s = e.props, o = {}, f = [];
  let u = !1;
  if (!X(e)) {
    const a = (l) => {
      u = !0;
      const [p, h] = Rs(l, t, !0);
      ue(o, p), h && f.push(...h);
    };
    !n && t.mixins.length && t.mixins.forEach(a), e.extends && a(e.extends), e.mixins && e.mixins.forEach(a);
  }
  if (!s && !u)
    return ie(e) && i.set(e, Jt), Jt;
  if (z(s))
    for (let a = 0; a < s.length; a++) {
      ({}).NODE_ENV !== "production" && !fe(s[a]) && N("props must be strings when using array syntax.", s[a]);
      const l = Je(s[a]);
      pr(l) && (o[l] = ee);
    }
  else if (s) {
    ({}).NODE_ENV !== "production" && !ie(s) && N("invalid props options", s);
    for (const a in s) {
      const l = Je(a);
      if (pr(l)) {
        const p = s[a], h = o[l] = z(p) || X(p) ? { type: p } : ue({}, p), w = h.type;
        let m = !1, O = !0;
        if (z(w))
          for (let k = 0; k < w.length; ++k) {
            const Q = w[k], R = X(Q) && Q.name;
            if (R === "Boolean") {
              m = !0;
              break;
            } else
              R === "String" && (O = !1);
          }
        else
          m = X(w) && w.name === "Boolean";
        h[
          0
          /* shouldCast */
        ] = m, h[
          1
          /* shouldCastTrue */
        ] = O, (m || W(h, "default")) && f.push(l);
      }
    }
  }
  const d = [o, f];
  return ie(e) && i.set(e, d), d;
}
function pr(e) {
  return e[0] !== "$" && !en(e) ? !0 : ({}.NODE_ENV !== "production" && N(`Invalid prop name: "${e}" is a reserved property.`), !1);
}
function nl(e) {
  return e === null ? "null" : typeof e == "function" ? e.name || "" : typeof e == "object" && e.constructor && e.constructor.name || "";
}
function Ts(e, t, n) {
  const i = G(t), r = n.propsOptions[0], s = Object.keys(e).map((o) => Je(o));
  for (const o in r) {
    let f = r[o];
    f != null && il(
      o,
      i[o],
      f,
      {}.NODE_ENV !== "production" ? _e(i) : i,
      !s.includes(o)
    );
  }
}
function il(e, t, n, i, r) {
  const { type: s, required: o, validator: f, skipCheck: u } = n;
  if (o && r) {
    N('Missing required prop: "' + e + '"');
    return;
  }
  if (!(t == null && !o)) {
    if (s != null && s !== !0 && !u) {
      let d = !1;
      const a = z(s) ? s : [s], l = [];
      for (let p = 0; p < a.length && !d; p++) {
        const { valid: h, expectedType: w } = sl(t, a[p]);
        l.push(w || ""), d = h;
      }
      if (!d) {
        N(ol(e, t, l));
        return;
      }
    }
    f && !f(t, i) && N('Invalid prop: custom validator check failed for prop "' + e + '".');
  }
}
const rl = /* @__PURE__ */ At(
  "String,Number,Boolean,Function,Symbol,BigInt"
);
function sl(e, t) {
  let n;
  const i = nl(t);
  if (i === "null")
    n = e === null;
  else if (rl(i)) {
    const r = typeof e;
    n = r === i.toLowerCase(), !n && r === "object" && (n = e instanceof t);
  } else
    i === "Object" ? n = ie(e) : i === "Array" ? n = z(e) : n = e instanceof t;
  return {
    valid: n,
    expectedType: i
  };
}
function ol(e, t, n) {
  if (n.length === 0)
    return `Prop type [] for prop "${e}" won't match anything. Did you mean to use type Array instead?`;
  let i = `Invalid prop: type check failed for prop "${e}". Expected ${n.map(Gn).join(" | ")}`;
  const r = n[0], s = ki(t), o = gr(t, r), f = gr(t, s);
  return n.length === 1 && hr(r) && !fl(r, s) && (i += ` with value ${o}`), i += `, got ${s} `, hr(s) && (i += `with value ${f}.`), i;
}
function gr(e, t) {
  return t === "String" ? `"${e}"` : t === "Number" ? `${Number(e)}` : `${e}`;
}
function hr(e) {
  return ["string", "number", "boolean"].some((n) => e.toLowerCase() === n);
}
function fl(...e) {
  return e.some((t) => t.toLowerCase() === "boolean");
}
const zs = (e) => e[0] === "_" || e === "$stable", Wi = (e) => z(e) ? e.map(je) : [je(e)], ll = (e, t, n) => {
  if (t._n)
    return t;
  const i = wf((...r) => ({}.NODE_ENV !== "production" && he && (!n || n.root === he.root) && N(
    `Slot "${e}" invoked outside of the render function: this will not track dependencies used in the slot. Invoke the slot function inside the render function instead.`
  ), Wi(t(...r))), n);
  return i._c = !1, i;
}, js = (e, t, n) => {
  const i = e._ctx;
  for (const r in e) {
    if (zs(r))
      continue;
    const s = e[r];
    if (X(s))
      t[r] = ll(r, s, i);
    else if (s != null) {
      ({}).NODE_ENV !== "production" && N(
        `Non-function value encountered for slot "${r}". Prefer function slots for better performance.`
      );
      const o = Wi(s);
      t[r] = () => o;
    }
  }
}, Xs = (e, t) => {
  ({}).NODE_ENV !== "production" && !Zi(e.vnode) && N(
    "Non-function value encountered for default slot. Prefer function slots for better performance."
  );
  const n = Wi(t);
  e.slots.default = () => n;
}, Pi = (e, t, n) => {
  for (const i in t)
    (n || i !== "_") && (e[i] = t[i]);
}, cl = (e, t, n) => {
  const i = e.slots = ks();
  if (e.vnode.shapeFlag & 32) {
    const r = t._;
    r ? (Pi(i, t, n), n && Mn(i, "_", r, !0)) : js(t, i);
  } else
    t && Xs(e, t);
}, ul = (e, t, n) => {
  const { vnode: i, slots: r } = e;
  let s = !0, o = ee;
  if (i.shapeFlag & 32) {
    const f = t._;
    f ? {}.NODE_ENV !== "production" && $e ? (Pi(r, t, n), Ue(e, "set", "$slots")) : n && f === 1 ? s = !1 : Pi(r, t, n) : (s = !t.$stable, js(t, r)), o = t;
  } else
    t && (Xs(e, t), o = { default: 1 });
  if (s)
    for (const f in r)
      !zs(f) && o[f] == null && delete r[f];
};
let Ut, bt;
function it(e, t) {
  e.appContext.config.performance && zn() && bt.mark(`vue-${t}-${e.uid}`), {}.NODE_ENV !== "production" && bf(e, t, zn() ? bt.now() : Date.now());
}
function rt(e, t) {
  if (e.appContext.config.performance && zn()) {
    const n = `vue-${t}-${e.uid}`, i = n + ":end";
    bt.mark(i), bt.measure(
      `<${ei(e, e.type)}> ${t}`,
      n,
      i
    ), bt.clearMarks(n), bt.clearMarks(i);
  }
  ({}).NODE_ENV !== "production" && yf(e, t, zn() ? bt.now() : Date.now());
}
function zn() {
  return Ut !== void 0 || (typeof window < "u" && window.performance ? (Ut = !0, bt = window.performance) : Ut = !1), Ut;
}
function Al() {
  const e = [];
  if ({}.NODE_ENV !== "production" && e.length) {
    const t = e.length > 1;
    console.warn(
      `Feature flag${t ? "s" : ""} ${e.join(", ")} ${t ? "are" : "is"} not explicitly defined. You are running the esm-bundler build of Vue, which expects these compile-time feature flags to be globally injected via the bundler config in order to get better tree-shaking in the production bundle.

For more details, see https://link.vuejs.org/feature-flags.`
    );
  }
}
const Le = Cl;
function al(e) {
  return dl(e);
}
function dl(e, t) {
  Al();
  const n = dn();
  n.__VUE__ = !0, {}.NODE_ENV !== "production" && bs(n.__VUE_DEVTOOLS_GLOBAL_HOOK__, n);
  const {
    insert: i,
    remove: r,
    patchProp: s,
    createElement: o,
    createText: f,
    createComment: u,
    setText: d,
    setElementText: a,
    parentNode: l,
    nextSibling: p,
    setScopeId: h = pe,
    insertStaticContent: w
  } = e, m = (c, A, g, b = null, v = null, I = null, B = void 0, D = null, x = {}.NODE_ENV !== "production" && $e ? !1 : !!A.dynamicChildren) => {
    if (c === A)
      return;
    c && !qt(c, A) && (b = Ge(c), re(c, v, I, !0), c = null), A.patchFlag === -2 && (x = !1, A.dynamicChildren = null);
    const { type: y, ref: j, shapeFlag: M } = A;
    switch (y) {
      case hn:
        O(c, A, g, b);
        break;
      case He:
        k(c, A, g, b);
        break;
      case xn:
        c == null ? Q(A, g, b, B) : {}.NODE_ENV !== "production" && R(c, A, g, B);
        break;
      case Ne:
        xt(
          c,
          A,
          g,
          b,
          v,
          I,
          B,
          D,
          x
        );
        break;
      default:
        M & 1 ? q(
          c,
          A,
          g,
          b,
          v,
          I,
          B,
          D,
          x
        ) : M & 6 ? J(
          c,
          A,
          g,
          b,
          v,
          I,
          B,
          D,
          x
        ) : M & 64 || M & 128 ? y.process(
          c,
          A,
          g,
          b,
          v,
          I,
          B,
          D,
          x,
          be
        ) : {}.NODE_ENV !== "production" && N("Invalid VNode type:", y, `(${typeof y})`);
    }
    j != null && v && Rn(j, c && c.ref, I, A || c, !A);
  }, O = (c, A, g, b) => {
    if (c == null)
      i(
        A.el = f(A.children),
        g,
        b
      );
    else {
      const v = A.el = c.el;
      A.children !== c.children && d(v, A.children);
    }
  }, k = (c, A, g, b) => {
    c == null ? i(
      A.el = u(A.children || ""),
      g,
      b
    ) : A.el = c.el;
  }, Q = (c, A, g, b) => {
    [c.el, c.anchor] = w(
      c.children,
      A,
      g,
      b,
      c.el,
      c.anchor
    );
  }, R = (c, A, g, b) => {
    if (A.children !== c.children) {
      const v = p(c.anchor);
      L(c), [A.el, A.anchor] = w(
        A.children,
        g,
        v,
        b
      );
    } else
      A.el = c.el, A.anchor = c.anchor;
  }, K = ({ el: c, anchor: A }, g, b) => {
    let v;
    for (; c && c !== A; )
      v = p(c), i(c, g, b), c = v;
    i(A, g, b);
  }, L = ({ el: c, anchor: A }) => {
    let g;
    for (; c && c !== A; )
      g = p(c), r(c), c = g;
    r(A);
  }, q = (c, A, g, b, v, I, B, D, x) => {
    A.type === "svg" ? B = "svg" : A.type === "math" && (B = "mathml"), c == null ? ae(
      A,
      g,
      b,
      v,
      I,
      B,
      D,
      x
    ) : te(
      c,
      A,
      v,
      I,
      B,
      D,
      x
    );
  }, ae = (c, A, g, b, v, I, B, D) => {
    let x, y;
    const { props: j, shapeFlag: M, transition: H, dirs: Z } = c;
    if (x = c.el = o(
      c.type,
      I,
      j && j.is,
      j
    ), M & 8 ? a(x, c.children) : M & 16 && oe(
      c.children,
      x,
      null,
      b,
      v,
      li(c, I),
      B,
      D
    ), Z && Dt(c, null, b, "created"), le(x, c, c.scopeId, B, b), j) {
      for (const se in j)
        se !== "value" && !en(se) && s(x, se, null, j[se], I, b);
      "value" in j && s(x, "value", null, j.value, I), (y = j.onVnodeBeforeMount) && Ve(y, b, c);
    }
    ({}).NODE_ENV !== "production" && (Mn(x, "__vnode", c, !0), Mn(x, "__vueParentComponent", b, !0)), Z && Dt(c, null, b, "beforeMount");
    const V = pl(v, H);
    V && H.beforeEnter(x), i(x, A, g), ((y = j && j.onVnodeMounted) || V || Z) && Le(() => {
      y && Ve(y, b, c), V && H.enter(x), Z && Dt(c, null, b, "mounted");
    }, v);
  }, le = (c, A, g, b, v) => {
    if (g && h(c, g), b)
      for (let I = 0; I < b.length; I++)
        h(c, b[I]);
    if (v) {
      let I = v.subTree;
      if ({}.NODE_ENV !== "production" && I.patchFlag > 0 && I.patchFlag & 2048 && (I = Ki(I.children) || I), A === I || Fs(I.type) && (I.ssContent === A || I.ssFallback === A)) {
        const B = v.vnode;
        le(
          c,
          B,
          B.scopeId,
          B.slotScopeIds,
          v.parent
        );
      }
    }
  }, oe = (c, A, g, b, v, I, B, D, x = 0) => {
    for (let y = x; y < c.length; y++) {
      const j = c[y] = D ? Et(c[y]) : je(c[y]);
      m(
        null,
        j,
        A,
        g,
        b,
        v,
        I,
        B,
        D
      );
    }
  }, te = (c, A, g, b, v, I, B) => {
    const D = A.el = c.el;
    ({}).NODE_ENV !== "production" && (D.__vnode = A);
    let { patchFlag: x, dynamicChildren: y, dirs: j } = A;
    x |= c.patchFlag & 16;
    const M = c.props || ee, H = A.props || ee;
    let Z;
    if (g && Nt(g, !1), (Z = H.onVnodeBeforeUpdate) && Ve(Z, g, A, c), j && Dt(A, c, g, "beforeUpdate"), g && Nt(g, !0), {}.NODE_ENV !== "production" && $e && (x = 0, B = !1, y = null), (M.innerHTML && H.innerHTML == null || M.textContent && H.textContent == null) && a(D, ""), y ? (F(
      c.dynamicChildren,
      y,
      D,
      g,
      b,
      li(A, v),
      I
    ), {}.NODE_ENV !== "production" && Cn(c, A)) : B || T(
      c,
      A,
      D,
      null,
      g,
      b,
      li(A, v),
      I,
      !1
    ), x > 0) {
      if (x & 16)
        Ee(D, M, H, g, v);
      else if (x & 2 && M.class !== H.class && s(D, "class", null, H.class, v), x & 4 && s(D, "style", M.style, H.style, v), x & 8) {
        const V = A.dynamicProps;
        for (let se = 0; se < V.length; se++) {
          const $ = V[se], Me = M[$], me = H[$];
          (me !== Me || $ === "value") && s(D, $, Me, me, v, g);
        }
      }
      x & 1 && c.children !== A.children && a(D, A.children);
    } else
      !B && y == null && Ee(D, M, H, g, v);
    ((Z = H.onVnodeUpdated) || j) && Le(() => {
      Z && Ve(Z, g, A, c), j && Dt(A, c, g, "updated");
    }, b);
  }, F = (c, A, g, b, v, I, B) => {
    for (let D = 0; D < A.length; D++) {
      const x = c[D], y = A[D], j = (
        // oldVNode may be an errored async setup() component inside Suspense
        // which will not have a mounted element
        x.el && // - In the case of a Fragment, we need to provide the actual parent
        // of the Fragment itself so it can move its children.
        (x.type === Ne || // - In the case of different nodes, there is going to be a replacement
        // which also requires the correct parent container
        !qt(x, y) || // - In the case of a component, it could contain anything.
        x.shapeFlag & 70) ? l(x.el) : (
          // In other cases, the parent container is not actually used so we
          // just pass the block element here to avoid a DOM parentNode call.
          g
        )
      );
      m(
        x,
        y,
        j,
        null,
        b,
        v,
        I,
        B,
        !0
      );
    }
  }, Ee = (c, A, g, b, v) => {
    if (A !== g) {
      if (A !== ee)
        for (const I in A)
          !en(I) && !(I in g) && s(
            c,
            I,
            A[I],
            null,
            v,
            b
          );
      for (const I in g) {
        if (en(I))
          continue;
        const B = g[I], D = A[I];
        B !== D && I !== "value" && s(c, I, D, B, v, b);
      }
      "value" in g && s(c, "value", A.value, g.value, v);
    }
  }, xt = (c, A, g, b, v, I, B, D, x) => {
    const y = A.el = c ? c.el : f(""), j = A.anchor = c ? c.anchor : f("");
    let { patchFlag: M, dynamicChildren: H, slotScopeIds: Z } = A;
    ({}).NODE_ENV !== "production" && // #5523 dev root fragment may inherit directives
    ($e || M & 2048) && (M = 0, x = !1, H = null), Z && (D = D ? D.concat(Z) : Z), c == null ? (i(y, g, b), i(j, g, b), oe(
      // #10007
      // such fragment like `<></>` will be compiled into
      // a fragment which doesn't have a children.
      // In this case fallback to an empty array
      A.children || [],
      g,
      j,
      v,
      I,
      B,
      D,
      x
    )) : M > 0 && M & 64 && H && // #2715 the previous fragment could've been a BAILed one as a result
    // of renderSlot() with no valid children
    c.dynamicChildren ? (F(
      c.dynamicChildren,
      H,
      g,
      v,
      I,
      B,
      D
    ), {}.NODE_ENV !== "production" ? Cn(c, A) : (
      // #2080 if the stable fragment has a key, it's a <template v-for> that may
      //  get moved around. Make sure all root level vnodes inherit el.
      // #2134 or if it's a component root, it may also get moved around
      // as the component is being moved.
      (A.key != null || v && A === v.subTree) && Cn(
        c,
        A,
        !0
        /* shallow */
      )
    )) : T(
      c,
      A,
      g,
      j,
      v,
      I,
      B,
      D,
      x
    );
  }, J = (c, A, g, b, v, I, B, D, x) => {
    A.slotScopeIds = D, c == null ? A.shapeFlag & 512 ? v.ctx.activate(
      A,
      g,
      b,
      B,
      x
    ) : S(
      A,
      g,
      b,
      v,
      I,
      B,
      x
    ) : C(c, A, x);
  }, S = (c, A, g, b, v, I, B) => {
    const D = c.component = kl(
      c,
      b,
      v
    );
    if ({}.NODE_ENV !== "production" && D.type.__hmrId && af(D), {}.NODE_ENV !== "production" && (bn(c), it(D, "mount")), Zi(c) && (D.ctx.renderer = be), {}.NODE_ENV !== "production" && it(D, "init"), Hl(D, !1, B), {}.NODE_ENV !== "production" && rt(D, "init"), D.asyncDep) {
      if ({}.NODE_ENV !== "production" && $e && (c.el = null), v && v.registerDep(D, P, B), !c.el) {
        const x = D.subTree = lt(He);
        k(null, x, A, g);
      }
    } else
      P(
        D,
        c,
        A,
        g,
        v,
        I,
        B
      );
    ({}).NODE_ENV !== "production" && (yn(), rt(D, "mount"));
  }, C = (c, A, g) => {
    const b = A.component = c.component;
    if (Pl(c, A, g))
      if (b.asyncDep && !b.asyncResolved) {
        ({}).NODE_ENV !== "production" && bn(A), E(b, A, g), {}.NODE_ENV !== "production" && yn();
        return;
      } else
        b.next = A, b.update();
    else
      A.el = c.el, b.vnode = A;
  }, P = (c, A, g, b, v, I, B) => {
    const D = () => {
      if (c.isMounted) {
        let { next: M, bu: H, u: Z, parent: V, vnode: se } = c;
        {
          const Se = Js(c);
          if (Se) {
            M && (M.el = se.el, E(c, M, B)), Se.asyncDep.then(() => {
              c.isUnmounted || D();
            });
            return;
          }
        }
        let $ = M, Me;
        ({}).NODE_ENV !== "production" && bn(M || c.vnode), Nt(c, !1), M ? (M.el = se.el, E(c, M, B)) : M = se, H && Wt(H), (Me = M.props && M.props.onVnodeBeforeUpdate) && Ve(Me, V, M, se), Nt(c, !0), {}.NODE_ENV !== "production" && it(c, "render");
        const me = ci(c);
        ({}).NODE_ENV !== "production" && rt(c, "render");
        const ze = c.subTree;
        c.subTree = me, {}.NODE_ENV !== "production" && it(c, "patch"), m(
          ze,
          me,
          // parent may have changed if it's in a teleport
          l(ze.el),
          // anchor may have changed if it's in a fragment
          Ge(ze),
          c,
          v,
          I
        ), {}.NODE_ENV !== "production" && rt(c, "patch"), M.el = me.el, $ === null && wl(c, me.el), Z && Le(Z, v), (Me = M.props && M.props.onVnodeUpdated) && Le(
          () => Ve(Me, V, M, se),
          v
        ), {}.NODE_ENV !== "production" && ys(c), {}.NODE_ENV !== "production" && yn();
      } else {
        let M;
        const { el: H, props: Z } = A, { bm: V, m: se, parent: $, root: Me, type: me } = c, ze = rn(A);
        if (Nt(c, !1), V && Wt(V), !ze && (M = Z && Z.onVnodeBeforeMount) && Ve(M, $, A), Nt(c, !0), H && ht) {
          const Se = () => {
            ({}).NODE_ENV !== "production" && it(c, "render"), c.subTree = ci(c), {}.NODE_ENV !== "production" && rt(c, "render"), {}.NODE_ENV !== "production" && it(c, "hydrate"), ht(
              H,
              c.subTree,
              c,
              v,
              null
            ), {}.NODE_ENV !== "production" && rt(c, "hydrate");
          };
          ze && me.__asyncHydrate ? me.__asyncHydrate(
            H,
            c,
            Se
          ) : Se();
        } else {
          Me.ce && Me.ce._injectChildStyle(me), {}.NODE_ENV !== "production" && it(c, "render");
          const Se = c.subTree = ci(c);
          ({}).NODE_ENV !== "production" && rt(c, "render"), {}.NODE_ENV !== "production" && it(c, "patch"), m(
            null,
            Se,
            g,
            b,
            c,
            v,
            I
          ), {}.NODE_ENV !== "production" && rt(c, "patch"), A.el = Se.el;
        }
        if (se && Le(se, v), !ze && (M = Z && Z.onVnodeMounted)) {
          const Se = A;
          Le(
            () => Ve(M, $, Se),
            v
          );
        }
        (A.shapeFlag & 256 || $ && rn($.vnode) && $.vnode.shapeFlag & 256) && c.a && Le(c.a, v), c.isMounted = !0, {}.NODE_ENV !== "production" && Ef(c), A = g = b = null;
      }
    };
    c.scope.on();
    const x = c.effect = new Kr(D);
    c.scope.off();
    const y = c.update = x.run.bind(x), j = c.job = x.runIfDirty.bind(x);
    j.i = c, j.id = c.uid, x.scheduler = () => Kn(j), Nt(c, !0), {}.NODE_ENV !== "production" && (x.onTrack = c.rtc ? (M) => Wt(c.rtc, M) : void 0, x.onTrigger = c.rtg ? (M) => Wt(c.rtg, M) : void 0), y();
  }, E = (c, A, g) => {
    A.component = c;
    const b = c.vnode.props;
    c.vnode = A, c.next = null, el(c, A.props, b, g), ul(c, A.children, g), at(), lr(c), dt();
  }, T = (c, A, g, b, v, I, B, D, x = !1) => {
    const y = c && c.children, j = c ? c.shapeFlag : 0, M = A.children, { patchFlag: H, shapeFlag: Z } = A;
    if (H > 0) {
      if (H & 128) {
        U(
          y,
          M,
          g,
          b,
          v,
          I,
          B,
          D,
          x
        );
        return;
      } else if (H & 256) {
        Y(
          y,
          M,
          g,
          b,
          v,
          I,
          B,
          D,
          x
        );
        return;
      }
    }
    Z & 8 ? (j & 16 && Be(y, v, I), M !== y && a(g, M)) : j & 16 ? Z & 16 ? U(
      y,
      M,
      g,
      b,
      v,
      I,
      B,
      D,
      x
    ) : Be(y, v, I, !0) : (j & 8 && a(g, ""), Z & 16 && oe(
      M,
      g,
      b,
      v,
      I,
      B,
      D,
      x
    ));
  }, Y = (c, A, g, b, v, I, B, D, x) => {
    c = c || Jt, A = A || Jt;
    const y = c.length, j = A.length, M = Math.min(y, j);
    let H;
    for (H = 0; H < M; H++) {
      const Z = A[H] = x ? Et(A[H]) : je(A[H]);
      m(
        c[H],
        Z,
        g,
        null,
        v,
        I,
        B,
        D,
        x
      );
    }
    y > j ? Be(
      c,
      v,
      I,
      !0,
      !1,
      M
    ) : oe(
      A,
      g,
      b,
      v,
      I,
      B,
      D,
      x,
      M
    );
  }, U = (c, A, g, b, v, I, B, D, x) => {
    let y = 0;
    const j = A.length;
    let M = c.length - 1, H = j - 1;
    for (; y <= M && y <= H; ) {
      const Z = c[y], V = A[y] = x ? Et(A[y]) : je(A[y]);
      if (qt(Z, V))
        m(
          Z,
          V,
          g,
          null,
          v,
          I,
          B,
          D,
          x
        );
      else
        break;
      y++;
    }
    for (; y <= M && y <= H; ) {
      const Z = c[M], V = A[H] = x ? Et(A[H]) : je(A[H]);
      if (qt(Z, V))
        m(
          Z,
          V,
          g,
          null,
          v,
          I,
          B,
          D,
          x
        );
      else
        break;
      M--, H--;
    }
    if (y > M) {
      if (y <= H) {
        const Z = H + 1, V = Z < j ? A[Z].el : b;
        for (; y <= H; )
          m(
            null,
            A[y] = x ? Et(A[y]) : je(A[y]),
            g,
            V,
            v,
            I,
            B,
            D,
            x
          ), y++;
      }
    } else if (y > H)
      for (; y <= M; )
        re(c[y], v, I, !0), y++;
    else {
      const Z = y, V = y, se = /* @__PURE__ */ new Map();
      for (y = V; y <= H; y++) {
        const ye = A[y] = x ? Et(A[y]) : je(A[y]);
        ye.key != null && ({}.NODE_ENV !== "production" && se.has(ye.key) && N(
          "Duplicate keys found during update:",
          JSON.stringify(ye.key),
          "Make sure keys are unique."
        ), se.set(ye.key, y));
      }
      let $, Me = 0;
      const me = H - V + 1;
      let ze = !1, Se = 0;
      const Vt = new Array(me);
      for (y = 0; y < me; y++)
        Vt[y] = 0;
      for (y = Z; y <= M; y++) {
        const ye = c[y];
        if (Me >= me) {
          re(ye, v, I, !0);
          continue;
        }
        let Fe;
        if (ye.key != null)
          Fe = se.get(ye.key);
        else
          for ($ = V; $ <= H; $++)
            if (Vt[$ - V] === 0 && qt(ye, A[$])) {
              Fe = $;
              break;
            }
        Fe === void 0 ? re(ye, v, I, !0) : (Vt[Fe - V] = y + 1, Fe >= Se ? Se = Fe : ze = !0, m(
          ye,
          A[Fe],
          g,
          null,
          v,
          I,
          B,
          D,
          x
        ), Me++);
      }
      const tr = ze ? gl(Vt) : Jt;
      for ($ = tr.length - 1, y = me - 1; y >= 0; y--) {
        const ye = V + y, Fe = A[ye], nr = ye + 1 < j ? A[ye + 1].el : b;
        Vt[y] === 0 ? m(
          null,
          Fe,
          g,
          nr,
          v,
          I,
          B,
          D,
          x
        ) : ze && ($ < 0 || y !== tr[$] ? ne(Fe, g, nr, 2) : $--);
      }
    }
  }, ne = (c, A, g, b, v = null) => {
    const { el: I, type: B, transition: D, children: x, shapeFlag: y } = c;
    if (y & 6) {
      ne(c.component.subTree, A, g, b);
      return;
    }
    if (y & 128) {
      c.suspense.move(A, g, b);
      return;
    }
    if (y & 64) {
      B.move(c, A, g, be);
      return;
    }
    if (B === Ne) {
      i(I, A, g);
      for (let M = 0; M < x.length; M++)
        ne(x[M], A, g, b);
      i(c.anchor, A, g);
      return;
    }
    if (B === xn) {
      K(c, A, g);
      return;
    }
    if (b !== 2 && y & 1 && D)
      if (b === 0)
        D.beforeEnter(I), i(I, A, g), Le(() => D.enter(I), v);
      else {
        const { leave: M, delayLeave: H, afterLeave: Z } = D, V = () => i(I, A, g), se = () => {
          M(I, () => {
            V(), Z && Z();
          });
        };
        H ? H(I, V, se) : se();
      }
    else
      i(I, A, g);
  }, re = (c, A, g, b = !1, v = !1) => {
    const {
      type: I,
      props: B,
      ref: D,
      children: x,
      dynamicChildren: y,
      shapeFlag: j,
      patchFlag: M,
      dirs: H,
      cacheIndex: Z
    } = c;
    if (M === -2 && (v = !1), D != null && Rn(D, null, g, c, !0), Z != null && (A.renderCache[Z] = void 0), j & 256) {
      A.ctx.deactivate(c);
      return;
    }
    const V = j & 1 && H, se = !rn(c);
    let $;
    if (se && ($ = B && B.onVnodeBeforeUnmount) && Ve($, A, c), j & 6)
      gt(c.component, g, b);
    else {
      if (j & 128) {
        c.suspense.unmount(g, b);
        return;
      }
      V && Dt(c, null, A, "beforeUnmount"), j & 64 ? c.type.remove(
        c,
        A,
        g,
        be,
        b
      ) : y && // #5154
      // when v-once is used inside a block, setBlockTracking(-1) marks the
      // parent block with hasOnce: true
      // so that it doesn't take the fast path during unmount - otherwise
      // components nested in v-once are never unmounted.
      !y.hasOnce && // #1153: fast path should not be taken for non-stable (v-for) fragments
      (I !== Ne || M > 0 && M & 64) ? Be(
        y,
        A,
        g,
        !1,
        !0
      ) : (I === Ne && M & 384 || !v && j & 16) && Be(x, A, g), b && Ae(c);
    }
    (se && ($ = B && B.onVnodeUnmounted) || V) && Le(() => {
      $ && Ve($, A, c), V && Dt(c, null, A, "unmounted");
    }, g);
  }, Ae = (c) => {
    const { type: A, el: g, anchor: b, transition: v } = c;
    if (A === Ne) {
      ({}).NODE_ENV !== "production" && c.patchFlag > 0 && c.patchFlag & 2048 && v && !v.persisted ? c.children.forEach((B) => {
        B.type === He ? r(B.el) : Ae(B);
      }) : De(g, b);
      return;
    }
    if (A === xn) {
      L(c);
      return;
    }
    const I = () => {
      r(g), v && !v.persisted && v.afterLeave && v.afterLeave();
    };
    if (c.shapeFlag & 1 && v && !v.persisted) {
      const { leave: B, delayLeave: D } = v, x = () => B(g, I);
      D ? D(c.el, I, x) : x();
    } else
      I();
  }, De = (c, A) => {
    let g;
    for (; c !== A; )
      g = p(c), r(c), c = g;
    r(A);
  }, gt = (c, A, g) => {
    ({}).NODE_ENV !== "production" && c.type.__hmrId && df(c);
    const { bum: b, scope: v, job: I, subTree: B, um: D, m: x, a: y } = c;
    vr(x), vr(y), b && Wt(b), v.stop(), I && (I.flags |= 8, re(B, c, A, g)), D && Le(D, A), Le(() => {
      c.isUnmounted = !0;
    }, A), A && A.pendingBranch && !A.isUnmounted && c.asyncDep && !c.asyncResolved && c.suspenseId === A.pendingId && (A.deps--, A.deps === 0 && A.resolve()), {}.NODE_ENV !== "production" && If(c);
  }, Be = (c, A, g, b = !1, v = !1, I = 0) => {
    for (let B = I; B < c.length; B++)
      re(c[B], A, g, b, v);
  }, Ge = (c) => {
    if (c.shapeFlag & 6)
      return Ge(c.component.subTree);
    if (c.shapeFlag & 128)
      return c.suspense.next();
    const A = p(c.anchor || c.el), g = A && A[Cf];
    return g ? p(g) : A;
  };
  let Te = !1;
  const ve = (c, A, g) => {
    c == null ? A._vnode && re(A._vnode, null, null, !0) : m(
      A._vnode || null,
      c,
      A,
      null,
      null,
      null,
      g
    ), A._vnode = c, Te || (Te = !0, lr(), Es(), Te = !1);
  }, be = {
    p: m,
    um: re,
    m: ne,
    r: Ae,
    mt: S,
    mc: oe,
    pc: T,
    pbc: F,
    n: Ge,
    o: e
  };
  let tt, ht;
  return t && ([tt, ht] = t(
    be
  )), {
    render: ve,
    hydrate: tt,
    createApp: Uf(ve, tt)
  };
}
function li({ type: e, props: t }, n) {
  return n === "svg" && e === "foreignObject" || n === "mathml" && e === "annotation-xml" && t && t.encoding && t.encoding.includes("html") ? void 0 : n;
}
function Nt({ effect: e, job: t }, n) {
  n ? (e.flags |= 32, t.flags |= 4) : (e.flags &= -33, t.flags &= -5);
}
function pl(e, t) {
  return (!e || e && !e.pendingBranch) && t && !t.persisted;
}
function Cn(e, t, n = !1) {
  const i = e.children, r = t.children;
  if (z(i) && z(r))
    for (let s = 0; s < i.length; s++) {
      const o = i[s];
      let f = r[s];
      f.shapeFlag & 1 && !f.dynamicChildren && ((f.patchFlag <= 0 || f.patchFlag === 32) && (f = r[s] = Et(r[s]), f.el = o.el), !n && f.patchFlag !== -2 && Cn(o, f)), f.type === hn && (f.el = o.el), {}.NODE_ENV !== "production" && f.type === He && !f.el && (f.el = o.el);
    }
}
function gl(e) {
  const t = e.slice(), n = [0];
  let i, r, s, o, f;
  const u = e.length;
  for (i = 0; i < u; i++) {
    const d = e[i];
    if (d !== 0) {
      if (r = n[n.length - 1], e[r] < d) {
        t[i] = r, n.push(i);
        continue;
      }
      for (s = 0, o = n.length - 1; s < o; )
        f = s + o >> 1, e[n[f]] < d ? s = f + 1 : o = f;
      d < e[n[s]] && (s > 0 && (t[i] = n[s - 1]), n[s] = i);
    }
  }
  for (s = n.length, o = n[s - 1]; s-- > 0; )
    n[s] = o, o = t[o];
  return n;
}
function Js(e) {
  const t = e.subTree.component;
  if (t)
    return t.asyncDep && !t.asyncResolved ? t : Js(t);
}
function vr(e) {
  if (e)
    for (let t = 0; t < e.length; t++)
      e[t].flags |= 8;
}
const hl = Symbol.for("v-scx"), vl = () => {
  {
    const e = wn(hl);
    return e || {}.NODE_ENV !== "production" && N(
      "Server rendering context not provided. Make sure to only call useSSRContext() conditionally in the server build."
    ), e;
  }
};
function ft(e, t, n) {
  return {}.NODE_ENV !== "production" && !X(t) && N(
    "`watch(fn, options?)` signature has been moved to a separate API. Use `watchEffect(fn, options?)` instead. `watch` now only supports `watch(source, cb, options?) signature."
  ), Qs(e, t, n);
}
function Qs(e, t, n = ee) {
  const { immediate: i, deep: r, flush: s, once: o } = n;
  ({}).NODE_ENV !== "production" && !t && (i !== void 0 && N(
    'watch() "immediate" option is only respected when using the watch(source, callback, options?) signature.'
  ), r !== void 0 && N(
    'watch() "deep" option is only respected when using the watch(source, callback, options?) signature.'
  ), o !== void 0 && N(
    'watch() "once" option is only respected when using the watch(source, callback, options?) signature.'
  ));
  const f = ue({}, n);
  ({}).NODE_ENV !== "production" && (f.onWarn = N);
  const u = t && i || !t && s !== "post";
  let d;
  if (An) {
    if (s === "sync") {
      const h = vl();
      d = h.__watcherHandles || (h.__watcherHandles = []);
    } else if (!u) {
      const h = () => {
      };
      return h.stop = pe, h.resume = pe, h.pause = pe, h;
    }
  }
  const a = he;
  f.call = (h, w, m) => et(h, a, w, m);
  let l = !1;
  s === "post" ? f.scheduler = (h) => {
    Le(h, a && a.suspense);
  } : s !== "sync" && (l = !0, f.scheduler = (h, w) => {
    w ? h() : Kn(h);
  }), f.augmentJob = (h) => {
    t && (h.flags |= 4), l && (h.flags |= 2, a && (h.id = a.uid, h.i = a));
  };
  const p = rf(e, t, f);
  return An && (d ? d.push(p) : u && p()), p;
}
function El(e, t, n) {
  const i = this.proxy, r = fe(e) ? e.includes(".") ? Ys(i, e) : () => i[e] : e.bind(i, i);
  let s;
  X(t) ? s = t : (s = t.handler, n = t);
  const o = vn(this), f = Qs(r, s.bind(i), n);
  return o(), f;
}
function Ys(e, t) {
  const n = t.split(".");
  return () => {
    let i = e;
    for (let r = 0; r < n.length && i; r++)
      i = i[n[r]];
    return i;
  };
}
const ml = (e, t) => t === "modelValue" || t === "model-value" ? e.modelModifiers : e[`${t}Modifiers`] || e[`${Je(t)}Modifiers`] || e[`${Pt(t)}Modifiers`];
function Il(e, t, ...n) {
  if (e.isUnmounted)
    return;
  const i = e.vnode.props || ee;
  if ({}.NODE_ENV !== "production") {
    const {
      emitsOptions: a,
      propsOptions: [l]
    } = e;
    if (a)
      if (!(t in a))
        (!l || !(Bt(Je(t)) in l)) && N(
          `Component emitted event "${t}" but it is neither declared in the emits option nor as an "${Bt(Je(t))}" prop.`
        );
      else {
        const p = a[t];
        X(p) && (p(...n) || N(
          `Invalid event arguments: event validation failed for event "${t}".`
        ));
      }
  }
  let r = n;
  const s = t.startsWith("update:"), o = s && ml(i, t.slice(7));
  if (o && (o.trim && (r = n.map((a) => fe(a) ? a.trim() : a)), o.number && (r = n.map(vo))), {}.NODE_ENV !== "production" && Pf(e, t, r), {}.NODE_ENV !== "production") {
    const a = t.toLowerCase();
    a !== t && i[Bt(a)] && N(
      `Event "${a}" is emitted in component ${ei(
        e,
        e.type
      )} but the handler is registered for "${t}". Note that HTML attributes are case-insensitive and you cannot use v-on to listen to camelCase events when using in-DOM templates. You should probably use "${Pt(
        t
      )}" instead of "${t}".`
    );
  }
  let f, u = i[f = Bt(t)] || // also try camelCase event handler (#2249)
  i[f = Bt(Je(t))];
  !u && s && (u = i[f = Bt(Pt(t))]), u && et(
    u,
    e,
    6,
    r
  );
  const d = i[f + "Once"];
  if (d) {
    if (!e.emitted)
      e.emitted = {};
    else if (e.emitted[f])
      return;
    e.emitted[f] = !0, et(
      d,
      e,
      6,
      r
    );
  }
}
function Zs(e, t, n = !1) {
  const i = t.emitsCache, r = i.get(e);
  if (r !== void 0)
    return r;
  const s = e.emits;
  let o = {}, f = !1;
  if (!X(e)) {
    const u = (d) => {
      const a = Zs(d, t, !0);
      a && (f = !0, ue(o, a));
    };
    !n && t.mixins.length && t.mixins.forEach(u), e.extends && u(e.extends), e.mixins && e.mixins.forEach(u);
  }
  return !s && !f ? (ie(e) && i.set(e, null), null) : (z(s) ? s.forEach((u) => o[u] = null) : ue(o, s), ie(e) && i.set(e, o), o);
}
function _n(e, t) {
  return !e || !an(t) ? !1 : (t = t.slice(2).replace(/Once$/, ""), W(e, t[0].toLowerCase() + t.slice(1)) || W(e, Pt(t)) || W(e, t));
}
let wi = !1;
function jn() {
  wi = !0;
}
function ci(e) {
  const {
    type: t,
    vnode: n,
    proxy: i,
    withProxy: r,
    propsOptions: [s],
    slots: o,
    attrs: f,
    emit: u,
    render: d,
    renderCache: a,
    props: l,
    data: p,
    setupState: h,
    ctx: w,
    inheritAttrs: m
  } = e, O = Hn(e);
  let k, Q;
  ({}).NODE_ENV !== "production" && (wi = !1);
  try {
    if (n.shapeFlag & 4) {
      const L = r || i, q = {}.NODE_ENV !== "production" && h.__isScriptSetup ? new Proxy(L, {
        get(ae, le, oe) {
          return N(
            `Property '${String(
              le
            )}' was accessed via 'this'. Avoid using 'this' in templates.`
          ), Reflect.get(ae, le, oe);
        }
      }) : L;
      k = je(
        d.call(
          q,
          L,
          a,
          {}.NODE_ENV !== "production" ? _e(l) : l,
          h,
          p,
          w
        )
      ), Q = f;
    } else {
      const L = t;
      ({}).NODE_ENV !== "production" && f === l && jn(), k = je(
        L.length > 1 ? L(
          {}.NODE_ENV !== "production" ? _e(l) : l,
          {}.NODE_ENV !== "production" ? {
            get attrs() {
              return jn(), _e(f);
            },
            slots: o,
            emit: u
          } : { attrs: f, slots: o, emit: u }
        ) : L(
          {}.NODE_ENV !== "production" ? _e(l) : l,
          null
        )
      ), Q = t.props ? f : bl(f);
    }
  } catch (L) {
    sn.length = 0, pn(L, e, 1), k = lt(He);
  }
  let R = k, K;
  if ({}.NODE_ENV !== "production" && k.patchFlag > 0 && k.patchFlag & 2048 && ([R, K] = Gs(k)), Q && m !== !1) {
    const L = Object.keys(Q), { shapeFlag: q } = R;
    if (L.length) {
      if (q & 7)
        s && L.some(Bn) && (Q = yl(
          Q,
          s
        )), R = wt(R, Q, !1, !0);
      else if ({}.NODE_ENV !== "production" && !wi && R.type !== He) {
        const ae = Object.keys(f), le = [], oe = [];
        for (let te = 0, F = ae.length; te < F; te++) {
          const Ee = ae[te];
          an(Ee) ? Bn(Ee) || le.push(Ee[2].toLowerCase() + Ee.slice(3)) : oe.push(Ee);
        }
        oe.length && N(
          `Extraneous non-props attributes (${oe.join(", ")}) were passed to component but could not be automatically inherited because component renders fragment or text or teleport root nodes.`
        ), le.length && N(
          `Extraneous non-emits event listeners (${le.join(", ")}) were passed to component but could not be automatically inherited because component renders fragment or text root nodes. If the listener is intended to be a component custom event listener only, declare it using the "emits" option.`
        );
      }
    }
  }
  return n.dirs && ({}.NODE_ENV !== "production" && !Er(R) && N(
    "Runtime directive used on component with non-element root node. The directives will not function as intended."
  ), R = wt(R, null, !1, !0), R.dirs = R.dirs ? R.dirs.concat(n.dirs) : n.dirs), n.transition && ({}.NODE_ENV !== "production" && !Er(R) && N(
    "Component inside <Transition> renders non-element root node that cannot be animated."
  ), Yi(R, n.transition)), {}.NODE_ENV !== "production" && K ? K(R) : k = R, Hn(O), k;
}
const Gs = (e) => {
  const t = e.children, n = e.dynamicChildren, i = Ki(t, !1);
  if (i) {
    if ({}.NODE_ENV !== "production" && i.patchFlag > 0 && i.patchFlag & 2048)
      return Gs(i);
  } else
    return [e, void 0];
  const r = t.indexOf(i), s = n ? n.indexOf(i) : -1, o = (f) => {
    t[r] = f, n && (s > -1 ? n[s] = f : f.patchFlag > 0 && (e.dynamicChildren = [...n, f]));
  };
  return [je(i), o];
};
function Ki(e, t = !0) {
  let n;
  for (let i = 0; i < e.length; i++) {
    const r = e[i];
    if ($n(r)) {
      if (r.type !== He || r.children === "v-if") {
        if (n)
          return;
        if (n = r, {}.NODE_ENV !== "production" && t && n.patchFlag > 0 && n.patchFlag & 2048)
          return Ki(n.children);
      }
    } else
      return;
  }
  return n;
}
const bl = (e) => {
  let t;
  for (const n in e)
    (n === "class" || n === "style" || an(n)) && ((t || (t = {}))[n] = e[n]);
  return t;
}, yl = (e, t) => {
  const n = {};
  for (const i in e)
    (!Bn(i) || !(i.slice(9) in t)) && (n[i] = e[i]);
  return n;
}, Er = (e) => e.shapeFlag & 7 || e.type === He;
function Pl(e, t, n) {
  const { props: i, children: r, component: s } = e, { props: o, children: f, patchFlag: u } = t, d = s.emitsOptions;
  if ({}.NODE_ENV !== "production" && (r || f) && $e || t.dirs || t.transition)
    return !0;
  if (n && u >= 0) {
    if (u & 1024)
      return !0;
    if (u & 16)
      return i ? mr(i, o, d) : !!o;
    if (u & 8) {
      const a = t.dynamicProps;
      for (let l = 0; l < a.length; l++) {
        const p = a[l];
        if (o[p] !== i[p] && !_n(d, p))
          return !0;
      }
    }
  } else
    return (r || f) && (!f || !f.$stable) ? !0 : i === o ? !1 : i ? o ? mr(i, o, d) : !0 : !!o;
  return !1;
}
function mr(e, t, n) {
  const i = Object.keys(t);
  if (i.length !== Object.keys(e).length)
    return !0;
  for (let r = 0; r < i.length; r++) {
    const s = i[r];
    if (t[s] !== e[s] && !_n(n, s))
      return !0;
  }
  return !1;
}
function wl({ vnode: e, parent: t }, n) {
  for (; t; ) {
    const i = t.subTree;
    if (i.suspense && i.suspense.activeBranch === e && (i.el = e.el), i === e)
      (e = t.vnode).el = n, t = t.parent;
    else
      break;
  }
}
const Fs = (e) => e.__isSuspense;
function Cl(e, t) {
  t && t.pendingBranch ? z(e) ? t.effects.push(...e) : t.effects.push(e) : vs(e);
}
const Ne = Symbol.for("v-fgt"), hn = Symbol.for("v-txt"), He = Symbol.for("v-cmt"), xn = Symbol.for("v-stc"), sn = [];
let Oe = null;
function ge(e = !1) {
  sn.push(Oe = e ? null : []);
}
function xl() {
  sn.pop(), Oe = sn[sn.length - 1] || null;
}
let un = 1;
function Ir(e, t = !1) {
  un += e, e < 0 && Oe && t && (Oe.hasOnce = !0);
}
function Vs(e) {
  return e.dynamicChildren = un > 0 ? Oe || Jt : null, xl(), un > 0 && Oe && Oe.push(e), e;
}
function Re(e, t, n, i, r, s) {
  return Vs(
    Ke(
      e,
      t,
      n,
      i,
      r,
      s,
      !0
    )
  );
}
function Zt(e, t, n, i, r) {
  return Vs(
    lt(
      e,
      t,
      n,
      i,
      r,
      !0
    )
  );
}
function $n(e) {
  return e ? e.__v_isVNode === !0 : !1;
}
function qt(e, t) {
  if ({}.NODE_ENV !== "production" && t.shapeFlag & 6 && e.component) {
    const n = Pn.get(t.type);
    if (n && n.has(e.component))
      return e.shapeFlag &= -257, t.shapeFlag &= -513, !1;
  }
  return e.type === t.type && e.key === t.key;
}
const Dl = (...e) => Ks(
  ...e
), Ws = ({ key: e }) => e ?? null, Dn = ({
  ref: e,
  ref_key: t,
  ref_for: n
}) => (typeof e == "number" && (e = "" + e), e != null ? fe(e) || ce(e) || X(e) ? { i: ke, r: e, k: t, f: !!n } : e : null);
function Ke(e, t = null, n = null, i = 0, r = null, s = e === Ne ? 0 : 1, o = !1, f = !1) {
  const u = {
    __v_isVNode: !0,
    __v_skip: !0,
    type: e,
    props: t,
    key: t && Ws(t),
    ref: t && Dn(t),
    scopeId: ws,
    slotScopeIds: null,
    children: n,
    component: null,
    suspense: null,
    ssContent: null,
    ssFallback: null,
    dirs: null,
    transition: null,
    el: null,
    anchor: null,
    target: null,
    targetStart: null,
    targetAnchor: null,
    staticCount: 0,
    shapeFlag: s,
    patchFlag: i,
    dynamicProps: r,
    dynamicChildren: null,
    appContext: null,
    ctx: ke
  };
  return f ? (Ui(u, n), s & 128 && e.normalize(u)) : n && (u.shapeFlag |= fe(n) ? 8 : 16), {}.NODE_ENV !== "production" && u.key !== u.key && N("VNode created with invalid key (NaN). VNode type:", u.type), un > 0 && // avoid a block node from tracking itself
  !o && // has current parent block
  Oe && // presence of a patch flag indicates this node needs patching on updates.
  // component nodes also should always be patched, because even if the
  // component doesn't need to update, it needs to persist the instance on to
  // the next vnode so that it can be properly unmounted later.
  (u.patchFlag > 0 || s & 6) && // the EVENTS flag is only for hydration and if it is the only flag, the
  // vnode should not be considered dynamic due to handler caching.
  u.patchFlag !== 32 && Oe.push(u), u;
}
const lt = {}.NODE_ENV !== "production" ? Dl : Ks;
function Ks(e, t = null, n = null, i = 0, r = null, s = !1) {
  if ((!e || e === jf) && ({}.NODE_ENV !== "production" && !e && N(`Invalid vnode type when creating vnode: ${e}.`), e = He), $n(e)) {
    const f = wt(
      e,
      t,
      !0
      /* mergeRef: true */
    );
    return n && Ui(f, n), un > 0 && !s && Oe && (f.shapeFlag & 6 ? Oe[Oe.indexOf(e)] = f : Oe.push(f)), f.patchFlag = -2, f;
  }
  if (eo(e) && (e = e.__vccOpts), t) {
    t = Nl(t);
    let { class: f, style: u } = t;
    f && !fe(f) && (t.class = ot(f)), ie(u) && (Sn(u) && !z(u) && (u = ue({}, u)), t.style = Gt(u));
  }
  const o = fe(e) ? 1 : Fs(e) ? 128 : xf(e) ? 64 : ie(e) ? 4 : X(e) ? 2 : 0;
  return {}.NODE_ENV !== "production" && o & 4 && Sn(e) && (e = G(e), N(
    "Vue received a Component that was made a reactive object. This can lead to unnecessary performance overhead and should be avoided by marking the component with `markRaw` or using `shallowRef` instead of `ref`.",
    `
Component that was made reactive: `,
    e
  )), Ke(
    e,
    t,
    n,
    i,
    r,
    o,
    s,
    !0
  );
}
function Nl(e) {
  return e ? Sn(e) || Os(e) ? ue({}, e) : e : null;
}
function wt(e, t, n = !1, i = !1) {
  const { props: r, ref: s, patchFlag: o, children: f, transition: u } = e, d = t ? Ml(r || {}, t) : r, a = {
    __v_isVNode: !0,
    __v_skip: !0,
    type: e.type,
    props: d,
    key: d && Ws(d),
    ref: t && t.ref ? (
      // #2078 in the case of <component :is="vnode" ref="extra"/>
      // if the vnode itself already has a ref, cloneVNode will need to merge
      // the refs so the single vnode can be set on multiple refs
      n && s ? z(s) ? s.concat(Dn(t)) : [s, Dn(t)] : Dn(t)
    ) : s,
    scopeId: e.scopeId,
    slotScopeIds: e.slotScopeIds,
    children: {}.NODE_ENV !== "production" && o === -1 && z(f) ? f.map(Us) : f,
    target: e.target,
    targetStart: e.targetStart,
    targetAnchor: e.targetAnchor,
    staticCount: e.staticCount,
    shapeFlag: e.shapeFlag,
    // if the vnode is cloned with extra props, we can no longer assume its
    // existing patch flag to be reliable and need to add the FULL_PROPS flag.
    // note: preserve flag for fragments since they use the flag for children
    // fast paths only.
    patchFlag: t && e.type !== Ne ? o === -1 ? 16 : o | 16 : o,
    dynamicProps: e.dynamicProps,
    dynamicChildren: e.dynamicChildren,
    appContext: e.appContext,
    dirs: e.dirs,
    transition: u,
    // These should technically only be non-null on mounted VNodes. However,
    // they *should* be copied for kept-alive vnodes. So we just always copy
    // them since them being non-null during a mount doesn't affect the logic as
    // they will simply be overwritten.
    component: e.component,
    suspense: e.suspense,
    ssContent: e.ssContent && wt(e.ssContent),
    ssFallback: e.ssFallback && wt(e.ssFallback),
    el: e.el,
    anchor: e.anchor,
    ctx: e.ctx,
    ce: e.ce
  };
  return u && i && Yi(
    a,
    u.clone(a)
  ), a;
}
function Us(e) {
  const t = wt(e);
  return z(e.children) && (t.children = e.children.map(Us)), t;
}
function Bl(e = " ", t = 0) {
  return lt(hn, null, e, t);
}
function br(e = "", t = !1) {
  return t ? (ge(), Zt(He, null, e)) : lt(He, null, e);
}
function je(e) {
  return e == null || typeof e == "boolean" ? lt(He) : z(e) ? lt(
    Ne,
    null,
    // #3666, avoid reference pollution when reusing vnode
    e.slice()
  ) : $n(e) ? Et(e) : lt(hn, null, String(e));
}
function Et(e) {
  return e.el === null && e.patchFlag !== -1 || e.memo ? e : wt(e);
}
function Ui(e, t) {
  let n = 0;
  const { shapeFlag: i } = e;
  if (t == null)
    t = null;
  else if (z(t))
    n = 16;
  else if (typeof t == "object")
    if (i & 65) {
      const r = t.default;
      r && (r._c && (r._d = !1), Ui(e, r()), r._c && (r._d = !0));
      return;
    } else {
      n = 32;
      const r = t._;
      !r && !Os(t) ? t._ctx = ke : r === 3 && ke && (ke.slots._ === 1 ? t._ = 1 : (t._ = 2, e.patchFlag |= 1024));
    }
  else
    X(t) ? (t = { default: t, _ctx: ke }, n = 32) : (t = String(t), i & 64 ? (n = 16, t = [Bl(t)]) : n = 8);
  e.children = t, e.shapeFlag |= n;
}
function Ml(...e) {
  const t = {};
  for (let n = 0; n < e.length; n++) {
    const i = e[n];
    for (const r in i)
      if (r === "class")
        t.class !== i.class && (t.class = ot([t.class, i.class]));
      else if (r === "style")
        t.style = Gt([t.style, i.style]);
      else if (an(r)) {
        const s = t[r], o = i[r];
        o && s !== o && !(z(s) && s.includes(o)) && (t[r] = s ? [].concat(s, o) : o);
      } else
        r !== "" && (t[r] = i[r]);
  }
  return t;
}
function Ve(e, t, n, i = null) {
  et(e, t, 7, [
    n,
    i
  ]);
}
const Sl = Ss();
let Ll = 0;
function kl(e, t, n) {
  const i = e.type, r = (t ? t.appContext : e.appContext) || Sl, s = {
    uid: Ll++,
    vnode: e,
    type: i,
    parent: t,
    appContext: r,
    root: null,
    // to be immediately set
    next: null,
    subTree: null,
    // will be set synchronously right after creation
    effect: null,
    update: null,
    // will be set synchronously right after creation
    job: null,
    scope: new Mo(
      !0
      /* detached */
    ),
    render: null,
    proxy: null,
    exposed: null,
    exposeProxy: null,
    withProxy: null,
    provides: t ? t.provides : Object.create(r.provides),
    ids: t ? t.ids : ["", 0, 0],
    accessCache: null,
    renderCache: [],
    // local resolved assets
    components: null,
    directives: null,
    // resolved props and emits options
    propsOptions: Rs(i, r),
    emitsOptions: Zs(i, r),
    // emit
    emit: null,
    // to be set immediately
    emitted: null,
    // props default value
    propsDefaults: ee,
    // inheritAttrs
    inheritAttrs: i.inheritAttrs,
    // state
    ctx: ee,
    data: ee,
    props: ee,
    attrs: ee,
    slots: ee,
    refs: ee,
    setupState: ee,
    setupContext: null,
    // suspense related
    suspense: n,
    suspenseId: n ? n.pendingId : 0,
    asyncDep: null,
    asyncResolved: !1,
    // lifecycle hooks
    // not using enums here because it results in computed properties
    isMounted: !1,
    isUnmounted: !1,
    isDeactivated: !1,
    bc: null,
    c: null,
    bm: null,
    m: null,
    bu: null,
    u: null,
    um: null,
    bum: null,
    da: null,
    a: null,
    rtg: null,
    rtc: null,
    ec: null,
    sp: null
  };
  return {}.NODE_ENV !== "production" ? s.ctx = Xf(s) : s.ctx = { _: s }, s.root = t ? t.root : s, s.emit = Il.bind(null, s), e.ce && e.ce(s), s;
}
let he = null;
const qi = () => he || ke;
let Xn, Ci;
{
  const e = dn(), t = (n, i) => {
    let r;
    return (r = e[n]) || (r = e[n] = []), r.push(i), (s) => {
      r.length > 1 ? r.forEach((o) => o(s)) : r[0](s);
    };
  };
  Xn = t(
    "__VUE_INSTANCE_SETTERS__",
    (n) => he = n
  ), Ci = t(
    "__VUE_SSR_SETTERS__",
    (n) => An = n
  );
}
const vn = (e) => {
  const t = he;
  return Xn(e), e.scope.on(), () => {
    e.scope.off(), Xn(t);
  };
}, yr = () => {
  he && he.scope.off(), Xn(null);
}, Ol = /* @__PURE__ */ At("slot,component");
function xi(e, { isNativeTag: t }) {
  (Ol(e) || t(e)) && N(
    "Do not use built-in or reserved HTML elements as component id: " + e
  );
}
function qs(e) {
  return e.vnode.shapeFlag & 4;
}
let An = !1;
function Hl(e, t = !1, n = !1) {
  t && Ci(t);
  const { props: i, children: r } = e.vnode, s = qs(e);
  _f(e, i, s, t), cl(e, r, n);
  const o = s ? Rl(e, t) : void 0;
  return t && Ci(!1), o;
}
function Rl(e, t) {
  var n;
  const i = e.type;
  if ({}.NODE_ENV !== "production") {
    if (i.name && xi(i.name, e.appContext.config), i.components) {
      const s = Object.keys(i.components);
      for (let o = 0; o < s.length; o++)
        xi(s[o], e.appContext.config);
    }
    if (i.directives) {
      const s = Object.keys(i.directives);
      for (let o = 0; o < s.length; o++)
        Cs(s[o]);
    }
    i.compilerOptions && Tl() && N(
      '"compilerOptions" is only supported when using a build of Vue that includes the runtime compiler. Since you are using a runtime-only build, the options should be passed via your build tool config instead.'
    );
  }
  e.accessCache = /* @__PURE__ */ Object.create(null), e.proxy = new Proxy(e.ctx, Bs), {}.NODE_ENV !== "production" && Jf(e);
  const { setup: r } = i;
  if (r) {
    at();
    const s = e.setupContext = r.length > 1 ? jl(e) : null, o = vn(e), f = Ft(
      r,
      e,
      0,
      [
        {}.NODE_ENV !== "production" ? _e(e.props) : e.props,
        s
      ]
    ), u = Li(f);
    if (dt(), o(), (u || e.sp) && !rn(e) && xs(e), u) {
      if (f.then(yr, yr), t)
        return f.then((d) => {
          Pr(e, d, t);
        }).catch((d) => {
          pn(d, e, 0);
        });
      if (e.asyncDep = f, {}.NODE_ENV !== "production" && !e.suspense) {
        const d = (n = i.name) != null ? n : "Anonymous";
        N(
          `Component <${d}>: setup function returned a promise, but no <Suspense> boundary was found in the parent component tree. A component with async setup() must be nested in a <Suspense> in order to be rendered.`
        );
      }
    } else
      Pr(e, f, t);
  } else
    _s(e, t);
}
function Pr(e, t, n) {
  X(t) ? e.type.__ssrInlineRender ? e.ssrRender = t : e.render = t : ie(t) ? ({}.NODE_ENV !== "production" && $n(t) && N(
    "setup() should not return VNodes directly - return a render function instead."
  ), {}.NODE_ENV !== "production" && (e.devtoolsRawSetupState = t), e.setupState = ds(t), {}.NODE_ENV !== "production" && Qf(e)) : {}.NODE_ENV !== "production" && t !== void 0 && N(
    `setup() should return an object. Received: ${t === null ? "null" : typeof t}`
  ), _s(e, n);
}
let Di;
const Tl = () => !Di;
function _s(e, t, n) {
  const i = e.type;
  if (!e.render) {
    if (!t && Di && !i.render) {
      const r = i.template || Vi(e).template;
      if (r) {
        ({}).NODE_ENV !== "production" && it(e, "compile");
        const { isCustomElement: s, compilerOptions: o } = e.appContext.config, { delimiters: f, compilerOptions: u } = i, d = ue(
          ue(
            {
              isCustomElement: s,
              delimiters: f
            },
            o
          ),
          u
        );
        i.render = Di(r, d), {}.NODE_ENV !== "production" && rt(e, "compile");
      }
    }
    e.render = i.render || pe;
  }
  {
    const r = vn(e);
    at();
    try {
      Zf(e);
    } finally {
      dt(), r();
    }
  }
  ({}).NODE_ENV !== "production" && !i.render && e.render === pe && !t && (i.template ? N(
    'Component provided template option but runtime compilation is not supported in this build of Vue. Configure your bundler to alias "vue" to "vue/dist/vue.esm-bundler.js".'
  ) : N("Component is missing template or render function: ", i));
}
const wr = {}.NODE_ENV !== "production" ? {
  get(e, t) {
    return jn(), de(e, "get", ""), e[t];
  },
  set() {
    return N("setupContext.attrs is readonly."), !1;
  },
  deleteProperty() {
    return N("setupContext.attrs is readonly."), !1;
  }
} : {
  get(e, t) {
    return de(e, "get", ""), e[t];
  }
};
function zl(e) {
  return new Proxy(e.slots, {
    get(t, n) {
      return de(e, "get", "$slots"), t[n];
    }
  });
}
function jl(e) {
  const t = (n) => {
    if ({}.NODE_ENV !== "production" && (e.exposed && N("expose() should be called only once per setup()."), n != null)) {
      let i = typeof n;
      i === "object" && (z(n) ? i = "array" : ce(n) && (i = "ref")), i !== "object" && N(
        `expose() should be passed a plain object, received ${i}.`
      );
    }
    e.exposed = n || {};
  };
  if ({}.NODE_ENV !== "production") {
    let n, i;
    return Object.freeze({
      get attrs() {
        return n || (n = new Proxy(e.attrs, wr));
      },
      get slots() {
        return i || (i = zl(e));
      },
      get emit() {
        return (r, ...s) => e.emit(r, ...s);
      },
      expose: t
    });
  } else
    return {
      attrs: new Proxy(e.attrs, wr),
      slots: e.slots,
      emit: e.emit,
      expose: t
    };
}
function _i(e) {
  return e.exposed ? e.exposeProxy || (e.exposeProxy = new Proxy(ds(qo(e.exposed)), {
    get(t, n) {
      if (n in t)
        return t[n];
      if (n in Tt)
        return Tt[n](e);
    },
    has(t, n) {
      return n in t || n in Tt;
    }
  })) : e.proxy;
}
const Xl = /(?:^|[-_])(\w)/g, Jl = (e) => e.replace(Xl, (t) => t.toUpperCase()).replace(/[-_]/g, "");
function $s(e, t = !0) {
  return X(e) ? e.displayName || e.name : e.name || t && e.__name;
}
function ei(e, t, n = !1) {
  let i = $s(t);
  if (!i && t.__file) {
    const r = t.__file.match(/([^/\\]+)\.\w+$/);
    r && (i = r[1]);
  }
  if (!i && e && e.parent) {
    const r = (s) => {
      for (const o in s)
        if (s[o] === t)
          return o;
    };
    i = r(
      e.components || e.parent.type.components
    ) || r(e.appContext.components);
  }
  return i ? Jl(i) : n ? "App" : "Anonymous";
}
function eo(e) {
  return X(e) && "__vccOpts" in e;
}
const Xe = (e, t) => {
  const n = tf(e, t, An);
  if ({}.NODE_ENV !== "production") {
    const i = qi();
    i && i.appContext.config.warnRecursiveComputed && (n._warnRecursive = !0);
  }
  return n;
};
function Ql() {
  if ({}.NODE_ENV === "production" || typeof window > "u")
    return;
  const e = { style: "color:#3ba776" }, t = { style: "color:#1677ff" }, n = { style: "color:#f5222d" }, i = { style: "color:#eb2f96" }, r = {
    __vue_custom_formatter: !0,
    header(l) {
      return ie(l) ? l.__isVue ? ["div", e, "VueInstance"] : ce(l) ? [
        "div",
        {},
        ["span", e, a(l)],
        "<",
        // avoid debugger accessing value affecting behavior
        f("_value" in l ? l._value : l),
        ">"
      ] : Ht(l) ? [
        "div",
        {},
        ["span", e, xe(l) ? "ShallowReactive" : "Reactive"],
        "<",
        f(l),
        `>${ut(l) ? " (readonly)" : ""}`
      ] : ut(l) ? [
        "div",
        {},
        ["span", e, xe(l) ? "ShallowReadonly" : "Readonly"],
        "<",
        f(l),
        ">"
      ] : null : null;
    },
    hasBody(l) {
      return l && l.__isVue;
    },
    body(l) {
      if (l && l.__isVue)
        return [
          "div",
          {},
          ...s(l.$)
        ];
    }
  };
  function s(l) {
    const p = [];
    l.type.props && l.props && p.push(o("props", G(l.props))), l.setupState !== ee && p.push(o("setup", l.setupState)), l.data !== ee && p.push(o("data", G(l.data)));
    const h = u(l, "computed");
    h && p.push(o("computed", h));
    const w = u(l, "inject");
    return w && p.push(o("injected", w)), p.push([
      "div",
      {},
      [
        "span",
        {
          style: i.style + ";opacity:0.66"
        },
        "$ (internal): "
      ],
      ["object", { object: l }]
    ]), p;
  }
  function o(l, p) {
    return p = ue({}, p), Object.keys(p).length ? [
      "div",
      { style: "line-height:1.25em;margin-bottom:0.6em" },
      [
        "div",
        {
          style: "color:#476582"
        },
        l
      ],
      [
        "div",
        {
          style: "padding-left:1.25em"
        },
        ...Object.keys(p).map((h) => [
          "div",
          {},
          ["span", i, h + ": "],
          f(p[h], !1)
        ])
      ]
    ] : ["span", {}];
  }
  function f(l, p = !0) {
    return typeof l == "number" ? ["span", t, l] : typeof l == "string" ? ["span", n, JSON.stringify(l)] : typeof l == "boolean" ? ["span", i, l] : ie(l) ? ["object", { object: p ? G(l) : l }] : ["span", n, String(l)];
  }
  function u(l, p) {
    const h = l.type;
    if (X(h))
      return;
    const w = {};
    for (const m in l.ctx)
      d(h, m, p) && (w[m] = l.ctx[m]);
    return w;
  }
  function d(l, p, h) {
    const w = l[h];
    if (z(w) && w.includes(p) || ie(w) && p in w || l.extends && d(l.extends, p, h) || l.mixins && l.mixins.some((m) => d(m, p, h)))
      return !0;
  }
  function a(l) {
    return xe(l) ? "ShallowRef" : l.effect ? "ComputedRef" : "Ref";
  }
  window.devtoolsFormatters ? window.devtoolsFormatters.push(r) : window.devtoolsFormatters = [r];
}
const Cr = "3.5.13", ct = {}.NODE_ENV !== "production" ? N : pe;
/**
* @vue/runtime-dom v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
let Ni;
const xr = typeof window < "u" && window.trustedTypes;
if (xr)
  try {
    Ni = /* @__PURE__ */ xr.createPolicy("vue", {
      createHTML: (e) => e
    });
  } catch (e) {
    ({}).NODE_ENV !== "production" && ct(`Error creating trusted types policy: ${e}`);
  }
const to = Ni ? (e) => Ni.createHTML(e) : (e) => e, Yl = "http://www.w3.org/2000/svg", Zl = "http://www.w3.org/1998/Math/MathML", st = typeof document < "u" ? document : null, Dr = st && /* @__PURE__ */ st.createElement("template"), Gl = {
  insert: (e, t, n) => {
    t.insertBefore(e, n || null);
  },
  remove: (e) => {
    const t = e.parentNode;
    t && t.removeChild(e);
  },
  createElement: (e, t, n, i) => {
    const r = t === "svg" ? st.createElementNS(Yl, e) : t === "mathml" ? st.createElementNS(Zl, e) : n ? st.createElement(e, { is: n }) : st.createElement(e);
    return e === "select" && i && i.multiple != null && r.setAttribute("multiple", i.multiple), r;
  },
  createText: (e) => st.createTextNode(e),
  createComment: (e) => st.createComment(e),
  setText: (e, t) => {
    e.nodeValue = t;
  },
  setElementText: (e, t) => {
    e.textContent = t;
  },
  parentNode: (e) => e.parentNode,
  nextSibling: (e) => e.nextSibling,
  querySelector: (e) => st.querySelector(e),
  setScopeId(e, t) {
    e.setAttribute(t, "");
  },
  // __UNSAFE__
  // Reason: innerHTML.
  // Static content here can only come from compiled templates.
  // As long as the user only uses trusted templates, this is safe.
  insertStaticContent(e, t, n, i, r, s) {
    const o = n ? n.previousSibling : t.lastChild;
    if (r && (r === s || r.nextSibling))
      for (; t.insertBefore(r.cloneNode(!0), n), !(r === s || !(r = r.nextSibling)); )
        ;
    else {
      Dr.innerHTML = to(
        i === "svg" ? `<svg>${e}</svg>` : i === "mathml" ? `<math>${e}</math>` : e
      );
      const f = Dr.content;
      if (i === "svg" || i === "mathml") {
        const u = f.firstChild;
        for (; u.firstChild; )
          f.appendChild(u.firstChild);
        f.removeChild(u);
      }
      t.insertBefore(f, n);
    }
    return [
      // first
      o ? o.nextSibling : t.firstChild,
      // last
      n ? n.previousSibling : t.lastChild
    ];
  }
}, Fl = Symbol("_vtc");
function Vl(e, t, n) {
  const i = e[Fl];
  i && (t = (t ? [t, ...i] : [...i]).join(" ")), t == null ? e.removeAttribute("class") : n ? e.setAttribute("class", t) : e.className = t;
}
const Nr = Symbol("_vod"), Wl = Symbol("_vsh"), Kl = Symbol({}.NODE_ENV !== "production" ? "CSS_VAR_TEXT" : ""), Ul = /(^|;)\s*display\s*:/;
function ql(e, t, n) {
  const i = e.style, r = fe(n);
  let s = !1;
  if (n && !r) {
    if (t)
      if (fe(t))
        for (const o of t.split(";")) {
          const f = o.slice(0, o.indexOf(":")).trim();
          n[f] == null && Nn(i, f, "");
        }
      else
        for (const o in t)
          n[o] == null && Nn(i, o, "");
    for (const o in n)
      o === "display" && (s = !0), Nn(i, o, n[o]);
  } else if (r) {
    if (t !== n) {
      const o = i[Kl];
      o && (n += ";" + o), i.cssText = n, s = Ul.test(n);
    }
  } else
    t && e.removeAttribute("style");
  Nr in e && (e[Nr] = s ? i.display : "", e[Wl] && (i.display = "none"));
}
const _l = /[^\\];\s*$/, Br = /\s*!important$/;
function Nn(e, t, n) {
  if (z(n))
    n.forEach((i) => Nn(e, t, i));
  else if (n == null && (n = ""), {}.NODE_ENV !== "production" && _l.test(n) && ct(
    `Unexpected semicolon at the end of '${t}' style value: '${n}'`
  ), t.startsWith("--"))
    e.setProperty(t, n);
  else {
    const i = $l(e, t);
    Br.test(n) ? e.setProperty(
      Pt(i),
      n.replace(Br, ""),
      "important"
    ) : e[i] = n;
  }
}
const Mr = ["Webkit", "Moz", "ms"], ui = {};
function $l(e, t) {
  const n = ui[t];
  if (n)
    return n;
  let i = Je(t);
  if (i !== "filter" && i in e)
    return ui[t] = i;
  i = Gn(i);
  for (let r = 0; r < Mr.length; r++) {
    const s = Mr[r] + i;
    if (s in e)
      return ui[t] = s;
  }
  return t;
}
const Sr = "http://www.w3.org/1999/xlink";
function Lr(e, t, n, i, r, s = Bo(t)) {
  i && t.startsWith("xlink:") ? n == null ? e.removeAttributeNS(Sr, t.slice(6, t.length)) : e.setAttributeNS(Sr, t, n) : n == null || s && !Gr(n) ? e.removeAttribute(t) : e.setAttribute(
    t,
    s ? "" : Ct(n) ? String(n) : n
  );
}
function kr(e, t, n, i, r) {
  if (t === "innerHTML" || t === "textContent") {
    n != null && (e[t] = t === "innerHTML" ? to(n) : n);
    return;
  }
  const s = e.tagName;
  if (t === "value" && s !== "PROGRESS" && // custom elements may use _value internally
  !s.includes("-")) {
    const f = s === "OPTION" ? e.getAttribute("value") || "" : e.value, u = n == null ? (
      // #11647: value should be set as empty string for null and undefined,
      // but <input type="checkbox"> should be set as 'on'.
      e.type === "checkbox" ? "on" : ""
    ) : String(n);
    (f !== u || !("_value" in e)) && (e.value = u), n == null && e.removeAttribute(t), e._value = n;
    return;
  }
  let o = !1;
  if (n === "" || n == null) {
    const f = typeof e[t];
    f === "boolean" ? n = Gr(n) : n == null && f === "string" ? (n = "", o = !0) : f === "number" && (n = 0, o = !0);
  }
  try {
    e[t] = n;
  } catch (f) {
    ({}).NODE_ENV !== "production" && !o && ct(
      `Failed setting prop "${t}" on <${s.toLowerCase()}>: value ${n} is invalid.`,
      f
    );
  }
  o && e.removeAttribute(r || t);
}
function ec(e, t, n, i) {
  e.addEventListener(t, n, i);
}
function tc(e, t, n, i) {
  e.removeEventListener(t, n, i);
}
const Or = Symbol("_vei");
function nc(e, t, n, i, r = null) {
  const s = e[Or] || (e[Or] = {}), o = s[t];
  if (i && o)
    o.value = {}.NODE_ENV !== "production" ? Rr(i, t) : i;
  else {
    const [f, u] = ic(t);
    if (i) {
      const d = s[t] = oc(
        {}.NODE_ENV !== "production" ? Rr(i, t) : i,
        r
      );
      ec(e, f, d, u);
    } else
      o && (tc(e, f, o, u), s[t] = void 0);
  }
}
const Hr = /(?:Once|Passive|Capture)$/;
function ic(e) {
  let t;
  if (Hr.test(e)) {
    t = {};
    let i;
    for (; i = e.match(Hr); )
      e = e.slice(0, e.length - i[0].length), t[i[0].toLowerCase()] = !0;
  }
  return [e[2] === ":" ? e.slice(3) : Pt(e.slice(2)), t];
}
let Ai = 0;
const rc = /* @__PURE__ */ Promise.resolve(), sc = () => Ai || (rc.then(() => Ai = 0), Ai = Date.now());
function oc(e, t) {
  const n = (i) => {
    if (!i._vts)
      i._vts = Date.now();
    else if (i._vts <= n.attached)
      return;
    et(
      fc(i, n.value),
      t,
      5,
      [i]
    );
  };
  return n.value = e, n.attached = sc(), n;
}
function Rr(e, t) {
  return X(e) || z(e) ? e : (ct(
    `Wrong type passed as event handler to ${t} - did you forget @ or : in front of your prop?
Expected function or array of functions, received type ${typeof e}.`
  ), pe);
}
function fc(e, t) {
  if (z(t)) {
    const n = e.stopImmediatePropagation;
    return e.stopImmediatePropagation = () => {
      n.call(e), e._stopped = !0;
    }, t.map(
      (i) => (r) => !r._stopped && i && i(r)
    );
  } else
    return t;
}
const Tr = (e) => e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && // lowercase letter
e.charCodeAt(2) > 96 && e.charCodeAt(2) < 123, lc = (e, t, n, i, r, s) => {
  const o = r === "svg";
  t === "class" ? Vl(e, i, o) : t === "style" ? ql(e, n, i) : an(t) ? Bn(t) || nc(e, t, n, i, s) : (t[0] === "." ? (t = t.slice(1), !0) : t[0] === "^" ? (t = t.slice(1), !1) : cc(e, t, i, o)) ? (kr(e, t, i), !e.tagName.includes("-") && (t === "value" || t === "checked" || t === "selected") && Lr(e, t, i, o, s, t !== "value")) : /* #11081 force set props for possible async custom element */ e._isVueCE && (/[A-Z]/.test(t) || !fe(i)) ? kr(e, Je(t), i, s, t) : (t === "true-value" ? e._trueValue = i : t === "false-value" && (e._falseValue = i), Lr(e, t, i, o));
};
function cc(e, t, n, i) {
  if (i)
    return !!(t === "innerHTML" || t === "textContent" || t in e && Tr(t) && X(n));
  if (t === "spellcheck" || t === "draggable" || t === "translate" || t === "form" || t === "list" && e.tagName === "INPUT" || t === "type" && e.tagName === "TEXTAREA")
    return !1;
  if (t === "width" || t === "height") {
    const r = e.tagName;
    if (r === "IMG" || r === "VIDEO" || r === "CANVAS" || r === "SOURCE")
      return !1;
  }
  return Tr(t) && fe(n) ? !1 : t in e;
}
const uc = /* @__PURE__ */ ue({ patchProp: lc }, Gl);
let zr;
function Ac() {
  return zr || (zr = al(uc));
}
const ac = (...e) => {
  const t = Ac().createApp(...e);
  ({}).NODE_ENV !== "production" && (pc(t), gc(t));
  const { mount: n } = t;
  return t.mount = (i) => {
    const r = hc(i);
    if (!r)
      return;
    const s = t._component;
    !X(s) && !s.render && !s.template && (s.template = r.innerHTML), r.nodeType === 1 && (r.textContent = "");
    const o = n(r, !1, dc(r));
    return r instanceof Element && (r.removeAttribute("v-cloak"), r.setAttribute("data-v-app", "")), o;
  }, t;
};
function dc(e) {
  if (e instanceof SVGElement)
    return "svg";
  if (typeof MathMLElement == "function" && e instanceof MathMLElement)
    return "mathml";
}
function pc(e) {
  Object.defineProperty(e.config, "isNativeTag", {
    value: (t) => Co(t) || xo(t) || Do(t),
    writable: !1
  });
}
function gc(e) {
  {
    const t = e.config.isCustomElement;
    Object.defineProperty(e.config, "isCustomElement", {
      get() {
        return t;
      },
      set() {
        ct(
          "The `isCustomElement` config option is deprecated. Use `compilerOptions.isCustomElement` instead."
        );
      }
    });
    const n = e.config.compilerOptions, i = 'The `compilerOptions` config option is only respected when using a build of Vue.js that includes the runtime compiler (aka "full build"). Since you are using the runtime-only build, `compilerOptions` must be passed to `@vue/compiler-dom` in the build setup instead.\n- For vue-loader: pass it via vue-loader\'s `compilerOptions` loader option.\n- For vue-cli: see https://cli.vuejs.org/guide/webpack.html#modifying-options-of-a-loader\n- For vite: pass it via @vitejs/plugin-vue options. See https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue#example-for-passing-options-to-vuecompiler-sfc';
    Object.defineProperty(e.config, "compilerOptions", {
      get() {
        return ct(i), n;
      },
      set() {
        ct(i);
      }
    });
  }
}
function hc(e) {
  if (fe(e)) {
    const t = document.querySelector(e);
    return {}.NODE_ENV !== "production" && !t && ct(
      `Failed to mount app: mount target selector "${e}" returned null.`
    ), t;
  }
  return {}.NODE_ENV !== "production" && window.ShadowRoot && e instanceof window.ShadowRoot && e.mode === "closed" && ct(
    'mounting on a ShadowRoot with `{mode: "closed"}` may lead to unpredictable bugs'
  ), e;
}
/**
* vue v3.5.13
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/
function vc() {
  Ql();
}
({}).NODE_ENV !== "production" && vc();
function no(e) {
  return Wr() ? (So(e), !0) : !1;
}
function $i(e) {
  return typeof e == "function" ? e() : mt(e);
}
const Ec = typeof window < "u" && typeof document < "u";
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const mc = Object.prototype.toString, Ic = (e) => mc.call(e) === "[object Object]", bc = () => {
};
function yc(e) {
  return e || qi();
}
function Pc(e, t = !0, n) {
  yc() ? qn(e, n) : t ? e() : ln(e);
}
const er = Ec ? window : void 0;
function Lt(e) {
  var t;
  const n = $i(e);
  return (t = n == null ? void 0 : n.$el) != null ? t : n;
}
function wc(...e) {
  let t, n, i, r;
  if (typeof e[0] == "string" || Array.isArray(e[0]) ? ([n, i, r] = e, t = er) : [t, n, i, r] = e, !t)
    return bc;
  Array.isArray(n) || (n = [n]), Array.isArray(i) || (i = [i]);
  const s = [], o = () => {
    s.forEach((a) => a()), s.length = 0;
  }, f = (a, l, p, h) => (a.addEventListener(l, p, h), () => a.removeEventListener(l, p, h)), u = ft(
    () => [Lt(t), $i(r)],
    ([a, l]) => {
      if (o(), !a)
        return;
      const p = Ic(l) ? { ...l } : l;
      s.push(
        ...n.flatMap((h) => i.map((w) => f(a, h, w, p)))
      );
    },
    { immediate: !0, flush: "post" }
  ), d = () => {
    u(), o();
  };
  return no(d), d;
}
function Cc() {
  const e = Ye(!1), t = qi();
  return t && qn(() => {
    e.value = !0;
  }, t), e;
}
function xc(e) {
  const t = Cc();
  return Xe(() => (t.value, !!e()));
}
function Dc(e, t, n = {}) {
  const { window: i = er, ...r } = n;
  let s;
  const o = xc(() => i && "ResizeObserver" in i), f = () => {
    s && (s.disconnect(), s = void 0);
  }, u = Xe(() => {
    const l = $i(e);
    return Array.isArray(l) ? l.map((p) => Lt(p)) : [Lt(l)];
  }), d = ft(
    u,
    (l) => {
      if (f(), o.value && i) {
        s = new ResizeObserver(t);
        for (const p of l)
          p && s.observe(p, r);
      }
    },
    { immediate: !0, flush: "post" }
  ), a = () => {
    f(), d();
  };
  return no(a), {
    isSupported: o,
    stop: a
  };
}
function Nc(e, t = { width: 0, height: 0 }, n = {}) {
  const { window: i = er, box: r = "content-box" } = n, s = Xe(() => {
    var l, p;
    return (p = (l = Lt(e)) == null ? void 0 : l.namespaceURI) == null ? void 0 : p.includes("svg");
  }), o = Ye(t.width), f = Ye(t.height), { stop: u } = Dc(
    e,
    ([l]) => {
      const p = r === "border-box" ? l.borderBoxSize : r === "content-box" ? l.contentBoxSize : l.devicePixelContentBoxSize;
      if (i && s.value) {
        const h = Lt(e);
        if (h) {
          const w = h.getBoundingClientRect();
          o.value = w.width, f.value = w.height;
        }
      } else if (p) {
        const h = Array.isArray(p) ? p : [p];
        o.value = h.reduce((w, { inlineSize: m }) => w + m, 0), f.value = h.reduce((w, { blockSize: m }) => w + m, 0);
      } else
        o.value = l.contentRect.width, f.value = l.contentRect.height;
    },
    n
  );
  Pc(() => {
    const l = Lt(e);
    l && (o.value = "offsetWidth" in l ? l.offsetWidth : t.width, f.value = "offsetHeight" in l ? l.offsetHeight : t.height);
  });
  const d = ft(
    () => Lt(e),
    (l) => {
      o.value = l ? t.width : 0, f.value = l ? t.height : 0;
    }
  );
  function a() {
    u(), d();
  }
  return {
    width: o,
    height: f,
    stop: a
  };
}
function Bc() {
  return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (e) => {
    const t = Math.random() * 16 | 0;
    return (e === "x" ? t : t & 3 | 8).toString(16);
  });
}
function Mc() {
  const e = navigator.userAgent.toLowerCase();
  return /android/.test(e) ? "android" : /iphone|ipad|ipod/.test(e) ? "ios" : "pc";
}
var io = typeof globalThis < "u" ? globalThis : typeof window < "u" ? window : typeof global < "u" ? global : typeof self < "u" ? self : {};
function ro(e) {
  return e && e.__esModule && Object.prototype.hasOwnProperty.call(e, "default") ? e.default : e;
}
var so = { exports: {} };
(function(e, t) {
  (function(n, i) {
    e.exports = i();
  })(io, function() {
    var n = 1e3, i = 6e4, r = 36e5, s = "millisecond", o = "second", f = "minute", u = "hour", d = "day", a = "week", l = "month", p = "quarter", h = "year", w = "date", m = "Invalid Date", O = /^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/, k = /\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g, Q = { name: "en", weekdays: "Sunday_Monday_Tuesday_Wednesday_Thursday_Friday_Saturday".split("_"), months: "January_February_March_April_May_June_July_August_September_October_November_December".split("_"), ordinal: function(J) {
      var S = ["th", "st", "nd", "rd"], C = J % 100;
      return "[" + J + (S[(C - 20) % 10] || S[C] || S[0]) + "]";
    } }, R = function(J, S, C) {
      var P = String(J);
      return !P || P.length >= S ? J : "" + Array(S + 1 - P.length).join(C) + J;
    }, K = { s: R, z: function(J) {
      var S = -J.utcOffset(), C = Math.abs(S), P = Math.floor(C / 60), E = C % 60;
      return (S <= 0 ? "+" : "-") + R(P, 2, "0") + ":" + R(E, 2, "0");
    }, m: function J(S, C) {
      if (S.date() < C.date())
        return -J(C, S);
      var P = 12 * (C.year() - S.year()) + (C.month() - S.month()), E = S.clone().add(P, l), T = C - E < 0, Y = S.clone().add(P + (T ? -1 : 1), l);
      return +(-(P + (C - E) / (T ? E - Y : Y - E)) || 0);
    }, a: function(J) {
      return J < 0 ? Math.ceil(J) || 0 : Math.floor(J);
    }, p: function(J) {
      return { M: l, y: h, w: a, d, D: w, h: u, m: f, s: o, ms: s, Q: p }[J] || String(J || "").toLowerCase().replace(/s$/, "");
    }, u: function(J) {
      return J === void 0;
    } }, L = "en", q = {};
    q[L] = Q;
    var ae = "$isDayjsObject", le = function(J) {
      return J instanceof Ee || !(!J || !J[ae]);
    }, oe = function J(S, C, P) {
      var E;
      if (!S)
        return L;
      if (typeof S == "string") {
        var T = S.toLowerCase();
        q[T] && (E = T), C && (q[T] = C, E = T);
        var Y = S.split("-");
        if (!E && Y.length > 1)
          return J(Y[0]);
      } else {
        var U = S.name;
        q[U] = S, E = U;
      }
      return !P && E && (L = E), E || !P && L;
    }, te = function(J, S) {
      if (le(J))
        return J.clone();
      var C = typeof S == "object" ? S : {};
      return C.date = J, C.args = arguments, new Ee(C);
    }, F = K;
    F.l = oe, F.i = le, F.w = function(J, S) {
      return te(J, { locale: S.$L, utc: S.$u, x: S.$x, $offset: S.$offset });
    };
    var Ee = function() {
      function J(C) {
        this.$L = oe(C.locale, null, !0), this.parse(C), this.$x = this.$x || C.x || {}, this[ae] = !0;
      }
      var S = J.prototype;
      return S.parse = function(C) {
        this.$d = function(P) {
          var E = P.date, T = P.utc;
          if (E === null)
            return /* @__PURE__ */ new Date(NaN);
          if (F.u(E))
            return /* @__PURE__ */ new Date();
          if (E instanceof Date)
            return new Date(E);
          if (typeof E == "string" && !/Z$/i.test(E)) {
            var Y = E.match(O);
            if (Y) {
              var U = Y[2] - 1 || 0, ne = (Y[7] || "0").substring(0, 3);
              return T ? new Date(Date.UTC(Y[1], U, Y[3] || 1, Y[4] || 0, Y[5] || 0, Y[6] || 0, ne)) : new Date(Y[1], U, Y[3] || 1, Y[4] || 0, Y[5] || 0, Y[6] || 0, ne);
            }
          }
          return new Date(E);
        }(C), this.init();
      }, S.init = function() {
        var C = this.$d;
        this.$y = C.getFullYear(), this.$M = C.getMonth(), this.$D = C.getDate(), this.$W = C.getDay(), this.$H = C.getHours(), this.$m = C.getMinutes(), this.$s = C.getSeconds(), this.$ms = C.getMilliseconds();
      }, S.$utils = function() {
        return F;
      }, S.isValid = function() {
        return this.$d.toString() !== m;
      }, S.isSame = function(C, P) {
        var E = te(C);
        return this.startOf(P) <= E && E <= this.endOf(P);
      }, S.isAfter = function(C, P) {
        return te(C) < this.startOf(P);
      }, S.isBefore = function(C, P) {
        return this.endOf(P) < te(C);
      }, S.$g = function(C, P, E) {
        return F.u(C) ? this[P] : this.set(E, C);
      }, S.unix = function() {
        return Math.floor(this.valueOf() / 1e3);
      }, S.valueOf = function() {
        return this.$d.getTime();
      }, S.startOf = function(C, P) {
        var E = this, T = !!F.u(P) || P, Y = F.p(C), U = function(Te, ve) {
          var be = F.w(E.$u ? Date.UTC(E.$y, ve, Te) : new Date(E.$y, ve, Te), E);
          return T ? be : be.endOf(d);
        }, ne = function(Te, ve) {
          return F.w(E.toDate()[Te].apply(E.toDate("s"), (T ? [0, 0, 0, 0] : [23, 59, 59, 999]).slice(ve)), E);
        }, re = this.$W, Ae = this.$M, De = this.$D, gt = "set" + (this.$u ? "UTC" : "");
        switch (Y) {
          case h:
            return T ? U(1, 0) : U(31, 11);
          case l:
            return T ? U(1, Ae) : U(0, Ae + 1);
          case a:
            var Be = this.$locale().weekStart || 0, Ge = (re < Be ? re + 7 : re) - Be;
            return U(T ? De - Ge : De + (6 - Ge), Ae);
          case d:
          case w:
            return ne(gt + "Hours", 0);
          case u:
            return ne(gt + "Minutes", 1);
          case f:
            return ne(gt + "Seconds", 2);
          case o:
            return ne(gt + "Milliseconds", 3);
          default:
            return this.clone();
        }
      }, S.endOf = function(C) {
        return this.startOf(C, !1);
      }, S.$set = function(C, P) {
        var E, T = F.p(C), Y = "set" + (this.$u ? "UTC" : ""), U = (E = {}, E[d] = Y + "Date", E[w] = Y + "Date", E[l] = Y + "Month", E[h] = Y + "FullYear", E[u] = Y + "Hours", E[f] = Y + "Minutes", E[o] = Y + "Seconds", E[s] = Y + "Milliseconds", E)[T], ne = T === d ? this.$D + (P - this.$W) : P;
        if (T === l || T === h) {
          var re = this.clone().set(w, 1);
          re.$d[U](ne), re.init(), this.$d = re.set(w, Math.min(this.$D, re.daysInMonth())).$d;
        } else
          U && this.$d[U](ne);
        return this.init(), this;
      }, S.set = function(C, P) {
        return this.clone().$set(C, P);
      }, S.get = function(C) {
        return this[F.p(C)]();
      }, S.add = function(C, P) {
        var E, T = this;
        C = Number(C);
        var Y = F.p(P), U = function(Ae) {
          var De = te(T);
          return F.w(De.date(De.date() + Math.round(Ae * C)), T);
        };
        if (Y === l)
          return this.set(l, this.$M + C);
        if (Y === h)
          return this.set(h, this.$y + C);
        if (Y === d)
          return U(1);
        if (Y === a)
          return U(7);
        var ne = (E = {}, E[f] = i, E[u] = r, E[o] = n, E)[Y] || 1, re = this.$d.getTime() + C * ne;
        return F.w(re, this);
      }, S.subtract = function(C, P) {
        return this.add(-1 * C, P);
      }, S.format = function(C) {
        var P = this, E = this.$locale();
        if (!this.isValid())
          return E.invalidDate || m;
        var T = C || "YYYY-MM-DDTHH:mm:ssZ", Y = F.z(this), U = this.$H, ne = this.$m, re = this.$M, Ae = E.weekdays, De = E.months, gt = E.meridiem, Be = function(ve, be, tt, ht) {
          return ve && (ve[be] || ve(P, T)) || tt[be].slice(0, ht);
        }, Ge = function(ve) {
          return F.s(U % 12 || 12, ve, "0");
        }, Te = gt || function(ve, be, tt) {
          var ht = ve < 12 ? "AM" : "PM";
          return tt ? ht.toLowerCase() : ht;
        };
        return T.replace(k, function(ve, be) {
          return be || function(tt) {
            switch (tt) {
              case "YY":
                return String(P.$y).slice(-2);
              case "YYYY":
                return F.s(P.$y, 4, "0");
              case "M":
                return re + 1;
              case "MM":
                return F.s(re + 1, 2, "0");
              case "MMM":
                return Be(E.monthsShort, re, De, 3);
              case "MMMM":
                return Be(De, re);
              case "D":
                return P.$D;
              case "DD":
                return F.s(P.$D, 2, "0");
              case "d":
                return String(P.$W);
              case "dd":
                return Be(E.weekdaysMin, P.$W, Ae, 2);
              case "ddd":
                return Be(E.weekdaysShort, P.$W, Ae, 3);
              case "dddd":
                return Ae[P.$W];
              case "H":
                return String(U);
              case "HH":
                return F.s(U, 2, "0");
              case "h":
                return Ge(1);
              case "hh":
                return Ge(2);
              case "a":
                return Te(U, ne, !0);
              case "A":
                return Te(U, ne, !1);
              case "m":
                return String(ne);
              case "mm":
                return F.s(ne, 2, "0");
              case "s":
                return String(P.$s);
              case "ss":
                return F.s(P.$s, 2, "0");
              case "SSS":
                return F.s(P.$ms, 3, "0");
              case "Z":
                return Y;
            }
            return null;
          }(ve) || Y.replace(":", "");
        });
      }, S.utcOffset = function() {
        return 15 * -Math.round(this.$d.getTimezoneOffset() / 15);
      }, S.diff = function(C, P, E) {
        var T, Y = this, U = F.p(P), ne = te(C), re = (ne.utcOffset() - this.utcOffset()) * i, Ae = this - ne, De = function() {
          return F.m(Y, ne);
        };
        switch (U) {
          case h:
            T = De() / 12;
            break;
          case l:
            T = De();
            break;
          case p:
            T = De() / 3;
            break;
          case a:
            T = (Ae - re) / 6048e5;
            break;
          case d:
            T = (Ae - re) / 864e5;
            break;
          case u:
            T = Ae / r;
            break;
          case f:
            T = Ae / i;
            break;
          case o:
            T = Ae / n;
            break;
          default:
            T = Ae;
        }
        return E ? T : F.a(T);
      }, S.daysInMonth = function() {
        return this.endOf(l).$D;
      }, S.$locale = function() {
        return q[this.$L];
      }, S.locale = function(C, P) {
        if (!C)
          return this.$L;
        var E = this.clone(), T = oe(C, P, !0);
        return T && (E.$L = T), E;
      }, S.clone = function() {
        return F.w(this.$d, this);
      }, S.toDate = function() {
        return new Date(this.valueOf());
      }, S.toJSON = function() {
        return this.isValid() ? this.toISOString() : null;
      }, S.toISOString = function() {
        return this.$d.toISOString();
      }, S.toString = function() {
        return this.$d.toUTCString();
      }, J;
    }(), xt = Ee.prototype;
    return te.prototype = xt, [["$ms", s], ["$s", o], ["$m", f], ["$H", u], ["$W", d], ["$M", l], ["$y", h], ["$D", w]].forEach(function(J) {
      xt[J[1]] = function(S) {
        return this.$g(S, J[0], J[1]);
      };
    }), te.extend = function(J, S) {
      return J.$i || (J(S, Ee, te), J.$i = !0), te;
    }, te.locale = oe, te.isDayjs = le, te.unix = function(J) {
      return te(1e3 * J);
    }, te.en = q[L], te.Ls = q, te.p = {}, te;
  });
})(so);
var Sc = so.exports;
const Jn = /* @__PURE__ */ ro(Sc);
var oo = { exports: {} };
(function(e, t) {
  (function(n, i) {
    e.exports = i();
  })(io, function() {
    var n = "minute", i = /[+-]\d\d(?::?\d\d)?/g, r = /([+-]|\d\d)/g;
    return function(s, o, f) {
      var u = o.prototype;
      f.utc = function(m) {
        var O = { date: m, utc: !0, args: arguments };
        return new o(O);
      }, u.utc = function(m) {
        var O = f(this.toDate(), { locale: this.$L, utc: !0 });
        return m ? O.add(this.utcOffset(), n) : O;
      }, u.local = function() {
        return f(this.toDate(), { locale: this.$L, utc: !1 });
      };
      var d = u.parse;
      u.parse = function(m) {
        m.utc && (this.$u = !0), this.$utils().u(m.$offset) || (this.$offset = m.$offset), d.call(this, m);
      };
      var a = u.init;
      u.init = function() {
        if (this.$u) {
          var m = this.$d;
          this.$y = m.getUTCFullYear(), this.$M = m.getUTCMonth(), this.$D = m.getUTCDate(), this.$W = m.getUTCDay(), this.$H = m.getUTCHours(), this.$m = m.getUTCMinutes(), this.$s = m.getUTCSeconds(), this.$ms = m.getUTCMilliseconds();
        } else
          a.call(this);
      };
      var l = u.utcOffset;
      u.utcOffset = function(m, O) {
        var k = this.$utils().u;
        if (k(m))
          return this.$u ? 0 : k(this.$offset) ? l.call(this) : this.$offset;
        if (typeof m == "string" && (m = function(L) {
          L === void 0 && (L = "");
          var q = L.match(i);
          if (!q)
            return null;
          var ae = ("" + q[0]).match(r) || ["-", 0, 0], le = ae[0], oe = 60 * +ae[1] + +ae[2];
          return oe === 0 ? 0 : le === "+" ? oe : -oe;
        }(m), m === null))
          return this;
        var Q = Math.abs(m) <= 16 ? 60 * m : m, R = this;
        if (O)
          return R.$offset = Q, R.$u = m === 0, R;
        if (m !== 0) {
          var K = this.$u ? this.toDate().getTimezoneOffset() : -1 * this.utcOffset();
          (R = this.local().add(Q + K, n)).$offset = Q, R.$x.$localOffset = K;
        } else
          R = this.utc();
        return R;
      };
      var p = u.format;
      u.format = function(m) {
        var O = m || (this.$u ? "YYYY-MM-DDTHH:mm:ss[Z]" : "");
        return p.call(this, O);
      }, u.valueOf = function() {
        var m = this.$utils().u(this.$offset) ? 0 : this.$offset + (this.$x.$localOffset || this.$d.getTimezoneOffset());
        return this.$d.valueOf() - 6e4 * m;
      }, u.isUTC = function() {
        return !!this.$u;
      }, u.toISOString = function() {
        return this.toDate().toISOString();
      }, u.toString = function() {
        return this.toDate().toUTCString();
      };
      var h = u.toDate;
      u.toDate = function(m) {
        return m === "s" && this.$offset ? f(this.format("YYYY-MM-DD HH:mm:ss:SSS")).toDate() : h.call(this);
      };
      var w = u.diff;
      u.diff = function(m, O, k) {
        if (m && this.$u === m.$u)
          return w.call(this, m, O, k);
        var Q = this.local(), R = f(m).local();
        return w.call(Q, R, O, k);
      };
    };
  });
})(oo);
var Lc = oo.exports;
const kc = /* @__PURE__ */ ro(Lc);
Jn.extend(kc);
function Oc() {
  return Jn.utc().format("YYYY-MM-DD");
}
function fo(e) {
  if (!e)
    return "";
  const t = Jn(), n = Jn(e), i = t.diff(n, "second");
  if (i < 60)
    return `${i}s ago`;
  const r = t.diff(n, "minute");
  if (r < 60)
    return `${r}m ago`;
  const s = t.diff(n, "hour");
  if (s < 24)
    return `${s}h ago`;
  const o = t.diff(n, "day");
  return o < 7 ? `${o}d ago` : t.diff(n, "year") <= 1 ? n.format("MMM D") : n.format("MMM YYYY");
}
const Qn = Mc() === "pc", Hc = Oc(), Rc = "https://mxnewsdata.pages.dev/news_en", Tc = "https://mxnews.pages.dev/";
async function zc(e = "usa") {
  const t = await fetch(`${Rc}/${Hc}/${e}.json`), n = [];
  try {
    const i = await t.json();
    if (typeof i == "object" && !i.code)
      for (const r in i) {
        const s = i[r];
        if (!s.cached || !s.image)
          return;
        const o = {
          ...s,
          hash: r,
          category: s.category,
          title: s.title,
          tag: s.source,
          date: fo(s.pubDate),
          imageList: [s.image, ...s.images || []],
          link: `${Tc}#/detail/${s.category.toLowerCase()}/${r}`
        };
        n.push(o);
      }
  } catch {
  }
  return {
    data: n
  };
}
const jr = Date.now(), lo = Qc(), jc = Yc(), Xc = `https://api.taboola.com/2.0/json/maxthonbrowser-${lo}/recommendations.get`;
async function Jc(...e) {
  const t = Zc(...e), i = await (await fetch(Xc, {
    method: "post",
    credentials: "include",
    body: JSON.stringify(t)
  })).json(), r = Gc(i);
  return {
    session: i.user.session,
    data: r
  };
}
function Qc() {
  const e = navigator.language;
  return /^en-us/.test(e) ? "us-english" : /^fr/.test(e) ? "france-french" : /^de/.test(e) ? "germany-german" : /^pl/.test(e) ? "poland-polish" : "global-english";
}
function Yc() {
  let e = localStorage.getItem("mx-deviceId");
  return e || (e = Bc(), localStorage.setItem("mx-deviceId", e)), e;
}
function Zc(e = "Trending", t = "init") {
  const n = {
    placements: [],
    // 用户信息
    user: {
      // 请求单元，第一次请求是 init，后续用上次返回的值
      session: t,
      // 设备ID，每个设备随机生成一个，后续从本地取，比如 'a3ad232c-9f08-45c5-9051-051409edb405'
      device: jc,
      // 以下固定：用户IP和设备信息
      // realip: '192.199.248.75',
      agent: navigator.userAgent
    },
    view: {
      // 页面ID，每次刷新随机生成一个，比如 'a558e7763d614902a3689c69b23c25a7'，可以用时间戳
      id: `view_${e}_${jr}`
    },
    app: {
      // 傲游key
      apiKey: "6d24fb0e092f4d688803aa8dd2b40fc43a8dad37",
      // SERVER or CLIENT
      origin: "CLIENT",
      // 以下固定
      type: "MOBILE",
      name: `maxthonbrowser-${lo}`
    },
    source: {
      // 页面ID，每次刷新随机生成一个，比如 'a558e7763d614902a3689c69b23c25a7'，可以用时间戳
      id: `source_${e}_${jr}`,
      // "home" or "article" or "video"
      type: "HOME",
      // 当前网址
      url: location.href
    }
  };
  for (let i = 0; i < 9; i++) {
    const r = i + 1;
    n.placements.push({
      // Editorial Trending 1
      name: r % 3 === 0 ? `AD Editorial ${e} ${r}` : `Editorial ${e} ${r}`,
      // 以下固定
      recCount: 1,
      organicType: "mix"
    });
  }
  return n;
}
function Gc(e) {
  const t = [], n = [];
  e.placements.forEach((o) => {
    var a;
    const f = o.list[0], u = !f.categories, d = {
      ...f,
      hash: f["raw-id"],
      category: u ? "ad" : f.categories[0],
      title: f.name,
      tag: f.branding,
      date: u ? f.origin : fo(f.created),
      imageList: ((a = f.thumbnail) == null ? void 0 : a.map((l) => l.url)) || [],
      link: f.url,
      visibleEvent: o.events.visible
    };
    u ? n.push(d) : t.push(d);
  });
  const i = t.length, r = n.length, s = Math.floor(i / r);
  for (let o = r - 1; o >= 0; o--) {
    const f = s * (o + 1);
    t.splice(f, 0, n[o]);
  }
  return t;
}
const Fc = {
  // 获取新闻
  getNewsList(e, t) {
    return Qn ? zc(e) : Jc(e, t);
  }
};
const Vc = { class: "mx-skeleton" }, co = {
  __name: "MxSkeleton",
  props: {
    // type: row, image, border
    // width, height 宽高
    // center 是否居中
    rows: { type: Array, default: null }
  },
  setup(e) {
    const t = e, n = Ye([]);
    i(t.rows);
    function i(r) {
      r.forEach((s) => {
        if (s.repeat)
          for (let o = 0; o < s.repeat; o++)
            s.rows ? i(s.rows) : n.value.push(s);
        else
          n.value.push(s);
      });
    }
    return (r, s) => (ge(), Re("div", Vc, [
      (ge(!0), Re(Ne, null, Gi(n.value, (o, f) => (ge(), Re("div", {
        key: f,
        class: ot([
          "mx-skeleton-item",
          `is-type-${o.type || "row"}`,
          {
            "is-center": o.center,
            "is-border": o.border
          }
        ]),
        style: Gt({
          width: o.width,
          height: o.height
        })
      }, null, 6))), 128))
    ]));
  }
};
typeof WorkerGlobalScope < "u" && globalThis instanceof WorkerGlobalScope;
const Xr = () => {
};
function Bi(e, t = !1, n = "Timeout") {
  return new Promise((i, r) => {
    setTimeout(t ? () => r(n) : i, e);
  });
}
function Mi(e, t = !1) {
  function n(l, { flush: p = "sync", deep: h = !1, timeout: w, throwOnTimeout: m } = {}) {
    let O = null;
    const Q = [new Promise((R) => {
      O = ft(
        e,
        (K) => {
          l(K) !== t && (O ? O() : ln(() => O == null ? void 0 : O()), R(K));
        },
        {
          flush: p,
          deep: h,
          immediate: !0
        }
      );
    })];
    return w != null && Q.push(
      Bi(w, m).then(() => St(e)).finally(() => O == null ? void 0 : O())
    ), Promise.race(Q);
  }
  function i(l, p) {
    if (!ce(l))
      return n((K) => K === l, p);
    const { flush: h = "sync", deep: w = !1, timeout: m, throwOnTimeout: O } = p ?? {};
    let k = null;
    const R = [new Promise((K) => {
      k = ft(
        [e, l],
        ([L, q]) => {
          t !== (L === q) && (k ? k() : ln(() => k == null ? void 0 : k()), K(L));
        },
        {
          flush: h,
          deep: w,
          immediate: !0
        }
      );
    })];
    return m != null && R.push(
      Bi(m, O).then(() => St(e)).finally(() => (k == null || k(), St(e)))
    ), Promise.race(R);
  }
  function r(l) {
    return n((p) => !!p, l);
  }
  function s(l) {
    return i(null, l);
  }
  function o(l) {
    return i(void 0, l);
  }
  function f(l) {
    return n(Number.isNaN, l);
  }
  function u(l, p) {
    return n((h) => {
      const w = Array.from(h);
      return w.includes(l) || w.includes(St(l));
    }, p);
  }
  function d(l) {
    return a(1, l);
  }
  function a(l = 1, p) {
    let h = -1;
    return n(() => (h += 1, h >= l), p);
  }
  return Array.isArray(St(e)) ? {
    toMatch: n,
    toContains: u,
    changed: d,
    changedTimes: a,
    get not() {
      return Mi(e, !t);
    }
  } : {
    toMatch: n,
    toBe: i,
    toBeTruthy: r,
    toBeNull: s,
    toBeNaN: f,
    toBeUndefined: o,
    changed: d,
    changedTimes: a,
    get not() {
      return Mi(e, !t);
    }
  };
}
function Wc(e) {
  return Mi(e);
}
function Kc(e, t, n) {
  const {
    immediate: i = !0,
    delay: r = 0,
    onError: s = Xr,
    onSuccess: o = Xr,
    resetOnExecute: f = !0,
    shallow: u = !0,
    throwError: d
  } = n ?? {}, a = u ? fr(t) : Ye(t), l = Ye(!1), p = Ye(!1), h = fr(void 0);
  async function w(k = 0, ...Q) {
    f && (a.value = t), h.value = void 0, l.value = !1, p.value = !0, k > 0 && await Bi(k);
    const R = typeof e == "function" ? e(...Q) : e;
    try {
      const K = await R;
      a.value = K, l.value = !0, o(K);
    } catch (K) {
      if (h.value = K, s(K), d)
        throw K;
    } finally {
      p.value = !1;
    }
    return a.value;
  }
  i && w(r);
  const m = {
    state: a,
    isReady: l,
    isLoading: p,
    error: h,
    execute: w
  };
  function O() {
    return new Promise((k, Q) => {
      Wc(p).toBe(!1).then(() => k(m)).catch(Q);
    });
  }
  return {
    ...m,
    then(k, Q) {
      return O().then(k, Q);
    }
  };
}
async function Uc(e) {
  return new Promise((t, n) => {
    const i = new Image(), { src: r, srcset: s, sizes: o, class: f, loading: u, crossorigin: d, referrerPolicy: a, width: l, height: p, decoding: h, fetchPriority: w, ismap: m, usemap: O } = e;
    i.src = r, s != null && (i.srcset = s), o != null && (i.sizes = o), f != null && (i.className = f), u != null && (i.loading = u), d != null && (i.crossOrigin = d), a != null && (i.referrerPolicy = a), l != null && (i.width = l), p != null && (i.height = p), h != null && (i.decoding = h), w != null && (i.fetchPriority = w), m != null && (i.isMap = m), O != null && (i.useMap = O), i.onload = () => t(i), i.onerror = n;
  });
}
function qc(e, t = {}) {
  const n = Kc(
    () => Uc(St(e)),
    void 0,
    {
      resetOnExecute: !0,
      ...t
    }
  );
  return ft(
    () => St(e),
    () => n.execute(t.delay),
    { deep: !0 }
  ), n;
}
const _c = "data:image/png;base64,";
const $c = ["src", "data-src"], eu = {
  __name: "MxImage",
  props: {
    // 图片地址
    src: { type: String, default: "" },
    // 是否圆角
    radius: { type: String, default: "" }
  },
  setup(e) {
    const t = e, { isLoading: n, error: i } = qc({ src: t.src });
    return (r, s) => (ge(), Re("div", {
      class: "mx-image",
      style: Gt({ "border-radius": e.radius })
    }, [
      mt(n) ? (ge(), Zt(co, {
        key: 0,
        rows: [{ type: "image" }]
      })) : (ge(), Re("img", {
        key: 1,
        src: mt(i) ? mt(_c) : t.src,
        "data-src": mt(i) ? e.src : null,
        class: ot(["mx-image-inner", { "is-error": mt(i) }])
      }, null, 10, $c))
    ], 4));
  }
};
const tu = ["href"], nu = { class: "mx-news-content" }, iu = ["innerHTML"], ru = { class: "mx-news-source" }, su = { class: "mx-news-date" }, Jr = {
  __name: "MxNewsItem",
  props: {
    // 数据
    data: { type: Object, default: null },
    // 是否顶部大图
    isTop: { type: Boolean, default: !1 }
  },
  setup(e) {
    const t = e, n = Xe(() => {
      if (t.data.imageList.length > 1) {
        const s = t.isTop ? 1 : 3;
        return t.data.imageList.slice(0, s);
      } else
        return t.data.imageList;
    }), i = Xe(() => t.isTop ? !1 : n.value.length === 1), r = Ye(null);
    return qn(() => {
      if (Qn)
        return;
      const s = new IntersectionObserver(
        (o) => {
          o.forEach((f) => {
            f.isIntersecting && (fetch(t.data.visibleEvent), s.disconnect());
          });
        },
        { threshold: 0.5 }
      );
      s.observe(r.value);
    }), (s, o) => (ge(), Re("a", {
      ref_key: "itemRef",
      ref: r,
      href: e.data.link,
      target: "_blank",
      class: ot(["mx-news-item", {
        "is-cols": i.value,
        "is-top": e.isTop
      }])
    }, [
      Ke("div", nu, [
        Ke("div", {
          class: ot(["mx-news-title", e.isTop ? "is-top" : "is-list"]),
          innerHTML: e.data.title
        }, null, 10, iu),
        Ke("div", {
          class: ot(["mx-news-meta", e.isTop ? "is-top" : "is-list"])
        }, [
          Ke("span", ru, ai(e.data.tag), 1),
          o[0] || (o[0] = Ke("span", null, "·", -1)),
          Ke("span", su, ai(e.data.date), 1)
        ], 2)
      ]),
      Ke("div", {
        class: "mx-news-imgs",
        style: Gt(`--mx-news-image-length: ${n.value.length}`)
      }, [
        (ge(!0), Re(Ne, null, Gi(n.value, (f) => (ge(), Zt(eu, {
          key: f,
          src: f
        }, null, 8, ["src"]))), 128))
      ], 4)
    ], 10, tu));
  }
};
const ou = { class: "mx-news-list" }, fu = {
  key: 1,
  class: "mx-news-empty"
}, lu = {
  key: 1,
  class: "mx-news-loading"
}, uo = {
  __name: "MxNewsList",
  props: {
    // 挂载节点选择器
    container: { type: String, default: null },
    // 滚动容器选择器：可选，默认 body
    scrollbar: { type: String, default: null },
    // 分类：可选，默认 Trending
    category: { type: String, default: null },
    // 是否需要顶部大图：默认 true
    needTop: { type: Boolean, default: !0 },
    // 最大条目：默认空，无限制
    maxLength: { type: Number, default: null },
    // 宽度过大时自动两列布局
    autoCols: { type: Boolean, default: !1 }
  },
  setup(e) {
    const t = e, n = document.querySelector(t.container), i = t.scrollbar ? document.querySelector(t.scrollbar) : document, r = t.scrollbar ? document.querySelector(t.scrollbar) : document.documentElement, { width: s } = Nc(n), o = Xe(() => t.autoCols && s.value >= 768), f = Ye(null), u = Xe(() => t.needTop && f.value ? f.value[0] : null), d = Xe(() => {
      var k, Q;
      const O = t.needTop ? 1 : 0;
      return t.maxLength ? (k = f.value) == null ? void 0 : k.slice(O, t.maxLength) : (Q = f.value) == null ? void 0 : Q.slice(O);
    }), a = Xe(() => {
      const O = [{ width: "80%", center: !0 }, { type: "image" }, { type: "border" }], k = [{ repeat: 3, rows: [{ width: "40%" }, { type: "row" }, { width: "60%" }, { type: "border" }] }];
      return t.needTop ? [...O, ...k] : [...k];
    }), l = Ye(!1), p = Ye("init"), h = Xe(() => t.category ? t.category : Qn ? "usa" : "Trending");
    async function w(O) {
      if (l.value)
        return;
      l.value = !0;
      const k = await Fc.getNewsList(h.value, p.value);
      l.value = !1, O && (f.value = []), f.value.push(...k.data), p.value = k.session, ln(() => {
        m();
      });
    }
    wc(i, "scroll", m);
    function m() {
      var R;
      if (Qn || n.style.display === "none" || t.maxLength && t.maxLength < ((R = f.value) == null ? void 0 : R.length))
        return;
      const { clientHeight: O, scrollTop: k, scrollHeight: Q } = r;
      O + k >= Q - 100 && w();
    }
    return w(!0), (O, k) => (ge(), Re("div", ou, [
      f.value ? f.value.length ? (ge(), Re(Ne, { key: 2 }, [
        e.needTop ? (ge(), Zt(Jr, {
          key: 0,
          data: u.value,
          "is-top": !0
        }, null, 8, ["data"])) : br("", !0),
        Ke("div", {
          class: ot(["mx-news-items", { "is-cols": o.value }])
        }, [
          (ge(!0), Re(Ne, null, Gi(d.value, (Q, R) => (ge(), Zt(Jr, {
            key: R,
            data: Q,
            "is-top": !1
          }, null, 8, ["data"]))), 128))
        ], 2),
        l.value ? (ge(), Re("div", lu, " Loading... ")) : br("", !0)
      ], 64)) : (ge(), Re("div", fu, " No Data ")) : (ge(), Zt(co, {
        key: 0,
        rows: a.value
      }, null, 8, ["rows"]))
    ]));
  }
};
function cu(e = {}) {
  const t = ac(uo, e), n = document.querySelector(e.container);
  return t.mount(n), {
    destroy() {
      t.unmount(), document.body.removeChild(n);
    }
  };
}
function uu(e) {
  e.component("MyComponent", uo);
}
export {
  cu as default,
  cu as init,
  uu as register
};
