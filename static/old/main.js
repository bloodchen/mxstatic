...
$(function() {
    ...
    if(!isZh) {
        $navigation.html('\
            <a class="mining-go-lk" href="http://extension.maxthon.com/detail/index.php?view_id=3127&category_id=" target="_blank">\
                <div class="mining-go-inner">\
                    <i class="mod_actmark mod_actmark_top"></i>\
                    <p class="mining-go-title">Most downloaded recently: Mining  Go</p>\
                    <p class="mining-go-sub-title">Join browser mining, earn LVT easily！</p>\
                    <button class="mining-go-download-btn" style="padding: 8px 20px;right:80px;">Download Now</button>\
                    <i class="iconfont mining-go-collapse"></i>\
                </div>\
            </a>\
            <span id="mining-go-ad">AD</span>\
        ')
    }
    if (window.Api.Maxthon.platform === 'Win' && window.localStorage && !localStorage.getItem('banner-coin-collapse')) {
        $navigation.addClass('isvisible');
    } else {
    }
    ...
    function bindeEvent() {
        ...
        $navigation.on('click', '.mining-go-collapse', function(e) {
            // 阻止冒泡
            e.stopPropagation();
            e.preventDefault();
            $navigation.removeClass('isvisible');
        ...
        $navigation.on('click', '.mining-go-lk', function() {
...
$(function () {
    ...
    $header.on('click', '.switch-action', function() {
    ...
    function login(data) {
        ...
        var lang = navigator.language.toLocaleLowerCase();
        data.securityUrl = 'https://pc-uc.uu.me/i/security.html?uid=' + data.id + '&ln=' + lang + '&mxver=' + Api.max_version + '&mxpn=' + Api.pn;
        data.personalUrl = 'https://pc-uc.uu.me/i/?uid=' + data.id + '&ln=' + lang + '&mxver=' + Api.max_version + '&mxpn=' + Api.pn;
        data.uuMailUrl = 'https://pc-uc.uu.me/mail/?uid=' + data.id + 'ln=' + lang + '&mxver=' + Api.max_version + '&mxpn=' + Api.pn;
    ...
    function logout() {
        $header.find('.header-action').replaceWith('<ul class="header-action fr">\
                            <li class="fl">\
                                <a class="switch-action" href="#">' + (lang === 'zh-cn' ? '热门插件' : 'Hot Addons') + '</a>\
                            </li>\
                            <li class="fl">\
                                <a href="mx://note/index.htm?id=-1" target="_blank" data-m="builtInpage" data-n="maxNote">' + Language.getLang('Maxnote') + '</a>\
                            </li>\
                            <li class="fl">\
    ...
    function isloginout() {
        ...
        if(maxthon.platform === 'Mac') {
            if((maxthon.pn === 'mxmac_appstore' && maxthon.max_version.cmpVersions('5.1.50') >= 0) ||
                (maxthon.pn !== 'mxmac_appstore' && maxthon.max_version.cmpVersions('5.1.114') >= 0)) {
                return true;
            }
        }
        return false;
    ...
    function render(data) {
        var tpl = '<ul class="header-action fr">\
                    ...
                    <li class="fl">\
                        <a href="'+ data.uuMailUrl + '" target="_blank" data-m="builtInpage" data-n="UUMail">' + Language.getLang('UUMail') + '</a>\
                    </li>\
					<li class="fl profile">\
						<span class="line">|</span>\
						...
						<div class="profile-dropdown">\
							...
							<div class="profile-continer">\
								...
								<ul>\
									...
									<li>\
										<p class="logout">' + (isloginout() ? Language.getLang('LogOut') : Language.getLang('Logout')) + '</p>\
									</li>\
								</ul>\
							</div>\
						</div>\
...
$(function () {
    ...
    /**
     * 点击切换搜索引擎按钮 pageload-overlay show pageload-loading
     */
    ...
    function initEngineList() {
        Api.useApi('config.getMultiValues', {keys:"['browser.general.search_engine_list2', 'browser.general.default_search_engine2']"}, function (data) {
            ...
            var defaultSearchEngin = JSON.parse(data[1] || {url: ''});
            // 初始化搜索框
...
var grid = {
    hot_grid_html: '<li class="top grid">\
                    <% if(!!this.image) {%>\
                        <a href="<%= this.url || \"\" %>" target="_blank" class="thumb" style="background-image:url(<%= this.image %>);">\
                        </a>\
                    <% } else { %>\
    ...
    var configPanel = (function () {
        ...
        function initOption() {
            ...
            getOption(function (config) {
                ...
                // 显示活动皮肤开关
                if(skinDisplay && skinDisplay === true) {
...
$(function () {
    ...
    function getPageData(lang) {
        ...
        if (_pn === 'ruixing' && lang === 'zh-cn' && (max_version === '5.1.1.1000' || max_version === '5.1.2.3000')) {
            subfix = 'ruixing';
        }
        ...
        $('.nav-header-menu li').on('mouseover mouseout', function (e) {
            ...
            hoverTimer = setTimeout(function (type) {
                if (type == "mouseover") {
                    ...
                    menuName === "activity" && window.Api.statistic.webSend({
                        'm': 'activityTab',
                        'n': 'activityTitle',
                        'data': {
...
(function () {
    ...
    var cache = {};  //缓存数据
    var currentIndex = 0;  //默认索引
    var hot_number = 0 ;  //热搜切换计数器
    var hot_count = 10;  //单词展示热搜条数
    ...
    function createFeedHeader() {
        ...
        $.each(categoryList, function (index, item) {
                ...
                box += '<div class="feed-pane"></div>';
        });
        $('#news-feed').find('.feed-header').html(html);
        $('#news-feed').find('.feed-content').append(box);
    }
    ...
    function initHotWord () {
        var xhr = getHotWord();
        $('#word-header-fresh').on('click', function (ev) {
            ev.preventDefault();
            if (xhr.status !== 200) {
                xhr.abort();
                xhr = getHotWord();
    ...
    function buildHotWord (data) {
        var str = '';
        var data = cache.hotWord.slice((hot_number % 2) * hot_count, (hot_number++ % 2) * hot_count + hot_count);
        $.each(data, function (index, item) {
            str += createHotWordItem(index + 1, item);
        });
        $('#word-content').html(str);
    }

    function createHotWordItem (index, data) {
        return '<a class="word-item" href="' + data.url +'" target="_blank">' +
                        '<div class="word-item-left">' + index + '.<span class="word-item-title">'+ data.key + '</span></div>'+
                        '<div class="word-item-right">' +
                            (data.tendency > 0 ? '<i></i>' : '') + data.count +
                        '</div>' +
                    '</a>'
    }
    ...
    function init () {
        ...
        attachEvent();
        initBackTopBtn();
        $('.feed-header a').eq(currentIndex).trigger('click');
    }

    function attachEvent () {
        $('.feed-header').on('click', 'a', function (ev) {
            ev.preventDefault();
            var curTab = $(this);
        ...
        $(window).on('scroll', function (ev) {
            ...
            if ($(document).height() - ($(window).height() + window.scrollY) < 100) {
                createFeedList(currentIndex);
            }
        ...
        $('#news-feed').on('click', '.feed-item', function (ev) {
            window.Api.statistic.webSend({
                'm': 'information flow',
                'n': categoryList[currentIndex].category,
                'o': 'click',
                'dt': 'user',
                'data': {
        ...
        $('#hot-word').on('click', '.word-item', function (ev) {
            window.Api.statistic.webSend({
                'm': 'hot search',
                'o': 'click',
                'dt': 'user',
        ...
        $('#back_top').on('click', function (ev) {
            ev.preventDefault();
            $('html,body').animate({scrollTop: 0}, 300);
        });

        $('.news-load-error').on('click', 'a', function (ev) {
            ev.preventDefault();
            getFeedData(currentIndex);
        });
    }
    ...
    function toggleLoading (status) {
        if (status) {
            ...
            $('.feed-pane').eq(currentIndex).show();
            $('.news-loading').hide();
        }
        $('.news-load-error').hide();
    }

    function getFeedData (index) {
        var category;
        if (typeof index !== 'number' || !(index in categoryList)) {
            index = 0;
        }
        category = categoryList[index].category;
        toggleLoading(true);
        $.ajax({
            url: '//pc-newtab.maxthon.com/static/newtab/' + category + '.json',
            dataType: 'json',
            cache: false,
            timeout: 6000,
            success: function (data) {
                var fragment;
                cache[category] = data;
                $('.feed-pane').eq(index).empty();
                fragment = createFeedList(index);
                toggleLoading(false);
            },
            error: function () {
                $('.feed-pane').hide();
    ...
    function createFeedList (index) {
        var data = (cache[categoryList[index].category] || []).splice(0, 10);
        var str = '';
        $.each(data, function (index, item) {
            if (item.images && item.images.length >= 3) {
                str += createFeedItem(item);
            } else {
                str += createFeedSingleItem(item);
            }
        });
        $('.feed-pane').eq(index).append(str);
    }

    function createFeedItem (data) {
        var str = '';
        $.each(data.images, function (index, item) {
            str += '<div class="feed-item-img"><img src="' + item+ '" alt="' + data.title + '"></div>'
        });

        str += '<div class="feed-item-img-more"><span>查看更多&nbsp;&gt;</span></div>'

        return '<a class="feed-item" target="_blank" href="' + data.link + '">' +
            '<h4 class="feed-item-header">' + data.title + '</h4>' +
            '<div class="feed-item-img-list clearfix">' + str + '</div>' +
            '<div class="feed-item-info"><span class="feed-item-time">' + data.time + '</span></div></a>'
    }

    function createFeedSingleItem (data) {
        return '<a class="feed-item single clearfix" href="' + data.link + '" target="_blank">' +
            '<div class="feed-item-img-list clearfix">' +
                '<div class="feed-item-img"><img src="' + data.images[0] + '" alt="' + data.title + '">' +
            '</div></div>' +
            '<h4 class="feed-item-header">' + data.title + '</h4>' +
            '<div class="feed-item-info"><span class="feed-item-time">' + data.time + '</span></div></a>'
    }
    ...
    var infoService = {
        ...
        initHotWord: initHotWord,
        init: init
    }


    window.infoService = infoService;
